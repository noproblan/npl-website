...
class ErrorController extends Zend_Controller_Action
{
	...
	public function errorAction()
    {
        ...
        $errorLogger->setEventItem("userIp", $errors->request->getServer('REMOTE_ADDR'));
        if (Zend_Auth::getInstance()->hasIdentity()) {
        	$errorLogger->setEventItem("userName", Zend_Auth::getInstance()->getIdentity()->username);
        } else {
			$errorLogger->setEventItem("userName", "guest");
        }
        $errorLogger->setEventItem("url", $this->_getAbsoluteRequestURI());
        $requestParams = $this->_filterPasswordRequests($errors->request->getParams());
    ...
    public function noaccessAction()
    {
		...
		/* @var $errorLogger Zend_Log */
		$errorLogger = Zend_Registry::get('errorLogger');
		$errorLogger->setEventItem("userIp", $this->getRequest()->getServer('REMOTE_ADDR'));
		if (Zend_Auth::getInstance()->hasIdentity()) {
			$errorLogger->setEventItem("userName", Zend_Auth::getInstance()->getIdentity()->username);
		} else {
			$errorLogger->setEventItem("userName", "guest");
		...
		$errorLogger->setEventItem("url", $this->_getAbsoluteRequestURI());
		$requestParams = $this->_filterPasswordRequests($this->getRequest()->getParams());
		$errorLogger->setEventItem("params", json_encode($requestParams));
