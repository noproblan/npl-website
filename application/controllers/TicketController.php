...
class TicketController extends Zend_Controller_Action
{
	...
	public function changeextrasAction()
	{
		...
		if (!$this->getRequest()->isPost()) return $this->_helper->redirector('list', 'lan');
		if (!$form->isValid($this->getRequest()->getPost())) return $this->_helper->redirector('list', 'lan');		
		if (!$this->_digitValidator->isValid($form->getValue('ticketId'))) return $this->_helper->redirector('list', 'lan');
		if (!$this->_ticketIDValidator->isValid($form->getValue('ticketId'))) return $this->_helper->redirector('list','lan');
		...
		if ($this->_currentUser->getId() != $ticket->getUserId()) return $this->_helper->redirector('list', 'lan');
		...
		$this->_flashMessenger->setNamespace('success')->addMessage('Die Optionen für die LAN wurden gespeichert.');//
	...
	public function reserveseatAction()
	{
		...
		if (is_null($ticketId) || is_null($seatId)) return $this->_helper->redirector('list', 'lan');
		...
		if (!$this->_ticketIDValidator->isValid($ticketId)) {
			$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_TICKETID_INVALID);
			return $this->_helper->redirector('list', 'lan');
		}
		...
		if (!$this->_seatIDValidator->isValid($seatId)) {
			$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_SEATID_INVALID);
			return $this->_helper->redirector('reservation', 'lan', null, array('lanid' => $ticket->getLanId()));
		}
		
		if ($ticket->getStatus() == Application_Model_Ticket::STATUS_NOTPAID) {
			$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_ONLY_PAID);
			return $this->_helper->redirector('reservation', 'lan', null, array('lanid' => $ticket->getLanId()));
		}
		...
		if (!$seat->isAvailable()) {
			$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_SEAT_TAKEN);
			return $this->_helper->redirector('reservation', 'lan', null, array('lanid' => $ticket->getLanId()));
		}
		
		$ticket->setSeatId($seatId);
		$ticketsMapper->save($ticket);
		$this->_flashMessenger->setNamespace('success')->addMessage(self::MSG_SEAT_RESERVED);
		return $this->_helper->redirector('reservation', 'lan', null, array('lanid' => $ticket->getLanId()));
	}
}