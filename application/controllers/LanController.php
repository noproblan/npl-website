...
class LanController extends Zend_Controller_Action
{
	...
	public function init()
	{
		...
		if (Zend_Auth::getInstance()->hasIdentity()) {
			$this->_mapperUsers->find(Zend_Auth::getInstance()->getIdentity()->id, $this->_currentUser);
		}
	...
	public function infoAction()
	{
		...
		if ($lanId != null) {
			...
			if ($this->_digitValidator->isValid($lanId) &&
				...
				$tickets = $mapperTickets->fetchAllByLanAndUserId($lanId, $this->_currentUser->getId());
				$this->view->hasTicket = $this->_currentUser->hasTicketOfLan($lanId);
				// TODO: Anpassen für mehrere Tickets
				$hasPaid = false;
				if (!empty($tickets)) {
					if ($tickets[0]->getStatus() == Application_Model_Ticket::STATUS_PAID
							|| $tickets[0]->getStatus() == Application_Model_Ticket::STATUS_PREPAID) {
						$hasPaid = true;
					}
				}
	...
	public function participateAction()
	{
			...
			if ($this->_digitValidator->isValid($lanId) &&
				...
				if ($this->_currentUser->hasTicketOfLan($lanId)) {
					if ($participate == 'no') {
						...
						$tickets = $ticketMapper->fetchAllByLanAndUserId($lanId, $this->_currentUser->getId());
						foreach ($tickets as $ticket) {
							if ($ticket->getStatus() == 'notpaid') {
								$ticketMapper->delete($ticket);
								$this->_flashMessenger->setNamespace('success')->addMessage('Du wurdest von der LAN abgemeldet.');
							} else {
								$this->_flashMessenger->setNamespace('notice')->addMessage('Du kannst dich nicht abmelden, da du bereits bezahlt hast.');
							}
						}
						if ($prevController == 'lan') {
							return $this->_helper->redirector('info', 'lan', 'default', array('lanid' => $lanId));
						}
						return $this->_helper->redirector('account', 'user');
					}
					$this->_flashMessenger->setNamespace('notice')->addMessage('Du bist bereits zur LAN angemeldet');
					if ($prevController == 'lan') {
						return $this->_helper->redirector('info', 'lan', 'default', array('lanid' => $lanId));
					}
					return $this->_helper->redirector('account', 'user');
					//return $this->_helper->redirector('reservation', 'lan', 'default', array('lanid' => $lanId));
				} else {
					...
					$this->_flashMessenger->setNamespace('success')->addMessage('Du wurdest zur LAN angemeldet');
					if ($prevController == 'lan') {
						return $this->_helper->redirector('info', 'lan', 'default', array('lanid' => $lanId));
					}
					return $this->_helper->redirector('account', 'user');
				...
				$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_LANID_INVALID);
				if ($prevController == 'lan') {
					return $this->_helper->redirector('list');
				}
				return $this->_helper->redirector('account', 'user');
		...
		$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_REQUEST_INVALID);
		if ($prevController == 'lan') {
			return $this->_helper->redirector('list');
		}
		return $this->_helper->redirector('account', 'user');
		...
		if ($lanId != null) {
			...
			if ($this->_digitValidator->isValid($lanId) &&
				...
				if ($this->_currentUser->hasTicketOfLan($lanId)) {
					...
					if ($tickets[0]->getStatus() != Application_Model_Ticket::STATUS_NOTPAID) {
						$ticketId = $tickets[0];
						$lan = new Application_Model_Lan();
						$this->_mapperLans->find($lanId, $lan);
						...
						$desks = array();
						foreach ($maps as $map) {
							$desks = $mapperDesks->fetchAllByMapId($map->getId());
						}
						...
						foreach($mapperTickets->fetchAllByLanId($lanId) as $ticket) { /* @var $ticket Application_Model_Ticket */
							...
							$this->_mapperUsers->find($ticket->getUserId(), $user);
						...
						$this->_flashMessenger->setNamespace('notice')->addMessage(self::MSG_NOT_PAID);
						return $this->_helper->redirector('info', 'lan', null, array('lanid' => $lanId));
					}
				} else {
					$this->_flashMessenger->setNamespace('notice')->addMessage(self::MSG_NO_TICKET);
					return $this->_helper->redirector('info', 'lan', null, array('lanid' => $lanId));
				}
			} else {
				$this->_flashMessenger->setNamespace('error')->addMessage(self::MSG_LANID_INVALID);
