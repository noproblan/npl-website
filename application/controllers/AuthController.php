...
class AuthController extends Zend_Controller_Action
{
	...
	protected function _loginUser($data)
	{
		...
		if (!$result->isValid()) {
			$forgotUsername = '<a href="' . self::URL_FORGOT_USERNAME . '">' . self::MSG_FORGOT_USERNAME . '</a>';
			$forgotPassword = '<a href="' . self::URL_FORGOT_PASSWORD . '">'. self::MSG_FORGOT_PASSWORD .'</a>';
			$this->_instantMessenger->addError(self::MSG_INVALID_LOGIN_CREDENTIALS);
			$this->_instantMessenger->addNotice($forgotUsername . " - " . $forgotPassword);
		} else {
			...
			if (!$user->getActive()) {
				$storage->clear();
				$resendUrl = self::URL_RESEND_ACTIVATION . '/id/' . $user->getId() . '/key/' . $user->getSalt();
				$this->_instantMessenger->addNotice(self::MSG_ACCOUNT_NOT_ACTIVATED . "<br />" . '<a href="' . $resendUrl . '">' . self::MSG_RESEND_ACTIVATION . '</a>');
			} else {
				$controller = $this->getRequest()->getParam('controller', 'user');
				...
				return $this->_helper->redirector($action, $controller, 'default', $params);
			}
		}
	}
}