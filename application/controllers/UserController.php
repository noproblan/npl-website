...
class UserController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        if (Zend_Auth::getInstance()->hasIdentity()) {
            $this->_users->find(Zend_Auth::getInstance()->getIdentity()->id,
                $this->_currentUser);
        }
        // Messengers
    ...
    /**
     * Name: Verify
     * Description:    Search for unactivated user with id and key and activate the
     * user
     * Access:        Guests, Members
     */
    public function verifyAction()
    {
        ...
        if (!is_Null($id) && !is_Null($key)) {
            ...
            if ($this->_digitValidator->isValid($id) &&
                $this->_userIDValidator->isValid($id)
            ) {
                    ...
                    if ($key ===
                        ...
                        if (!$user->getMailVerified()) {
                            ...
                            $this->_flashMessenger->setNamespace('success')->addMessage(
                                self::MSG_VERIFICATION_SUCCESSFUL);
                            return $this->_helper->redirector('index', 'index');
                        } else {
                            $this->_flashMessenger->setNamespace('notice')->addMessage(
                                self::MSG_ALREADY_VERIFICATED);
                            return $this->_helper->redirector('index', 'index');
                        }
                    }
                }
    ...
    public function activateAction()
    {
        ...
        if (!is_Null($id) && !is_Null($key)) {
            ...
            if ($this->_digitValidator->isValid($id) &&
                ...
                if ($this->_shaLengthValidator->isValid($key) &&
                    $this->_userSaltValidator->isValid($key)
                ) {
                    $user = new Application_Model_User();
                    $this->_users->find($id, $user);
                    if (!$user->getActive()) {
                        ...
                        $this->_users->save($user);
                        $this->_flashMessenger->setNamespace('success')->addMessage(
                            self::MSG_ACTIVATION_SUCCESSFUL);
                        return $this->_helper->redirector('index', 'index');
                    } else {
                        $this->_flashMessenger->setNamespace('notice')->addMessage(
                            self::MSG_ALREADY_ACTIVATED);
                        return $this->_helper->redirector('index', 'index');
                    }
                }
            }
    ...
    /**
     * Name: ForgotPassword
     * Description:    Shows a form to enter mail address of a user to send password
     * reset instructions
     * Access:        Guests
     */
...
// TODO: Refactor!!
    /**
     * Name: ResetPassword
     * Description:    Resets the password of a user with a new one if parameters are
     * correct.
     * Access:        Guests
     */
    public function resetpasswordAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                if ($this->_digitValidator->isValid($id) && $this->_userIDValidator->isValid(
                        $id)
                ) {
                    if ($lenghtValidator->isValid($key)) {
                        $subkey = substr($key, strlen($key) - 40, 40);
                        $timestamp = substr($key, 0, strlen($key) - 40);
                        if ($this->_digitValidator->isValid($timestamp) &&
                            $this->_userSaltValidator->isValid($subkey)
                        ) {
                            if (($timestamp + 604800) >= time()) {
                                $user = new Application_Model_User();
                                $this->_users->find($id, $user);
                                $user->setPassword(
                                    Application_Model_User::encryptPassword($password,
                                        $user->getSalt()));
                                $this->_users->save($user);
                                $this->_flashMessenger->setNamespace('success')->addMessage(
                                    self::MSG_PASSWORD_CHANGE_SUCCESSFUL);
                                return $this->_helper->redirector('index', 'index');
                            }
                            $this->_instantMessenger->addError(
                                'Die Anfrage ist älter als 7 Tage.');
                            $this->_instantMessenger->addNotice(
                                '<a href="' . self::URL_FORGOT_PASSWORD . '">' .
                                self::MSG_RESEND_PASSWORD_REQUEST . '</a>');
    ...
    public function accountAction()
    {
        ...
        if (!$this->_currentUser->getMailVerified()) {
            $this->_instantMessenger->addNotice(
                self::MSG_MAIL_NOT_VERIFIED . "<br />" . '<a href="' .
                self::URL_RESEND_VERIFICATION . '">' . self::MSG_RESEND_VERIFICATION .
                '</a>');
        }
        ...
        foreach ($comingLans as $key => $lanModel) {
            if ($this->_currentUser->hasTicketOfLan($lanModel->getId())) {
                $tickets = $mapperTickets->fetchAllByLanAndUserId($lanModel->getId(),
    ...
    public function changepasswordAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                $oldPasswordHashed = Application_Model_User::encryptPassword($oldPassword,
                    $this->_currentUser->getSalt());
                $newPasswordHashed = Application_Model_User::encryptPassword($newPassword,
                    $this->_currentUser->getSalt());
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                if ($this->_currentUser->getMail() !== $mail) {
                    ...
                    $this->_flashMessenger->setNamespace('success')->addMessage(
                        self::MSG_MAIL_CHANGE_SUCCESSFUL);
                    return $this->_helper->redirector('profile', 'user');
                } else {
    ...
    public function deleteAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                $data = $form->getValues();
                if ($this->_currentUser->getPassword() ===
                    ...
                    $this->_flashMessenger->setNamespace('success')->addMessage(
                        self::MSG_ACCOUNT_DELETED_SUCCESSFUL);
                    return $this->_helper->redirector('index', 'index');
                } else {
    ...
    public function migrateAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!is_Null($key)) {
                $key = $this->_stripFilter->filter($key);
                if ($this->_shaLengthValidator->isValid($key) &&
                    $this->_oldUserSaltValidator->isValid($key)
                ) {
    ...
    public function lannewsmailerAction()
    {
        $users = $this->_users->fetchAll();
        $lanMapper = new Application_Model_Mapper_LansMapper();
        $comingLans = $lanMapper->fetchComing();
        if (isset($comingLans[0])) {
            /* @var $nextLan Application_Model_Lan */
            $nextLan = $comingLans[0];
            $lanId = $nextLan->getId();

            set_time_limit(0);
            foreach ($users as $user) {
                /* @var $user Application_Model_User */
                if ($user->getId() > 2 && $user->hasTicketOfLan($lanId)) {
                    $user->sendLanNewsMail();
                }
            }
            set_time_limit(30);
        } else {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                'Keine nächste LAN im System.');
        }
        $this->_helper->redirector('index', 'index');
    }
    ...
    protected function _registerUser($data)
    {
        ...
        $user->sendActivationMail();
        $this->_flashMessenger->setNamespace('success')->addMessage(
            self::MSG_ACCOUNT_CREATED_SUCCESSFUL);
        return $this->_helper->redirector('index', 'index');
    }
    ...
    protected function _migrateUser($data)
    {
        ...
        $this->_flashMessenger->setNamespace('success')->addMessage(
            self::MSG_ACCOUNT_MIGRATED_SUCCESSFUL);
        return $this->_helper->redirector('index', 'index');
    }
