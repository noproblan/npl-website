...
class Application_Model_Mapper_TicketsMapper
{
	...
	public function save(Application_Model_Ticket $ticket)
	{
		...
		if ($data['seat_id'] == 0) unset($data['seat_id']);
		if (is_null($data['extras'])) $data['extras'] = "";
	...
	{
		return $this->_fetch($this->getDbTable()->fetchAllByLanAndUserId($lanId, $userId));
	}
	
	public function fetchAllByLanId($lanId)
	{
		return $this->_fetch($this->getDbTable()->fetchAllByLanId($lanId));
	}
	
	public function fetchAllBySeatId($seatId)
	{
		return $this->_fetch($this->getDbTable()->fetchAllBySeatId($seatId));
	}
	
	public function fetchAllByUserId($userId)
	{
		return $this->_fetch($this->getDbTable()->fetchAllByUserId($userId));
	}
	
	public function fetchAllWithSeatByLanId($lanId)
	{
		return $this->_fetch($this->getDbTable()->fetchAllWithSeatByLanId($lanId));
	}
