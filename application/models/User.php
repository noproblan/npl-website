...
class Application_Model_User
{
    ...
    public function sendActivationMail ()
    {
        ...
        try {
            ...
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Eine E-Mail zur Aktivierung deines Accounts wurde an deine Mailadresse verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Die E-Mail zur Aktivierung deines Accounts konnte nicht versendet werden.');
        }
    }
    ...
    public function sendVerificationMail ()
    {
        ...
        try {
            $mail->send();
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Eine E-Mail zur Überprüfung deiner Mailadresse wurde an deine Mailadresse verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Die E-Mail zur Überprüfung deiner Mailadresse konnte nicht versendet werden.');
        }
    }
    ...
    public function sendUsernameMail ()
    {
        ...
        try {
            $mail->send();
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Eine E-Mail mit deinem Benutzernamen wurde an deine Mailadresse verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Die E-Mail mit deinem Benutzernamen konnte nicht versendet werden.');
        }
    }

    public function sendPasswordResetMail ()
    {
        // Notice: Password only will be sent. A new one has to be generated
        // with Mapper
        $mail = new Npl_Mail("utf-8");
        $mail->setRecipient($this->getMail(), $this->getUsername());
        ...
        try {
            $mail->send();
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Eine E-Mail zum Zurücksetzen deines Passworts wurde an deine Mailadresse verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Die E-Mail zum Zurücksetzen deines Passworts konnte nicht versendet werden.');
        }
    }
    ...
    public function sendNewsletterMail ()
    {
        ...
        try {
            $mail->send();
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Der Newsletter wurde erfolgreich an ' . $this->getUsername() .
                             ' verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Der Newsletter konnte nicht an ' . $this->getUsername() .
    ...
    public function sendTeamMail ()
    {
        ...
        try {
            $mail->send();
            $this->_flashMessenger->setNamespace('success')->addMessage(
                    'Das Teammail wurde erfolgreich an ' . $this->getUsername() .
                             ' verschickt.');
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->_flashMessenger->setNamespace('error')->addMessage(
                    'Der Teammail konnte nicht an ' . $this->getUsername() . ' (' .
                             $this->getMail() . ') versendet werden.');
        }
    }
