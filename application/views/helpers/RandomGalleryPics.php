...
class Zend_View_Helper_RandomGalleryPics extends Zend_View_Helper_Abstract
{
     ...
     * TODO: ein bisschen wahnsinnig, für jeden Seitenaufruf dermassen viele Festplattenzugriffe...
			...
			while ($counter <= $count) {
				...
				if (!empty($galleryDir)) {
					...
					foreach ($this->imageTypes as $imgType => $mimeType) {
						$fileNamesWithType = glob(self::GALLERY_PATH.$galleryDir."/*." . $imgType);
						if (!empty($fileNamesWithType)) {
							$fileNames = array_merge($fileNames, $fileNamesWithType);
						}
					}
