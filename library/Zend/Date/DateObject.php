...
abstract class Zend_Date_DateObject
{
    ...
    /**
     * Internal function.
     * Returns time(). This method exists to allow unit tests to work-around
     * methods that might otherwise
     * be hard-coded to use time(). For example, this makes it possible to test
     * isYesterday() in Date.php.
     *
     * @param integer $sync
     ...
     * The timestamp returned by mktime() can exceed the precision of
     ...
     * @param integer $year            
     * @param boolean $gmt
     *            OPTIONAL true = other arguments are for UTC time, false =
     *            arguments are for local time/date
     * @return integer|float timestamp (number of seconds elapsed relative to
     *         1970/01/01 00:00:00 GMT/UTC)
     */
            ...
            $gmt = false)
    {
        ...
        if ((1901 < $year) and ($year < 2038)) {
            ...
            // Timezone also includes DST settings, therefor substracting the
            // GMT offset is not enough
            // We have to set the correct timezone to get the right value
            if (($this->_timezone != $oldzone) and ($gmt === false)) {
                date_default_timezone_set($this->_timezone);
            }
            $result = ($gmt) ? @gmmktime($hour, $minute, $second, $month, $day, 
                    $year) : @mktime($hour, $minute, $second, $month, $day, 
                    $year);
        ...
        if (isset(self::$_cache)) {
            $id = strtr(
                    'Zend_DateObject_mkTime_' . $this->_offset . '_' . $year .
    ...
    protected function date ($format, $timestamp = null, $gmt = false)
    {
        ...
        if (abs($timestamp) <= 0x7FFFFFFF) {
            $result = ($gmt) ? @gmdate($format, $timestamp) : @date($format, 
                    $timestamp);
            date_default_timezone_set($oldzone);
            return $result;
        }
        ...
        if (isset(self::$_cache)) {
            $idstamp = strtr(
                    'Zend_DateObject_date_' . $this->_offset . '_' . $timestamp .
                             '_' . (int) $gmt, '-', '_');
        ...
        for ($i = 0; $i < $length; $i ++) {
            switch ($format[$i]) {
                ...
                case 'N': // ISO 8601 numeric day of week, 1 - 7
                    $day = self::dayOfWeek($date['year'], $date['mon'], 
                            $date['mday']);
                    if ($day == 0) {
                        $day = 7;
                    }
                ...
                case 'w': // numeric day of week, 0 - 6
                    $output .= self::dayOfWeek($date['year'], $date['mon'], 
                            $date['mday']);
                    break;
                ...
                case 'W': // ISO 8601, week number of year
                    $output .= $this->weekNumber($date['year'], $date['mon'], 
                            $date['mday']);
                    break;
                ...
                case 'o': // ISO 8601 year number
                    $week = $this->weekNumber($date['year'], $date['mon'], 
                            $date['mday']);
                ...
                case 'y': // year number, 2 digits
                    $output .= substr($date['year'], strlen($date['year']) - 2, 
                            2);
                    break;
                ...
                case 'e': // timezone identifier
                    if ($gmt === true) {
                        $output .= gmdate('e', 
                                mktime($date['hours'], $date['minutes'], 
                                        $date['seconds'], $date['mon'], 
                                        $date['mday'], 2000));
                    } else {
                        $output .= date('e', 
                                mktime($date['hours'], $date['minutes'], 
                ...
                case 'O': // difference to GMT in hours
                    ...
                    $output .= sprintf('%s%04d', ($gmtstr <= 0) ? '+' : '-', 
                            abs($gmtstr) / 36);
                    break;
                
                case 'P': // difference to GMT with colon
                    $gmtstr = ($gmt === true) ? 0 : $this->getGmtOffset();
                    $gmtstr = sprintf('%s%04d', ($gmtstr <= 0) ? '+' : '-', 
                            abs($gmtstr) / 36);
                    $output = $output . substr($gmtstr, 0, 3) . ':' .
                             substr($gmtstr, 3);
                    break;
                ...
                case 'c': // ISO 8601 date format
                    ...
                    $difference = sprintf('%s%04d', 
                            ($difference <= 0) ? '+' : '-', 
                            abs($difference) / 36);
                    $difference = substr($difference, 0, 3) . ':' .
                             substr($difference, 3);
                    $output .= $date['year'] . '-' .
...
$correction = 0;
if (($year < 1582) or
         (($year == 1582) and (($month < 10) or (($month == 10) && ($day < 15))))) {
    $correction = 3;
}
...
$day = floor((13 * $month - 1) / 5) + $day + ($year % 100) +
         floor(($year % 100) / 4);
 ...
 *
 * Returns an array of date parts for $timestamp, relative to 1970/01/01
 * 00:00:00 GMT/UTC.
 *
 * $fast specifies ALL date parts should be returned (slower)
 * Default is false, and excludes $dayofweek, weekday, month and timestamp from
 * parts returned.
 *
 * @param mixed $timestamp            
 * @param boolean $fast
...
if (isset(self::$_cache)) {
    $id = strtr('Zend_DateObject_getDateParts_' . $timestamp . '_' . (int) $fast, 
            '-', '_');
    if ($result = self::$_cache->load($id)) {
        return unserialize($result);
    }
...
if (($month == 1) and (($firstday < 1) or ($firstday > 4)) and ($day < 4)) {
    ...
    if (($month == 12) and ((self::dayOfWeek($year + 1, 1, 1) < 5) and
             (self::dayOfWeek($year + 1, 1, 1) > 0))) {
        return 1;
    }

return intval(
        ((self::dayOfWeek($year, 1, 1) < 5) and
                 (self::dayOfWeek($year, 1, 1) > 0)) + 4 * ($month - 1) +
                 (2 * ($month - 1) + ($day - 1) + $firstday - $dayofweek + 6) *
                 36 / 256);
}
 ...
 *            Location for calculation MUST include 'latitude', 'longitude',
 *            'horizon'
 * @param bool $horizon
...
if (abs($this->_unixTimestamp) <= 0x7FFFFFFF) {
    if ($rise === false) {
        return date_sunset($this->_unixTimestamp, SUNFUNCS_RET_TIMESTAMP, 
                $location['latitude'], $location['longitude'], 90 + $horizon, 
                $this->getGmtOffset() / 3600);
    }
    return date_sunrise($this->_unixTimestamp, SUNFUNCS_RET_TIMESTAMP, 
            $location['latitude'], $location['longitude'], 90 + $horizon, 
            $this->getGmtOffset() / 3600);
}
...
$radDay = $this->date('z', $this->_unixTimestamp) +
         ($tmpRise - $radLongitude) / $fullCircle;
...
if (($solLongitude / $quarterCircle) - intval($solLongitude / $quarterCircle) ==
         0) {
...
return $this->mktime($hour, $min, $sec, $this->date('m', $this->_unixTimestamp), 
        ...
        $this->date('Y', $this->_unixTimestamp), - 1, true);
}

/**
 * Sets a new timezone for calculation of $this object's gmt offset.
 * For a list of supported timezones look here: http://php.net/timezones
 * If no timezone can be detected or the given timezone is wrong UTC will be
 * set.
 *
 * @param string $zone
 *            OPTIONAL timezone for date calculation; defaults to
...
// throw an error on false input, but only if the new date extension is
// available
...
// this can generate an error if the date extension is not available and a false
// timezone is given
 ...
 * The offset to GMT is initially set when the object is instantiated using the
 * currently,
 * in effect, default timezone for PHP functions.
 *
 * @return integer seconds difference between GMT timezone and timezone when
