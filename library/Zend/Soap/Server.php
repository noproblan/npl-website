...
class Zend_Soap_Server implements Zend_Server_Interface
{
        ...
        foreach ($options as $key => $value) {
            switch ($key) {
                ...
                case 'featues':
                    trigger_error(
                            __METHOD__ .
                                     ': the option "featues" is deprecated as of 1.10.x and will be removed with 2.0.0; use "features" instead', 
                                    E_USER_NOTICE);
                case 'features':
    ...
    public function setSoapVersion ($version)
    {
        if (! in_array($version, array(
            ...
            throw new Zend_Soap_Server_Exception(
                    'Invalid soap version specified');
        }
     ...
     *
     * See {@link setObject()} to set preconfigured object instances as request
     * handlers.
     *
     * @param string $class
     ...
     *
     * Uses {@link $_wsdl} and return value of {@link getOptions()} to
     * instantiate
    ...
    /**
     * Method initalizes the error context that the SOAPServer enviroment will
     * run in.
     *
    ...
    {
        $this->_faultExceptions = array_merge($this->_faultExceptions, 
                (array) $class);
        return $this;
    }
