...
class Zend_Soap_Wsdl_Strategy_ArrayOfTypeSequence extends Zend_Soap_Wsdl_Strategy_DefaultComplexType
{
    ...
    /**
     * Add an unbounded ArrayOfType based on the xsd:sequence syntax if type[]
     * is detected in return value doc comment.
     *
     * @param string $type            
    ...
    public function addComplexType ($type)
    {
        ...
        if ($nestedCounter > 0) {
            ...
            for ($i = 1; $i <= $nestedCounter; $i ++) {
                $complexTypeName = substr(
                        $this->_getTypeNameBasedOnNestingLevel($singularType, 
                                $i), 4);
                $childTypeName = $this->_getTypeNameBasedOnNestingLevel(
                        $singularType, $i - 1);
                
                $this->_addElementFromWsdlAndChildTypes($complexTypeName, 
                        $childTypeName);
            }
            // adding the PHP type which is resolved to a nested XSD type.
            // therefore add only once.
    ...
    /**
     * Return the ArrayOf or simple type name based on the singular xsdtype and
     * the nesting level
     *
    ...
    /**
     * Strip the xsd: from a singularType and Format it nice for ArrayOf<Type>
     * naming
     *
     * @param string $singularType            
     * @return string
    ...
    {
        $singulartype = $this->getContext()->getType(
                str_replace("[]", "", $type));
        return $singulartype;
    }
     ...
     * @return void
     */
    protected function _addElementFromWsdlAndChildTypes ($arrayType, 
            $childTypeName)
    {
