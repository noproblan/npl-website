...
class Zend_Soap_AutoDiscover implements Zend_Server_Interface
{
    ...
    public function setUri ($uri)
    {
        if (! is_string($uri) && ! ($uri instanceof Zend_Uri)) {
            ...
            throw new Zend_Soap_AutoDiscover_Exception(
                    "No uri given to Zend_Soap_AutoDiscover::setUri as string or Zend_Uri instance.");
        }
        $this->_uri = $uri;
    ...
    {
        if (! is_string($wsdlClass) &&
                 ! is_subclass_of($wsdlClass, 'Zend_Soap_Wsdl')) {
            require_once "Zend/Soap/AutoDiscover/Exception.php";
            throw new Zend_Soap_AutoDiscover_Exception(
                    "No Zend_Soap_Wsdl subclass given to Zend_Soap_AutoDiscover::setWsdlClass as string.");
        }
    ...
    {
        if (isset($_SERVER['HTTP_X_REWRITE_URL'])) { // check this first so IIS
                                                     // will catch
    ...
    public function setClass ($class, $namespace = '', $argv = null)
    {
        ...
        // The wsdl:types element must precede all other elements (WS-I Basic
        // Profile 1.1 R2023)
        $wsdl->addSchemaTypeSection();
        
        $port = $wsdl->addPortType($class . 'Port');
        $binding = $wsdl->addBinding($class . 'Binding', 'tns:' . $class .
                 'Port');
        
        $wsdl->addSoapBinding($binding, $this->_bindingStyle['style'], 
                $this->_bindingStyle['transport']);
        $wsdl->addService($class . 'Service', $class . 'Port', 
                'tns:' . $class . 'Binding', $uri);
    ...
    public function addFunction ($function, $namespace = '')
    {
        ...
        if (! ($this->_wsdl instanceof Zend_Soap_Wsdl)) {
            ...
            // The wsdl:types element must precede all other elements (WS-I
            // Basic Profile 1.1 R2023)
            $wsdl->addSchemaTypeSection();
            
            $port = $wsdl->addPortType($name . 'Port');
            $binding = $wsdl->addBinding($name . 'Binding', 
                    'tns:' . $name . 'Port');
            
            $wsdl->addSoapBinding($binding, $this->_bindingStyle['style'], 
                    $this->_bindingStyle['transport']);
            $wsdl->addService($name . 'Service', $name . 'Port', 
                    'tns:' . $name . 'Binding', $uri);
        } else {
            $wsdl = $this->_wsdl;
        }
    ...
    protected function _addFunctionToWsdl ($function, $wsdl, $port, $binding)
    {
        ...
        // We only support one prototype: the one with the maximum number of
        // arguments
        $prototype = null;
        ...
        if ($prototype === null) {
            require_once "Zend/Soap/AutoDiscover/Exception.php";
            throw new Zend_Soap_AutoDiscover_Exception(
                    "No prototypes could be found for the '" .
        ...
        if ($isOneWayMessage == false) {
            ...
            if ($this->_bindingStyle['style'] == 'document') {
                ...
                // Add the wrapper element part, which must be named
                // 'parameters'
                $args['parameters'] = array(
                        'element' => $wsdl->addElement($element)
                );
        ...
        if ($isOneWayMessage == false) {
            $portOperation = $wsdl->addPortOperation($port, 
                    $function->getName(), 'tns:' . $function->getName() . 'In', 
                    'tns:' . $function->getName() . 'Out');
        } else {
            $portOperation = $wsdl->addPortOperation($port, 
                    $function->getName(), 'tns:' . $function->getName() . 'In', 
                    false);
        ...
        // When using the RPC style, make sure the operation style includes a
        // 'namespace' attribute (WS-I Basic Profile 1.1 R2717)
        if ($this->_bindingStyle['style'] == 'rpc' &&
                 ! isset($this->_operationBodyStyle['namespace'])) {
        ...
        if ($isOneWayMessage == false) {
            $operation = $wsdl->addBindingOperation($binding, 
                    $function->getName(), $this->_operationBodyStyle, 
    ...
    public function fault ($fault = null, $code = null)
    {
        require_once "Zend/Soap/AutoDiscover/Exception.php";
        throw new Zend_Soap_AutoDiscover_Exception(
                "Function has no use in AutoDiscover.");
    }
    ...
    public function dump ($filename)
    {
        if ($this->_wsdl !== null) {
            ...
            require_once "Zend/Soap/AutoDiscover/Exception.php";
            throw new Zend_Soap_AutoDiscover_Exception(
                    "Cannot dump autodiscovered contents, WSDL file has not been generated yet.");
        }
    }
    ...
    public function toXml ()
    {
        if ($this->_wsdl !== null) {
            ...
            require_once "Zend/Soap/AutoDiscover/Exception.php";
            throw new Zend_Soap_AutoDiscover_Exception(
                    "Cannot return autodiscovered contents, WSDL file has not been generated yet.");
        }
    }
    ...
    public function getType ($type)
    {
        if (! ($this->_wsdl instanceof Zend_Soap_Wsdl)) {
            ...
            // WSDL is not defined yet, so we can't recognize type in context of
            // current service
            return '';
        } else {
