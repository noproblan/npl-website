...
class Zend_Log
{
    ...
    static public function factory ($config = array())
    {
        ...
        if (array_key_exists('timestampFormat', $config)) {
            if (null != $config['timestampFormat'] &&
    ...
    {
        $writer = $this->_constructFromConfig('writer', $config, 
                $this->_defaultWriterNamespace);
    ...
    {
        $filter = $this->_constructFromConfig('filter', $config, 
                $this->_defaultFilterNamespace);
    ...
    {
        $formatter = $this->_constructFromConfig('formatter', $config, 
                $this->_defaultFormatterNamespace);
    ...
    protected function _constructFromConfig ($type, $config, $namespace)
    {
        ...
        if (! $reflection->implementsInterface('Zend_Log_FactoryInterface')) {
            ...
            throw new Zend_Log_Exception(
                    $className .
                             ' does not implement Zend_Log_FactoryInterface and can not be constructed from config.');
        }
    ...
    protected function getClassName ($config, $type, $defaultNamespace)
    {
        if (! isset($config[$type . 'Name'])) {
            require_once 'Zend/Log/Exception.php';
            throw new Zend_Log_Exception(
                    "Specify {$type}Name in the configuration array");
        }
    ...
    public function addPriority ($name, $priority)
    {
        ...
        if (isset($this->_priorities[$priority]) ||
            ...
            require_once 'Zend/Log/Exception.php';
            throw new Zend_Log_Exception(
                    'Existing priorities cannot be overwritten');
        }
     ...
     * Note: it still calls the original error handler if set_error_handler is
     * able to return it.
     *
     ...
     * (unknown/other) => INFO
     *
     * @link http://www.php.net/manual/en/function.set-error-handler.php Custom
     *       error handler
     *      
     * @return Zend_Log
     */
    public function registerErrorHandler ()
    {
        ...
        $this->_origErrorHandler = set_error_handler(
                array(
                        $this,
                        'errorHandler'
                ));
    ...
    /**
     * Error Handler will convert error into log message, and then call the
            ...
            $errcontext)
    {
        ...
        if ($this->_origErrorHandler !== null) {
            return call_user_func($this->_origErrorHandler, $errno, $errstr, 
