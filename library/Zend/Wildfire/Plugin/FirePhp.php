...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface
{
     ...
     * @return Zend_Wildfire_Plugin_FirePhp Returns the singleton
     *         Zend_Wildfire_Plugin_FirePhp instance
    ...
    public static function init ($class = null)
    {
        if (self::$_instance !== null) {
            ...
            throw new Zend_Wildfire_Exception(
                    'Singleton instance of Zend_Wildfire_Plugin_FirePhp already exists!');
        }
        if ($class !== null) {
            if (! is_string($class)) {
                require_once 'Zend/Wildfire/Exception.php';
                throw new Zend_Wildfire_Exception(
                        'Third argument is not a class string');
            }
            ...
            if (! self::$_instance instanceof Zend_Wildfire_Plugin_FirePhp) {
                ...
                require_once 'Zend/Wildfire/Exception.php';
                throw new Zend_Wildfire_Exception(
                        'Invalid class to third argument. Must be subclass of Zend_Wildfire_Plugin_FirePhp.');
            }
        } else {
    ...
    public function setEnabled ($enabled)
    {
        ...
        if (! $this->_enabled) {
            ...
            $this->_channel->getProtocol(self::PROTOCOL_URI)->clearMessages(
                    $this);
        }
        return $previous;
    }
     ...
     * @param array $options
     *            OPTIONAL Setting 'Collapsed' to true will initialize group
     *            collapsed instead of expanded
     * @return TRUE if the group instruction was added to the response headers
     *         or buffered.
     */
     ...
     * @return boolean Returns TRUE if the variable was added to the response
     *         headers or buffered.
     * @throws Zend_Wildfire_Exception
     */
    public static function send ($var, $label = null, $style = null, $options = array())
    {
        ...
        if ($var instanceof Exception) {
            ...
            if ($meta['Type'] == self::TRACE) {
                ...
                if (! $trace) {
                    $trace = $firephp->_getStackTrace(
                            array_merge($options, 
                                    array(
                                            'maxTraceDepth' => $options['maxTraceDepth'] +
                                                     1
                                    )));
                }
                ...
                $var = array(
                        ...
                        'Args' => isset($trace[0]['args']) ? $firephp->_encodeObject(
                                $trace[0]['args']) : '',
                        'Trace' => $firephp->_encodeTrace(
                                array_splice($trace, 1))
                );
        ...
        switch ($meta['Type']) {
            ...
            default:
                require_once 'Zend/Wildfire/Exception.php';
                throw new Zend_Wildfire_Exception(
                        'Log style "' . $meta['Type'] . '" not recognized!');
                break;
        }
    ...
    protected function _getStackTrace ($options)
    {
        ...
        if (isset($options['fixZendLogOffsetIfApplicable']) &&
                 $options['fixZendLogOffsetIfApplicable']) {
            if (count($trace) >= 3 && isset($trace[0]['file']) &&
             substr($trace[0]['file'], - 7, 7) == 'Log.php' &&
...
switch ($structure) {
    ...
    case self::STRUCTURE_URI_FIREBUGCONSOLE:
        ...
        if (! isset($data['meta']) || ! is_array($data['meta']) ||
            ...
            require_once 'Zend/Wildfire/Exception.php';
            throw new Zend_Wildfire_Exception(
                    'You must supply a "Type" in the meta information.');
        }
        if (! array_key_exists('data', $data)) {
            require_once 'Zend/Wildfire/Exception.php';
            throw new Zend_Wildfire_Exception('You must supply data.');
    ...
    default:
        require_once 'Zend/Wildfire/Exception.php';
        throw new Zend_Wildfire_Exception(
                'Structure of name "' . $structure . '" is not recognized.');
        break;
}
return false;
}
...
if (is_resource($object)) {
    ...
    if (is_object($object)) {
        ...
        foreach ($properties as $just_name => $property) {
            ...
            $return[$name] = $this->_encodeObject($members[$raw_name], 
                    $objectDepth + 1, 1);
        } else {
            if (method_exists($property, 'setAccessible')) {
                $property->setAccessible(true);
                $return[$name] = $this->_encodeObject(
                        $property->getValue($object), $objectDepth + 1, 1);
            } else 
                if ($property->isPublic()) {
                    $return[$name] = $this->_encodeObject(
                            $property->getValue($object), $objectDepth + 1, 1);
