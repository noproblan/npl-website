...
/**
 * Implements communication via HTTP request and response headers for Wildfire
 * Protocols.
 *
 * @category Zend
 * @package Zend_Wildfire
        ...
        Zend_Wildfire_Channel_Interface
{
     ...
     * @return Zend_Wildfire_Channel_HttpHeaders Returns the singleton
     *         Zend_Wildfire_Channel_HttpHeaders instance
    ...
    protected function _initProtocol ($uri)
    {
        ...
        require_once 'Zend/Wildfire/Exception.php';
        throw new Zend_Wildfire_Exception(
                'Tyring to initialize unknown protocol for URI "' . $uri . '".');
    }
            ...
            if ($payload) {
                foreach ($payload as $message) {
                    ...
                    $this->getResponse()->setHeader(
                            self::$_headerPrefix . $message[0], $message[1], 
                            true);
                }
            }
        }
    ...
    protected function _registerControllerPlugin ()
    {
        ...
        if (! $controller->hasPlugin(get_class($this))) {
            $controller->registerPlugin($this, 
                    self::$_controllerPluginStackIndex);
        }
    }
     ...
     *
     * The channel is ready as long as the request and response objects are
     * initialized,
     ...
     *
     * A timing issue arises when messages are logged before the
     * request/response
     ...
     * for sure.
     *
     * If the request/response objects are not yet initialized we assume if
     * messages are
     * logged, the client will be able to receive them. As soon as the
     * request/response
     * objects are availoable and a message is logged this assumption is
     * challenged.
     * If the client cannot accept the messages any further messages are dropped
     * and messages sent prior are kept but discarded when the channel is
     * finally
     * flushed at the end of the request.
     *
     * When the channel is flushed the $forceCheckRequest option is used to
     * force
     * a check of the request/response objects. This is the last verification to
     * ensure
    ...
    public function isReady ($forceCheckRequest = false)
    {
        ...
        return ($this->getResponse()->canSendHeaders() && (preg_match_all(
                '/\s?FirePHP\/([\.\d]*)\s?/si', 
                $this->getRequest()->getHeader('User-Agent'), $m) || (($header = $this->getRequest()->getHeader(
                'X-FirePHP-Version')) &&
    ...
    /**
     * Flush messages to headers as late as possible but before headers have
     * been sent.
     *
     * @return void
     */
