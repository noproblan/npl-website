...
class Zend_Wildfire_Protocol_JsonStream
{
     ...
     * @return boolean Returns TRUE if plugin was registered, false if it was
     *         already registered
     */
     ...
     */
    public function recordMessage (Zend_Wildfire_Plugin_Interface $plugin, 
            $structure, $data)
    {
    ...
    {
        return Zend_Json::encode($value, true, 
                array(
    ...
    public function getPayload (Zend_Wildfire_Channel_Interface $channel)
    {
        if (! $channel instanceof Zend_Wildfire_Channel_HttpHeaders) {
            ...
            throw new Zend_Wildfire_Exception(
                    'The ' . get_class($channel) .
                             ' channel is not supported by the ' .
                             get_class($this) . ' protocol.');
        }
                ...
                foreach ($messages as $message) {
                    ...
                    for ($i = 0; $i < count($parts); $i ++) {
                        ...
                        if ($part) {
                            ...
                            $payload[] = array(
                                    $protocol_index . '-' . $structure_index .
                                             '-' . $plugin_index . '-' .
                                             $message_index,
                                            $msg
                            );
                            ...
                            if ($message_index > 99999) {
                                require_once 'Zend/Wildfire/Exception.php';
                                throw new Zend_Wildfire_Exception(
                                        'Maximum number (99,999) of messages reached!');
                            }
                        }
                    }
                }
