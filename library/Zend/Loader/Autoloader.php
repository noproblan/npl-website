...
class Zend_Loader_Autoloader
{
    ...
    public function setDefaultAutoloader ($callback)
    {
        if (! is_callable($callback)) {
            throw new Zend_Loader_Exception(
                    'Invalid callback specified for default autoloader');
        }
     ...
     *            Array of PHP callbacks (or Zend_Loader_Autoloader_Interface
     ...
     * @return bool|Zend_Loader_Autoloader Returns boolean if no argument is
     *         passed, object instance otherwise
     */
     ...
     *            PHP callback or Zend_Loader_Autoloader_Interface
     *            implementation
     * @param string|array $namespace
     ...
     * @return Zend_Loader_Autoloader
     */
    protected function _setNamespaceAutoloaders (array $autoloaders, 
            $namespace = '')
    {
        $namespace = (string) $namespace;
    ...
    protected function _getVersionPath ($path, $version)
    {
        ...
        if (empty($availableVersions)) {
            throw new Zend_Loader_Exception(
                    'No valid ZF installations discovered');
        }
    ...
    protected function _getAvailableVersions ($path, $version)
    {
        ...
        foreach ((array) $dirs as $dir) {
            ...
            if (('latest' == $version) || ((strlen($matchedVersion) >=
                     $versionLen) && (0 === strpos($matchedVersion, $version)))) {
