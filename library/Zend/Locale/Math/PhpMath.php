 ...
 * otherwise to PHP builtin math operators, with limited detection of overflow
 * conditions.
 * Sampling of PHP environments and platforms suggests that at least 80% to 90%
 * support bcmath.
 * This file should only be loaded for the 10% to 20% lacking access to the
 * bcmath extension.
 *
 * @category Zend
 * @package Zend_Locale
...
class Zend_Locale_Math_PhpMath extends Zend_Locale_Math
{
    ...
    public static function Add ($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result) or (abs($result - $op2 - $op1) > $precision)) {
            ...
            throw new Zend_Locale_Math_Exception(
                    "addition overflow: $op1 + $op2 != $result", $op1, $op2, 
                    $result);
        }
    ...
    public static function Sub ($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result) or (abs($result + $op2 - $op1) > $precision)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception(
                    "subtraction overflow: $op1 - $op2 != $result", $op1, $op2, 
                    $result);
        }
    ...
    public static function Pow ($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result) or is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception("power overflow: $op1 ^ $op2", 
                    $op1, $op2, $result);
        }
    ...
    public static function Mul ($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result) or is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception(
                    "multiplication overflow: $op1 * $op2 != $result", $op1, 
                    $op2, $result);
        }
    ...
    public static function Div ($op1, $op2, $scale = null)
    {
        ...
        if (empty($op2)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception("can not divide by zero", $op1, 
                    $op2, null);
        }
        if (empty($op1)) {
            $op1 = 0;
        ...
        if (is_infinite($result) or is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception(
                    "division overflow: $op1 / $op2 != $result", $op1, $op2, 
                    $result);
        }
    ...
    public static function Mod ($op1, $op2)
    {
        ...
        if (is_nan($result) or (($op1 - $result) % $op2 != 0)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception(
                    "modulus calculation error: $op1 % $op2 != $result", $op1, 
                    $op2, $result);
        }
    ...
    public static function Scale ($scale)
    {
        if ($scale > 9) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new Zend_Locale_Math_Exception(
                    "can not scale to precision $scale", $scale, null, null);
        }
