...
class Zend_Locale_Format
{
     ...
     * will be returned
     * The 'precision' option of a value is used to truncate or stretch extra
     * digits.
     * -1 means not to touch the extra digits.
     * The 'locale' option helps when parsing numbers and dates using separators
     * and month names.
     * The date format 'format_type' option selects between CLDR/ISO date format
     * specifier tokens and PHP's date() tokens.
     * The 'fix_date' option enables or disables heuristics that attempt to
     * correct invalid dates.
     * The 'number_format' option can be used to specify a default number format
     * string
     * The 'date_format' option can be used to specify a default date format
     * string, but beware of using getDate(),
     * checkDateFormat() and getTime() after using setOptions() with a 'format'.
     * To use these four methods
     * with the default date format for a locale, use array('date_format' =>
     * null, 'locale' => $locale) for their options.
     *
     * @param array $options
     *            Array of options, keyed by option name: format_type = 'iso' |
     *            'php', fix_date = true | false,
     *            locale = Zend_Locale | locale string, precision = whole number
     *            between -1 and 30
            ...
            switch ($name) {
                case 'number_format':
                    if ($value == Zend_Locale_Format::STANDARD) {
                        ...
                        $options['number_format'] = Zend_Locale_Data::getContent(
                                $locale, 'decimalnumber');
                    } else 
                ...
                case 'date_format':
                    if ($value == Zend_Locale_Format::STANDARD) {
                        ...
                        $options['date_format'] = Zend_Locale_Format::getDateFormat(
                                $locale);
                    } else 
                        if ((gettype($value) !== 'string') and ($value !== NULL)) {
                            require_once 'Zend/Locale/Exception.php';
                            throw new Zend_Locale_Exception(
                                    "Unknown dateformat type '" . gettype(
                                            $value) . "'. " .
                                             "Format '$value' must be a valid ISO or PHP date format string.");
                        } else {
                            if (((isset($options['format_type']) === true) and
                                     ($options['format_type'] == 'php')) or
                                     ((isset($options['format_type']) === false) and
                                     (self::$_options['format_type'] == 'php'))) {
                                $options['date_format'] = Zend_Locale_Format::convertPhpToIsoFormat(
                                        $value);
                            }
                        }
                    break;
    ...
    /**
     * Changes the numbers/digits within a given string from one script to
     * another
     * 'Decimal' representated the stardard numbers 0-9, if a script does not
     * exist
     ...
     *            String to convert
     * @param string $from
     *            Script to parse, see {@link Zend_Locale::getScriptList()} for
     *            details.
     * @param string $to
     ...
     * @param array $options
     *            Options: locale, precision. See {@link setOptions()} for
     *            details.
        ...
        if (strpos($input, $symbols['decimal']) !== false) {
            ...
            if (strlen($pre) >= $options['precision']) {
                $input = substr($input, 0, 
                        strlen($input) - strlen($pre) + $options['precision']);
            }
     ...
     * @param array $options
     *            Options: number_format, locale, precision. See {@link
    ...
    public static function toNumber ($value, array $options = array())
    {
        ...
        if ($format === null) {
            $format = Zend_Locale_Data::getContent($options['locale'], 
                    'decimalnumber');
            $format = self::_seperateFormat($format, $value, 
                    $options['precision']);
            
            if ($options['precision'] !== null) {
                $value = Zend_Locale_Math::normalize(
                        Zend_Locale_Math::round($value, $options['precision']));
            }
        } else {
            ...
            if (strpos($format, '.')) {
                if (is_numeric($options['precision'])) {
                    $value = Zend_Locale_Math::round($value, 
                            $options['precision']);
                } else {
                    if (substr($format, iconv_strpos($format, '.') + 1, 3) ==
                        ...
                        $options['precision'] = iconv_strlen(
                                iconv_substr($format, 
                                        iconv_strpos($format, '.') + 1, 
                                        iconv_strrpos($format, '0') -
                                                 iconv_strpos($format, '.')));
                        $format = iconv_substr($format, 0, 
                                iconv_strpos($format, '.') + 1) . '###' .
                                 iconv_substr($format, 
                                        iconv_strrpos($format, '0') + 1);
                    }
                }
            } else {
        ...
        $prec = call_user_func(Zend_Locale_Math::$sub, $value, $number, 
                $options['precision']);
        ...
        if (($options['precision'] + 2) > iconv_strlen($prec)) {
            $prec = str_pad((string) $prec, $options['precision'] + 2, "0", 
                    STR_PAD_RIGHT);
        }
        ...
        if (iconv_strpos($format, '.')) {
            ...
            $format = iconv_substr($format, 0, 
                    iconv_strlen($format) - iconv_strlen($rest));
        }
            ...
            if ($group == $group2) {
                ...
                for ($x = iconv_strlen($number); $x > $seperation; $x -= $seperation) {
                    if (iconv_substr($number, 0, $x - $seperation) !== "") {
                        $number = iconv_substr($number, 0, $x - $seperation) .
                                 $symbols['group'] .
                                 iconv_substr($number, $x - $seperation);
                    }
                }
                $format = iconv_substr($format, 0, iconv_strpos($format, '#')) .
                         $number . iconv_substr($format, $point);
            } else {
                
                // 2 seperations
                if (iconv_strlen($number) > ($point - $group)) {
                    $seperation = ($point - $group);
                    $number = iconv_substr($number, 0, 
                            iconv_strlen($number) - $seperation) .
                             $symbols['group'] .
                             iconv_substr($number, 
                                    iconv_strlen($number) - $seperation);
                    ...
                    if ((iconv_strlen($number) - 1) > ($point - $group + 1)) {
                        ...
                        for ($x = iconv_strlen($number) - $seperation2 - 2; $x >
                                 $seperation2; $x -= $seperation2) {
                            $number = iconv_substr($number, 0, 
                                    $x - $seperation2) . $symbols['group'] .
                             iconv_substr($number, $x - $seperation2);
                }
            }
        }
        $format = iconv_substr($format, 0, iconv_strpos($format, '#')) . $number .
                 iconv_substr($format, $point);
    }
// set negative sign
...
$regexs = array_merge($regexs, 
        Zend_Locale_Format::_getRegexForType('scientificnumber', $options));
                ...
                case '-#':
                    ...
                    if (($parts[$key + 1]) == '##0') {
                        ...
                        if (($parts[$key + 1]) == '##') {
                            ...
                            throw new Zend_Locale_Exception(
                                    'Unsupported token for numberformat (Pos 1):"' .
                                             $pattern . '"');
                        }
                    break;
                case '##':
                    if ($parts[$key + 1] == '##0') {
                        $regex[$pkey] .= '(\\' . $symbols['group'] .
                                 '{0,1}[0-9]{2})*';
                    } else {
                        throw new Zend_Locale_Exception(
                                'Unsupported token for numberformat (Pos 2):"' .
                                         $pattern . '"');
                    }
                    break;
                case '##0':
                    ...
                    throw new Zend_Locale_Exception(
                            'Unsupported token for numberformat (Pos 3):"' .
                                     $pattern . '"');
                }
            break;
        case '#0':
            if ($key == 0) {
                ...
                throw new Zend_Locale_Exception(
                        'Unsupported token for numberformat (Pos 4):"' . $pattern .
                                 '"');
            }
            break;
    }
...
if (($pattern == '#E0') || ($pattern == '#E00')) {
    $regex[$pkey] .= '[' . $symbols['plus'] . '+]{0,1}[0-9]{1,}(\\' .
             $symbols['decimal'] . '[0-9]{1,})*[eE][' . $symbols['plus'] .
             '+]{0,1}[0-9]{1,}';
} else 
    if (($pattern == '-#E0') || ($pattern == '-#E00')) {
        $regex[$pkey] .= '[' . $symbols['minus'] . '-]{0,1}[0-9]{1,}(\\' .
                 $symbols['decimal'] . '[0-9]{1,})*[eE][' . $symbols['minus'] .
                 '-]{0,1}[0-9]{1,}';
    } else {
        throw new Zend_Locale_Exception(
                'Unsupported token for numberformat (Pos 5):"' . $pattern . '"');
    }
}
...
if ($end == '###') {
    ...
    if ($end == '###-') {
        ...
        throw new Zend_Locale_Exception(
                'Unsupported token for numberformat (Pos 6):"' . $pattern . '"');
    }
}
 ...
 * Remember that Zend Date always returns localized string, so a month name
 * which returns the english
 * month in php's date() will return the translated month name with this
 * function...
 * use 'en' as locale
 * if you are in need of the original english names
 *
 * The conversion has the following restrictions:
 * 'a', 'A' - Meridiem is not explicit upper/lowercase, you have to
 * upper/lowercase the translated value yourself
 *
 * @param string $format
 ...
 * @param array $options
 *            Options: format_type, fix_date, locale, date_format. See {@link
 *            setOptions()} for details.
...
$result['date_format'] = $format; // save the format used to normalize $number
                                  // (convenience)
$result['locale'] = $options['locale']; // save the locale used to normalize
                                        // $number (convenience)
...
$position = self::_replaceMonth($number, 
        Zend_Locale_Data::getList($options['locale'], 'month'));
if ($position === false) {
    $position = self::_replaceMonth($number, 
            Zend_Locale_Data::getList($options['locale'], 'month', 
                    array(
...
require_once 'Zend/Locale/Exception.php';
throw new Zend_Locale_Exception(
    "Unknown date format, neither date nor time in '" . $format . "' found");
}
ksort($parse);

// get daytime
if (iconv_strpos($format, 'a') !== false) {
if (iconv_strpos(strtoupper($number), 
    strtoupper(Zend_Locale_Data::getContent($options['locale'], 'am'))) !== false) {
$am = true;
} else 
if (iconv_strpos(strtoupper($number), 
        strtoupper(Zend_Locale_Data::getContent($options['locale'], 'pm'))) !==
         false) {
    $am = false;
}
}
...
$result['fixed'] = 0; // nothing has been "fixed" by swapping date parts around
                      // (yet)
}
...
if (($position !== false) and
         ((iconv_strpos($date, $result['day']) === false) or
         (isset($result['year']) and
         (iconv_strpos($date, $result['year']) === false)))) {
    if ($options['fix_date'] !== true) {
        iconv_set_encoding('internal_encoding', $oenc);
        require_once 'Zend/Locale/Exception.php';
        throw new Zend_Locale_Exception(
                "Unable to parse date '$date' using '" . $format .
...
if ($result['day'] > 31) {
    if ($options['fix_date'] !== true) {
        ...
        require_once 'Zend/Locale/Exception.php';
        throw new Zend_Locale_Exception(
                "Unable to parse date '$date' using '" . $format . "' (d <> y)");
    }
    $temp = $result['year'];
...
if ($result['month'] > 31) {
    if ($options['fix_date'] !== true) {
        ...
        require_once 'Zend/Locale/Exception.php';
        throw new Zend_Locale_Exception(
                "Unable to parse date '$date' using '" . $format . "' (M <> y)");
    }
    $temp = $result['year'];
...
if ($result['month'] > 12) {
    if ($options['fix_date'] !== true || $result['month'] > 31) {
        ...
        require_once 'Zend/Locale/Exception.php';
        throw new Zend_Locale_Exception(
                "Unable to parse date '$date' using '" . $format . "' (M <> d)");
    }
    $temp = $result['day'];
    $result['day'] = $result['month'];
    $result['month'] = $temp;
...
if (((iconv_strlen($result['year']) == 2) && ($result['year'] < 10)) ||
     (((iconv_strpos($format, 'yy') !== false) &&
     (iconv_strpos($format, 'yyyy') === false)) ||
     ((iconv_strpos($format, 'YY') !== false) &&
     (iconv_strpos($format, 'YYYY') === false)))) {
...
// If no $locale was given, or $locale was invalid, do not use this identity
// mapping to normalize.
// Otherwise, translate locale aware month names in $number to their numeric
// equivalents.
$position = false;
...
/**
 * Returns if the given datestring contains all date parts from the given
 * format.
 * If no format is given, the default date format from the locale is used
 * If you want to check if the date is a proper date you should use
 * Zend_Date::isDate()
 *
 * @param string $date
 *            Date string
...
/**
 * Returns an array with 'hour', 'minute', and 'second' elements extracted from
 * $time
...
/**
 * Returns an array with 'year', 'month', 'day', 'hour', 'minute', and 'second'
 * elements
 ...
 * array ('year' => 1985, 'month' => 5, 'day' => 10, 'hour' => 11, 'minute' =>
 * 20, 'second' => 55)
 * The optional $locale parameter may be used to help extract times from strings
 * containing both a time and a day or month name.
 *
