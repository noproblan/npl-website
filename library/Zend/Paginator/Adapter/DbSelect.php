 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Paginator_Adapter_DbSelect implements 
        Zend_Paginator_Adapter_Interface
{
    ...
    public function setRowCount ($rowCount)
    {
        if ($rowCount instanceof Zend_Db_Select) {
            ...
            $rowCountColumn = $this->_select->getAdapter()->foldCase(
                    self::ROW_COUNT_COLUMN);
            
            // The select query can contain only one column, which should be the
            // row count column
     ...
     *
     * TODO: Have a look at queries that have both GROUP BY and DISTINCT
     * specified.
    ...
    public function getCountSelect ()
    {
        ...
        if (! empty($unionParts)) {
             ...
             */
            if (($isDistinct && count($columnParts) > 1) ||
                     count($groupParts) > 1 || ! empty($havingParts)) {
                ...
                if ($isDistinct) {
                    ...
                    if ($part[1] !== Zend_Db_Select::SQL_WILDCARD &&
                        ...
                        if (! empty($part[0])) {
                            $column = $db->quoteIdentifier($part[0], true) . '.' .
                                     $column;
                        }
