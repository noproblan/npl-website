...
class Zend_Measure_Number extends Zend_Measure_Abstract
{
    ...
    /**
     * Zend_Measure_Abstract is an abstract class for the different measurement
     * types
     *
     * @param integer $value
     *            Value
    ...
    public function __construct ($value, $type, $locale = null)
    {
        ...
        if (! Zend_Locale::isLocale($locale, true, false)) {
            if (! Zend_Locale::isLocale($locale, true, false)) {
                ...
                throw new Zend_Measure_Exception(
                        "Language (" . (string) $locale . ") is unknown");
            }
    ...
    public function setValue ($value, $type = null, $locale = null)
    {
        ...
        switch ($type) {
            ...
            case 'HEXADECIMAL':
                preg_match('/[0123456789ABCDEF]+/', strtoupper($value), 
                        $ergebnis);
                $value = $ergebnis[0];
                break;
            ...
            default:
                try {
                    $value = Zend_Locale_Format::getInteger($value, 
                            array(
                                    'locale' => $locale
                            ));
                } catch (Exception $e) {
                ...
                if (call_user_func(Zend_Locale_Math::$comp, $value, 0) < 0) {
                    $value = call_user_func(Zend_Locale_Math::$sqrt, 
                            call_user_func(Zend_Locale_Math::$pow, $value, 2));
                }
                break;
        }
    ...
    private function _toDecimal ($input, $type)
    {
            ...
            for ($x = 0; $x < $length; ++ $x) {
                ...
                $value = call_user_func(Zend_Locale_Math::$add, $value, 
                        call_user_func(Zend_Locale_Math::$mul, $split[$x], 
                                call_user_func(Zend_Locale_Math::$pow, 
                                        $this->_units[$type][0], 
                                        ($length - $x - 1))));
            }
        }
        ...
        if ($type === 'ROMAN') {
            ...
            $input = preg_replace(array_keys(self::$_romanconvert), 
                    array_values(self::$_romanconvert), $input);
            ...
            for ($x = 0; $x < sizeof($split); $x ++) {
                ...
                if (($x > 0 and ($split[$x - 1] != '/') and
                         ($num < self::$_roman[$split[$x - 1]]))) {
                    $num -= $num;
                }
    ...
    private function _fromDecimal ($value, $type)
    {
        ...
        if ($this->_units[$type][0] <= 16) {
            ...
            while (call_user_func(Zend_Locale_Math::$comp, $value, 0, 25) != 0) {
                ...
                $value = call_user_func(Zend_Locale_Math::$sub, $value, $target, 
                        0);
                $value = call_user_func(Zend_Locale_Math::$div, $value, $base, 
                        0);
        ...
        if ($type === 'ROMAN') {
            ...
            $newvalue = str_replace('/', '', 
                    preg_replace(array_values(self::$_romanconvert), 
                            array_keys(self::$_romanconvert), $newvalue));
        }
