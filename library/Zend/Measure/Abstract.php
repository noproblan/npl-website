...
abstract class Zend_Measure_Abstract
{
    ...
    public function getValue ($round = -1, $locale = null)
    {
        ...
        if ($locale !== null) {
            ...
            return Zend_Locale_Format::toNumber($return, 
                    array(
                            'locale' => $locale
                    ));
        }
    ...
    public function setType ($type)
    {
        ...
        if (empty($this->_type)) {
            ...
            if (is_array($this->_units[$this->getType()][0])) {
                foreach ($this->_units[$this->getType()][0] as $key => $found) {
                    switch ($key) {
                        case "/":
                            if ($found != 0) {
                                $value = call_user_func(Zend_Locale_Math::$div, 
                                        $value, $found, 25);
                            }
                            break;
                        case "+":
                            $value = call_user_func(Zend_Locale_Math::$add, 
                                    $value, $found, 25);
                            break;
                        case "-":
                            $value = call_user_func(Zend_Locale_Math::$sub, 
                                    $value, $found, 25);
                            break;
                        default:
                            $value = call_user_func(Zend_Locale_Math::$mul, 
                                    $value, $found, 25);
                            break;
                    }
                }
            } else {
                $value = call_user_func(Zend_Locale_Math::$mul, $value, 
                        $this->_units[$this->getType()][0], 25);
            }
            ...
            if (is_array($this->_units[$type][0])) {
                ...
                $value = call_user_func(Zend_Locale_Math::$div, $value, 
                        $this->_units[$type][0], 25);
            }
    ...
    public function toString ($round = -1, $locale = null)
    {
        ...
        return $this->getValue($round, $locale) . ' ' .
                 $this->_units[$this->getType()][1];
    }
