...
class Zend_Json
{
     ...
     *            Optional; whether or not to check for object recursion; off by
     *            default
     * @param array $options
            ...
            $options = array())
    {
        ...
        if (isset($options['enableJsonExprFinder']) &&
            ...
            $valueToEncode = self::_recursiveJsonExprFinder($valueToEncode, 
        ...
        if (function_exists('json_encode') &&
            ...
            require_once 'Zend/Json/Encoder.php';
            $encodedResult = Zend_Json_Encoder::encode($valueToEncode, 
                    $cycleCheck, $options);
        }
        ...
        if (count($javascriptExpressions) > 0) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                $encodedResult = str_replace(
                        // instead of replacing "key:magicKey", we replace
                        // directly magicKey by value because "key" never
                        // changes.
     ...
     * @return void
     */
    protected static function _recursiveJsonExprFinder (&$value, 
            array &$javascriptExpressions, $currentKey = null)
    {
        if ($value instanceof Zend_Json_Expr) {
            // TODO: Optimize with ascii keys, if performance is bad
            $magicKey = "____" . $currentKey . "_" .
                     (count($javascriptExpressions));
            $javascriptExpressions[] = array(
                    
                    // if currentKey is integer, encodeUnicodeString call is not
                    // required.
                    "magicKey" => (is_int($currentKey)) ? $magicKey : Zend_Json_Encoder::encodeUnicodeString(
                            $magicKey),
     ...
     * caller.
     *
     * Throws a Zend_Json_Exception if the XML tree is deeper than the allowed
     * limit.
     *
     * @param SimpleXMLElement $simpleXmlElementObject            
            ...
            $ignoreXmlAttributes, $recursionDepth = 0)
    {
        ...
        if (count($children) == 0) {
            ...
            foreach ($children as $child) {
                ...
                $element = self::_processXml($child, $ignoreXmlAttributes, 
                        $recursionDepth + 1);
     ...
     * NOTE: Encoding native javascript expressions via Zend_Json_Expr is not
     * possible.
     *
     * @static
     *
            ...
            $ignoreXmlAttributes = true)
    {
        ...
        $resultArray = self::_processXml($simpleXmlElementObject, 
                $ignoreXmlAttributes);
     ...
     *
     * Use 'format' option to select output format - currently html and txt
     * supported, txt is default
    ...
    public static function prettyPrint ($json, $options = array())
    {
        ...
        foreach ($tokens as $token) {
            ...
            if (! $inLiteral && ($token == '{' || $token == '[')) {
                $indent ++;
                if (($result != '') &&
                         ($result[(strlen($result) - 1)] == $lineBreak)) {
                    $result .= $prefix;
                }
                ...
                if ((substr_count($token, "\"") - substr_count($token, "\\\"")) %
                         2 != 0) {
                    $inLiteral = ! $inLiteral;
                }
            }
