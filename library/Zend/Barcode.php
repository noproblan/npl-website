...
class Zend_Barcode
{
     ...
     *            OPTIONAL; an array or Zend_Config object with barcode
     *            parameters.
     * @param mixed $rendererConfig
     *            OPTIONAL; an array or Zend_Config object with renderer
     *            parameters.
    ...
    public static function factory ($barcode, $renderer = 'image', 
            $barcodeConfig = array(), $rendererConfig = array(), $automaticRenderError = true)
    {
        /*
    ...
    public static function makeBarcode ($barcode, $barcodeConfig = array())
    {
        ...
        if ($barcode instanceof Zend_Config) {
            if (isset($barcode->barcodeParams) &&
                     $barcode->barcodeParams instanceof Zend_Config) {
                $barcodeConfig = $barcode->barcodeParams->toArray();
            }
            if (isset($barcode->barcode)) {
                $barcode = (string) $barcode->barcode;
        ...
        $barcodeName = str_replace(' ', '_', 
                ucwords(str_replace('_', ' ', $barcodeName)));
    ...
    public static function makeRenderer ($renderer = 'image', $rendererConfig = array())
    {
        ...
        $rendererName = str_replace(' ', '_', 
                ucwords(str_replace('_', ' ', $rendererName)));
     ...
     */
    public static function render ($barcode, $renderer, $barcodeConfig = array(), 
     ...
     * @return mixed
     */
    public static function draw ($barcode, $renderer, $barcodeConfig = array(), 
            $rendererConfig = array())
    {
        return self::factory($barcode, $renderer, $barcodeConfig, 
                $rendererConfig)->draw();
    }
