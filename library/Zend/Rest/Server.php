...
class Zend_Rest_Server implements Zend_Server_Interface
{
    ...
    /**
     * Whether or not {@link handle()} should send output or return the
     * response.
     * 
     ...
     *
     * @param boolean $flag            
     * @return boolean|Zend_Rest_Server Returns Zend_Rest_Server when used to
     *         set the flag; returns boolean flag value otherwise.
     */
                ...
                if ($this->_functions[$this->_method] instanceof Zend_Server_Reflection_Function ||
                    ...
                    foreach ($func_args as $arg) {
                        if (isset($request[strtolower($arg->getName())])) {
                            $calling_args[] = $request[strtolower(
                                    $arg->getName())];
                    ...
                    foreach ($request as $key => $value) {
                        if (substr($key, 0, 3) == 'arg') {
                            ...
                            if (($index = array_search($key, $missing_args)) !==
                                     false) {
                    ...
                    if (! $result &&
                        ...
                        if ($this->_functions[$this->_method]->isStatic()) {
                            // for some reason, invokeArgs() does not work the
                            // same as
                            // invoke(), and expects the first argument to be an
                            // object.
                            // So, using a callback if the method is static.
                            $result = $this->_callStaticMethod($class, 
                                    $calling_args);
                        } else {
                            // Object method
                            $result = $this->_callObjectMethod($class, 
                                    $calling_args);
                        }
                    } elseif (! $result) {
                        try {
                            $result = call_user_func_array(
                                    ...
                                    $calling_args); // $this->_functions[$this->_method]->invokeArgs($calling_args);
                        } catch (Exception $e) {
                            $result = $this->fault($e);
                        }
                    }
                    ...
                    $result = $this->fault(
                            new Zend_Rest_Server_Exception(
                                    "Unknown Method '$this->_method'."), 404);
                }
            } else {
                require_once "Zend/Rest/Server/Exception.php";
                $result = $this->fault(
     ...
     * @return void
     */
    protected function _structValue ($struct, DOMDocument $dom, 
            DOMElement $parent)
    {
        $struct = (array) $struct;
    ...
    public function addFunction ($function, $namespace = '')
    {
        ...
        foreach ($function as $func) {
            if (is_callable($func) && ! in_array($func, self::$magicMethods)) {
                $this->_functions[$func] = $this->_reflection->reflectFunction(
                        $func);
            } else {
                require_once 'Zend/Rest/Server/Exception.php';
    ...
    protected function _callObjectMethod ($class, array $args)
    {
        try {
            if ($this->_functions[$this->_method]->getDeclaringClass()->getConstructor()) {
                $object = $this->_functions[$this->_method]->getDeclaringClass()->newInstanceArgs(
                        $this->_args);
            } else {
            ...
            require_once 'Zend/Rest/Server/Exception.php';
            throw new Zend_Rest_Server_Exception(
                    'Error instantiating class ' . $class . ' to invoke method ' .
                             $this->_functions[$this->_method]->getName() . ' (' .
                             $e->getMessage() . ') ', 500, $e);
        }
        
        try {
            $result = $this->_functions[$this->_method]->invokeArgs($object, 
                    $args);
        } catch (Exception $e) {
            $result = $this->fault($e);
        }
