...
class Zend_Serializer_Adapter_PythonPickle extends Zend_Serializer_Adapter_AdapterAbstract
{
    ...
    const OP_POP_MARK = '1'; // discard stack top through topmost
                                    // markobject

    const OP_DUP = '2'; // duplicate top stack item

    const OP_FLOAT = 'F'; // push float object; decimal string
                                    // argument

    const OP_INT = 'I'; // push integer or bool; decimal string
                                    // argument
    ...
    const OP_PERSID = 'P'; // push persistent object; id is taken from
                                    // string arg
    ...
    const OP_STRING = 'S'; // push string; NL-terminated string
                                    // argument

    const OP_BINSTRING = 'T'; // push string; counted binary string
                                    // argument

    const OP_SHORT_BINSTRING = 'U'; // " " ; " " " " < 256 bytes

    const OP_UNICODE = 'V'; // push Unicode string;
                                    // raw-unicode-escaped'd argument
    ...
    const OP_GLOBAL = 'c'; // push self.find_class(modname, name); 2
                                    // string args
    ...
    const OP_APPENDS = 'e'; // extend list on stack by topmost stack
                                    // slice

    const OP_GET = 'g'; // push item from memo on stack; index is
                                    // string arg
    ...
    const OP_LONG_BINGET = 'j'; // push item from memo on stack; index is
                                    // 4-byte arg
    ...
    const OP_PUT = 'p'; // store stack top in memo; index is string
                                    // arg
    ...
    const OP_SETITEMS = 'u'; // modify dict by adding topmost key+value
                                    // pairs
    ...
    const OP_NEWOBJ = "\x81"; // build object by applying cls.__new__
                                       // to argtuple

    const OP_EXT1 = "\x82"; // push object from extension registry;
                                       // 1-byte index
    ...
    const OP_TUPLE2 = "\x86"; // build 2-tuple from two topmost stack
                                       // items

    const OP_TUPLE3 = "\x87"; // build 3-tuple from three topmost stack
                                       // items
    ...
    const OP_BINBYTES = 'B'; // push bytes; counted binary string
                                    // argument
    ...
    protected function _checkProtocolNumber ($number)
    {
        ...
        if ($int < 0 || $int > 3) {
            ...
            throw new Zend_Serializer_Exception(
                    'Invalid or unknown protocol version "' . $number . '"');
        }
        return $int;
    }
        ...
        if ($value === null) {
            ...
            throw new Zend_Serializer_Exception(
                    'PHP-Type "' . gettype($value) .
                             '" isn\'t serializable with ' . get_class($this));
        }
    }
    ...
    protected function _writeArrayDict (array $value)
    {
        ...
        foreach ($value as $k => $v) {
            $this->_pickle .= $this->_write($k) . $this->_write($v) .
                     self::OP_SETITEM;
        }
    }
    ...
    protected function _hex2Utf8 ($hex)
    {
        ...
        if ($uniCode < 0x80) { // 1Byte
            ...
            require_once 'Zend/Serializer/Exception.php';
            throw new Zend_Serializer_Exception(
                    'Unsupported unicode character found "' . dechex($uniCode) .
                             '"');
        }
     ...
     *
     * @return void
     * @throws Zend_Serializer_Exception if Pickle version does not support this
     *         feature
     */
