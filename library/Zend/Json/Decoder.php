...
class Zend_Json_Decoder
{
     ...
     *            {@link Zend_Json::TYPE_ARRAY} and {@link
     *            Zend_Json::TYPE_OBJECT} for
     *            valid values
     * @return void
     */
    ...
    protected function _decodeObject ()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                require_once 'Zend/Json/Exception.php';
                throw new Zend_Json_Exception(
                        'Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                require_once 'Zend/Json/Exception.php';
                throw new Zend_Json_Exception(
                        'Missing ":" in object encoding: ' . $this->_source);
            }
    ...
    protected function _decodeArray ()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                require_once 'Zend/Json/Exception.php';
                throw new Zend_Json_Exception(
                        'Missing "," in array encoding: ' . $this->_source);
            }
    ...
    {
        if (preg_match('/([\t\b\f\n\r ])*/s', $this->_source, $matches, 
                PREG_OFFSET_CAPTURE, $this->_offset) &&
    ...
    protected function _getNextToken ()
    {
        ...
        switch ($str{$i}) {
            ...
            case '"':
                ...
                // $this->_tokenValue = substr($str, $start + 1, $i - $start -
                // 1);
            ...
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s', 
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        require_once 'Zend/Json/Exception.php';
                        throw new Zend_Json_Exception(
                                "Octal notation not supported by JSON (value: $datum)");
                    } else {
                        $val = intval($datum);
                        $fVal = floatval($datum);
