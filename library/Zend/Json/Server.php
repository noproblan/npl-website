...
class Zend_Json_Server extends Zend_Server_Abstract
{
    ...
    public function addFunction ($function, $namespace = '')
    {
        ...
        if (! is_callable($function)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new Zend_Json_Server_Exception(
                    'Unable to attach function; does not exist');
        }
        ...
        if (is_string($function)) {
            $method = Zend_Server_Reflection::reflectFunction($function, $argv, 
                    $namespace);
        } else {
            $class = array_shift($function);
            $action = array_shift($function);
            $reflection = Zend_Server_Reflection::reflectClass($class, $argv, 
                    $namespace);
    ...
    {
        if ((false !== $request) &&
                 (! $request instanceof Zend_Json_Server_Request)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new Zend_Json_Server_Exception(
                    'Invalid request type provided; cannot handle');
        } elseif ($request) {
    ...
    public function loadFunctions ($definition)
    {
        if (! is_array($definition) &&
            ...
            require_once 'Zend/Json/Server/Exception.php';
            throw new Zend_Json_Server_Exception(
                    'Invalid definition provided to loadFunctions()');
        }
     ...
     * @return void
     */
    protected function _addMethodServiceMap (
            Zend_Server_Method_Definition $method)
    {
        $serviceInfo = array(
                'name' => $method->getName(),
