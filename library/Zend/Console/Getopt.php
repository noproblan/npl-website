...
class Zend_Console_Getopt
{
     ...
     *
     * @param array $argv            
     * @throws Zend_Console_Getopt_Exception When not given an array as
     *         parameter
        ...
        while (count($argv) > 0) {
            ...
            if (substr($argv[0], 0, 2) == '--') {
                ...
                if (substr($argv[0], 0, 1) == '-' &&
                    ...
                    if ($this->_getoptConfig[self::CONFIG_PARSEALL]) {
                        $this->_remainingArgs[] = array_shift($argv);
                    } else {
                        /*
                         * We should put all other arguments in _remainingArgs
                         * and stop parsing
                         * since CONFIG_PARSEALL is false.
                         */
                        $this->_remainingArgs = array_merge(
                                $this->_remainingArgs, $argv);
                        break;
                    }
        }
        $this->_parsed = true;
    ...
    protected function _addRulesModeZend ($rules)
    {
        foreach ($rules as $ruleCode => $helpMessage) {
            ...
            // are any complaints that =string will not work (even though that
            // use
