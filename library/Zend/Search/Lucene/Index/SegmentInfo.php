 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Search_Lucene_Index_SegmentInfo implements 
        Zend_Search_Lucene_Index_TermsStream_Interface
{
     ...
     *
     * If filter selectivity is less than this value, then full scan is
     * performed
     ...
     *
     * Array of arrays (Zend_Search_Lucene_Index_Term objects are represented as
     * arrays because
     * of performance considerations)
     * [0] -> $termValue
     * [1] -> $termFieldNum
     *
     * Corresponding Zend_Search_Lucene_Index_TermInfo object stored in the
     ...
     *
     * Array of arrays (Zend_Search_Lucene_Index_TermInfo objects are
     * represented as arrays because
     * of performance considerations)
     * [0] -> $docFreq
     * [1] -> $freqPointer
     * [2] -> $proxPointer
    ...
    /**
     * Associative array where the key is the file name and the value is data
     * offset
    ...
    /**
     * Associative array where the key is the file name and the value is file
     * size (.csf).
     *
     * @var array
     */
     ...
     * - 'offset' => $docStoreOffset The starting document in the shared doc
     * store files where this segment's documents begin
     * - 'segment' => $docStoreSegment The name of the segment that has the
     * shared doc store files.
     * - 'isCompound' => $docStoreIsCompoundFile True, if compound file format
     * is used for the shared doc store files (.cfx file).
     */
    ...
    public function __construct (Zend_Search_Lucene_Storage_Directory $directory, 
            $name, $docCount, $delGen = 0, $docStoreOptions = null, 
            $hasSingleNormFile = false, $isCompound = null)
    {
        ...
        if ($docStoreOptions !== null) {
            ...
            if ($docStoreOptions['isCompound']) {
                $cfxFile = $this->_directory->getFileObject(
                        $docStoreOptions['segment'] . '.cfx');
                ...
                if ($count != 0) {
                    $cfxFileSizes[$fileName] = $this->_directory->fileLength(
                            $docStoreOptions['segment'] . '.cfx') - $dataOffset;
                }
        ...
        if ($isCompound !== null) {
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'is not readable') !== false) {
                    ...
                    throw new Zend_Search_Lucene_Exception($e->getMessage(), 
                            $e->getCode(), $e);
                }
            }
        }
        ...
        if ($this->_isCompound) {
            ...
            for ($count = 0; $count < $segFilesCount; $count ++) {
                ...
                if ($count != 0) {
                    $this->_segFileSizes[$fileName] = $dataOffset -
                             end($this->_segFiles);
                }
            ...
            if ($count != 0) {
                $this->_segFileSizes[$fileName] = $this->_directory->fileLength(
                        $name . '.cfs') - $dataOffset;
            }
        }
        ...
        for ($count = 0; $count < $fieldsCount; $count ++) {
            ...
            $this->_fields[$count] = new Zend_Search_Lucene_Index_FieldInfo(
                    $fieldName, $fieldBits & 0x01 /* field is indexed */,
    ...
    {
        $delFile = $this->_directory->getFileObject(
                $this->_name . '_' . base_convert($this->_delGen, 10, 36) .
                         '.del');
        ...
        if ($format == (int) 0xFFFFFFFF) {
            ...
            $delFileSize = $this->_directory->fileLength(
                    $this->_name . '_' . base_convert($this->_delGen, 10, 36) .
                             '.del');
            $byteNum = 0;
    ...
    public function openCompoundFile ($extension, $shareHandler = true)
    {
        if (($extension == '.fdx' || $extension == '.fdt') &&
            ...
            if (! $this->_sharedDocStoreOptions['isCompound']) {
                $fdxFile = $this->_directory->getFileObject($fdxFName, 
                        $shareHandler);
                $fdxFile->seek($this->_sharedDocStoreOptions['offset'] * 8, 
                        SEEK_CUR);
                ...
                if ($extension == '.fdx') {
                    ...
                    $fdtFile = $this->_directory->getFileObject($fdtFName, 
                            $shareHandler);
            ...
            $cfxFile = $this->_directory->getFileObject(
                    $this->_sharedDocStoreOptions['segment'] . '.cfx', 
                    $shareHandler);
            ...
            $cfxFile->seek($this->_sharedDocStoreOptions['offset'] * 8, 
                    SEEK_CUR);
            ...
            if ($extension == '.fdx') {
                ...
                $cfxFile->seek(
                        $this->_sharedDocStoreOptions['files'][$fdtFName]);
                // Seek to the start of current segment documents section
        ...
        if (! isset($this->_segFiles[$filename])) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Segment compound file doesn\'t contain ' . $filename .
                             ' file.');
        }
    ...
    public function compoundFileLength ($extension)
    {
        ...
        if (! isset($this->_segFileSizes[$filename])) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Index compound file doesn\'t contain ' . $filename .
                             ' file.');
        }
     ...
     *
     * Size is 1024.
     * Numbers are used instead of class constants because of performance
     * considerations
     *
     * @var array
     */
    ...
    private function _loadDictionaryIndex ()
    {
        ...
        if ($this->_directory->fileExists($this->_name . '.sti')) {
            ...
            $stiFileData = $stiFile->readBytes(
                    $this->_directory->fileLength($this->_name . '.sti'));
        ...
        // Load dictionary index data
        list ($this->_termDictionary, $this->_termDictionaryInfos) = Zend_Search_Lucene_Index_DictionaryLoader::load(
                $tiiFileData);
        
        $stiFileData = serialize(
                array(
    ...
    public function getTermInfo (Zend_Search_Lucene_Index_Term $term)
    {
        ...
        while ($highIndex >= $lowIndex) {
            ...
            if ($delta < 0) {
                ...
                $termInfo = new Zend_Search_Lucene_Index_TermInfo($a[0], $a[1], 
                        $a[2], $a[3], $a[4]);
        ...
        $tisFile->seek(
                $prevTermInfo[4] /* indexPointer */ - (($tiVersion == (int) 0xFFFFFFFD) ? 24 : 20) /* header size*/, SEEK_CUR);
        ...
        for ($count = $prevPosition * $indexInterval + 1; $count <= $termCount && ($this->_getFieldPosition(
                $termFieldNum) < $searchDicField || ($this->_getFieldPosition(
                $termFieldNum) == $searchDicField &&
            ...
            $termValue = Zend_Search_Lucene_Index_Term::getPrefix($termValue, 
        ...
        if ($termFieldNum == $searchField && $termValue == $term->text) {
            $termInfo = new Zend_Search_Lucene_Index_TermInfo($docFreq, 
     ...
     * @return array
     */
    public function termDocs (Zend_Search_Lucene_Index_Term $term, $shift = 0, 
            $docsFilter = null)
    {
        ...
        if (! $termInfo instanceof Zend_Search_Lucene_Index_TermInfo) {
            if ($docsFilter !== null &&
                     $docsFilter instanceof Zend_Search_Lucene_Index_DocsFilter) {
        ...
        if ($docsFilter !== null) {
            ...
            if (isset($docsFilter->segmentFilters[$this->_name])) {
                ...
                // Make short name for the filter (which doesn't need additional
                // dereferencing)
                ...
                if ($this->_docCount / count($filter) <
                         self::FULL_SCAN_VS_FETCH_BOUNDARY) {
                    // Perform fetching
                    // ---------------------------------------------------------------
                    ...
                    for ($count = 0; $count < $termInfo->docFreq; $count ++) {
                        ...
                        if (isset($filter[$docId])) {
                            $result[] = $shift + $docId;
                            $updatedFilterData[$docId] = 1; // 1 is just a some
                                                            // constant value,
                                                            // so we don't need
                                                            // additional var
                                                            // dereference here
                ...
                // Filter is present, but doesn't has data for the current
                // segment yet
                $filterData = array();
                for ($count = 0; $count < $termInfo->docFreq; $count ++) {
                    $docDelta = $frqFile->readVInt();
                    ...
                    $result[] = $shift + $docId;
                    $filterData[$docId] = 1; // 1 is just a some constant value,
                                             // so we don't need additional var
                                             // dereference here
                }
            ...
            if (isset($docsFilter->segmentFilters[$this->_name])) {
                ...
                if ($this->_docCount / count($filter) <
                    ...
                    for ($count = 0; $count < $termInfo->docFreq; $count ++) {
                        ...
                        if ($docDelta % 2 == 1) {
                            ...
                            if (isset($filter[$docId])) {
                                ...
                                $updatedFilterData[$docId] = 1; // 1 is just a
                                                                // some constant
                                                                // value, so we
                                                                // don't need
                                                                // additional
                    ...
                    for ($count = 0; $count < $termInfo->docFreq; $count ++) {
                        ...
                        if ($docDelta % 2 == 1) {
                            ...
                            if (isset($filter[$docId])) {
                                $result[$shift + $docId] = 1;
                                $updatedFilterData[$docId] = 1; // 1 is just
                                                                // some constant
                                                                // value, so we
                                                                // don't need
                                                                // additional
                ...
                for ($count = 0; $count < $termInfo->docFreq; $count ++) {
                    ...
                    if ($docDelta % 2 == 1) {
                        $docId += ($docDelta - 1) / 2;
                        $result[$shift + $docId] = 1;
                        $filterData[$docId] = 1; // 1 is just a some constant
                                                 // value, so we don't need
                                                 // additional var dereference
                                                 // here
                    } else {
                        $docId += $docDelta / 2;
            ...
            if (isset($docsFilter->segmentFilters[$this->_name])) {
                ...
                if ($this->_docCount / count($filter) <
                    ...
                    foreach ($freqs as $docId => $freq) {
                        ...
                        // we have to read .prx file to get right position for
                        // next doc
                        ...
                        // Include into updated filter and into result only if
                        // doc is matched by filter
                        if (isset($filter[$docId])) {
                            $updatedFilterData[$docId] = 1; // 1 is just a some
                                                            // constant value,
                                                            // so we don't need
        ...
        if ($this->_hasSingleNormFile) {
            ...
            foreach ($this->_fields as $fNum => $fieldInfo) {
                if ($fieldInfo->isIndexed) {
                    $this->_norms[$fNum] = $normfFile->readBytes(
                            $this->_docCount);
                }
            }
        } else {
    ...
    public function norm ($id, $fieldName)
    {
        ...
        return Zend_Search_Lucene_Search_Similarity::decodeNorm(
                ord($this->_norms[$fieldNum][$id]));
    }
            ...
            return str_repeat(
                    chr(
                            $similarity->encodeNorm(
                                    $similarity->lengthNorm($fieldName, 0))), 
                    $this->_docCount);
        }
     ...
     *
     * Is actualy used from writeChanges() method or from the constructor if
     * it's invoked from
     * Index writer. In both cases index write lock is already obtained, so we
     * shouldn't care
     * about it
     *
     * @return integer
    ...
    private function _detectLatestDelGen ()
    {
        ...
        foreach ($this->_directory->fileList() as $file) {
            if ($file == $this->_name . '.del') {
                ...
                if (preg_match('/^' . $this->_name . '_([a-zA-Z0-9]+)\.del$/i', 
                        $file, $matches)) {
        ...
        if (count($delFileList) == 0) {
            ...
            // There are some deletions files for current segment in the
            // directory
     ...
     *
     * This method must be invoked only from the Writer _updateSegments()
     * method,
    ...
    public function writeChanges ()
    {
        ...
        $delFile = $this->_directory->createFile(
                $this->_name . '_' . base_convert($this->_delGen, 10, 36) .
                         '.del');
     ...
     * self::SM_FULL_INFO - terms are scanned, frequency and position info is
     * retrieved
     * self::SM_MERGE_INFO - terms are scanned, frequency and position info is
     * retrieved
     ...
     * Scan modes
     */
    const SM_TERMS_ONLY = 0; // terms are scanned, no additional info is
                             // retrieved

    const SM_FULL_INFO = 1; // terms are scanned, frequency and position info
                             // is retrieved

    const SM_MERGE_INFO = 2; // terms are scanned, frequency and position info
                             // is retrieved
                             // document numbers are compacted (shifted if
                             // segment contains deleted documents)
    ...
    {
        /**
         * SegmentInfo->resetTermsStream() method actually takes two optional
         * parameters:
        ...
        $this->_indexInterval = $this->_tisFile->readInt(); // Read Index
                                                            // interval
        $this->_skipInterval = $this->_tisFile->readInt(); // Read skip
                                                            // interval
        if ($tiVersion == (int) 0xFFFFFFFD /* 2.1+ format */) {
        ...
        switch ($mode) {
            ...
            case self::SM_MERGE_INFO:
                ...
                for ($count = 0; $count < $this->_docCount; $count ++) {
                    if (! $this->isDeleted($count)) {
                        $this->_docMap[$count] = $startId +
                                 (($mode == self::SM_MERGE_INFO) ? count(
                                        $this->_docMap) : $count);
                    }
                }
                break;
            ...
            default:
                require_once 'Zend/Search/Lucene/Exception.php';
                throw new Zend_Search_Lucene_Exception(
                        'Wrong terms scaning mode specified.');
                break;
        }
        
        // Calculate next segment start id (since $this->_docMap structure may
        // be cleaned by $this->nextTerm() call)
        $nextSegmentStartId = $startId +
                 (($mode == self::SM_MERGE_INFO) ? count($this->_docMap) : $this->_docCount);
        $this->nextTerm();
    ...
    public function skipTo (Zend_Search_Lucene_Index_Term $prefix)
    {
        ...
        if ($this->_tisFile === null) {
            // The end of terms stream is reached and terms dictionary file is
            // closed
            // Perform mini-reset operation
            $this->_tisFile = $this->openCompoundFile('.tis', false);
            
            if ($this->_termsScanMode == self::SM_FULL_INFO ||
                     $this->_termsScanMode == self::SM_MERGE_INFO) {
                $this->_frqFile = $this->openCompoundFile('.frq', false);
                $this->_prxFile = $this->openCompoundFile('.prx', false);
            }
        }
        $this->_tisFile->seek($this->_tisFileOffset + $prevTermInfo[4], 
                SEEK_SET);
        
        $this->_lastTerm = new Zend_Search_Lucene_Index_Term($prevTerm[1] /* text */,
                                                                 ($prevTerm[0] == - 1) ? '' : $this->_fields[$prevTerm[0] /*
                                                                         * field
                                                                         */
]->name);
        ...
        $this->_termCount = $this->_termNum - $prevPosition *
                 $this->_indexInterval;
        ...
        if ($highIndex == 0) {
            ...
            if ($prefix->field == $this->_lastTerm->field &&
                     $prefix->text == $this->_lastTerm->text) {
            ...
            $this->_frqFile->seek(
                    $this->_lastTermInfo->freqPointer + $this->_frqFileOffset, 
                    SEEK_SET);
            $freqs = array();
            $docId = 0;
            ...
            $this->_prxFile->seek(
                    $this->_lastTermInfo->proxPointer + $this->_prxFileOffset, 
                    SEEK_SET);
            foreach ($freqs as $docId => $freq) {
                $termPosition = 0;
                $positions = array();
...
$termValue = Zend_Search_Lucene_Index_Term::getPrefix($this->_lastTerm->text, 
        $termPrefixLength) . $termSuffix;

$this->_lastTerm = new Zend_Search_Lucene_Index_Term($termValue, 
        $this->_fields[$termFieldNum]->name);
...
$this->_lastTermInfo = new Zend_Search_Lucene_Index_TermInfo($docFreq, 
        $freqPointer, $proxPointer, $skipOffset);
...
if ($this->_termsScanMode == self::SM_FULL_INFO ||
    ...
    $this->_frqFile->seek(
            $this->_lastTermInfo->freqPointer + $this->_frqFileOffset, SEEK_SET);
    $freqs = array();
    $docId = 0;
    for ($count = 0; $count < $this->_lastTermInfo->docFreq; $count ++) {
        $docDelta = $this->_frqFile->readVInt();
    ...
    $this->_prxFile->seek(
            $this->_lastTermInfo->proxPointer + $this->_prxFileOffset, SEEK_SET);
    foreach ($freqs as $docId => $freq) {
        $termPosition = 0;
        $positions = array();
