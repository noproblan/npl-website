        ...
        foreach ($document->getFieldNames() as $fieldName) {
            ...
            if ($field->isIndexed) {
                if ($field->isTokenized) {
                    ...
                    if ($tokenCounter == 0) {
                        // Field contains empty value. Treat it as non-indexed
                        // and non-tokenized
                        $field = clone ($field);
                        ...
                        $docNorms[$field->name] = chr(
                                $similarity->encodeNorm(
                                        $similarity->lengthNorm($field->name, 
                                                $tokenCounter) * $document->boost *
                                                 $field->boost));
                    }
                } else 
