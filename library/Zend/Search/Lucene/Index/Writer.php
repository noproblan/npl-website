...
class Zend_Search_Lucene_Index_Writer
{
     ...
     * @todo Implement Zend_Search_Lucene_Storage_DirectoryRAM and
     *       Zend_Search_Lucene_Storage_FileRAM to use it for
     ...
     *
     * It's a reference to the corresponding Zend_Search_Lucene::$_segmentInfos
     * array
     *
    ...
    public static function createIndex (
            Zend_Search_Lucene_Storage_Directory $directory, $generation, 
            $nameCount)
    {
        if ($generation == 0) {
            ...
            foreach ($directory->fileList() as $file) {
                if ($file == 'deletable' || $file == 'segments' ||
                         isset(
                                self::$_indexExtensions[substr($file, 
                                        strlen($file) - 4)]) ||
            ...
            $segmentsFile = $directory->createFile(
    ...
    public function addDocument (Zend_Search_Lucene_Document $document)
    {
        ...
        if ($this->_currentSegment === null) {
            $this->_currentSegment = new Zend_Search_Lucene_Index_SegmentWriter_DocumentWriter(
    ...
    {
        if (Zend_Search_Lucene_LockManager::obtainOptimizationLock(
                $this->_directory) === false) {
            return;
        }
        
        if (! $this->_hasAnythingToMerge()) {
            Zend_Search_Lucene_LockManager::releaseOptimizationLock(
                    $this->_directory);
            return;
        }
        
        // Update segments list to be sure all segments are not merged yet by
        // another process
        //
        // Segment merging functionality is concentrated in this class and
        // surrounded
        // by optimization lock obtaining/releasing.
        // _updateSegments() refreshes segments list from the latest index
        // generation.
        // So only new segments can be added to the index while we are merging
        // some already existing
        // segments.
        // Newly added segments will be also included into the index by the
        // _updateSegments() call
        // either by another process or by the current process with the commit()
        // call at the end of _mergeSegments() method.
        // That's guaranteed by the serialisation of _updateSegments() execution
    ...
    private function _updateSegments ()
    {
        ...
        $segmentsFile = $this->_directory->getFileObject(
                Zend_Search_Lucene::getSegmentFileName($generation), false);
        $newSegmentFile = $this->_directory->createFile(
                Zend_Search_Lucene::getSegmentFileName(++ $generation), false);
                ...
                if (! in_array($segName, $this->_segmentsToDelete)) {
                    ...
                    if (! isset($this->_segmentInfos[$segName])) {
                        ...
                        $this->_segmentInfos[$segName] = new Zend_Search_Lucene_Index_SegmentInfo(
                                $this->_directory, $segName, $segSize, $delGen, 
                                $docStoreOptions, $hasSingleNormFile, 
                                $isCompound);
                    } else {
                    ...
                    if ($this->_targetFormatVersion ==
                        ...
                        if ($docStoreOptions !== null) {
                            ...
                            Zend_Search_Lucene_LockManager::releaseWriteLock(
                                    $this->_directory);
        ...
        if (Zend_Search_Lucene_LockManager::escalateReadLock($this->_directory)) {
            ...
            foreach ($this->_directory->fileList() as $file) {
                if ($file == 'deletable') {
                    // 'deletable' file
                    $filesToDelete[] = $file;
                    $filesTypes[] = 0; // delete this file first, since it's
                                          // not used starting from Lucene v2.1
                    $filesNumbers[] = 0;
                } else 
                    if ($file == 'segments') {
                        // 'segments' file
                        $filesToDelete[] = $file;
                        $filesTypes[] = 1; // second file to be deleted
                                              // "zero" version of segments file
                                              // (Lucene pre-2.1)
                        $filesNumbers[] = 0;
                    } else 
                        if (preg_match('/^segments_[a-zA-Z0-9]+$/i', $file)) {
                            ...
                            if ($file !=
                                     Zend_Search_Lucene::getSegmentFileName(
                                            $generation)) {
                                $filesToDelete[] = $file;
                                $filesTypes[] = 2; // first group of files
                                                      // for deletions
                                $filesNumbers[] = (int) base_convert(
                                        substr($file, 9), 36, 10); // ordered
                                                                                                        // by
                                                                                                        // segment
                                                                                                        // generation
                                                                                                        // numbers
                            ...
                            if (preg_match('/(^_([a-zA-Z0-9]+))\.f\d+$/i', 
                                    $file, $matches)) {
                                // one of per segment files
                                // ('<segment_name>.f<decimal_number>')
                                // Check if it's not one of the segments in the
                                // current segments set
                                if (! isset($segments[$matches[1]])) {
                                    $filesToDelete[] = $file;
                                    $filesTypes[] = 3; // second group of
                                                          // files for deletions
                                    $filesNumbers[] = (int) base_convert(
                                            $matches[2], 36, 10); // order
                                                                                                       // by
                                                                                                       // segment
                                                                                                       // number
                                }
                                ...
                                if (preg_match(
                                        '/(^_([a-zA-Z0-9]+))(_([a-zA-Z0-9]+))\.del$/i', 
                                        $file, $matches)) {
                                    // one of per segment files
                                    // ('<segment_name>_<del_generation>.del'
                                    // where <segment_name> is
                                    // '_<segment_number>')
                                    // Check if it's not one of the segments in
                                    // the current segments set
                                    if (! isset($segments[$matches[1]])) {
                                        $filesToDelete[] = $file;
                                        $filesTypes[] = 3; // second group of
                                                              // files for
                                        ...
                                        $segmentNumber = (int) base_convert(
                                                $matches[2], 36, 10);
                                        $delGeneration = (int) base_convert(
                                                $matches[4], 36, 10);
                                    ...
                                    if (isset(
                                            self::$_indexExtensions[substr(
                                                    $file, strlen($file) - 4)])) {
                                        // one of per segment files
                                        // ('<segment_name>.<ext>')
                                        $segmentName = substr($file, 0, 
                                                strlen($file) - 4);
                                        // Check if it's not one of the segments
                                        // in the current segments set
                                        if (! isset($segments[$segmentName]) &&
                                                 ($this->_currentSegment === null ||
                                                 $this->_currentSegment->getName() !=
                                                 $segmentName)) {
                                            $filesToDelete[] = $file;
                                            $filesTypes[] = 3; // second
                                                                  // group of
            ...
            foreach ($delFiles as $segmentNumber => $segmentDelFiles) {
                ...
                array_pop($delFiles[$segmentNumber]); // remove last delete file
                                                      // generation from
            ...
            foreach ($delFiles as $segmentNumber => $segmentDelFiles) {
                foreach ($segmentDelFiles as $delGeneration => $file) {
                    ...
                    $filesNumbers[] = $segmentNumber * $maxGenNumber +
                             $delGeneration; // order
                                                                                              // by
                                                                                              // <segment_number>,<del_generation>
                                                                                              // pair
                }
            }
            
            // Reorder files for deleting
            array_multisort($filesTypes, SORT_ASC, SORT_NUMERIC, $filesNumbers, 
                    SORT_ASC, SORT_NUMERIC, $filesToDelete, SORT_ASC, 
                    SORT_STRING);
            ...
            foreach ($filesToDelete as $file) {
                try {
                     ...
                     */
                    /**
                     * @todo Process '.cfx' files to check if them are already
                     * unused
                     */
                    ...
                    if (strpos($e->getMessage(), 'Can\'t delete file') === false) {
                        // That's not "file is under processing or already
                        // deleted" exception
                        // Pass it through
                        throw new Zend_Search_Lucene_Exception($e->getMessage(), 
                                $e->getCode(), $e);
            ...
            Zend_Search_Lucene_LockManager::deEscalateReadLock(
                    $this->_directory);
        } else {
     ...
     *
     * Returns true on success and false if another optimization or
    ...
    private function _newSegmentName ()
    {
        ...
        $segmentsFile->seek(12); // 12 = 4 (int, file format marker) + 8 (long,
                                 // index version)
        ...
        // Flash output to guarantee that wrong value will not be loaded between
        // unlock and
