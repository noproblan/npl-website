...
class Zend_Search_Lucene_Index_SegmentMerger
{
    ...
    public function __construct ($directory, $name)
    {
        ...
        $this->_writer = new Zend_Search_Lucene_Index_SegmentWriter_StreamWriter(
                $directory, $name);
    }
    ...
    private function _mergeFields ()
    {
        foreach ($this->_segmentInfos as $segName => $segmentInfo) {
            foreach ($segmentInfo->getFieldInfos() as $fieldInfo) {
                $this->_fieldsMap[$segName][$fieldInfo->number] = $this->_writer->addFieldInfo(
                        $fieldInfo);
            }
        }
    }
            ...
            if ($fieldInfo->isIndexed) {
                foreach ($this->_segmentInfos as $segName => $segmentInfo) {
                    if ($segmentInfo->hasDeletions()) {
                        ...
                        $this->_writer->addNorm($fieldInfo->name, 
        ...
        foreach ($this->_segmentInfos as $segName => $segmentInfo) {
            ...
            for ($count = 0; $count < $segmentInfo->count(); $count ++) {
                ...
                for ($count2 = 0; $count2 < $fieldCount; $count2 ++) {
                    ...
                    if (! ($bits & 2)) { // Text data
                        $storedFields[] = new Zend_Search_Lucene_Field(
                                $fieldInfo->name, $fdtFile->readString(), 
                                'UTF-8', true, $fieldInfo->isIndexed, $bits & 1);
                    } else { // Binary data
                        $storedFields[] = new Zend_Search_Lucene_Field(
                                $fieldInfo->name, $fdtFile->readBinary(), '', 
                                true, $fieldInfo->isIndexed, $bits & 1, true);
                    }
                }
    ...
    private function _mergeTerms ()
    {
        ...
        $segmentStartId = 0;
        foreach ($this->_segmentInfos as $segName => $segmentInfo) {
            $segmentStartId = $segmentInfo->resetTermsStream($segmentStartId, 
        ...
        while (($segmentInfo = $segmentInfoQueue->pop()) !== null) {
            ...
            if ($segmentInfoQueue->top() === null || $segmentInfoQueue->top()
                ...
                if (count($termDocs) > 0) {
                    $this->_writer->addTerm($segmentInfo->currentTerm(), 
                            $termDocs);
                }
                $termDocs = array();
            }
