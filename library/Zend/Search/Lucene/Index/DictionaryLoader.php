 ...
 * Manual "method inlining" is performed to increase dictionary index loading
 * operation
...
class Zend_Search_Lucene_Index_DictionaryLoader
{
     ...
     *
     * It takes a string which is actually <segment_name>.tii index file data
     * and
    ...
    public static function load ($data)
    {
        ...
        $tiVersion = ord($data[0]) << 24 | ord($data[1]) << 16 |
        ...
        if (PHP_INT_SIZE > 4) {
            $indexTermCount = ord($data[$pos]) << 56 | ord($data[$pos + 1]) << 48 |
                     ord($data[$pos + 2]) << 40 | ord($data[$pos + 3]) << 32 |
                     ord($data[$pos + 4]) << 24 | ord($data[$pos + 5]) << 16 |
                     ord($data[$pos + 6]) << 8 | ord($data[$pos + 7]);
        } else {
            if ((ord($data[$pos]) != 0) || (ord($data[$pos + 1]) != 0) ||
                     (ord($data[$pos + 2]) != 0) ||
                     (ord($data[$pos + 3]) != 0) ||
            ...
            $indexTermCount = ord($data[$pos + 4]) << 24 |
                     ord($data[$pos + 5]) << 16 | ord($data[$pos + 6]) << 8 |
                     ord($data[$pos + 7]);
        }
        $pos += 8;
        ...
        $skipInterval = ord($data[$pos]) << 24 | ord($data[$pos + 1]) << 16 |
                 ord($data[$pos + 2]) << 8 | ord($data[$pos + 3]);
        $pos += 4;
        if ($indexTermCount < 1) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Wrong number of terms in a term dictionary index');
        }
        ...
        for ($count = 0; $count < $indexTermCount; $count ++) {
            ...
            if ($len == 0) {
                ...
                for ($count1 = 0; $count1 < $len; $count1 ++) {
                    if ((ord($termSuffix[$count1]) & 0xC0) == 0xC0) {
                        ...
                        // Check for null character. Java2 encodes null
                        // character
                        // in two bytes.
            ...
            // $termValue = Zend_Search_Lucene_Index_Term::getPrefix($prevTerm,
            ...
            // $this->_termDictionary[] = new
            // Zend_Search_Lucene_Index_Term($termValue, $termFieldNum);
            $termDictionary[] = array(
                    $termFieldNum,
                    $termValue
            );
            ...
            $termInfos[] = 
            // new Zend_Search_Lucene_Index_TermInfo($docFreq, $freqPointer,
            // $proxPointer, $skipDelta, $indexPointer);
            array(
                    $docFreq,
                    $freqPointer,
                    $proxPointer,
