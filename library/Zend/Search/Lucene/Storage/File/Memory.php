...
class Zend_Search_Lucene_Storage_File_Memory extends Zend_Search_Lucene_Storage_File
{
    ...
    public function readInt ()
    {
        ...
        return ord($str[0]) << 24 | ord($str[1]) << 16 | ord($str[2]) << 8 |
                 ord($str[3]);
    }
    ...
    public function writeInt ($value)
    {
        ...
        $this->_data .= chr($value >> 24 & 0xFF) . chr($value >> 16 & 0xFF) .
    ...
    public function readLong ()
    {
         ...
         * fseek() uses long for offset. Thus, largest index segment file size
         * in 32bit mode is 2Gb
         */
        if (PHP_INT_SIZE > 4) {
            $str = substr($this->_data, $this->_position, 8);
            $this->_position += 8;
            
            return ord($str[0]) << 56 | ord($str[1]) << 48 | ord($str[2]) << 40 |
                     ord($str[3]) << 32 | ord($str[4]) << 24 | ord($str[5]) << 16 |
                     ord($str[6]) << 8 | ord($str[7]);
        } else {
    ...
    public function writeLong ($value)
    {
        ...
        /**
         * Check, that we work in 64-bit mode.
         * fseek() and ftell() use long for offset. Thus, largest index segment
         * file size in 32bit mode is 2Gb
         */
        if (PHP_INT_SIZE > 4) {
            settype($value, 'integer');
            $this->_data .= chr($value >> 56 & 0xFF) . chr($value >> 48 & 0xFF) .
                     chr($value >> 40 & 0xFF) . chr($value >> 32 & 0xFF) .
    ...
    public function writeLong32Bit ($value)
    {
        ...
        if ($value < 0) {
            ...
            if ($wordLow > 0x7FFFFFFF) {
                // Highest bit of low word is set. Translate it to the
                // corresponding negative integer value
                $wordLow -= 0x80000000;
                $wordLow |= 0x80000000;
            }
        }
            ...
            for ($count = 0; $count < $strlen; $count ++) {
                if ((ord($str_val[$count]) & 0xC0) == 0xC0) {
                    ...
                    if (ord($str_val[$count]) & 0x20) {
                        ...
                        // Never used. Java2 doesn't encode strings in four
                        // bytes
                        if (ord($str_val[$count]) & 0x10) {
                            $addBytes ++;
                        }
                    }
                    $str_val .= substr($this->_data, $this->_position, 
                            $addBytes);
