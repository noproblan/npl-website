...
abstract class Zend_Search_Lucene_Search_Similarity
{
     ...
     * stored in an index and multipled into scores for hits on each field by
     * the
     * search code.
     *
    ...
    private static function _floatToByte ($f)
    {
        ...
        if ($highIndex != 255 &&
                 $f - self::$_normTable[$highIndex] >
                 self::$_normTable[$highIndex + 1] - $f) {
            return $highIndex + 1;
        } else {
            return $highIndex;
        }
    ...
    public function idf ($input, Zend_Search_Lucene_Interface $reader)
    {
        if (! is_array($input)) {
            ...
            foreach ($input as $term) {
                $idf += $this->idfFreq($reader->docFreq($term), 
                        $reader->count());
            }
            return $idf;
        }
     ...
     * implemenations of this method usually return larger values for rare
     * terms,
