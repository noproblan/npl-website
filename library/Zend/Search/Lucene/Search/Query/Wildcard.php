...
class Zend_Search_Lucene_Search_Query_Wildcard extends Zend_Search_Lucene_Search_Query
{
        ...
        if ($questionMarkPosition !== false) {
            if ($astrericPosition !== false) {
                return substr($word, 0, 
                        min($questionMarkPosition, $astrericPosition));
            }
    ...
    public function rewrite (Zend_Search_Lucene_Interface $index)
    {
        ...
        if ($prefixLength < self::$_minPrefixLength) {
            ...
            throw new Zend_Search_Lucene_Exception(
                    'At least ' . self::$_minPrefixLength .
        ...
        foreach ($fields as $field) {
            ...
            if ($prefix != '') {
                ...
                while ($index->currentTerm() !== null &&
                         ...
                         substr($index->currentTerm()->text, 0, $prefixLength) ==
                         $prefix) {
                            if (preg_match($matchExpression, 
                                    $index->currentTerm()->text) === 1) {
    ...
    public function getQueryTerms ()
    {
        if ($this->_matches === null) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Search has to be performed first to get matched terms');
        }
