...
class Zend_Search_Lucene_Search_Query_Boolean extends Zend_Search_Lucene_Search_Query
{
     ...
     * @return void
     */
    public function addSubquery (Zend_Search_Lucene_Search_Query $subquery, 
            $sign = null)
    {
        if ($sign !== true || $this->_signs !== null) { // Skip, if all
                                                        // subqueries are
                                                        // required
            if ($this->_signs === null) { // Check, If all previous subqueries
                                          // are required
                $this->_signs = array();
    ...
    public function optimize (Zend_Search_Lucene_Interface $index)
    {
        ...
        if (count($subqueries) == 1) {
            ...
            $optimizedQuery->setBoost(
                    $optimizedQuery->getBoost() * $this->getBoost());
            
            return $optimizedQuery;
        }
        
        // Prepare first candidate for optimized query
        $optimizedQuery = new Zend_Search_Lucene_Search_Query_Boolean(
                $subqueries, $signs);
            ...
            if ($subquery instanceof Zend_Search_Lucene_Search_Query_Term) {
                ...
                if ($subquery instanceof Zend_Search_Lucene_Search_Query_MultiTerm) {
                    ...
                    if ($signs[$id] === true) {
                        ...
                        // Multi-term required subquery can be decomposed only
                        // if it contains
                        ...
                        // Continue if subquery has prohibited terms or doesn't
                        // have required terms
                        ...
                        unset($signs[$id]);
                             ...
                             // It's an optional or prohibited multi-term
                             // subquery.
                             // Something like '... (+term1 -term2 term3 ...)
                             // ...'
                             // or
                        ...
                        // Multi-term optional and required subqueries can be
                             // decomposed
                        ...
                        // Continue if non-optional terms are presented in this
                        // multi-term subquery
                        if (! $onlyOptional) {
                            continue;
                        }
        ...
        // Check, if all subqueries have been decomposed and all terms has the
        // same boost factor
        if (count($subqueries) == 0 && count(array_unique($boostFactors)) == 1) {
            require_once 'Zend/Search/Lucene/Search/Query/MultiTerm.php';
            $optimizedQuery = new Zend_Search_Lucene_Search_Query_MultiTerm(
                    $terms, $tsigns);
        ...
        // This boolean query can't be transformed to Term/MultiTerm query and
        // still contains
        // several subqueries
        ...
        if (count($terms) == 1) {
            ...
            if (count($terms) > 1 && count(array_unique($boostFactors)) == 1) {
                require_once 'Zend/Search/Lucene/Search/Query/MultiTerm.php';
                $clause = new Zend_Search_Lucene_Search_Query_MultiTerm($terms, 
                        $tsigns);
                $clause->setBoost(reset($boostFactors));
                
                $subqueries[] = $clause;
                // Clause sign is 'required' if clause contains required terms.
                // 'Optional' otherwise.
        ...
        if (count($prohibitedTerms) == 1) {
            ...
            require_once 'Zend/Search/Lucene/Search/Query/Term.php';
            $subqueries[] = new Zend_Search_Lucene_Search_Query_Term(
                    reset($prohibitedTerms));
            $signs[] = false;
            ...
            if (count($prohibitedTerms) > 1) {
                ...
                foreach ($prohibitedTerms as $id => $term) {
                    // all prohibited term are grouped as optional into
                    // multi-term query
                ...
                // (boost factors are not significant for prohibited clauses)
                require_once 'Zend/Search/Lucene/Search/Query/MultiTerm.php';
                $subqueries[] = new Zend_Search_Lucene_Search_Query_MultiTerm(
                        $prohibitedTerms, $prohibitedSigns);
    ...
    public function createWeight (Zend_Search_Lucene_Interface $reader)
    {
        ...
        $this->_weight = new Zend_Search_Lucene_Search_Weight_Boolean($this, 
                $reader);
        return $this->_weight;
    }
    ...
    private function _calculateConjunctionResult ()
    {
        ...
        array_multisort($resVectorsSizes, SORT_ASC, SORT_NUMERIC, 
                $resVectorsIds, SORT_ASC, SORT_NUMERIC, $resVectors);
    ...
    private function _calculateNonConjunctionResult ()
    {
        ...
        foreach ($this->_subqueries as $subqueryId => $subquery) {
            if ($this->_signs[$subqueryId] === true) {
                ...
                // Calculating prohibited vector may take significant time, but
        ...
        array_multisort($requiredVectorsSizes, SORT_ASC, SORT_NUMERIC, 
                $requiredVectorsIds, SORT_ASC, SORT_NUMERIC, $requiredVectors);
            ...
            Zend_Search_Lucene_Interface $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = $reader->getSimilarity()->coord(
                    count($this->_subqueries), count($this->_subqueries));
        }
    ...
    /**
     * Score calculator for non conjunction queries (not all subqueries are
     * required)
     *
     * @param integer $docId            
     * @param Zend_Search_Lucene_Interface $reader            
     * @return float
     */
    public function _nonConjunctionScore ($docId, 
            Zend_Search_Lucene_Interface $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = array();
            ...
            for ($count = 0; $count <= $maxCoord; $count ++) {
                $this->_coord[$count] = $reader->getSimilarity()->coord($count, 
                        $maxCoord);
            }
        }
