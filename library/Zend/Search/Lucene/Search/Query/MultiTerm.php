...
class Zend_Search_Lucene_Search_Query_MultiTerm extends Zend_Search_Lucene_Search_Query
{
    ...
    {
        if ($sign !== true || $this->_signs !== null) { // Skip, if all terms
                                                        // are required
            if ($this->_signs === null) { // Check, If all previous terms are
                                          // required
                $this->_signs = array();
    ...
    public function rewrite (Zend_Search_Lucene_Interface $index)
    {
        ...
        if ($allQualified) {
            ...
            /**
             * transform multiterm query to boolean and apply rewrite() method
             * to subqueries.
             */
    ...
    public function optimize (Zend_Search_Lucene_Interface $index)
    {
        ...
        $optimizedQuery = new Zend_Search_Lucene_Search_Query_MultiTerm($terms, 
                $signs);
        $optimizedQuery->setBoost($this->getBoost());
        return $optimizedQuery;
    }
    ...
    public function createWeight (Zend_Search_Lucene_Interface $reader)
    {
        ...
        $this->_weight = new Zend_Search_Lucene_Search_Weight_MultiTerm($this, 
                $reader);
        return $this->_weight;
    }
            ...
            Zend_Search_Lucene_Interface $reader)
    {
        ...
        foreach ($this->_terms as $id => $term) {
            ...
            $ids[] = $id; // Used to keep original order for terms with the
                               // same selectivity and omit terms comparison
        }
        array_multisort($docFreqs, SORT_ASC, SORT_NUMERIC, $ids, SORT_ASC, 
                SORT_NUMERIC, $this->_terms);
        ...
        // Docs are returned ordered. Used algorithms doesn't change elements
    // order.
    }
            ...
            Zend_Search_Lucene_Interface $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = $reader->getSimilarity()->coord(
                    count($this->_terms), count($this->_terms));
        }
