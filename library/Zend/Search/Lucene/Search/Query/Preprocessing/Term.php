...
class Zend_Search_Lucene_Search_Query_Preprocessing_Term extends Zend_Search_Lucene_Search_Query_Preprocessing
{
    ...
    /**
     * Word encoding (field name is always provided using UTF-8 encoding since
    ...
    public function rewrite (Zend_Search_Lucene_Interface $index)
    {
        if ($this->_field === null) {
            ...
            foreach ($searchFields as $fieldName) {
                $subquery = new Zend_Search_Lucene_Search_Query_Preprocessing_Term(
                        $this->_word, $this->_encoding, $fieldName);
         ...
         * Zend_Environment in some future
         */
        ...
        $subPatterns = preg_split($wildcardsPattern, $word, - 1, 
        ...
        if (count($subPatterns) > 1) {
            ...
            foreach ($subPatterns as $id => $subPattern) {
                // Append corresponding wildcard character to the pattern before
                // each sub-pattern (except first)
                if ($id != 0) {
                    $pattern .= $word[$subPattern[1] - 1];
                }
                
                // Check if each subputtern is a single word in terms of current
                // analyzer
                $tokens = Zend_Search_Lucene_Analysis_Analyzer::getDefault()->tokenize(
                        $subPattern[0], $subPatternsEncoding);
                if (count($tokens) > 1) {
            ...
            // Get rewritten query. Important! It also fills terms matching
            // container.
