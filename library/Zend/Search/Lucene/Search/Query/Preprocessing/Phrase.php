...
/**
 * It's an internal abstract class intended to finalize ase a query processing
 * after query parsing.
...
class Zend_Search_Lucene_Search_Query_Preprocessing_Phrase extends Zend_Search_Lucene_Search_Query_Preprocessing
{
    ...
    /**
     * Phrase encoding (field name is always provided using UTF-8 encoding since
     * it may be retrieved from index).
     *
     * @var string
     */
    ...
    public function rewrite (Zend_Search_Lucene_Interface $index)
    {
        ...
        // They are either removed by text analyzer or used as a part of keyword
        // for keyword fields
        //
        // if (strpos($this->_phrase, '?') !== false || strpos($this->_phrase,
        // '*') !== false) {
        // require_once 'Zend/Search/Lucene/Search/QueryParserException.php';
        // throw new Zend_Search_Lucene_Search_QueryParserException('Wildcards
        // are only allowed in a single terms.');
        // }
        ...
        if ($this->_field === null) {
            ...
            foreach ($searchFields as $fieldName) {
                $subquery = new Zend_Search_Lucene_Search_Query_Preprocessing_Phrase(
                        $this->_phrase, $this->_phraseEncoding, $fieldName);
        ...
        // Recognize exact term matching (it corresponds to Keyword fields
        // stored in the index)
        ...
        // tokenize phrase using current analyzer and process it as a phrase
        // query
        require_once 'Zend/Search/Lucene/Analysis/Analyzer.php';
        $tokens = Zend_Search_Lucene_Analysis_Analyzer::getDefault()->tokenize(
                $this->_phrase, $this->_phraseEncoding);
        ...
        if (count($tokens) == 1) {
            require_once 'Zend/Search/Lucene/Index/Term.php';
            $term = new Zend_Search_Lucene_Index_Term($tokens[0]->getTermText(), 
                    $this->_field);
            require_once 'Zend/Search/Lucene/Search/Query/Term.php';
            $query = new Zend_Search_Lucene_Search_Query_Term($term);
            $query->setBoost($this->getBoost());
        ...
        foreach ($tokens as $token) {
            ...
            $term = new Zend_Search_Lucene_Index_Term($token->getTermText(), 
                    $this->_field);
            ...
            Zend_Search_Lucene_Search_Highlighter_Interface $highlighter)
    {
         ...
         * We don't need it, since we expect all fields presented in the HTML
         * body and don't differentiate them
         */
        
        /**
         * Skip exact term matching recognition, keyword fields highlighting is
         * not supported
         */
