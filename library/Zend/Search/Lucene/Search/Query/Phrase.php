...
class Zend_Search_Lucene_Search_Query_Phrase extends Zend_Search_Lucene_Search_Query
{
     ...
     * switch
     * the order of two words requires two moves (the first move places the
     * words
    ...
    public function __construct ($terms = null, $offsets = null, $field = null)
    {
        ...
        if (is_array($terms)) {
            ...
            foreach ($terms as $termId => $termText) {
                $this->_terms[$termId] = ($field !== null) ? new Zend_Search_Lucene_Index_Term(
                        $termText, $field) : new Zend_Search_Lucene_Index_Term(
                        $termText);
            }
        } else 
            if ($terms === null) {
                ...
                throw new Zend_Search_Lucene_Exception(
                        'terms argument must be array of strings or null');
            }
        ...
        if (is_array($offsets)) {
            if (count($this->_terms) != count($offsets)) {
                require_once 'Zend/Search/Lucene/Exception.php';
                throw new Zend_Search_Lucene_Exception(
                        'terms and offsets arguments must have the same size.');
            }
            ...
            if ($offsets === null) {
                ...
                require_once 'Zend/Search/Lucene/Exception.php';
                throw new Zend_Search_Lucene_Exception(
                        'offsets argument must be array of strings or null');
            }
    }
     ...
     * The relative position of the term is specified explicitly or the one
     * immediately
     ...
     */
    public function addTerm (Zend_Search_Lucene_Index_Term $term, 
            $position = null)
    {
        if ((count($this->_terms) != 0) &&
                 (end($this->_terms)->field != $term->field)) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'All phrase terms must be in the same field: ' . $term->field .
                             ':' . $term->text);
        }
        ...
        if (count($this->_terms) == 0) {
            ...
            if ($this->_terms[0]->field !== null) {
                ...
                foreach ($index->getFieldNames(true) as $fieldName) {
                    ...
                    require_once 'Zend/Search/Lucene/Index/Term.php';
                    foreach ($this->_terms as $termId => $term) {
                        $qualifiedTerm = new Zend_Search_Lucene_Index_Term(
                                $term->text, $fieldName);
                        
                        $subquery->addTerm($qualifiedTerm, 
    ...
    public function optimize (Zend_Search_Lucene_Interface $index)
    {
        ...
        if (count($this->_terms) == 1) {
            ...
            $optimizedQuery = new Zend_Search_Lucene_Search_Query_Term(
                    reset($this->_terms));
    ...
    public function createWeight (Zend_Search_Lucene_Interface $reader)
    {
        ...
        $this->_weight = new Zend_Search_Lucene_Search_Weight_Phrase($this, 
                $reader);
        return $this->_weight;
    }
        ...
        foreach ($this->_termsPositions[$lowCardTermId][$docId] as $lowCardPos) {
            ...
            foreach ($this->_terms as $termId => $term) {
                if ($termId != $lowCardTermId) {
                    $expectedPosition = $lowCardPos + ($this->_offsets[$termId] -
                             $this->_offsets[$lowCardTermId]);
                    
                    if (! in_array($expectedPosition, 
                            $this->_termsPositions[$termId][$docId])) {
     ...
     * @return float
     */
    public function _sloppyPhraseFreq ($docId, 
            Zend_Search_Lucene_Interface $reader)
    {
                ...
                if ($firstPass) {
                    ...
                    for ($count = 0; $count < $queueSize; $count ++) {
                        if ($lastTerm !== null &&
                                 abs(
                                        $termPosition -
                                         $phraseQueue[$count][$lastTerm] -
                                         ($this->_offsets[$termId] -
                                         $this->_offsets[$lastTerm])) >
                                 $this->_slop) {
                            continue;
                        }
        ...
        foreach ($phraseQueue as $phrasePos) {
            ...
            for ($shift = - $this->_slop; $shift <= $this->_slop; $shift ++) {
                ...
                foreach ($this->_terms as $termId => $term) {
                    $distance += abs(
                            $phrasePos[$termId] - $this->_offsets[$termId] -
                                     $start);
