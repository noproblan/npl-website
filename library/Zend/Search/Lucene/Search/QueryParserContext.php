...
class Zend_Search_Lucene_Search_QueryParserContext
{
     ...
     * Zend_Search_Lucene_Search_Query::rewriteQuery($index) transletes such
     * queries to several
     *
     * @var string|null
     */
    ...
    public function processFuzzyProximityModifier ($parameter = null)
    {
        ...
        if ($this->_nextEntryField !== null || $this->_nextEntrySign !== null) {
            require_once 'Zend/Search/Lucene/Search/QueryParserException.php';
            throw new Zend_Search_Lucene_Search_QueryParserException(
                    '\'~\' modifier must follow word or phrase.');
        }
    ...
    private function _booleanExpressionQuery ()
    {
        ...
        try {
            foreach ($this->_entries as $entry) {
                if ($entry instanceof Zend_Search_Lucene_Search_QueryEntry) {
                    ...
                    switch ($entry) {
                        ...
                        default:
                            throw new Zend_Search_Lucene(
                                    'Boolean expression error. Unknown operator type.');
                    }
                }
            }
            ...
            // throw new Zend_Search_Lucene_Search_QueryParserException('Boolean
            ...
            if (count($conjuction) == 1) {
                ...
                foreach ($conjuction as $conjuctionEntry) {
                    $subquery->addSubquery(
                            $conjuctionEntry[0]->getQuery($this->_encoding), 
                            $conjuctionEntry[1]);
                }
