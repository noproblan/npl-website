 ...
 *
 * Any type of Transducers (Moore machine or Mealy machine) also may be
 * implemented by using this abstract FSM.
...
abstract class Zend_Search_Lucene_FSM
{
     ...
     *
     * $states is an array of integers or strings with a list of possible
     * machine states
     * constructor treats fist list element as a sturt state (assignes it to
     * $_current state).
     * It may be reassigned by setState() call.
     * States list may be empty and can be extended later by addState() or
     * addStates() calls.
     *
     * $inputAphabet is the same as $states, but represents input alphabet
     * it also may be extended later by addInputSymbols() or addInputSymbol()
     * calls.
     *
     ...
     *
     * FSM actions are very flexible and may be defined by addEntryAction(),
     * addExitAction(),
    ...
    public function setState ($state)
    {
        if (! isset($this->_states[$state])) {
            ...
            throw new Zend_Search_Exception(
                    'State \'' . $state .
                             '\' is not on of the possible FSM states.');
        }
     ...
     * @throws Zend_Search_Exception
     */
    public function addRule ($sourceState, $input, $targetState, 
            $inputAction = null)
    {
        if (! isset($this->_states[$sourceState])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Undefined source state (' . $sourceState . ').');
        }
        if (! isset($this->_states[$targetState])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Undefined target state (' . $targetState . ').');
        }
        if (! isset($this->_inputAphabet[$input])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Undefined input symbol (' . $input . ').');
        }
        ...
        if (isset($this->_rules[$sourceState][$input])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Rule for {state,input} pair (' . $sourceState . ', ' .
     ...
     * @param Zend_Search_Lucene_FSMAction $action            
     */
    public function addInputAction ($state, $inputSymbol, 
            Zend_Search_Lucene_FSMAction $action)
    {
        ...
        if (! isset($this->_inputAphabet[$inputSymbol])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Undefined input symbol (' . $inputSymbol . ').');
        }
     ...
     * @param Zend_Search_Lucene_FSMAction $action            
     */
    public function addTransitionAction ($sourceState, $targetState, 
            Zend_Search_Lucene_FSMAction $action)
    {
        if (! isset($this->_states[$sourceState])) {
            require_once 'Zend/Search/Exception.php';
        ...
        if (! isset($this->_states[$targetState])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'Undefined source state (' . $targetState . ').');
        }
    ...
    public function process ($input)
    {
        if (! isset($this->_rules[$this->_currentState])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'There is no any rule for current state (' .
                             $this->_currentState . ').');
        }
        if (! isset($this->_rules[$this->_currentState][$input])) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'There is no any rule for {current state, input} pair (' .
        ...
        if ($sourceState != $targetState &&
                 isset($this->_exitActions[$sourceState])) {
        ...
        if ($sourceState != $targetState &&
                 isset($this->_entryActions[$targetState])) {
    ...
    public function reset ()
    {
        if (count($this->_states) == 0) {
            require_once 'Zend/Search/Exception.php';
            throw new Zend_Search_Exception(
                    'There is no any state defined for FSM.');
        }
