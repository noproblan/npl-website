...
class Zend_Search_Lucene_Document_Xlsx extends Zend_Search_Lucene_Document_OpenXml
{
    ...
    private function __construct ($fileName, $storeContent)
    {
        ...
        if ($relationsXml === false) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Invalid archive or corrupted .xlsx file.');
        }
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] ==
                ...
                foreach ($workbookRelations->Relationship as $workbookRelation) {
                    if ($workbookRelation["Type"] ==
                             Zend_Search_Lucene_Document_Xlsx::SCHEMA_WORKSHEETRELATION) {
                        $worksheets[str_replace('rId', '', 
                                (string) $workbookRelation["Id"])] = simplexml_load_string(
                                $package->getFromName(
                                        $this->absoluteZipPath(
                                                dirname($rel["Target"]) . "/" .
                                                 dirname(
