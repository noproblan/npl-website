...
class Zend_Search_Lucene_Document_Docx extends Zend_Search_Lucene_Document_OpenXml
{
    ...
    private function __construct ($fileName, $storeContent)
    {
        ...
        if ($relationsXml === false) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Invalid archive or corrupted .docx file.');
        }
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] ==
                ...
                $contents->registerXPathNamespace('w', 
                        Zend_Search_Lucene_Document_Docx::SCHEMA_WORDPROCESSINGML);
                $paragraphs = $contents->xpath('//w:body/w:p');
                
                foreach ($paragraphs as $paragraph) {
                    $runs = $paragraph->xpath(
                            './/w:r/*[name() = "w:t" or name() = "w:br"]');
                    ...
                    // Add space after each paragraph. So they are not bound
                    // together.
                    $documentBody[] = ' ';
                }
    ...
    public static function loadDocxFile ($fileName, $storeContent = false)
    {
        if (! is_readable($fileName)) {
            ...
            throw new Zend_Search_Lucene_Document_Exception(
                    'Provided file \'' . $fileName . '\' is not readable.');
        }
