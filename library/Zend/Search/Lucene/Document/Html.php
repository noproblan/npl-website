...
class Zend_Search_Lucene_Document_Html extends Zend_Search_Lucene_Document
{
     ...
     *            HTML encoding, is used if it's not specified using
            ...
            $defaultEncoding = '')
    {
        ...
        if ($this->_doc->encoding === null) {
             ...
             */
            if (preg_match('/<html[^>]*>/i', $htmlData, $matches, 
                    PREG_OFFSET_CAPTURE)) {
                ...
                @$this->_doc->loadHTML(
                        iconv($defaultEncoding, 'UTF-8//IGNORE', 
                                substr($htmlData, 0, $htmlTagOffset)) .
                                 '<head><META HTTP-EQUIV="Content-type" CONTENT="text/html; charset=UTF-8"/></head>' .
                                 iconv($defaultEncoding, 'UTF-8//IGNORE', 
                                        substr($htmlData, $htmlTagOffset)));
         ...
         * The case is:
         * Content-type HTTP-EQUIV meta tag is presented, but ISO-8859-5
        ...
        $this->addField(
                Zend_Search_Lucene_Field::Text('title', $docTitle, 'UTF-8'));
        ...
        foreach ($metaNodes as $metaNode) {
            $this->addField(
                    Zend_Search_Lucene_Field::Text(
                            ...
                            $metaNode->getAttribute('content'), 'UTF-8'));
        }
        ...
        if ($storeContent) {
            $this->addField(
                    Zend_Search_Lucene_Field::Text('body', $docBody, 'UTF-8'));
        } else {
            $this->addField(
                    Zend_Search_Lucene_Field::UnStored('body', $docBody, 
                            'UTF-8'));
        }
     ...
     * @param array $params
     *            Array of additionall callback parameters (first non-optional
     *            parameter is a text to transform)
     * @throws Zend_Search_Lucene_Exception
     */
    protected function _highlightTextNode (DOMText $node, $wordsToHighlight, 
            $callback, $params)
    {
        /**
        ...
        foreach ($matchedTokens as $token) {
            ...
            $highlightedWordNodeSetHtml = call_user_func_array($callback, 
                    $fullCallbackparamsList);
            
            // Transform HTML string to a DOM representation and automatically
            // transform retrieved string
            ...
            $success = @$highlightedWordNodeSetDomDocument->loadHTML(
                    '<html><head><meta http-equiv="Content-type" content="text/html; charset=UTF-8"/></head><body>' .
                             $highlightedWordNodeSetHtml . '</body></html>');
            if (! $success) {
            ...
            $highlightedWordNodeSetXpath = new DOMXPath(
                    $highlightedWordNodeSetDomDocument);
            $highlightedWordNodeSet = $highlightedWordNodeSetXpath->query(
                    '/html/body')->item(0)->childNodes;
    ...
    protected function _highlightNodeRecursive (DOMNode $contextNode, 
            $wordsToHighlight, $callback, $params)
    {
        $textNodes = array();
        ...
        foreach ($contextNode->childNodes as $childNode) {
            if ($childNode->nodeType == XML_TEXT_NODE) {
                ...
                if ($childNode->nodeName != 'script') {
                    $this->_highlightNodeRecursive($childNode, 
                            $wordsToHighlight, $callback, $params);
                }
            }
        }
        ...
        foreach ($textNodes as $textNode) {
            $this->_highlightTextNode($textNode, $wordsToHighlight, $callback, 
                    $params);
        }
    }
    ...
    {
        return '<b style="color:black;background-color:' . $colour . '">' .
                 $stringToHighlight . '</b>';
    }
     ...
     *
     * @param string|array $words
     *            Words to highlight. Words could be organized using the array
     *            or string.
     * @param callback $callback
     *            Callback method, used to transform (highlighting) text.
     * @param array $params
     *            Array of additionall callback parameters passed through into
     *            it
     *            (first non-optional parameter is an HTML fragment for
     *            highlighting)
     * @return string
     * @throws Zend_Search_Lucene_Exception
     */
    public function highlightExtended ($words, $callback, $params = array())
    {
        ...
        $wordsToHighlight = call_user_func_array('array_merge', 
        ...
        foreach ($matchedNodes as $matchedNode) {
            $this->_highlightNodeRecursive($matchedNode, 
                    $wordsToHighlightFlipped, $callback, $params);
        }
    }
