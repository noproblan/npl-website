 ...
 *
 * We do recommend to provide all words in lowercase and concatenate this class
 * after the lowercase filter.
 *
 * @category Zend
 * @package Zend_Search_Lucene
 * @subpackage Analysis
...
class Zend_Search_Lucene_Analysis_TokenFilter_StopWords extends Zend_Search_Lucene_Analysis_TokenFilter
{
     ...
     * @throws Zend_Search_Exception When the file doesn`t exists or is not
     *         readable.
     */
    public function loadFromFile ($filepath = null)
    {
        if (! $filepath || ! file_exists($filepath)) {
            ...
            throw new Zend_Search_Lucene_Exception(
                    'You have to provide valid file path');
        }
        $fd = fopen($filepath, "r");
        if (! $fd) {
            require_once 'Zend/Search/Lucene/Exception.php';
