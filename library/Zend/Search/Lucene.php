...
/**
 * User land classes and interfaces turned on by Zend/Search/Lucene.php file
 * inclusion.
 */
/**
...
class Zend_Search_Lucene implements Zend_Search_Lucene_Interface
{
    ...
    /**
     * Array of Zend_Search_Lucene_Index_SegmentInfo objects for current version
     * of index.
     *
    ...
    public static function create ($directory)
    {
        ...
        return new Zend_Search_Lucene_Proxy(
                new Zend_Search_Lucene($directory, true));
    }
    ...
    public static function open ($directory)
    {
        ...
        return new Zend_Search_Lucene_Proxy(
                new Zend_Search_Lucene($directory, false));
    }
     ...
     */
    public static function getActualGeneration (
    ...
    private function _readPre21SegmentsFile ()
    {
        ...
        for ($count = 0; $count < $segments; $count ++) {
            ...
            $this->_segmentInfos[$segName] = new Zend_Search_Lucene_Index_SegmentInfo(
                    $this->_directory, $segName, $segSize);
        }
    ...
    private function _readSegmentsFile ()
    {
        $segmentsFile = $this->_directory->getFileObject(
                self::getSegmentFileName($this->_generation));
        ...
        for ($count = 0; $count < $segments; $count ++) {
            ...
            $this->_segmentInfos[$segName] = new Zend_Search_Lucene_Index_SegmentInfo(
                    ...
                    $docStoreOptions, $hasSingleNormFile, $isCompound);
        }
    }
    ...
    public function __construct ($directory = null, $create = false)
    {
        ...
        if (is_string($directory)) {
            ...
            $this->_directory = new Zend_Search_Lucene_Storage_Directory_Filesystem(
                    $directory);
        ...
        if ($create) {
            require_once 'Zend/Search/Lucene/Exception.php';
            try {
                Zend_Search_Lucene_LockManager::obtainWriteLock(
                        $this->_directory);
            } catch (Zend_Search_Lucene_Exception $e) {
                Zend_Search_Lucene_LockManager::releaseReadLock(
                        $this->_directory);
                
                if (strpos($e->getMessage(), 
                        'Can\'t obtain exclusive index lock') === false) {
                    throw new Zend_Search_Lucene_Exception($e->getMessage(), 
                            $e->getCode(), $e);
                } else {
                    throw new Zend_Search_Lucene_Exception(
                            'Can\'t create index. It\'s under processing now', 0, 
                            $e);
                }
            }
    ...
    private function _getIndexWriter ()
    {
        if ($this->_writer === null) {
            ...
            $this->_writer = new Zend_Search_Lucene_Index_Writer(
                    $this->_directory, $this->_segmentInfos, 
                    $this->_formatVersion);
        }
     ...
     * @param integer $id            
     * @return boolean
     * @throws Zend_Search_Lucene_Exception Exception is thrown if $id is out of
     *         the range
     */
    public function isDeleted ($id)
    {
        ...
        if ($id >= $this->_docCount) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Document id is out of the range.');
        }
    ...
    public function find ($query)
    {
        ...
        if (! $query instanceof Zend_Search_Lucene_Search_Query) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
                    'Query must be a string or Zend_Search_Lucene_Search_Query object');
        }
        ...
        foreach ($query->matchedDocs() as $id => $num) {
            ...
            if (self::$_resultSetLimit != 0 &&
                     count($hits) >= self::$_resultSetLimit) {
                break;
            }
        }
        ...
        if (func_num_args() == 1) {
            // sort by scores
            array_multisort($scores, SORT_DESC, SORT_NUMERIC, $ids, SORT_ASC, 
                    SORT_NUMERIC, $hits);
        } else {
            ...
            // since constants can't be passed by reference, create some
            ...
            for ($count = 1; $count < count($argList); $count ++) {
                ...
                if (strtolower($fieldName) == 'score') {
                    ...
                    if (! isset($sortFieldValues[$fieldName])) {
                        ...
                        foreach ($hits as $hit) {
                            try {
                                $value = $hit->getDocument()->getFieldValue(
                                        $fieldName);
                            } catch (Zend_Search_Lucene_Exception $e) {
                        ...
                        // Required for PHP 5.3 which translates references into
                        // values when source
                ...
                if ($count + 1 < count($argList) &&
                         is_integer($argList[$count + 1])) {
                    $count ++;
        ...
        for ($count = 0; $count < $fieldCount; $count ++) {
            ...
            if (! ($bits & 2)) { // Text data
                $field = new Zend_Search_Lucene_Field($fieldInfo->name, 
                        $fdtFile->readString(), 'UTF-8', true, 
                        $fieldInfo->isIndexed, $bits & 1);
            } else { // Binary data
                $field = new Zend_Search_Lucene_Field($fieldInfo->name, 
                        $fdtFile->readBinary(), '', true, $fieldInfo->isIndexed, 
                        $bits & 1, true);
            }
            ...
            $docsFilter = null)
    {
        ...
        foreach ($this->_segmentInfos as $segmentInfo) {
            $subResults[] = $segmentInfo->termDocs($term, $segmentStartDocId, 
                    $docsFilter);
     ...
     */
    public function termDocsFilter (Zend_Search_Lucene_Index_Term $term, 
            $docsFilter = null)
    {
        $segmentStartDocId = 0;
    ...
    public function terms ()
    {
        ...
        while (($segmentInfo = $segmentInfoQueue->pop()) !== null) {
            if ($segmentInfoQueue->top() === null || $segmentInfoQueue->top()
                ->currentTerm()
                ->key() != $segmentInfo->currentTerm()->key()) {
                // We got new term
    ...
    public function resetTermsStream ()
    {
        if ($this->_termsStream === null) {
            ...
            $this->_termsStream = new Zend_Search_Lucene_TermStreamsPriorityQueue(
                    $this->_segmentInfos);
        } else {
