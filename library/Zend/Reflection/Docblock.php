...
class Zend_Reflection_Docblock implements Reflector
{
    ...
    public function __construct ($commentOrReflector)
    {
        if ($commentOrReflector instanceof Reflector) {
            ...
            $this->_startLine = $this->_reflector->getStartLine() - $lineCount -
                     1;
    ...
    protected function _parse ()
    {
        ...
        while (($newlinePos = strpos($parsedDocComment, "\n")) !== false) {
            ...
            if ((strpos($line, '@') === 0) &&
                     (preg_match('#^(@\w+.*?)(\n)(?:@|\r?\n|$)#s', 
                            $parsedDocComment, $matches))) {
                $this->_tags[] = Zend_Reflection_Docblock_Tag::factory(
                        $matches[1]);
                $parsedDocComment = str_replace($matches[1] . $matches[2], '', 
                        $parsedDocComment);
            } else {
