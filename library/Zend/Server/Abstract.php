...
abstract class Zend_Server_Abstract implements Zend_Server_Interface
{
    ...
    public function __construct ()
    {
        ...
        $this->_table->setOverwriteExistingMethods(
                $this->_overwriteExistingMethods);
    }
            ...
            Zend_Server_Reflection_Function_Abstract $reflection)
    {
        ...
        if ($reflection instanceof Zend_Server_Reflection_Method) {
            $callback->setType($reflection->isStatic() ? 'static' : 'instance')
                ->setClass($reflection->getDeclaringClass()
                ->getName())
    ...
    protected function _buildSignature (
            Zend_Server_Reflection_Function_Abstract $reflection, $class = null)
    {
        $ns = $reflection->getNamespace();
        $name = $reflection->getName();
        $method = empty($ns) ? $name : $ns . '.' . $name;
        
        if (! $this->_overwriteExistingMethods &&
                 $this->_table->hasMethod($method)) {
            require_once 'Zend/Server/Exception.php';
            throw new Zend_Server_Exception(
                    'Duplicate method registered: ' . $method);
        }
     ...
     * @return mixed
     */
    protected function _dispatch (Zend_Server_Method_Definition $invocable, 
            array $params)
    {
