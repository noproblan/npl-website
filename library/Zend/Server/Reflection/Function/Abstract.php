...
abstract class Zend_Server_Reflection_Function_Abstract
{
     ...
     * @return array
     */
    protected function _buildSignatures ($return, $returnDesc, $paramTypes, 
            $paramDesc)
    {
        $this->_return = $return;
        ...
        foreach ($signatures as $signature) {
            $return = new Zend_Server_Reflection_ReturnValue(
                    array_shift($signature), $this->_returnDesc);
            $tmp = array();
            foreach ($signature as $key => $type) {
                $param = new Zend_Server_Reflection_Parameter($params[$key], 
                        $type, 
                        (isset($this->_paramDesc[$key]) ? $this->_paramDesc[$key] : null));
                $param->setPosition($key);
                $tmp[] = $param;
            }
            
            $this->_prototypes[] = new Zend_Server_Reflection_Prototype($return, 
                    $tmp);
        }
    }
    ...
    protected function _reflect ()
    {
        ...
        if (! empty($docBlock)) {
            ...
            if (preg_match('/@return\s+(\S+)/', $docBlock, $matches)) {
                ...
                if (preg_match('/@return\s+\S+\s+(.*?)(@|\*\/)/s', $docBlock, 
                        $matches)) {
                    $value = $matches[1];
        ...
        if (count($paramTypesTmp) != $paramCount) {
            ...
            throw new Zend_Server_Reflection_Exception(
                    'Variable number of arguments is not supported for services (except optional parameters). ' .
                             'Number of function arguments in ' .
                             $function->getDeclaringClass()->getName() . '::' .
                             $function->getName() .
                             '() must correspond to actual number of arguments described in the ' .
                             'docblock.');
        }
    ...
    public function __call ($method, $args)
    {
        ...
        require_once 'Zend/Server/Reflection/Exception.php';
        throw new Zend_Server_Reflection_Exception(
                'Invalid reflection method ("' . $method . '")');
    }
    ...
    public function setNamespace ($namespace)
    {
        ...
        if (! is_string($namespace) || ! preg_match('/[a-z0-9_\.]+/i', 
                $namespace)) {
            require_once 'Zend/Server/Reflection/Exception.php';
     ...
     * {@link Zend_Server_Reflection_Prototype
     * Zend_Server_Reflection_Prototypes}
     *
     * @return array
     */
    ...
    public function __wakeup ()
    {
        if ($this->_reflection instanceof ReflectionMethod) {
            ...
            $this->_reflection = new ReflectionMethod($class->newInstance(), 
                    $this->getName());
        } else {
