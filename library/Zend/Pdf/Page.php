...
class Zend_Pdf_Page extends Zend_Pdf_Canvas_Abstract
{
    ...
    /**
     * Page dictionary (refers to an inderect Zend_Pdf_Element_Dictionary
     * object).
     *
    ...
    /**
     * Flag which signals, that page is created separately from any PDF document
     * or
     * attached to anyone.
     *
     * @var boolean
     ...
     * context of previous contents stream (ex. drawing coordinate system may be
     * rotated).
     ...
     * New page is created in the same context as source page. Object factory is
     * shared.
     * Thus it will be attached to the document, but need to be placed into
     * Zend_Pdf::$pages array
     ...
     * If $factory is null then it will be created and page must be attached to
     * the document to be
     * included into output.
     * ---------------------------------------------------------
     * new Zend_Pdf_Page(string $pagesize, Zend_Pdf_ElementFactory_Interface
     * $factory = null);
     * ---------------------------------------------------------
     *
     * 4. Create new page with a specified pagesize (in default user space
     * units).
     * If $factory is null then it will be created and page must be attached to
     * the document to be
     * included into output.
     ...
     * Zend_Pdf_ElementFactory_Interface $factory = null);
     * ---------------------------------------------------------
     *
     *
     * @param mixed $param1            
    ...
    public function __construct ($param1, $param2 = null, $param3 = null)
    {
        if (($param1 instanceof Zend_Pdf_Element_Reference ||
            ...
            if ($param1 instanceof Zend_Pdf_Page && $param2 === null &&
                     $param3 === null) {
                // Duplicate existing page.
                // Let already existing content and resources to be shared
                // between pages
                // We don't give existing content modification functionality, so
                // we don't need "deep copy"
                $this->_objFactory = $param1->_objFactory;
                $this->_attached = &$param1->_attached;
                $this->_safeGS = false;
                
                $this->_dictionary = $this->_objFactory->newObject(
                ...
                foreach ($param1->_dictionary->getKeys() as $key) {
                    if ($key == 'Contents') {
                        ...
                        if ($param1->_dictionary->Contents->getType() !=
                                 Zend_Pdf_Element::TYPE_ARRAY) {
                            // Prepare array of content streams and add existing
                            // stream
                ...
                if (is_string($param1) &&
                    ...
                    if ($param2 !== null) {
                        ...
                        $this->_objFactory = Zend_Pdf_ElementFactory::createFactory(
                                1);
                    }
                    ...
                    if (count($pageDim) == 2 || count($pageDim) == 3) {
                        ...
                        /**
                         *
                         * @todo support of user defined pagesize notations,
                         *       like:
                ...
                /**
                 *
                 * @todo support of pagesize recalculation to "default user
                 *       space units"
                 */
                } else 
        ...
        $this->_dictionary->LastModified = new Zend_Pdf_Element_String(
                Zend_Pdf::pdfDate());
        ...
        $this->_dictionary->MediaBox->items[] = new Zend_Pdf_Element_Numeric(0);
        $this->_dictionary->MediaBox->items[] = new Zend_Pdf_Element_Numeric(
                $pageWidth);
        $this->_dictionary->MediaBox->items[] = new Zend_Pdf_Element_Numeric(
                $pageHeight);
        $this->_dictionary->Contents = new Zend_Pdf_Element_Array();
    ...
    protected function _addProcSet ($procSetName)
    {
        ...
        $this->_dictionary->Resources->ProcSet->items[] = new Zend_Pdf_Element_Name(
                $procSetName);
    }
        ...
        foreach ($resDictionary->getKeys() as $resType) {
            ...
            if ($resType == 'ProcSet') {
                ...
                foreach ($resMap->getKeys() as $resId) {
                    $resources[$resType][$resId] = new Zend_Pdf_Resource_Unified(
                            $resMap->$resId);
                }
            }
        }
    ...
    public function clonePage ($factory, &$processed)
    {
        ...
        $clonedPage = new Zend_Pdf_Page($factory->newObject($dictionary), 
                $factory);
     ...
     *
     * @todo Don't forget to close all current graphics operations (like path
     *       drawing)
     *      
     * @throws Zend_Pdf_Exception
     */
    public function flush ()
    {
        ...
        if ($this->_dictionary->Contents->getType() !=
                 Zend_Pdf_Element::TYPE_ARRAY) {
            /**
        ...
        if ((! $this->_safeGS) &&
                 (count($this->_dictionary->Contents->items) != 0)) {
            /**
            ...
            $newContentsArray->items[] = $this->_objFactory->newStreamObject(
                    " q\n");
        ...
        $this->_dictionary->Contents->items[] = $this->_objFactory->newStreamObject(
                $this->_contents);
     ...
     *
     * This method is not intended to be used in userland, but helps to optimize
     * some document wide operations
     *
    ...
    public function extractFonts ()
    {
        ...
        foreach ($fontResourcesUnique as $resourceId => $fontDictionary) {
            try {
                ...
                // Try to extract font
                $extractedFont = new Zend_Pdf_Resource_Font_Extracted(
                        $fontDictionary);
                
                $fonts[$resourceId] = $extractedFont;
            } catch (Zend_Pdf_Exception $e) {
                if ($e->getMessage() != 'Unsupported font type.') {
                    throw new Zend_Pdf_Exception($e->getMessage(), $e->getCode(), 
                            $e);
                }
            }
        }
    ...
    public function attachAnnotation (Zend_Pdf_Annotation $annotation)
    {
        ...
        if (! $annotationDictionary instanceof Zend_Pdf_Element_Object &&
            ...
            $annotationDictionary = $this->_objFactory->newObject(
