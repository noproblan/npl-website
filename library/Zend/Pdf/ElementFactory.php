...
class Zend_Pdf_ElementFactory implements Zend_Pdf_ElementFactory_Interface
{
    ...
    static public function createFactory ($objCount)
    {
        ...
        return new Zend_Pdf_ElementFactory_Proxy(
                new Zend_Pdf_ElementFactory($objCount));
    }
    ...
    public function getObjectCount ()
    {
        ...
        foreach ($this->_attachedFactories as $attached) {
            $count += $attached->getObjectCount() - 1; // -1 as "0" object is a
                                                       // special case and
                                                       // shared between
                                                       // factories
        }
    ...
    {
        if ($factory === $this ||
                 isset($this->_attachedFactories[$factory->getId()])) {
            /**
             * Don't attach factory twice.
             * We do not check recusively because of nature of attach operation
             * (Pages are always attached to the Documents, Fonts are always
             * attached
             * to the pages even if pages already use Document level object
             * factory and so on)
             */
            return;
        }
    ...
    public function calculateShift (Zend_Pdf_ElementFactory_Interface $factory)
    {
        ...
        foreach ($this->_attachedFactories as $subFactory) {
            ...
            if ($subFactoryShift != - 1) {
                // context found
                $this->_shiftCalculationCache[$factory->_factoryId] = $shift +
                         $subFactoryShift;
     ...
     * Has to be used after PDF render operation to let followed updates be
     * correct.
     */
     ...
     */
    public function getEnumerationShift (
    ...
    public function markAsModified (Zend_Pdf_Element_Object $obj)
    {
        if ($obj->getFactory() !== $this) {
            ...
            throw new Zend_Pdf_Exception(
                    'Object is not generated by this factory');
        }
     ...
     *
     * @todo Reusage of the freed object. It's not a support of new feature, but
     *       only improvement.
     *      
    ...
    public function newObject (Zend_Pdf_Element $objectValue)
    {
        ...
        $obj = new Zend_Pdf_Element_Object($objectValue, $this->_objectCount ++, 
                0, $this);
        $this->_modifiedObjects[$obj->getObjNum()] = $obj;
        return $obj;
    }
    ...
    public function newStreamObject ($streamValue)
    {
        ...
        $obj = new Zend_Pdf_Element_Object_Stream($streamValue, 
                $this->_objectCount ++, 0, $this);
        $this->_modifiedObjects[$obj->getObjNum()] = $obj;
        return $obj;
    }
        ...
        foreach ($this->_modifiedObjects as $objNum => $obj) {
            if ($this->_removedObjects->contains($obj)) {
                $result[$objNum + $shift] = new Zend_Pdf_UpdateInfoContainer(
                        $objNum + $shift, $obj->getGenNum() + 1, true);
            } else {
                $result[$objNum + $shift] = new Zend_Pdf_UpdateInfoContainer(
                        $objNum + $shift, $obj->getGenNum(), false, 
     ...
     *
     * It's used to clear "parent object" referencies when factory is closed and
     * clean up resources
     *
