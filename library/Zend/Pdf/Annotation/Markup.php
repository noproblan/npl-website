...
class Zend_Pdf_Annotation_Markup extends Zend_Pdf_Annotation
{
    ...
    public function __construct (Zend_Pdf_Element $annotationDictionary)
    {
        ...
        if ($annotationDictionary->Subtype === null ||
                 $annotationDictionary->Subtype->getType() !=
                 Zend_Pdf_Element::TYPE_NAME || ! in_array(
     ...
     * for for a highlight, underline, squiggly-underline, or strikeout
     * annotation,
     * respectively.
     *
     * $quadPoints is an array of 8xN numbers specifying the coordinates of
     * N quadrilaterals default user space. Each quadrilateral encompasses a
     * word or
     ...
     * @throws Zend_Pdf_Exception
     */
    public static function create ($x1, $y1, $x2, $y2, $text, $subType, 
            $quadPoints)
    {
        ...
        if (! is_array($quadPoints) || count($quadPoints) == 0 ||
                 count($quadPoints) % 8 != 0) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
