...
class Zend_Pdf_Cmap_SegmentToDelta extends Zend_Pdf_Cmap
{
    ...
    public function glyphNumbersForCharacters ($characterCodes)
    {
        ...
        foreach ($characterCodes as $key => $characterCode) {
            ...
            if ($this->_segmentTableIdRangeOffsets[$subtableIndex] == 0) {
                 ...
                 * glyph number.
                 */
                $glyphNumbers[$key] = ($characterCode +
                         $this->_segmentTableIdDeltas[$subtableIndex]) % 65536;
            } else {
                /*
                 ...
                 */
                $glyphIndex = ($characterCode -
                         $this->_segmentTableStartCodes[$subtableIndex] +
                         $this->_segmentTableIdRangeOffsets[$subtableIndex] -
                         $this->_segmentCount + $subtableIndex - 1);
    ...
    public function getCoveredCharacters ()
    {
        ...
        for ($i = 1; $i <= $this->_segmentCount; $i ++) {
            for ($code = $this->_segmentTableStartCodes[$i]; $code <=
                     $this->_segmentTableEndCodes[$i]; $code ++) {
                $characterCodes[] = $code;
            }
        }
    ...
    /**
     * Returns an array containing the glyphs numbers that have entries in this
     * character map.
     * Keys are Unicode character codes (integers)
     *
     * This functionality is partially covered by
     * glyphNumbersForCharacters(getCoveredCharacters())
     * call, but this method do it in more effective way (prepare complete list
     * instead of searching
     * glyph for each character code).
     *
     * @internal
     *
     * @return array Array representing <Unicode character code> => <glyph
     *         number> pairs.
     */
    public function getCoveredCharactersGlyphs ()
    {
        ...
        for ($segmentNum = 1; $segmentNum <= $this->_segmentCount; $segmentNum ++) {
            if ($this->_segmentTableIdRangeOffsets[$segmentNum] == 0) {
                ...
                for ($code = $this->_segmentTableStartCodes[$segmentNum]; $code <=
                         $this->_segmentTableEndCodes[$segmentNum]; $code ++) {
                    $glyphNumbers[$code] = ($code + $delta) % 65536;
                }
            } else {
                $code = $this->_segmentTableStartCodes[$segmentNum];
                $glyphIndex = $this->_segmentTableIdRangeOffsets[$segmentNum] -
                         ($this->_segmentCount - $segmentNum) - 1;
    ...
    public function __construct ($cmapData)
    {
        ...
        for ($i = 1; $i <= $this->_segmentCount; $i ++, $offset += 2) {
            $this->_segmentTableEndCodes[$i] = $this->_extractUInt2($cmapData, 
                    $offset);
        }
        ...
        for ($i = 1; $i <= $this->_segmentCount; $i ++, $offset += 2) {
            $this->_segmentTableStartCodes[$i] = $this->_extractUInt2($cmapData, 
                    $offset);
        }
        
        for ($i = 1; $i <= $this->_segmentCount; $i ++, $offset += 2) {
            $this->_segmentTableIdDeltas[$i] = $this->_extractInt2($cmapData, 
                    $offset); // signed
        }
        
        /*
         * The range offset helps determine the index into the glyph index
         * array.
         ...
         */
        for ($i = 1; $i <= $this->_segmentCount; $i ++, $offset += 2) {
            $this->_segmentTableIdRangeOffsets[$i] = $this->_extractUInt2(
                    $cmapData, $offset) >> 1;
        }
