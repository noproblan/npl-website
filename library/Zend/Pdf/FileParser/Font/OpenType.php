...
abstract class Zend_Pdf_FileParser_Font_OpenType extends Zend_Pdf_FileParser_Font
{
    ...
    protected function _parseTableDirectory ()
    {
        ...
        for ($tableIndex = 0; $tableIndex < $tableCount; $tableIndex ++) {
            ...
            /*
             * We ignore the checksum here for two reasons: First, the PDF
             * viewer
            ...
            $this->_debugLog('%s offset: 0x%x; length: %d', $tableName, 
    ...
    protected function _parseHeadTable ()
    {
        ...
        if ($magicNumber != 0x5f0f3cf5) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'Wrong magic number. Expected: 0x5f0f3cf5; actual: ' .
        ...
        $this->_debugLog('Font bounding box: %d %d %d %d', $this->xMin, 
                $this->yMin, $this->xMax, $this->yMax);
    ...
    protected function _parseNameTable ()
    {
        ...
        for ($nameIndex = 0; $nameIndex < $nameCount; $nameIndex ++) {
            ...
            if (! ((($platformID == 3) && ($encodingID == 1)) || // Microsoft
                                                                          // Unicode
            ...
            $languageCode = $this->_languageCodeForPlatform($platformID, 
                    $languageID);
            if ($languageCode === null) {
                $this->_debugLog('Skipping languageID: 0x%x; platformID %d', 
                        $languageID, $platformID);
                continue; // Not a supported language. Move on.
            }
            
            $this->_debugLog(
                    'Adding nameID: %d; languageID: 0x%x; platformID: %d; offset: 0x%x (0x%x); length: %d', 
                    $nameID, $languageID, $platformID, $baseOffset + $nameOffset, 
                    $nameOffset, $nameLength);
            
            /*
             * Entries in the name table are sorted by platform ID. If an entry
             * exists for both Mac Roman and Microsoft Unicode, the Unicode
             * entry
     ...
     *
     * The post table contains additional information required for using the
     * font
    ...
    protected function _parseMaxpTable ()
    {
        ...
        // Skip other maxp table entries (if presented with table version
    // 1.0)...
    }
    ...
    protected function _parseOs2Table ()
    {
        ...
        try {
            ...
            if ($e->getCode() == Zend_Pdf_Exception::REQUIRED_TABLE_NOT_FOUND) {
                ...
                $this->isSerifFont = false; // the style of the font is
                                                 // unknown
                ...
                /*
                 * Something else went wrong. Throw this exception higher up the
                 * chain.
                 */
                throw $e;
        ...
        if ($this->isBitSet(9, $embeddingFlags)) {
            ...
            if ($this->isBitSet(1, $embeddingFlags)) {
                /*
                 * Restricted license embedding. We currently don't have any way
                 * to
                 * enforce this, so interpret this as no embed. This may be
                 * revised
                 * in the future...
                 */
                $this->isEmbeddable = false;
                ...
                /*
                 * The remainder of the bit settings grant us permission to
                 * embed
                 * the font. There may be additional usage rights granted or
                 * denied
                 * but those only affect the PDF viewer application, not our
                 * code.
                 */
                $this->isEmbeddable = true;
            }
        $this->_debugLog(
                'Font ' . ($this->isEmbeddable ? 'may' : 'may not') .
                         ' be embedded');
        ...
        $this->_debugLog('Unicode ranges: 0x%x 0x%x 0x%x 0x%x', $unicodeRange1, 
                $unicodeRange2, $unicodeRange3, $unicodeRange4);
        ...
        /**
         *
         * @todo Font is recognized as Adobe Latin subset font if it only
         *       contains
         *       Basic Latin characters (only bit 0 of Unicode range bits is
         *       set).
         *       Actually, other Unicode subranges like General Punctuation (bit
         *       31) also
         *       fall into Adobe Latin characters. So this code has to be
         *       modified.
         */
        $this->isAdobeLatinSubset = (($unicodeRange1 == 1) &&
                 ($unicodeRange2 == 0) && ($unicodeRange3 == 0) &&
                 ($unicodeRange4 == 0));
        $this->_debugLog(
                ($this->isAdobeLatinSubset ? 'Is' : 'Is not') .
                         ' a subset of Adobe Latin');
        
        $this->vendorID = $this->readBytes(4);
        
        /*
         * Skip the font style bits. We use the values found in the 'head'
         * table.
        ...
        if ($tableVersion < 2) {
            ...
            /*
             * Ignore the remaining fields in this table. They are
             * Windows-specific.
             */
        }
    /**
     ...
     * There are usually at least two character maps in a font: Microsoft
     * Unicode
     ...
     * characters.
     *
     * @todo Need to rework the selection logic for picking a subtable. We
     *       should
     *       have an explicit list of preferences, followed by a list of those
     *       that
     *       are tolerable. Most specifically, since everything above this layer
     *       deals
     *       in Unicode, we need to be sure to only accept format 0 MacRoman
     *       tables.
     *      
     * @throws Zend_Pdf_Exception
     */
    protected function _parseCmapTable ()
    {
        ...
        for ($subtableIndex = 0; $subtableIndex < $subtableCount; $subtableIndex ++) {
            ...
            if (! ((($platformID == 0) && ($encodingID == 3)) || // Unicode
                     ...
                     (($platformID == 3) && ($encodingID == 1)) || // Microsoft
                                                                  // Unicode
                     (($platformID == 1) && ($encodingID == 0))) // Mac Roman
) {
                $this->_debugLog(
            ...
            if ($subtableOffset < 0) { // Sanity check for 4-byte unsigned on
                                       // 32-bit platform
                $this->_debugLog(
            ...
            $this->_debugLog(
                    ...
                    $platformID, $encodingID, $baseOffset + $subtableOffset, 
                    $subtableOffset);
        ...
        foreach ($offsets as $offset) {
            ...
            switch ($format) {
                case 0x0:
                    ...
                    if ($language != 0) {
                        $this->_debugLog(
                                'Type 0 cmap tables must be language-independent;' .
                ...
                case 0x6:
                    ...
                    if ($language != 0) {
                        $this->_debugLog(
                                'Warning: cmap tables must be language-independent - this font' .
                                         ' may not work properly; language: %d', 
                                        $language);
                    }
                    break;
                ...
                // if ($language != 0) {
                // $this->_debugLog('Warning: cmap tables must be
                // language-independent - this font'
                // . ' may not work properly; language: %d', $language);
                // }
                // break;
                
                default:
                    $this->_debugLog('Unknown subtable format: 0x%x; skipping', 
                            $format);
                    continue;
            }
            $cmapType = $format;
    ...
    {
        if (empty($this->_tableDirectory[$tableName])) { // do not allow NULL or
                                                         // zero
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
