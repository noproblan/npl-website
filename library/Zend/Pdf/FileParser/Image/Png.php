...
class Zend_Pdf_FileParser_Image_Png extends Zend_Pdf_FileParser_Image
{
    ...
    protected function _parseChunks ()
    {
        ...
        // Start processing chunks. If there are no more bytes to read parsing
                                 // is complete.
        $size = $this->getSize();
        while ($size - $this->getOffset() >= 8) {
            $chunkLength = $this->readUInt(4);
            if ($chunkLength < 0 ||
                     ($chunkLength + $this->getOffset() + 4) > $size) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        "PNG Corrupt: Invalid Chunk Size In File.");
            }
            
            $chunkType = $this->readBytes(4);
            $offset = $this->getOffset();
            
            // If we know how to process the chunk, do it here, else ignore the
            // chunk and move on to the next
            switch ($chunkType) {
                case 'IDAT': // This chunk may appear more than once. It
                ...
                case 'tRNS': // This chunk contains non-alpha channel
                             // transparency data
                ...
                // @TODO Implement the rest of the PNG chunks. (There are many
                             // not implemented here)
            }
            if ($offset + $chunkLength + 4 < $size) {
                $this->moveToOffset($offset + $chunkLength + 4); // Skip past
                                                                 // the data
                                                                 // finalizer.
                                                                 // (Don't rely
                                                                 // on the parse
    ...
    {
        $this->moveToOffset(12); // IHDR must always start at offset 12 and run
                                 // for 17 bytes
    ...
    protected function _parseTRNSChunk ($chunkOffset, $chunkLength)
    {
        ...
        switch ($this->_color) {
            ...
            case Zend_Pdf_Image::PNG_CHANNEL_RGB:
                ...
                // @TODO Fix this hack.
                // This parser cheats and only uses the lsb's (and only works
                // with < 16 bit depth images)
                 ...
                 *
                 * (If the image bit depth is less than 16, the least
                 * significant bits are used and the others are 0.)
                 * Pixels of the specified color value are to be treated as
                 * transparent (equivalent to alpha value 0);
                 * all other pixels are to be treated as fully opaque (alpha
                 * value 2bitdepth-1).
                 *
                 */
            ...
            case Zend_Pdf_Image::PNG_CHANNEL_INDEXED:
                ...
                // @TODO Fix this hack.
                // This parser cheats too. It only masks the first color in the
                // palette.
                 ...
                 * contain more alpha values than there are palette entries,
                 * but tRNS can contain fewer values than there are palette
                 * entries. In this case, the alpha value for all remaining
                 * palette
                 * entries is assumed to be 255. In the common case in which
                 * only palette index 0 need be made transparent, only a
                 * one-byte
