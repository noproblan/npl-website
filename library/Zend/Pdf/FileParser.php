...
abstract class Zend_Pdf_FileParser
{
    ...
    /**
     * Performs a cursory check to verify that the binary file is in the
     * expected
     * format.
    ...
    public function __construct (Zend_Pdf_FileParserDataSource $dataSource)
    {
        if ($dataSource->getSize() == 0) {
            ...
            throw new Zend_Pdf_Exception(
                    'The data source has not been properly initialized', 
     ...
     *            Use the BYTE_ORDER_ constants defined in {@link
     *            Zend_Pdf_FileParser}.
     *            If omitted, uses big-endian.
     * @return integer
     * @throws Zend_Pdf_Exception
     */
    public function readInt ($size, 
     ...
     * resulting value WILL BE SIGNED because PHP uses signed integers
     * internally
     * for everything. To guarantee portability, be sure to use bitwise
     * operators
            ...
            $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN)
    {
        ...
        $number = $this->readInt(($bitsToRead >> 3), $byteOrder) /
                 (1 << $fractionBits);
        return $number;
    }
     ...
     *            You may use any character set supported by {@link iconv()}. If
     *            omitted,
     *            uses 'current locale'.
     * @return string
     * @throws Zend_Pdf_Exception
    ...
    public function readStringUTF16 ($byteCount, 
            $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN, $characterSet = '')
    {
        if ($byteCount == 0) {
            return '';
        }
