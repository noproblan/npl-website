...
class Zend_Pdf_Filter_RunLength implements Zend_Pdf_Filter_Interface
{
    ...
    public static function encode ($data, $params = null)
    {
        ...
        while ($offset < strlen($data)) {
            // Do not encode 2 char chains since they produce 2 char run
            // sequence,
            // but it takes more time to decode such output (because of
            // processing additional run)
            if (($repeatedCharChainLength = strspn($data, $data[$offset], 
                    $offset + 1, 127) + 1) > 2) {
                if ($chainStartOffset != $offset) {
                    // Drop down previouse (non-repeatable chars) run
                    $output .= chr($offset - $chainStartOffset - 1) .
                             substr($data, $chainStartOffset, 
                                    $offset - $chainStartOffset);
                }
