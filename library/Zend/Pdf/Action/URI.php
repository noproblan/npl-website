...
class Zend_Pdf_Action_URI extends Zend_Pdf_Action
{
            ...
            SplObjectStorage $processedActions)
    {
        ...
        if ($dictionary->URI === null) {
            ...
            throw new Zend_Pdf_Exception(
                    'URI action dictionary entry is required');
        }
    }
     ...
     * @param boolean $isMap
     *            A flag specifying whether to track the mouse position when the
     *            URI is resolved
     ...
     * Set IsMap property
     *
     * If the IsMap flag is true and the user has triggered the URI action by
     * clicking
     * an annotation, the coordinates of the mouse position at the time the
     * action is
     * performed should be transformed from device space to user space and then
     * offset
