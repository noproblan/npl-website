 ...
 *
 * @todo Implement an ability to associate an outline item with a structure
 *       element (PDF 1.3 feature)
 *      
 * @package Zend_Pdf
 * @subpackage Outlines
...
class Zend_Pdf_Outline_Created extends Zend_Pdf_Outline
{
            ...
            SplObjectStorage $processedOutlines = null)
    {
        ...
        $outlineDictionary = $factory->newObject(
                new Zend_Pdf_Element_Dictionary());
        
        $outlineDictionary->Title = new Zend_Pdf_Element_String(
                $this->getTitle());
        ...
        if ($color !== null) {
            ...
            $outlineDictionary->C = new Zend_Pdf_Element_Array(
                    $colorComponentElements);
        }
        
        if ($this->isItalic() || $this->isBold()) {
            $outlineDictionary->F = new Zend_Pdf_Element_Numeric(
                    ($this->isItalic() ? 1 : 0) | // Bit
                                                                                                       // 1
                                                                                                       // -
                                                                                                       // Italic
($this->isBold() ? 2 : 0)); // Bit 2 - Bold
        ...
        foreach ($this->childOutlines as $childOutline) {
            ...
            if ($lastChild === null) {
                $lastChild = $childOutline->dumpOutline($factory, true, 
                        $outlineDictionary, null, $processedOutlines);
                $outlineDictionary->First = $lastChild;
            } else {
                $childOutlineDictionary = $childOutline->dumpOutline($factory, 
        ...
        if (count($this->childOutlines) != 0) {
            $outlineDictionary->Count = new Zend_Pdf_Element_Numeric(
                    ($this->isOpen() ? 1 : - 1) * count($this->childOutlines));
        }
