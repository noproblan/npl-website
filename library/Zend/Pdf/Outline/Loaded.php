 ...
 *
 * Instances of this class trace object update uperations. That allows to avoid
 * outlines PDF tree update
...
class Zend_Pdf_Outline_Loaded extends Zend_Pdf_Outline
{
    ...
    public function getTitle ()
    {
        if ($this->_outlineDictionary->Title === null) {
            ...
            throw new Zend_Pdf_Exception(
                    'Outline dictionary Title entry is required.');
        }
    ...
    public function setIsOpen ($isOpen)
    {
        ...
        if ($isOpen != $isOpenCurrentState) {
            ...
            $this->_outlineDictionary->Count->value = ($isOpen ? 1 : - 1) *
                     abs($childrenCount);
        }
    ...
    public function setIsItalic ($isItalic)
    {
        if ($this->_outlineDictionary->F === null) {
            ...
            $this->_outlineDictionary->F = new Zend_Pdf_Element_Numeric(
                    $isItalic ? 1 : 0);
        } else {
            $this->_outlineDictionary->F->touch();
            if ($isItalic) {
                $this->_outlineDictionary->F->value = $this->_outlineDictionary->F->value |
                         1;
            } else {
                $this->_outlineDictionary->F->value = $this->_outlineDictionary->F->value |
                         ~ 1;
    ...
    public function getColor ()
    {
        ...
        return new Zend_Pdf_Color_Rgb($components[0], $components[1], 
                $components[2]);
    }
    ...
    public function setColor (Zend_Pdf_Color_Rgb $color)
    {
        ...
        if ($color === null) {
            ...
            $this->_outlineDictionary->C = new Zend_Pdf_Element_Array(
            ...
            SplObjectStorage $processedDictionaries = null)
    {
        if ($dictionary->getType() != Zend_Pdf_Element::TYPE_DICTIONARY) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    '$dictionary mast be an indirect dictionary object.');
        }
        ...
        if ($dictionary->Count !== null) {
            ...
            while ($childDictionary !== null) {
                ...
                if (! $processedDictionaries->contains($childDictionary)) {
                    $this->childOutlines[] = new Zend_Pdf_Outline_Loaded(
                            $childDictionary, $processedDictionaries);
                }
    ...
    public function dumpOutline (Zend_Pdf_ElementFactory_Interface $factory, 
            $updateNavigation, Zend_Pdf_Element $parent, 
            ...
            SplObjectStorage $processedOutlines = null)
    {
        ...
        if (count($this->_originalChildOutlines) != count($this->childOutlines)) {
            ...
            if (! (array_keys($this->_originalChildOutlines) ===
                     array_keys($this->childOutlines))) {
        ...
        if ($updateChildNavigation) {
            ...
            foreach ($this->childOutlines as $childOutline) {
                if ($processedOutlines->contains($childOutline)) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
                            'Outlines cyclyc reference is detected.');
                }
                
                if ($lastChild === null) {
                    // First pass. Update Outlines dictionary First entry using
                    // corresponding value
                    $lastChild = $childOutline->dumpOutline($factory, 
                            $updateChildNavigation, $this->_outlineDictionary, 
                            null, $processedOutlines);
                    $this->_outlineDictionary->First = $lastChild;
                } else {
                    // Update previous outline dictionary Next entry (Prev is
                    // updated within dumpOutline() method)
                    $childOutlineDictionary = $childOutline->dumpOutline(
                            $factory, $updateChildNavigation, 
                            $this->_outlineDictionary, $lastChild, 
                            $processedOutlines);
            ...
            if (count($this->childOutlines) != 0) {
                $this->_outlineDictionary->Count = new Zend_Pdf_Element_Numeric(
                        ($this->isOpen() ? 1 : - 1) * count(
                                $this->childOutlines));
            } else {
    ...
    public function dump ($level = 0)
    {
        printf(":%3d:%s:%s:%s%s  :\n", count($this->childOutlines), 
                $this->isItalic() ? 'i' : ' ', $this->isBold() ? 'b' : ' ', 
                str_pad('', 4 * $level), $this->getTitle());
