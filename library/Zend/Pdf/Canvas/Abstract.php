...
abstract class Zend_Pdf_Canvas_Abstract implements Zend_Pdf_Canvas_Interface
{
     ...
     * @return string
     */
    abstract protected function _attachResource ($type, 
     ...
     */
    public function drawCanvas (Zend_Pdf_Canvas_Interface $canvas, $x1, $y1, 
            $x2 = null, $y2 = null)
    {
        $this->saveGS();
     ...
     *
     * Pattern is an array of floats: array(on_length, off_length, on_length,
     * off_length, ...)
    ...
    public function setFont (Zend_Pdf_Resource_Font $font, $fontSize)
    {
        ...
        $this->_contents .= $fontNameObj->toString() . ' ' .
                 $fontSizeObj->toString() . " Tf\n";
     ...
     * This takes a snapshot of the currently applied style, position, clipping
     * area and
    ...
    public function restoreGS ()
    {
        if ($this->_saveCount -- <= 0) {
            ...
            throw new Zend_Pdf_Exception(
                    'Restoring graphics state which is not saved');
        }
        $this->_contents .= " Q\n";
     ...
     * Normal (default), Multiply, Screen, Overlay, Darken, Lighten, ColorDodge,
     * ColorBurn, HardLight,
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function clipCircle ($x, $y, $radius, $startAngle = null, 
            $endAngle = null)
    {
        $this->clipEllipse($x - $radius, $y - $radius, $x + $radius, 
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function clipEllipse ($x1, $y1, $x2, $y2, $startAngle = null, 
            $endAngle = null)
    {
        $this->_addProcSet('PDF');
        ...
        if ($startAngle !== null) {
            ...
            for ($count = 0; $count <= $clipSectors; $count ++) {
                ...
                $pX = new Zend_Pdf_Element_Numeric($x + cos($pAngle) *
                         $clipRadius);
                $pY = new Zend_Pdf_Element_Numeric($y + sin($pAngle) *
                         $clipRadius);
        ...
        $this->_contents .= $xC->toString() . ' ' . $yUp->toString() . " m\n" .
                 ...
                 $xl->toString() . ' ' . $yUp->toString() . ' ' . $xC->toString() .
                 ' ' . $yUp->toString() . " c\n" . "h\nW\nn\n";
     ...
     * (at PI/8, 3*PI/8, 5*PI/8, 7*PI/8, 9*PI/8, 11*PI/8, 13*PI/8 and 15*PI/8
     * angles).
     * At 0, PI/4, PI/2, 3*PI/4, PI, 5*PI/4, 3*PI/2 and 7*PI/4 it's exactly a
     * tangent to a circle.
     *
     * @param float $x            
     * @param float $y            
    ...
    public function drawCircle ($x, $y, $radius, $param4 = null, $param5 = null, 
            $param6 = null)
    {
        $this->drawEllipse($x - $radius, $y - $radius, $x + $radius, 
                $y + $radius, $param4, $param5, $param6);
     ...
     * @return Zend_Pdf_Canvas_Interface
     */
    public function drawImage (Zend_Pdf_Resource_Image $image, $x1, $y1, $x2, 
            $y2)
    {
        $this->_addProcSet('PDF');
        ...
        $this->_contents .= "q\n" . '1 0 0 1 ' . $x1Obj->toString() . ' ' .
                 $y1Obj->toString() . " cm\n" . $widthObj->toString() . ' 0 0 ' .
                 $heightObj->toString() . " 0 0 cm\n" . $imageNameObj->toString() .
                 " Do\n" . "Q\n";
     ...
     *
     * Fill types:
     * Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE - fill rectangle and stroke
     * (default)
            ...
            $fillType = Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE)
    {
        ...
        if ($radius[1] != 0) {
            ...
            $this->_contents .= $x1Obj->toString() . ' ' . $y1Obj->toString() .
                     ' ' . $x2Obj->toString() . ' ' . $y2Obj->toString() . ' ' .
                     $x3Obj->toString() . ' ' . $y3Obj->toString() . ' ' . " c\n";
        }
    ...
    public function drawText ($text, $x, $y, $charEncoding = '')
    {
        ...
        $textObj = new Zend_Pdf_Element_String(
                $this->_font->encodeString($text, $charEncoding));
        $xObj = new Zend_Pdf_Element_Numeric($x);
        $yObj = new Zend_Pdf_Element_Numeric($y);
        
        $this->_contents .= "BT\n" . $xObj->toString() . ' ' . $yObj->toString() .
                 " Td\n" . $textObj->toString() . " Tj\n" . "ET\n";
     ...
     *          
     * @throws Zend_Pdf_Exception - if a path hasn't been started with
     *         pathMove()
     * @return Zend_Pdf_Canvas_Interface
     */
    ...
    public function rotate ($x, $y, $angle)
    {
        ...
        $this->_addProcSet('PDF');
        $this->_contents .= '1 0 0 1 ' . $xObj->toString() . ' ' .
                 $yObj->toString() . " cm\n" . $cos->toString() . ' ' .
                 $sin->toString() . ' ' . $mSin->toString() . ' ' .
    ...
    public function scale ($xScale, $yScale)
    {
        ...
        $this->_addProcSet('PDF');
        $this->_contents .= $xScaleObj->toString() . ' 0 0 ' .
                 $yScaleObj->toString() . " 0 0 cm\n";
    ...
    public function translate ($xShift, $yShift)
    {
        ...
        $this->_addProcSet('PDF');
        $this->_contents .= '1 0 0 1 ' . $xShiftObj->toString() . ' ' .
                 $yShiftObj->toString() . " cm\n";
    ...
    public function skew ($x, $y, $xAngle, $yAngle)
    {
        ...
        $this->_contents .= '1 0 0 1 ' . $xObj->toString() . ' ' .
                 $yObj->toString() . " cm\n" . '1 ' . $tanXObj->toString() . ' ' .
                 $tanYObj->toString() . " 1 0 0 cm\n" . '1 0 0 1 ' .
                 $mXObj->toString() . ' ' . $mYObj->toString() . " cm\n";
