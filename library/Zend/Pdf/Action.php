 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
abstract class Zend_Pdf_Action extends Zend_Pdf_Target implements 
        RecursiveIterator, Countable
{
     ...
     *            list of already processed action dictionaries, used to avoid
     *            cyclic references
     * @throws Zend_Pdf_Exception
     */
    public function __construct (Zend_Pdf_Element $dictionary, 
        ...
        if ($dictionary->Next !== null) {
            if ($dictionary->Next instanceof Zend_Pdf_Element_Dictionary) {
                ...
                if (! $processedActions->contains($dictionary->Next)) {
                    ...
                    $this->next[] = Zend_Pdf_Action::load($dictionary->Next, 
                            $processedActions);
                }
            } else 
                if ($dictionary->Next instanceof Zend_Pdf_Element_Array) {
                    foreach ($dictionary->Next->items as $chainedActionDictionary) {
                        // Check if dictionary object is not already processed
                        if (! $processedActions->contains(
                                $chainedActionDictionary)) {
                            $processedActions->attach($chainedActionDictionary);
                            $this->next[] = Zend_Pdf_Action::load(
                                    $chainedActionDictionary, $processedActions);
                        }
                    }
                } else {
                    require_once 'Zend/Pdf/Exception.php';
     ...
     *            (It's actually Dictionary or Dictionary Object or Reference to
     *            a Dictionary Object)
     * @param SplObjectStorage $processedActions
     *            list of already processed action dictionaries, used to avoid
     *            cyclic references
     ...
     * @throws Zend_Pdf_Exception
     */
    public static function load (Zend_Pdf_Element $dictionary, 
            SplObjectStorage $processedActions = null)
    {
        ...
        if ($dictionary->S === null) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'Action dictionary must contain S entry');
        }
        ...
        switch ($dictionary->S->value) {
            ...
            case 'SubmitForm':
                ...
                return new Zend_Pdf_Action_SubmitForm($dictionary, 
                        $processedActions);
                break;
            
            case 'ResetForm':
                require_once 'Zend/Pdf/Action/ResetForm.php';
                return new Zend_Pdf_Action_ResetForm($dictionary, 
                        $processedActions);
                break;
            
            case 'ImportData':
                require_once 'Zend/Pdf/Action/ImportData.php';
                return new Zend_Pdf_Action_ImportData($dictionary, 
                        $processedActions);
                break;
            
            case 'JavaScript':
                require_once 'Zend/Pdf/Action/JavaScript.php';
                return new Zend_Pdf_Action_JavaScript($dictionary, 
                        $processedActions);
                break;
            
            case 'SetOCGState':
                require_once 'Zend/Pdf/Action/SetOCGState.php';
                return new Zend_Pdf_Action_SetOCGState($dictionary, 
                        $processedActions);
                break;
            
            case 'Rendition':
                require_once 'Zend/Pdf/Action/Rendition.php';
                return new Zend_Pdf_Action_Rendition($dictionary, 
                        $processedActions);
                break;
            ...
            case 'GoTo3DView':
                ...
                return new Zend_Pdf_Action_GoTo3DView($dictionary, 
                        $processedActions);
                break;
            
            default:
                require_once 'Zend/Pdf/Action/Unknown.php';
                return new Zend_Pdf_Action_Unknown($dictionary, 
                        $processedActions);
                break;
        }
    }
     ...
     * @param SplObjectStorage $processedActions
     *            list of already processed actions (used to prevent infinity
     *            loop caused by cyclic references)
     * @return Zend_Pdf_Element_Object|Zend_Pdf_Element_Reference Dictionary
     *         indirect object
     */
    public function dumpAction (Zend_Pdf_ElementFactory_Interface $factory, 
            SplObjectStorage $processedActions = null)
    {
        if ($processedActions === null) {
            $processedActions = new SplObjectStorage();
        ...
        if ($processedActions->contains($this)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'Action chain cyclyc reference is detected.');
        }
        $processedActions->attach($this);
        
        $childListUpdated = false;
        if (count($this->_originalNextList) != count($this->next)) {
            // If original and current children arrays have different size then
            // children list was updated
            $childListUpdated = true;
        } else 
            if (! (array_keys($this->_originalNextList) ===
                     array_keys($this->next))) {
                // If original and current children arrays have different keys
                // (with a glance to an order) then children list was updated
                $childListUpdated = true;
            } else {
        ...
        if ($childListUpdated) {
            ...
            switch (count($this->next)) {
                ...
                case 1:
                    ...
                    $this->_actionDictionary->Next = $child->dumpAction(
                ...
                default:
                    ...
                    foreach ($this->next as $child) {
                        ...
                        $pdfChildArray->items[] = $child->dumpAction($factory, 
                                $processedActions);
                    }
        ...
        if ($this->_actionDictionary instanceof Zend_Pdf_Element_Dictionary) {
            // It's a newly created action. Register it within object factory
            // and return indirect object
