...
class Zend_Pdf_Element_Object_Stream extends Zend_Pdf_Element_Object
{
     ...
     */
    public function __construct ($val, $objNum, $genNum, 
            Zend_Pdf_ElementFactory $factory, $dictionary = null)
    {
        parent::__construct(new Zend_Pdf_Element_Stream($val), $objNum, $genNum, 
                $factory);
        
        if ($dictionary === null) {
            $this->_dictionary = new Zend_Pdf_Element_Dictionary();
            $this->_dictionary->Length = new Zend_Pdf_Element_Numeric(
                    strlen($val));
    ...
    /**
     * Extract dictionary data which are used to store information and to
     * normalize filters
    ...
    private function _extractDictionaryData ()
    {
        ...
        if ($this->_dictionary->Filter === null) {
            ...
            if ($this->_dictionary->Filter->getType() ==
                ...
                foreach ($this->_dictionary->Filter->items as $id => $filter) {
                    ...
                    if ($this->_dictionary->DecodeParms !== null) {
                        if ($this->_dictionary->DecodeParms->items[$id] !== null &&
                                 $this->_dictionary->DecodeParms->items[$id]->value !==
                                 null) {
        ...
        if ($this->_dictionary->FFilter === null) {
            ...
            if ($this->_dictionary->FFilter->getType() ==
                     Zend_Pdf_Element::TYPE_ARRAY) {
                foreach ($this->_dictionary->FFilter->items as $id => $filter) {
                    ...
                    if ($this->_dictionary->FDecodeParms !== null) {
                        if ($this->_dictionary->FDecodeParms->items[$id] !== null &&
                                 $this->_dictionary->FDecodeParms->items[$id]->value !==
                                 null) {
    ...
    private function _decodeStream ()
    {
         ...
         * If we don't recognize any of applied filetrs an exception should be
         * thrown here
         */
        if (isset($this->_initialDictionaryData['F'])) {
            ...
            throw new Zend_Pdf_Exception(
                    'External filters are not supported now.');
        }
    ...
    public function __get ($property)
    {
        if ($property == 'dictionary') {
            /**
             * If stream is not decoded yet, then store original decoding
             * options (do it only once).
             */
            if ((! $this->_streamDecoded) &&
                     ($this->_initialDictionaryData === null)) {
                $this->_initialDictionaryData = $this->_extractDictionaryData();
            }
    ...
    public function __set ($property, $value)
    {
        ...
        require_once 'Zend/Pdf/Exception.php';
        throw new Zend_Pdf_Exception(
                'Unknown stream object property: \'' . $property . '\'.');
    }
            ...
            array &$processed, $mode)
    {
        ...
        $streamDictionary = $this->_dictionary->makeClone($factory, $processed, 
                $mode);
        
        // Make new empty instance of stream object and register it in
        // $processed container
        $processed[$id] = $clonedObject = $factory->newStreamObject('');
        
        // Copy current object data and state
        $clonedObject->_dictionary = $this->_dictionary->makeClone($factory, 
                $processed, $mode);
        $clonedObject->_value = $this->_value->makeClone($factory, $processed, 
                $mode);
