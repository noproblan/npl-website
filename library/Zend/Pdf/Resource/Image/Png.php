...
class Zend_Pdf_Resource_Image_Png extends Zend_Pdf_Resource_Image
{
     ...
     * @todo Add compression conversions to support compression strategys other
     *       than PNG_COMPRESSION_DEFAULT_STRATEGY.
     * @todo Add pre-compression filtering.
     * @todo Add interlaced image handling.
     * @todo Add support for 16-bit images. Requires PDF version bump to 1.5 at
     *       least.
    ...
    public function __construct ($imageFileName)
    {
        if (($imageFile = @fopen($imageFileName, 'rb')) === false) {
            ...
            throw new Zend_Pdf_Exception(
                    "Can not open '$imageFileName' file for reading.");
        }
        ...
        fseek($imageFile, 12, SEEK_CUR); // Signature bytes (Includes the IHDR
                                         // chunk) IHDR processed linerarly
                                         // because it doesnt contain a variable
                                         // chunk length
        $wtmp = unpack('Ni', fread($imageFile, 4)); // Unpack a 4-Byte Long
        $width = $wtmp['i'];
        $htmp = unpack('Ni', fread($imageFile, 4));
        $height = $htmp['i'];
        $bits = ord(fread($imageFile, 1)); // Higher than 8 bit depths are only
        ...
        if (($interlacing = ord(fread($imageFile, 1))) !=
                 Zend_Pdf_Resource_Image_Png::PNG_INTERLACING_DISABLED) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
        ...
        /*
         * The following loop processes PNG chunks. 4 Byte Longs are packed
         * first give the chunk length
         * followed by the chunk signature, a four byte code. IDAT and IEND are
         * manditory in any PNG.
         */
        while (! feof($imageFile)) {
            ...
            if ($chunkLengthBytes === false) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        'Error ocuured while image file reading.');
            }
            ...
            switch ($chunkType) {
                case 'IDAT': // Image Data
                    /*
                     * Reads the actual image data from the PNG file. Since we
                     * know at this point that the compression
                     * strategy is the default strategy, we also know that this
                     * data is Zip compressed. We will either copy
                     * the data directly to the PDF and provide the correct
                     * FlateDecode predictor, or decompress the data
                     * decode the filters and output the data as a raw pixel
                     * map.
                     */
                ...
                case 'tRNS': // Basic (non-alpha channel) transparency.
                    ...
                    switch ($color) {
                        ...
                        case Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_INDEXED:
                            ...
                            // Find the first transparent color in the index, we
                            // will mask that. (This is a bit of a hack. This
                            // should be a SMask and mask all entries values).
                        ...
                        case Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_RGB_ALPHA:
                            require_once 'Zend/Pdf/Exception.php';
                            throw new Zend_Pdf_Exception(
                                    "tRNS chunk illegal for Alpha Channel Images");
                            break;
                    }
                    fseek($imageFile, 4, SEEK_CUR); // 4 Byte Ending Sequence
                    break;
                ...
                default:
                    fseek($imageFile, $chunkLength + 4, SEEK_CUR); // Skip the
                                                                   // section
                    break;
            }
        }
        ...
        switch ($color) {
            ...
            case Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_INDEXED:
                ...
                $colorSpace->items[] = new Zend_Pdf_Element_Numeric(
                        (strlen($paletteData) / 3 - 1));
                $paletteObject = $this->_objectFactory->newObject(
                        new Zend_Pdf_Element_String_Binary($paletteData));
            ...
            case Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_GRAY_ALPHA:
                ...
                if ($bits > 8) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
                            "Alpha PNGs with bit depth > 8 are not yet supported");
                }
                ...
                $decodingStream = $decodingObjFactory->newStreamObject(
                        $imageData);
                $decodingStream->dictionary->Filter = new Zend_Pdf_Element_Name(
                        'FlateDecode');
                $decodingStream->dictionary->DecodeParms = new Zend_Pdf_Element_Dictionary();
                $decodingStream->dictionary->DecodeParms->Predictor = new Zend_Pdf_Element_Numeric(
                        15);
                $decodingStream->dictionary->DecodeParms->Columns = new Zend_Pdf_Element_Numeric(
                        $width);
                $decodingStream->dictionary->DecodeParms->Colors = new Zend_Pdf_Element_Numeric(
                        2); // GreyAlpha
                $decodingStream->dictionary->DecodeParms->BitsPerComponent = new Zend_Pdf_Element_Numeric(
                        $bits);
                $decodingStream->skipFilters();
                
                $pngDataRawDecoded = $decodingStream->value;
                
                // Iterate every pixel and copy out gray data and alpha channel
                // (this will be slow)
                for ($pixel = 0, $pixelcount = ($width * $height); $pixel <
                         $pixelcount; $pixel ++) {
                    $imageDataTmp .= $pngDataRawDecoded[($pixel * 2)];
                    $smaskData .= $pngDataRawDecoded[($pixel * 2) + 1];
                }
                $compressed = false;
                $imageData = $imageDataTmp; // Overwrite image data with the
                                             // gray channel without alpha
                break;
            ...
            case Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_RGB_ALPHA:
                ...
                // Iterate every pixel and copy out rgb data and alpha channel
                // (this will be slow)
                for ($pixel = 0, $pixelcount = ($width * $height); $pixel <
                         $pixelcount; $pixel ++) {
                    $imageDataTmp .= $pngDataRawDecoded[($pixel * 4) + 0] .
                     $pngDataRawDecoded[($pixel * 4) + 1] .
                     $pngDataRawDecoded[($pixel * 4) + 2];
            $smaskData .= $pngDataRawDecoded[($pixel * 4) + 3];
        }
        
        $compressed = false;
        $imageData = $imageDataTmp; // Overwrite image data with the RGB
                                     // channel without alpha
        break;
...
if (empty($imageData)) {
    require_once 'Zend/Pdf/Exception.php';
    throw new Zend_Pdf_Exception(
            "Corrupt PNG Image. Mandatory IDAT chunk not found.");
}

$imageDictionary = $this->_resource->dictionary;
if (! empty($smaskData)) {
    /*
     * Includes the Alpha transparency data as a Gray Image, then assigns the
     * image as the Shadow Mask for the main image data.
     */
    ...
    $smaskStream->dictionary->ColorSpace = new Zend_Pdf_Element_Name(
            'DeviceGray');
    $smaskStream->dictionary->BitsPerComponent = new Zend_Pdf_Element_Numeric(
            $bits);
    ...
    $smaskStream->dictionary->DecodeParms = new Zend_Pdf_Element_Dictionary(
...
$decodeParms = array();
$decodeParms['Predictor'] = new Zend_Pdf_Element_Numeric(15); // Optimal
                                                                     // prediction
$decodeParms['Columns'] = new Zend_Pdf_Element_Numeric($width);
$decodeParms['Colors'] = new Zend_Pdf_Element_Numeric(
        (($color == Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_RGB ||
                 $color == Zend_Pdf_Resource_Image_Png::PNG_CHANNEL_RGB_ALPHA) ? (3) : (1)));
