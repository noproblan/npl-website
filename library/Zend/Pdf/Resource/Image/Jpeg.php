...
class Zend_Pdf_Resource_Image_Jpeg extends Zend_Pdf_Resource_Image
{
    ...
    public function __construct ($imageFileName)
    {
        ...
        $gd_options = gd_info();
        if ((! isset($gd_options['JPG Support']) ||
                 $gd_options['JPG Support'] != true) &&
                 (! isset($gd_options['JPEG Support']) ||
                 $gd_options['JPEG Support'] != true)) {
            require_once 'Zend/Pdf/Exception.php';
        ...
        if (($imageInfo = getimagesize($imageFileName)) === false) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'Corrupted image or image doesn\'t exist.');
        }
        if ($imageInfo[2] != IMAGETYPE_JPEG &&
                 $imageInfo[2] != IMAGETYPE_JPEG2000) {
            require_once 'Zend/Pdf/Exception.php';
        ...
        while ($byteCount > 0 && ! feof($imageFile)) {
            ...
            if ($nextBlock === false) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        "Error occured while '$imageFileName' file reading.");
            }
