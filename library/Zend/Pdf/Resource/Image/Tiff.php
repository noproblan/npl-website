...
class Zend_Pdf_Resource_Image_Tiff extends Zend_Pdf_Resource_Image
{
     ...
     *
     * Makes it possible to unpack bytes in one statement for enhanced logic
     * readability.
     *
     * @param int $type            
     * @param string $bytes            
    ...
    protected function unpackBytes ($type, $bytes)
    {
        ...
        switch ($type) {
            ...
            case Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT:
                $format = ($this->_endianType ==
                         Zend_Pdf_Resource_Image_Tiff::TIFF_ENDIAN_LITTLE) ? 'v' : 'n';
                $unpacked = unpack($format, $bytes);
                return $unpacked[1];
                break;
            ...
            case Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_RATIONAL:
                $format = ($this->_endianType ==
                         Zend_Pdf_Resource_Image_Tiff::TIFF_ENDIAN_LITTLE) ? 'V2' : 'N2';
                $unpacked = unpack($format, $bytes);
    ...
    public function __construct ($imageFileName)
    {
        ...
        $version = $this->unpackBytes(
                Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT, 
                fread($imageFile, 2));
        ...
        $ifdOffset = $this->unpackBytes(
                Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_LONG, 
                fread($imageFile, 4));
        ...
        /*
         * Tiff files are stored as a series of Image File Directories (IFD)
         * each direcctory
         * has a specific number of entries each 12 bytes in length. At the end
         * of the directories
         * is four bytes pointing to the offset of the next IFD.
         */
        
        while ($ifdOffset > 0) {
            if (fseek($imageFile, $ifdOffset, SEEK_SET) == - 1 ||
                     $ifdOffset + 2 >= $this->_fileSize) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        "Could not seek to the image file directory as indexed by the file. Likely cause is TIFF corruption. Offset: " .
                                 $ifdOffset);
            }
            
            $numDirEntries = $this->unpackBytes(
                    Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT, 
                    fread($imageFile, 2));
            
            /*
             * Since we now know how many entries are in this (IFD) we can
             * extract the data.
             * The format of a TIFF directory entry is:
             *
             * 2 bytes (short) tag code; See TIFF_TAG constants at the top for
             * supported values. (There are many more in the spec)
             * 2 bytes (short) field type
             * 4 bytes (long) number of values, or value count.
             * 4 bytes (mixed) data if the data will fit into 4 bytes or an
             * offset if the data is too large.
             */
            for ($dirEntryIdx = 1; $dirEntryIdx <= $numDirEntries; $dirEntryIdx ++) {
                $tag = $this->unpackBytes(
                        Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT, 
                        fread($imageFile, 2));
                $fieldType = $this->unpackBytes(
                        Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT, 
                        fread($imageFile, 2));
                $valueCount = $this->unpackBytes(
                        Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_LONG, 
                        fread($imageFile, 4));
                ...
                if ($fieldLength <= 4) {
                    switch ($fieldType) {
                        case Zend_Pdf_Resource_Image_Tiff::TIFF_FIELD_TYPE_BYTE:
                            $value = $this->unpackBytes(
                                    Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_BYTE, 
                                    $offsetBytes);
                            break;
                        case Zend_Pdf_Resource_Image_Tiff::TIFF_FIELD_TYPE_ASCII:
                    ...
                    $refOffset = $this->unpackBytes(
                            Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_LONG, 
                            $offsetBytes);
                }
                /*
                 * Linear tag processing is probably not the best way to do
                 * this. I've processed the tags according to the
                 * Tiff 6 specification and make some assumptions about when
                 * tags will be < 4 bytes and fit into $value and when
                 * they will be > 4 bytes and require seek/extraction of the
                 * offset. Same goes for extracting arrays of data, like
                 * the data offsets and length. This should be fixed in the
                 * future.
                 */
                switch ($tag) {
                    ...
                    case Zend_Pdf_Resource_Image_Tiff::TIFF_TAG_BITS_PER_SAMPLE:
                        if ($valueCount > 1) {
                            ...
                            $this->_bitsPerSample = $this->unpackBytes(
                                    Zend_Pdf_Resource_Image_Tiff::UNPACK_TYPE_SHORT, 
                                    fread($imageFile, 2));
                    ...
                    case Zend_Pdf_Resource_Image_Tiff::TIFF_TAG_COMPRESSION:
                        ...
                        switch ($value) {
                            ...
                            case Zend_Pdf_Resource_Image_Tiff::TIFF_COMPRESSION_LZW:
                                ...
                                require_once 'Zend/Pdf/Exception.php';
                                throw new Zend_Pdf_Exception(
                                        "LZWDecode Compression Mode Not Currently Supported");
                                break;
                            case Zend_Pdf_Resource_Image_Tiff::TIFF_COMPRESSION_JPEG:
                                $this->_filter = 'DCTDecode'; // Should work,
                                                              // doesnt...
                                require_once 'Zend/Pdf/Exception.php';
                                throw new Zend_Pdf_Exception(
                                        "JPEG Compression Mode Not Currently Supported");
                                break;
                            ...
                            case Zend_Pdf_Resource_Image_Tiff::TIFF_COMPRESSION_FLATE_OBSOLETE_CODE:
                                ...
                                require_once 'Zend/Pdf/Exception.php';
                                throw new Zend_Pdf_Exception(
                                        "ZIP/Flate Compression Mode Not Currently Supported");
                                break;
                    ...
                    case Zend_Pdf_Resource_Image_Tiff::TIFF_TAG_PHOTOMETRIC_INTERPRETATION:
                        ...
                        switch ($value) {
                            ...
                            default:
                                require_once 'Zend/Pdf/Exception.php';
                                throw new Zend_Pdf_Exception(
                                        'TIFF: Unknown or Unsupported Color Type: ' .
                                                 $value);
                        }
                        break;
                    case Zend_Pdf_Resource_Image_Tiff::TIFF_TAG_STRIP_OFFSETS:
                        if ($valueCount > 1) {
                            ...
                            $this->_imageDataOffset = unpack($format, 
                                    $stripOffsetsBytes);
                            fseek($imageFile, $fp, SEEK_SET);
                        } else {
                    ...
                    case Zend_Pdf_Resource_Image_Tiff::TIFF_TAG_STRIP_BYTE_COUNTS:
                        if ($valueCount > 1) {
                            ...
                            $fp = ftell($imageFile);
                            fseek($imageFile, $refOffset, SEEK_SET);
                            $stripByteCountsBytes = fread($imageFile, 
                                    $fieldLength);
                            $this->_imageDataLength = unpack($format, 
                                    $stripByteCountsBytes);
                            fseek($imageFile, $fp, SEEK_SET);
                        } else {
                        ...
                        break;
                    default:
                    // For debugging. It should be harmless to ignore unknown
                // tags, though there is some good info in them.
        ...
        if (is_array($this->_imageDataOffset)) {
            ...
            foreach ($this->_imageDataOffset as $idx => $offset) {
                ...
                $imageDataBytes .= fread($imageFile, 
                        $this->_imageDataLength[$idx]);
            }
        } else {
        ...
        if (! isset($this->_width) || ! isset($this->_width)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    "Problem reading tiff file. Tiff is probably corrupt.");
        }
        ...
        if ($this->_whiteIsZero === true) {
            $imageDictionary->Decode = new Zend_Pdf_Element_Array(
                    array(
        ...
        $imageDictionary->ColorSpace = new Zend_Pdf_Element_Name(
                $this->_colorSpace);
        $imageDictionary->BitsPerComponent = new Zend_Pdf_Element_Numeric(
                $this->_bitsPerSample);
