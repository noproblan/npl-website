 ...
 *
 * A font descriptor specifies metrics and other attributes of a simple font or
 * a
 * CIDFont as a whole, as distinct from the metrics of individual glyphs. These
 * font
 * metrics provide information that enables a viewer application to synthesize a
 * substitute font or select a similar font when the font program is
 * unavailable. The
...
class Zend_Pdf_Resource_Font_FontDescriptor
{
     ...
     * @throws Zend_Pdf_Exception
     */
    static public function factory (Zend_Pdf_Resource_Font $font, 
            Zend_Pdf_FileParser_Font_OpenType $fontParser, $embeddingOptions)
    {
        /*
        ...
        $fontDescriptor->FontName = new Zend_Pdf_Element_Name(
                $font->getResource()->BaseFont->value);
        ...
        $fontDescriptor->ItalicAngle = new Zend_Pdf_Element_Numeric(
                $fontParser->italicAngle);
        
        $fontDescriptor->Ascent = new Zend_Pdf_Element_Numeric(
                $font->toEmSpace($fontParser->ascent));
        $fontDescriptor->Descent = new Zend_Pdf_Element_Numeric(
                $font->toEmSpace($fontParser->descent));
        
        $fontDescriptor->CapHeight = new Zend_Pdf_Element_Numeric(
                $fontParser->capitalHeight);
        /**
        ...
        $fontDescriptor->MissingWidth = new Zend_Pdf_Element_Numeric(
                $fontParser->glyphWidths[0]);
         ...
         *
         * First, the developer might specifically request not to embed the
         * font.
         */
        if (! ($embeddingOptions & Zend_Pdf_Font::EMBED_DONT_EMBED)) {
            ...
            if (! $fontParser->isEmbeddable) {
                /*
                 * This exception may be suppressed if the developer decides
                 * that
            ...
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception($message, 
                    Zend_Pdf_Exception::FONT_CANT_BE_EMBEDDED);
        }
    } else {
        /*
        ...
        $fontFileObject->dictionary->Length1 = new Zend_Pdf_Element_Numeric(
                strlen($fontFile));
        if (! ($embeddingOptions & Zend_Pdf_Font::EMBED_DONT_COMPRESS)) {
             ...
             * sizes by about half!
             */
            $fontFileObject->dictionary->Filter = new Zend_Pdf_Element_Name(
                    'FlateDecode');
        }
