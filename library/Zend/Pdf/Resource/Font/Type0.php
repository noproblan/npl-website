 ...
 *
 * A composite font is one whose glyphs are obtained from other fonts or from
 * fontlike
 * objects called CIDFonts ({@link Zend_Pdf_Resource_Font_CidFont}), organized
 * hierarchically.
 * In PDF, a composite font is represented by a font dictionary whose Subtype
 * value is Type0;
 * this is also called a Type 0 font (the Type 0 font at the top level of the
 * hierarchy is the
 * root font).
 *
 * In PDF, a Type 0 font is a CID-keyed font.
 *
 * CID-keyed fonts provide effective method to operate with multi-byte character
 * encodings.
 *
 * The CID-keyed font architecture specifies the external representation of
 * certain font programs,
 * called CMap and CIDFont files, along with some conventions for combining and
 * using those files.
 *
 * A CID-keyed font is the combination of a CMap with one or more CIDFonts,
 * simple fonts,
...
class Zend_Pdf_Resource_Font_Type0 extends Zend_Pdf_Resource_Font
{
    ...
    public function __construct (Zend_Pdf_Resource_Font_CidFont $descendantFont)
    {
        ...
        $this->_resource->BaseFont = new Zend_Pdf_Element_Name(
                $descendantFont->getResource()->BaseFont->value);
        $this->_resource->DescendantFonts = new Zend_Pdf_Element_Array(
                array(
                        $descendantFont->getResource()
                ));
        $this->_resource->Encoding = new Zend_Pdf_Element_Name('Identity-H');
        
        $toUnicode = $this->_objectFactory->newStreamObject(
                self::getToUnicodeCMapData());
    ...
    {
        return $this->_descendantFont->getCoveredPercentage($string, 
                $charEncoding);
    }
