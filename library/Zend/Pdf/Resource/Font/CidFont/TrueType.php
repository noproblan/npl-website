 ...
 *
 * For Type 2, the CIDFont program is actually a TrueType font program, which
 * has
 * no native notion of CIDs. In a TrueType font program, glyph descriptions are
 * identified by glyph index values. Glyph indices are internal to the font and
 * are not
 * defined consistently from one font to another. Instead, a TrueType font
 * program
 * contains a 'cmap' table that provides mappings directly from character codes
 * to
...
class Zend_Pdf_Resource_Font_CidFont_TrueType extends Zend_Pdf_Resource_Font_CidFont
{
    ...
    public function __construct (
            Zend_Pdf_FileParser_Font_OpenType_TrueType $fontParser, 
            $embeddingOptions)
    {
        ...
        $fontDescriptor = Zend_Pdf_Resource_Font_FontDescriptor::factory($this, 
                $fontParser, $embeddingOptions);
        $this->_resource->FontDescriptor = $this->_objectFactory->newObject(
                $fontDescriptor);
        
        /* Prepare CIDToGIDMap */
        // Initialize 128K string of null characters (65536 2 byte integers)
        $cidToGidMapData = str_repeat(
        ...
        $cidToGidMap->dictionary->Filter = new Zend_Pdf_Element_Name(
                'FlateDecode');
