 ...
 *
 * A CIDFont program contains glyph descriptions that are accessed using a CID
 * as
 * the character selector. There are two types of CIDFont. A Type 0 CIDFont
 * contains
 ...
 * CIDFont program.
 * Although its Type value is Font, a CIDFont is not actually a font. It does
 * not have an Encoding
 * entry, it cannot be listed in the Font subdictionary of a resource
 * dictionary, and it cannot be
 * used as the operand of the Tf operator. It is used only as a descendant of a
 * Type 0 font.
 * The CMap in the Type 0 font is what defines the encoding that maps character
 * codes to CIDs
 * in the CIDFont.
 *
...
abstract class Zend_Pdf_Resource_Font_CidFont extends Zend_Pdf_Resource_Font
{
    ...
    /**
     * Array containing the widths of each character that have entries in used
     * character map.
     *
     * @var array
     */
    ...
    public function __construct (Zend_Pdf_FileParser_Font_OpenType $fontParser)
    {
         ...
         */
        /*
         * Constract characters widths array using font CMap and glyphs widths
         * array
         */
        ...
        $this->_resource->DW = new Zend_Pdf_Element_Numeric(
                $this->toEmSpace($defaultWidth));
        
        // Remove characters which corresponds to default width from the widths
        // array
        ...
        // Save last sequence, if widths array is not empty (it may happens for
        // monospaced fonts)
        ...
        foreach ($widthsSequences as $startCode => $widthsSequence) {
            ...
            foreach ($widthsSequence as $width) {
                if ($lastWidth != $width) {
                    ...
                    if ($widthsInSequence != 0) {
                        // Previous width value was a part of the widths
                        // sequence. Save it as 'c_1st c_last w'.
                        $pdfCharsWidths[] = new Zend_Pdf_Element_Numeric(
                                $startCode); // First
                                                                                              // character
                                                                                              // code
                        $pdfCharsWidths[] = new Zend_Pdf_Element_Numeric(
                                $startCode + $widthsInSequence - 1); // Last
                                                                                                                      // character
                                                                                                                      // code
                        $pdfCharsWidths[] = new Zend_Pdf_Element_Numeric(
                                $this->toEmSpace($lastWidth)); // Width
                    ...
                    // Collect new width
                    $pdfWidths[] = new Zend_Pdf_Element_Numeric(
                            $this->toEmSpace($width));
                    ...
                    if (count($pdfWidths) != 0) {
                        ...
                        // Remove last element from widths list, since it's a
                        // part of widths sequence
                        array_pop($pdfWidths);
                        ...
                        if (count($pdfWidths) != 0) {
                            ...
                            $pdfCharsWidths[] = new Zend_Pdf_Element_Numeric(
                                    $startCode); // First
                                                                                                  // character
                                                                                                  // code
                            $pdfCharsWidths[] = new Zend_Pdf_Element_Array(
                                    $pdfWidths); // Widths
                                                                                                // array
            ...
            // Check if we have widths collection or widths sequence to wite it
            // down
            if (count($pdfWidths) != 0) {
                // We have some widths collected
                // Save it as 'c_1st [w1 w2 ... wn]'.
                $pdfCharsWidths[] = new Zend_Pdf_Element_Numeric($startCode); // First
                                                                              // character
                                                                              // code
                $pdfCharsWidths[] = new Zend_Pdf_Element_Array($pdfWidths); // Widths
                                                                            // array
            } else 
                if ($widthsInSequence != 0) {
    ...
    /**
     * Returns an array of glyph numbers corresponding to the Unicode
     * characters.
     *
     * If a particular character doesn't exist in this font, the special
     * 'missing
    ...
    public function glyphNumbersForCharacters ($characterCodes)
    {
         ...
         * it cannot be listed in the Font subdictionary of a resource
         * dictionary, and
