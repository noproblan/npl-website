 ...
 * - Glyphs in the font are selected by single-byte character codes obtained
 * from a
 * string that is shown by the text-showing operators. Logically, these codes
 * index
 * into a table of 256 glyphs; the mapping from codes to glyphs is called the
 * font’s
 * encoding.
 * PDF specification provides a possibility to specify any user defined encoding
 * in addition
 * to the standard built-in encodings: Standard-Encoding, MacRomanEncoding,
 * WinAnsiEncoding,
 * and PDFDocEncoding, but Zend_Pdf simple fonts implementation operates only
 * with
 * Windows ANSI encoding (except Symbol and ZapfDingbats built-in fonts).
 *
 * - Each glyph has a single set of metrics, including a horizontal displacement
 * or
...
abstract class Zend_Pdf_Resource_Font_Simple extends Zend_Pdf_Resource_Font
{
     ...
     *
     * Font character map may contain gaps for actually used glyphs,
     * nevertheless glyphWidths array
     ...
     *
     * Note: Adobe PDF specfication (V1.4 - V1.6) doesn't define behavior for
     * rendering
     * characters missed in the standard PDF fonts (such us 0x7F (DEL) Windows
     * ANSI code)
     * Adobe Font Metrics files doesn't also define metrics for "missed glyph".
     * We provide character width as "0" for this case, but actually it depends
     * on PDF viewer
     * implementation.
     *
     * @var integer
    ...
    public function __construct ()
    {
        ...
        /**
         *
         * @todo It's easy to add other encodings support now
         *       (Standard-Encoding, MacRomanEncoding,
         *       PDFDocEncoding, MacExpertEncoding, Symbol, and ZapfDingbats).
         *       Steps for the implementation:
         *       - completely describe all PDF single byte encodings in the
         *       documentation
         *       - implement non-WinAnsi encodings processing into
         *       encodeString()/decodeString() methods
         *      
         *       These encodings will be automatically supported for standard
         *       builtin PDF fonts as well
    ...
    public function getCoveredPercentage ($string, $charEncoding = '')
    {
        ...
        $charCount = (PHP_OS != 'AIX') ? iconv_strlen($string, 'UTF-16BE') : strlen(
                $string);
        if ($charCount == 0) {
            return 0;
        }
     ...
     *
     * The method is used to convert strings retrieved from existing content
     * streams
     *
     * @param string $string            
     * @param string $charEncoding
