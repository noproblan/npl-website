...
class Zend_Pdf_Parser
{
    ...
    private function _loadXRefTable ($offset)
    {
        ...
        $context = new Zend_Pdf_Element_Reference_Context($this->_stringParser, 
                $refTable);
        ...
        if ($nextLexeme == 'xref') {
            ...
            while (($nextLexeme = $this->_stringParser->readLexeme()) !=
                     'trailer') {
                        if (! ctype_digit($nextLexeme)) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
                            sprintf(
                                    'PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.', 
                ...
                $this->_stringParser->skipWhiteSpace();
                while ($refCount > 0) {
                    $objectOffset = substr($this->_stringParser->data, 
                            $this->_stringParser->offset, 10);
                    if (! ctype_digit($objectOffset)) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Zend_Pdf_Exception(
                                sprintf(
                                        'PDF file cross-reference table syntax error. Offset - 0x%X. Offset must contain only digits.', 
                                        $this->_stringParser->offset));
                    }
                    // Force $objectOffset to be treated as decimal instead of
                    // octal number
                    ...
                    if (strpos("\x00\t\n\f\r ", 
                            $this->_stringParser->data[$this->_stringParser->offset]) ===
                             false) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Zend_Pdf_Exception(
                                sprintf(
                                        'PDF file cross-reference table syntax error. Offset - 0x%X. Value separator must be white space.', 
                                        $this->_stringParser->offset));
                    }
                    $this->_stringParser->offset ++;
                    
                    $genNumber = substr($this->_stringParser->data, 
                    ...
                    switch ($inUseKey) {
                        case 'f':
                            ...
                            // free entry
                            unset(
                                    $this->_refTable[$objNum . ' ' . $genNumber .
                                             ' R']);
                    ...
                    if (! Zend_Pdf_StringParser::isWhiteSpace(
                            ord(
                                    $this->_stringParser->data[$this->_stringParser->offset]))) {
                        require_once 'Zend/Pdf/Exception.php';
                        throw new Zend_Pdf_Exception(
            ...
            if (! $trailerDict instanceof Zend_Pdf_Element_Dictionary) {
                ...
                throw new Zend_Pdf_Exception(
                        sprintf(
                                'PDF file syntax error. Offset - 0x%X.  Dictionary expected after \'trailer\' keyword.', 
                                $trailerDictOffset));
            }
        } else {
            ...
            if (! $xrefStream instanceof Zend_Pdf_Element_Object_Stream) {
                ...
                throw new Zend_Pdf_Exception(
                        sprintf(
                                'PDF file syntax error. Offset - 0x%X.  Cross-reference stream expected.', 
                                $offset));
            }
            ...
            if ($trailerDict->Type->value != 'XRef') {
                ...
                throw new Zend_Pdf_Exception(
                        sprintf(
                                'PDF file syntax error. Offset - 0x%X.  Cross-reference stream object must have /Type property assigned to /XRef.', 
                                $offset));
            }
            if ($trailerDict->W === null ||
                     $trailerDict->W->getType() != Zend_Pdf_Element::TYPE_ARRAY) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        sprintf(
                                'PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary doesn\'t have W entry or it\'s not an array.', 
                                $offset));
            }
            ...
            if ($entryField2Size == 0 || $entryField3Size == 0) {
                ...
                throw new Zend_Pdf_Exception(
                        sprintf(
                                'PDF file syntax error. Offset - 0x%X. Wrong W dictionary entry. Only type field of stream entries has default value and could be zero length.', 
                                $offset));
            }
            ...
            if ($trailerDict->Index !== null) {
                if ($trailerDict->Index->getType() !=
                         Zend_Pdf_Element::TYPE_ARRAY) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
                            sprintf(
                                    'PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary Index entry must be an array.', 
                                    $offset));
                }
            ...
            for ($count = 0; $count < $sections; $count ++) {
                ...
                for ($count2 = 0; $count2 < $entries; $count2 ++) {
                    if ($entryField1Size == 0) {
                        ...
                        if ($entryField1Size == 1) { // Optimyze one-byte
                                                            // field case
                            $type = ord($xrefStreamData[$streamOffset ++]);
                        } else {
                            $type = Zend_Pdf_StringParser::parseIntFromStream(
                                    $xrefStreamData, $streamOffset, 
                                    $entryField1Size);
                    ...
                    if ($entryField2Size == 1) { // Optimyze one-byte field case
                        ...
                        $field2 = Zend_Pdf_StringParser::parseIntFromStream(
                                $xrefStreamData, $streamOffset, $entryField2Size);
                    ...
                    if ($entryField3Size == 1) { // Optimyze one-byte field case
                        ...
                        $field3 = Zend_Pdf_StringParser::parseIntFromStream(
                                $xrefStreamData, $streamOffset, $entryField3Size);
                    ...
                    switch ($type) {
                        case 0:
                            ...
                            // Free object
                            $refTable->addReference(
                                    $objNum . ' ' . $field3 . ' R', $field2, 
                                    false);
                            // Debug output:
                            // echo "Free object - $objNum $field3 R, next free
                            // - $field2\n";
                            break;
                        ...
                        case 1:
                            ...
                            $refTable->addReference(
                                    ...
                                    true);
                            // Debug output:
                            // echo "In-use object - $objNum $field3 R, offset -
                            // $field2\n";
                            break;
                        ...
                        case 2:
                            ...
                            // Debug output:
                            // echo "Compressed object - $objNum 0 R, object
                            // stream - $field2 0 R, offset - $field3\n";
                            break;
                    }
        ...
        if ($trailerDict->Prev instanceof Zend_Pdf_Element_Numeric ||
            ...
            $trailerObj->setPrev(
                    $this->_loadXRefTable($trailerDict->Prev->value));
            $context->getRefTable()->setParent(
                    $trailerObj->getPrev()
                        ->getRefTable());
        }
        
        /**
         * We set '/Prev' dictionary property to the current cross-reference
         * section offset.
         * It doesn't correspond to the actual data, but is true when trailer
         * will be used
     ...
     * Object constructor
     *
     * Note: PHP duplicates string, which is sent by value, only of it's
     * updated.
    ...
    public function __construct ($source, 
            Zend_Pdf_ElementFactory_Interface $factory, $load)
    {
        if ($load) {
            ...
            while ($byteCount > 0 && ! feof($pdfFile)) {
                ...
                if ($nextBlock === false) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
                            "Error occured while '$source' file reading.");
                }
        ...
        if (version_compare($pdfVersion, '0.9', '<') ||
            ...
            /**
             *
             * @todo To support PDF versions 1.5 (Acrobat 6) and PDF version 1.7
             *       (Acrobat 7)
             *       Stream compression filter must be implemented (for
            ...
            throw new Zend_Pdf_Exception(
                    sprintf(
                            'Unsupported PDF version. Zend_Pdf supports PDF 1.0-1.4. Current version - \'%f\'', 
                            $pdfVersion));
        }
        $this->_pdfVersion = $pdfVersion;
        
        $this->_stringParser->offset = strrpos($this->_stringParser->data, 
                '%%EOF');
        if ($this->_stringParser->offset === false ||
                 strlen($this->_stringParser->data) -
                 $this->_stringParser->offset > 7) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
         ...
         */
        while (Zend_Pdf_StringParser::isWhiteSpace(
                ord($this->_stringParser->data[$this->_stringParser->offset])) &&
        ...
        if ($nextLexeme != 'startxref') {
            ...
            throw new Zend_Pdf_Exception(
                    sprintf(
                            'Pdf file syntax error. \'startxref\' keyword expected. Offset - 0x%X.', 
                            $this->_stringParser->offset - strlen($nextLexeme)));
        }
        ...
        if (! ctype_digit($startXref)) {
            ...
            throw new Zend_Pdf_Exception(
                    sprintf(
                            'Pdf file syntax error. Cross-reference table offset must contain only digits. Offset - 0x%X.', 
                            $this->_stringParser->offset - strlen($nextLexeme)));
        }
