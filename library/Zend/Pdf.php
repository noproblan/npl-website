...
class Zend_Pdf
{
     ...
     * document properties.
     *
     * Standard document properties: Title (must be set for PDF/X documents),
     * Author,
     * Subject, Keywords (comma separated list), Creator (the name of the
     * application,
     * that created document, if it was converted from other format), Trapped
     * (must be
    ...
    /**
     * True if the object is a newly created PDF document (affects save() method
     * behavior)
     * False otherwise
     *
     * @var boolean
    ...
    public function save ($filename, $updateOnly = false)
    {
        if (($file = @fopen($filename, $updateOnly ? 'ab' : 'wb')) === false) {
            ...
            throw new Zend_Pdf_Exception(
                    "Can not open '$filename' file for writing.");
        }
    ...
    public function __construct ($source = null, $revision = null, $load = false)
    {
        ...
        if ($source !== null) {
            ...
            $this->_parser = new Zend_Pdf_Parser($source, $this->_objFactory, 
                    $load);
            ...
            if ($this->_trailer->Encrypt !== null) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                        'Encrypted document modification is not supported');
            }
            if ($revision !== null) {
            ...
            $this->_loadNamedDestinations($this->_trailer->Root, 
                    $this->_parser->getPDFVersion());
            ...
            $trailerDictionary->ID->items[] = new Zend_Pdf_Element_String_Binary(
                    $docIdLow);
            $trailerDictionary->ID->items[] = new Zend_Pdf_Element_String_Binary(
                    $docIdHigh);
             ...
             */
            $docCatalog = $this->_objFactory->newObject(
                    new Zend_Pdf_Element_Dictionary());
            $docCatalog->Type = new Zend_Pdf_Element_Name('Catalog');
            $docCatalog->Version = new Zend_Pdf_Element_Name(
                    Zend_Pdf::PDF_VERSION);
            $this->_trailer->Root = $docCatalog;
            
            /**
             * Pages container
             */
            $docPages = $this->_objFactory->newObject(
                    new Zend_Pdf_Element_Dictionary());
    ...
    public function revisions ()
    {
        ...
        while ($currentTrailer->getPrev() !== null &&
                 $currentTrailer->getPrev()->Root !== null) {
                    $revisions ++;
    ...
    public function rollback ($steps)
    {
        for ($count = 0; $count < $steps; $count ++) {
            if ($this->_trailer->getPrev() !== null &&
                     $this->_trailer->getPrev()->Root !== null) {
        ...
        // Mark content as modified to force new trailer generation at render
        // time
     ...
     */
    protected function _loadPages (Zend_Pdf_Element_Reference $pages, 
            $attributes = array())
    {
                ...
                if ($child->Type->value == 'Page') {
                    foreach (self::$_inheritableAttributes as $property) {
                        if ($child->$property === null &&
                                 array_key_exists($property, $attributes)) {
                            /**
                             * Important note.
                             * If any attribute or dependant object is an
                             * indirect object, then it's still
                        ...
                        $child->$property = $this->_objFactory->newObject(
                                $attributes[$property]);
                    }
                }
            }
 ...
 * @throws Zend_Pdf_Exception
 */
protected function _loadNamedDestinations (Zend_Pdf_Element_Reference $root, 
    $pdfHeaderVersion)
{
if ($root->Version !== null &&
         version_compare($root->Version->value, $pdfHeaderVersion, '>')) {
...
if ($versionIs_1_2_plus) {
    ...
    if ($root->Dests !== null) {
        if ($root->Dests->getType() != Zend_Pdf_Element::TYPE_DICTIONARY) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'Document catalog Dests entry must be a dictionary.');
        }
        
        require_once 'Zend/Pdf/Target.php';
        foreach ($root->Dests->getKeys() as $destKey) {
            $this->_namedTargets[$destKey] = Zend_Pdf_Target::load(
                    $root->Dests->$destKey);
        }
    }
}
...
if ($root->Outlines->Type !== null && $root->Outlines->Type->value != 'Outlines') {
    require_once 'Zend/Pdf/Exception.php';
    throw new Zend_Pdf_Exception(
            'Outlines Type entry must be an \'Outlines\' string.');
}
...
while ($outlineDictionary !== null &&
         ! $processedDictionaries->contains($outlineDictionary)) {
...
foreach ($this->_namedTargets as $name => $namedTarget) {
    if ($namedTarget instanceof Zend_Pdf_Destination_Explicit) {
        ...
        if ($namedTarget instanceof Zend_Pdf_Action) {
...
$iterator = new RecursiveIteratorIterator(
        new Zend_Pdf_RecursivelyIteratableObjectsContainer($this->outlines), 
...
if ($openAction !== null) {
    if ($openAction instanceof Zend_Pdf_Action) {
        ...
        if ($openAction instanceof Zend_Pdf_Destination) {
            ...
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
                    'OpenAction has to be either PDF Action or Destination.');
        }
}
}
...
foreach ($this->_namedTargets as $name => $destination) {
    ...
    if ($destination instanceof Zend_Pdf_Target) {
        ...
        require_once 'Zend/Pdf/Exception.php';
        throw new Zend_Pdf_Exception(
                'PDF named destinations must be a Zend_Pdf_Target object.');
    }
}
$destArray = $this->_objFactory->newObject(
        new Zend_Pdf_Element_Array($destArrayItems));
...
if ($root->Names === null) {
    $root->touch();
    $root->Names = $this->_objFactory->newObject(
            new Zend_Pdf_Element_Dictionary());
} else {
    $root->Names->touch();
}
...
if ($root->Outlines === null) {
    if (count($this->outlines) == 0) {
        ...
        $root->Outlines = $this->_objFactory->newObject(
                new Zend_Pdf_Element_Dictionary());
    ...
    if (count($this->_originalOutlines) != count($this->outlines)) {
        // If original and current outlines arrays have different size then
        // outlines list was updated
        $updateOutlinesNavigation = true;
    } else 
        if (! (array_keys($this->_originalOutlines) ===
                 array_keys($this->outlines))) {
            // If original and current outlines arrays have different keys (with
            // a glance to an order) then outlines list was updated
            $updateOutlinesNavigation = true;
        } else {
...
if ($updateOutlinesNavigation) {
    ...
    foreach ($this->outlines as $outline) {
        if ($lastOutline === null) {
            // First pass. Update Outlines dictionary First entry using
            // corresponding value
            $lastOutline = $outline->dumpOutline($this->_objFactory, 
                    $updateOutlinesNavigation, $root->Outlines);
            $root->Outlines->First = $lastOutline;
        } else {
            // Update previous outline dictionary Next entry (Prev is updated
            // within dumpOutline() method)
            $currentOutlineDictionary = $outline->dumpOutline(
                    $this->_objFactory, $updateOutlinesNavigation, 
                    $root->Outlines, $lastOutline);
 ...
 * If $factory is null then it will be created and page must be attached to the
 * document to be
...
/**
 * Set open Action which is actually Zend_Pdf_Destination or Zend_Pdf_Action
 * object
 *
...
/**
 * Return an associative array containing all the named destinations (or GoTo
 * actions) in the PDF.
...
{
if ($destination !== null && ! $destination instanceof Zend_Pdf_Action_GoTo &&
         ! $destination instanceof Zend_Pdf_Destination_Explicit) {
    require_once 'Zend/Pdf/Exception.php';
    throw new Zend_Pdf_Exception(
            'PDF named destination must refer an explicit destination or a GoTo PDF action.');
}
...
$count = 1;
foreach ($this->pages as $page) {
    $pageDictionaryHashId = spl_object_hash(
            $page->getPageDictionary()->getObject());
 ...
 * Resolve destination.
 *
 * Returns Zend_Pdf_Page page object or null if destination is not found within
 * PDF document.
 *
 ...
 * @throws Zend_Pdf_Exception
 */
public function resolveDestination (Zend_Pdf_Destination $destination, 
...
if ($destination instanceof Zend_Pdf_Destination_Named) {
    ...
    if (! $destination instanceof Zend_Pdf_Destination_Explicit) {
        require_once 'Zend/Pdf/Exception.php';
        throw new Zend_Pdf_Exception(
                'Named destination target has to be an explicit destination.');
    }
}
 ...
 */
protected function _cleanUpAction (Zend_Pdf_Action $action, 
    $refreshPageCollectionHashes = true)
{
if ($this->_pageReferences === null || $refreshPageCollectionHashes) {
    $this->_refreshPagesHash();
...
$iterator = new RecursiveIteratorIterator($action, 
        RecursiveIteratorIterator::SELF_FIRST);
...
foreach ($iterator as $chainedAction) {
    if ($chainedAction instanceof Zend_Pdf_Action_GoTo &&
             $this->resolveDestination($chainedAction->getDestination(), false) ===
             null) {
        ...
        switch ($key) {
                    ...
                    default:
                        ...
                        throw new Zend_Pdf_Exception(
                                'Wrong Trapped document property vale: \'' .
                                         $value .
                                         '\'. Only true, false and null values are allowed.');
                        break;
                }
            ...
            case 'Producer':
                if (extension_loaded('mbstring') === true) {
                    ...
                    if ($detected !== 'ASCII') {
                        $value = "\xfe\xff" .
                                 mb_convert_encoding($value, 'UTF-16', 
                                        $detected);
                    }
                }
                $docInfo->$key = new Zend_Pdf_Element_String((string) $value);
...
// File is always modified by _dumpPages() now, but future implementations may
// eliminate this.
if (! $this->_objFactory->isModified()) {
    ...
    if ($outputStream === null) {
        ...
        while (strlen($pdfData) > 0 &&
                 ($byteCount = fwrite($outputStream, $pdfData)) != false) {
...
$xrefSection = array();
// Dummy initialization of the first element (specail case - header of linked
...
foreach ($this->_objFactory->listModifiedObjects() as $updateInfo) {
    ...
    if ($updateInfo->isFree()) {
        ...
        if ($outputStream === null) {
            ...
            while (strlen($pdfBlock) > 0 &&
                     ($byteCount = fwrite($outputStream, $pdfBlock)) != false) {
 ...
 *       probably actually exist as Zend_Pdf_Element_Date or something like
 *       that.
 *      
