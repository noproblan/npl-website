...
class Zend_Cache_Core
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (! is_array($options)) {
            Zend_Cache::throwException(
                    "Options passed were not an array" .
     ...
     * @param string $id
     *            Cache id
     * @return int|false Last modified time of cache entry if it is available,
     *         false otherwise
     */
            ...
            $priority = 8)
    {
        ...
        if ($this->_options['automatic_cleaning_factor'] > 0) {
            ...
            if ($rand == 1) {
                ...
                if ($this->_extendedBackend ||
                    ...
                    $this->_log(
                            "Zend_Cache_Core::save(): automatic cleaning running", 
                            7);
                    $this->clean(Zend_Cache::CLEANING_MODE_OLD);
                } else {
                    $this->_log(
                            "Zend_Cache_Core::save(): automatic cleaning is not available/necessary with current backend", 
                            4);
                }
            }
        }
        ...
        if (! $result) {
            ...
            $this->_log(
                    "Zend_Cache_Core::save(): failed to save item '{$id}' -> removing it", 
                    4);
        ...
        if ($this->_options['write_control']) {
            ...
            if ($data != $data2) {
                $this->_log(
                        "Zend_Cache_Core::save(): write control of item '{$id}' failed -> removing it", 
                        4);
                $this->_backend->remove($id);
                return false;
            }
