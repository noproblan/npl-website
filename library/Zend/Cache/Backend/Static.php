 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cache_Backend_Static extends Zend_Cache_Backend implements 
        Zend_Cache_Backend_Interface
{
    ...
    protected function _createDirectoriesFor ($path)
    {
        if (! is_dir($path)) {
            $oldUmask = umask(0);
            if (! @mkdir($path, 
                    $this->_octdec($this->_options['cache_directory_umask']), 
                    true)) {
    ...
    protected static function _validateIdOrTag ($string)
    {
        ...
        // Validation assumes no query string, fragments or scheme included -
        // only the path
        if (! preg_match(
