...
/**
 * Impementation of Zend Cache Backend using the Zend Platform (Output Content
 * Caching)
 *
 * @package Zend_Cache
 * @subpackage Zend_Cache_Backend
 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cache_Backend_ZendPlatform extends Zend_Cache_Backend implements 
        Zend_Cache_Backend_Interface
{
    ...
    public function __construct (array $options = array())
    {
        if (! function_exists('accelerator_license_info')) {
            Zend_Cache::throwException(
                    'The Zend Platform extension must be loaded for using this backend !');
        }
        if (! function_exists('accelerator_get_configuration')) {
            $licenseInfo = accelerator_license_info();
            Zend_Cache::throwException(
                    'The Zend Platform extension is not loaded correctly: ' .
    ...
    {
        // doNotTestCacheValidity implemented by giving zero lifetime to the
        // cache
     ...
     * @param string $id
     *            Cache id
     * @return mixed|false false (a cache is not available) or "last modified"
    ...
    public function clean ($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
            ...
            case Zend_Cache::CLEANING_MODE_NOT_MATCHING_TAG:
                $this->_log(
                        "Zend_Cache_Backend_ZendPlatform::clean() : CLEANING_MODE_NOT_MATCHING_TAG is not supported by the Zend Platform backend");
                return false;
                break;
     ...
     *
     * Remove all the cached files that need to be cleaned (according to mode
     * and files mtime)
     *
     * @param string $dir
     *            Path of directory ot clean
     * @param string $mode
     *            The same parameter as in
    ...
    private function _remove ($file)
    {
        if (! @unlink($file)) {
            ...
            return @touch($file, time() - 2 *
                     abs($this->_directives['lifetime']));
        }
        return true;
    }
}
