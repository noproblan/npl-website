 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cache_Backend_TwoLevels extends Zend_Cache_Backend implements 
        Zend_Cache_Backend_ExtendedInterface
{
     ...
     * x (integer) > 1 => computation of the fast backend filling percentage
     * randomly 1 times on x cache write
     *
        ...
        if (($priority > 0) && (10 * $priority >= $usage)) {
            ...
            if ($boolSlow === true) {
                ...
                if (! $boolFast && ! $this->_fastBackend->test($id)) {
                    // some backends return false on remove() even if the key
                    // never existed. (and it won't if fast is full)
    ...
    public function load ($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_options['auto_refresh_fast_cache']) {
            ...
            if (($array['priority'] > 0) && (10 * $array['priority'] >= $usage)) {
                ...
                $preparedData = $this->_prepareData($array['data'], 
                        $array['lifetime'], $array['priority']);
