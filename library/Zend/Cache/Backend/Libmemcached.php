 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cache_Backend_Libmemcached extends Zend_Cache_Backend implements 
        Zend_Cache_Backend_ExtendedInterface
{
     ...
     *
     * =====> (array) servers :
     * an array of memcached server ; each memcached server is described by an
     * associative array :
     ...
     * =====> (array) client :
     * an array of memcached client options ; the memcached client is described
     * by an associative array :
     * 
     * @see http://php.net/manual/memcached.constants.php - The option name can
     *      be the name of the constant without the prefix 'OPT_'
    ...
    public function __construct (array $options = array())
    {
        ...
        if (isset($this->_options['servers'])) {
            ...
            if (isset($value['host'])) {
                // in this case, $value seems to be a simple associative array
                // (one server only)
                $value = array(
                        0 => $value
                ); // let's transform it into a
                                             // classical array of associative
                                             // arrays
            }
     ...
     * @param string $id
     *            Cache id
     * @return int|false (a cache is not available) or "last modified" timestamp
    ...
    public function save ($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        // ZF-8856: using set because add needs a second request if item already
        // exists
    ...
    public function clean ($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
            ...
            case Zend_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                $this->_log(
                        self::TAGS_UNSUPPORTED_BY_CLEAN_OF_LIBMEMCACHED_BACKEND);
                break;
            default:
    ...
    public function setDirectives ($directives)
    {
        ...
        if ($lifetime > 2592000) {
            // #ZF-3490 : For the memcached backend, there is a lifetime limit
