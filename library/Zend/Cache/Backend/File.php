        ...
        Zend_Cache_Backend_ExtendedInterface
{
     ...
     * - Can avoid cache corruption under bad circumstances but it doesn't work
     * on multithread
     ...
     * 'crc32' for a crc32 hash control (lightly less safe but faster, better
     * choice)
     * 'adler32' for an adler32 hash control (excellent choice too, faster than
     * crc32)
     ...
     * - be really carefull with this option because a too generic value in a
     * system cache dir
    ...
    public function __construct (array $options = array())
    {
        ...
        if ($this->_options['cache_dir'] !== null) { // particular case for this
                                                     // option
            $this->setCacheDir($this->_options['cache_dir']);
        } else {
            $this->setCacheDir(self::getTmpDir() . DIRECTORY_SEPARATOR, false);
        }
        if (isset($this->_options['file_name_prefix'])) { // particular case for
                                                          // this option
    ...
    public function save ($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $metadatas = array(
                ...
                'expire' => $this->_expireTime(
                        $this->getLifetime($specificLifetime)),
                'tags' => $tags
        );
     ...
     * Remove a file
     *
     * If we can't remove the file (because of locks or any problem), we will
     * touch
        ...
        foreach ($glob as $file) {
            if (is_file($file)) {
                ...
                if ($this->_isMetadatasFile($fileName)) {
                    // in CLEANING_MODE_ALL, we drop anything, even remainings
                    // old metadatas files
                ...
                switch ($mode) {
                    case Zend_Cache::CLEANING_MODE_ALL:
                        $res = $this->remove($id);
                        if (! $res) {
                            // in this case only, we accept a problem with the
                            // metadatas file drop
            ...
            if ((is_dir($file)) and ($this->_options['hashed_directory_level'] >
                     0)) {
                // Recursive call
                $result = $this->_clean($file . DIRECTORY_SEPARATOR, $mode, 
                        $tags) && $result;
                if ($mode == Zend_Cache::CLEANING_MODE_ALL) {
     ...
     *            Cache id
     * @param boolean $parts
     *            if true, returns array of directory parts instead of single
     *            string
    ...
    protected function _recursiveMkdirAndChmod ($id)
    {
        ...
        foreach ($partsArray as $part) {
            if (! is_dir($part)) {
                ...
                @chmod($part, $this->_options['hashed_directory_umask']); // see
                                                                          // #ZF-320
                                                                          // (this
                                                                          // line
                                                                          // is
     ...
     * @param boolean $doNotTestCacheValidity
     *            If set to true, the cache validity won't be tested
     * @return boolean|mixed false (a cache is not available) or "last modified"
     *         timestamp (int) of the available cache record
     */
