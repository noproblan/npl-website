...
class Zend_Cache_Frontend_Page extends Zend_Cache_Core
{
     ...
     * WARNING : http_conditional OPTION IS NOT IMPLEMENTED FOR THE MOMENT
     * (TODO)
     *
        ...
        if (isset($this->_specificOptions['http_conditional'])) {
            if ($this->_specificOptions['http_conditional']) {
                Zend_Cache::throwException(
                        'http_conditional is not implemented for the moment !');
            }
        }
    ...
    protected function _setRegexps ($regexps)
    {
        ...
        foreach ($regexps as $regexp => $conf) {
            if (! is_array($conf)) {
                Zend_Cache::throwException(
                        'regexps option must be an array of arrays !');
            }
     ...
     *
     * @param string $id
     *            (optional) A cache id (if you set a value here, maybe you have
     *            to use Output frontend instead)
    ...
    public function _flush ($data)
    {
        ...
        $this->save($array, null, $this->_activeOptions['tags'], 
     ...
     * options)
     *
     * @return mixed|false a cache id (string), false if the cache should have
     *         not to be used
     */
    protected function _makeId ()
    {
        ...
        foreach (array(
            ...
            $tmp2 = $this->_makePartialId($arrayName, 
                    $this->_activeOptions['cache_with_' . strtolower($arrayName) .
                             '_variables'], 
                            $this->_activeOptions['make_id_with_' .
                             strtolower($arrayName) . '_variables']);
            if ($tmp2 === false) {
                return false;
            }
            $tmp = $tmp . $tmp2;
     ...
     * @return mixed|false Partial id (string) or false if the cache should have
     *         not to be used
     */
