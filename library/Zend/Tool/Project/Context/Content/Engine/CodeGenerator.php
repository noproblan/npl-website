...
class Zend_Tool_Project_Context_Content_Engine_CodeGenerator
{
     ...
     * @return string
     */
    public function hasContent (Zend_Tool_Project_Context_Interface $context, 
            $method)
    {
        return $this->_storage->has(
                $this->_contentPrefix . '/' . $context->getName() . '/' . $method .
                         '.php');
    }
            ...
            $method, $parameters)
    {
        ...
        if (! $codeGenerator instanceof Zend_CodeGenerator_Abstract) {
            throw new Zend_Tool_Project_Exception(
                    'Custom file at ' . $streamUri .
                             ' did not return the $codeGenerator object.');
        }
