 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Project_Context_Zf_ActionMethod implements 
        Zend_Tool_Project_Context_Interface
{
    ...
    public function init ()
    {
        ...
        if (! $this->_controllerResource->getContext() instanceof Zend_Tool_Project_Context_Zf_ControllerFile) {
            ...
            throw new Zend_Tool_Project_Context_Exception(
                    'ActionMethod must be a sub resource of a ControllerFile');
        }
        // make the ControllerFile node appendable so we can tack on the
        // actionMethod.
         ...
         * self::hasActionMethod($this->_controllerPath, $this->_actionName)) {
         * require_once 'Zend/Tool/Project/Context/Exception.php';
         * throw new Zend_Tool_Project_Context_Exception('An action named ' .
         * $this->_actionName . 'Action already exists in this controller');
         * }
         */
    ...
    {
        if (self::createActionMethod($this->_controllerPath, $this->_actionName) ===
                 false) {
            require_once 'Zend/Tool/Project/Context/Exception.php';
            throw new Zend_Tool_Project_Context_Exception(
                    'Could not create action within controller ' .
                             $this->_controllerPath . ' with action name ' .
                             $this->_actionName);
        }
        return $this;
    }
     ...
     * @return true
     */
    public static function createActionMethod ($controllerPath, $actionName, 
            $body = '        // action body')
    {
        ...
        $controllerCodeGenFile = Zend_CodeGenerator_Php_File::fromReflectedFileName(
                $controllerPath, true, true);
    ...
    public static function hasActionMethod ($controllerPath, $actionName)
    {
        ...
        $controllerCodeGenFile = Zend_CodeGenerator_Php_File::fromReflectedFileName(
                $controllerPath, true, true);
        return $controllerCodeGenFile->getClass()->hasMethod(
                $actionName . 'Action');
    }
}