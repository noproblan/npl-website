...
class Zend_Tool_Project_Context_Zf_TestApplicationControllerFile extends Zend_Tool_Project_Context_Filesystem_File
{
    ...
    {
        $this->_forControllerName = $this->_resource->getAttribute(
                'forControllerName');
        $this->_filesystemName = ucfirst($this->_forControllerName) .
                 'ControllerTest.php';
        parent::init();
        return $this;
    }
    ...
    public function getContents ()
    {
        ...
        $className = $filter->filter($this->_forControllerName) .
                 'ControllerTest';
        ...
        if ($controllerDirectoryResource->getParentResource()->getName() ==
            ...
            $className = $filter->filter(
                    ucfirst(
                            $controllerDirectoryResource->getParentResource()
                                ->getForModuleName())) . '_' . $className;
        }
