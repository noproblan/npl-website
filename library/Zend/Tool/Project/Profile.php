...
class Zend_Tool_Project_Profile extends Zend_Tool_Project_Profile_Resource_Container
{
    ...
    public function getIterator ()
    {
        ...
        return new RecursiveIteratorIterator(
                new Zend_Tool_Project_Profile_Iterator_EnabledResourceFilter(
    ...
    public function loadFromData ()
    {
        if (! isset($this->_attributes['profileData'])) {
            ...
            throw new Zend_Tool_Project_Exception(
                    'loadFromData() must have "profileData" set.');
        }
        
        $profileFileParser = new Zend_Tool_Project_Profile_FileParser_Xml();
        $profileFileParser->unserialize($this->_attributes['profileData'], 
                $this);
    ...
    public function isLoadableFromFile ()
    {
        if (! isset($this->_attributes['projectProfileFile']) &&
                 ! isset($this->_attributes['projectDirectory'])) {
            return false;
        }
        ...
        if (isset($this->_attributes['projectProfileFile'])) {
            ...
            $projectProfileFilePath = rtrim(
                    $this->_attributes['projectDirectory'], '/\\') .
                     '/.zfproject.xml';
            if (! file_exists($projectProfileFilePath)) {
                return false;
            }
     ...
     *
     * this attempts to load a project profile file from a variety of locations
     * depending
    ...
    {
        // if no data is supplied, need either a projectProfileFile or a
        // projectDirectory
        if (! isset($this->_attributes['projectProfileFile']) &&
                 ! isset($this->_attributes['projectDirectory'])) {
            require_once 'Zend/Tool/Project/Exception.php';
        ...
        if (isset($this->_attributes['projectProfileFile'])) {
            ...
            if (! file_exists($projectProfileFilePath)) {
                require_once 'Zend/Tool/Project/Exception.php';
                throw new Zend_Tool_Project_Exception(
                        '"projectProfileFile" was supplied but file was not found at location ' .
                                 $projectProfileFilePath);
            }
            $this->_attributes['projectDirectory'] = dirname(
                    $projectProfileFilePath);
        } else {
            $projectProfileFilePath = rtrim(
                    $this->_attributes['projectDirectory'], '/\\') .
            ...
            if (! file_exists($projectProfileFilePath)) {
                require_once 'Zend/Tool/Project/Exception.php';
                throw new Zend_Tool_Project_Exception(
                        '"projectDirectory" was supplied but no profile file file was not found at location ' .
                                 $projectProfileFilePath);
            }
     ...
     *
     * This will store the profile in memory to a place on disk determined by
     * the attributes
    ...
    public function storeToFile ()
    {
        ...
        if ($file == null) {
            require_once 'Zend/Tool/Project/Exception.php';
            throw new Zend_Tool_Project_Exception(
                    'storeToFile() must have a "projectProfileFile" attribute set.');
        }
    ...
    public function __toString ()
    {
        ...
        foreach ($this as $resource) {
            ...
            $rii = new RecursiveIteratorIterator($resource, 
                    RecursiveIteratorIterator::SELF_FIRST);
            foreach ($rii as $item) {
                $string .= str_repeat('  ', $rii->getDepth() + 1) .
                         $item->getName() .
                         ((count($attributes = $item->getAttributes()) > 0) ? ' [' .
                         http_build_query($attributes) . ']' : '') . PHP_EOL;
            }
        }
        return $string;
    }
