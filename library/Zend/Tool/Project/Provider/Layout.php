        ...
        Zend_Tool_Framework_Provider_Pretendable
{
            ...
            $layoutName = 'layout')
    {
        ...
        if ($layoutDirectory == false) {
            $layoutDirectory = $applicationDirectory->createResource(
                    'layoutsDirectory');
        }
        
        $layoutScriptsDirectory = $layoutDirectory->search(
                'layoutScriptsDirectory');
        
        if ($layoutScriptsDirectory == false) {
            $layoutScriptsDirectory = $layoutDirectory->createResource(
                    'layoutScriptsDirectory');
        }
        
        $layoutScriptFile = $layoutScriptsDirectory->search('layoutScriptFile', 
                array(
                        'layoutName' => 'layout'
                ));
    ...
    public function enable ()
    {
        ...
        if ($this->_registry->getRequest()->isPretend()) {
            ...
            $applicationConfigResource->addStringItem(
                    'resources.layout.layoutPath', $this->_layoutPath, 
                    'production', false);
    ...
    public function disable ()
    {
        ...
        if ($this->_registry->getRequest()->isPretend()) {
            ...
            // Remove the resources.layout.layoutPath directive from application
            // config
