 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Project_Provider_Module extends Zend_Tool_Project_Provider_Abstract implements 
        Zend_Tool_Framework_Provider_Pretendable
{

    public static function createResources (Zend_Tool_Project_Profile $profile, 
            $moduleName, 
            Zend_Tool_Project_Profile_Resource $targetModuleResource = null)
    {
        ...
        $moduleDirectory = $modulesDirectory->createResource('moduleDirectory', 
                array(
                        'moduleName' => $moduleName
                ));
        
        // create a context filter so that we can pull out only what we need
        // from the module skeleton
        ...
        foreach ($targetIterator as $targetSubResource) {
            ...
            if ($depthDifference === 1) {
                ...
                $i = $depthDifference;
                do {
                    // if we went out more than 1 parent, get to the correct
                    // parent
            ...
            $currentChildResource = $currentResource->createResource(
                    $targetSubResource->getName(), $params);
            
            // based of the provided list (Currently up top), enable specific
            // resources
            if (isset($targetModuleEnabledResources)) {
                $currentChildResource->setEnabled(
                        in_array($targetSubResource->getName(), 
                                $targetModuleEnabledResources));
            } else {
                $currentChildResource->setEnabled(
                        $targetSubResource->isEnabled());
            }
        }
    ...
    public function create ($name) // , $moduleProfile = null)
    {
        ...
        // $testingEnabled =
        // Zend_Tool_Project_Provider_Test::isTestingEnabled($this->_loadedProfile);
        ...
        if ($this->_registry->getRequest()->isPretend()) {
            $response->appendContent(
                    'I would create the following module and artifacts:');
            foreach (new RecursiveIteratorIterator($resources, 
                    RecursiveIteratorIterator::SELF_FIRST) as $resource) {
            ...
            $enabledFilter = new Zend_Tool_Project_Profile_Iterator_EnabledResourceFilter(
                    $resources);
            foreach (new RecursiveIteratorIterator($enabledFilter, 
                    RecursiveIteratorIterator::SELF_FIRST) as $resource) {
                $response->appendContent($resource->getContext()
                    ->getPath());
                $resource->create();
            ...
            $appConfigFile = $this->_loadedProfile->search(
                    'ApplicationConfigFile');
            $appConfigFile->removeStringItem(
                    'resources.frontController.moduleDirectory', 'production');
            $appConfigFile->addStringItem(
                    'resources.frontController.moduleDirectory', 
                    'APPLICATION_PATH "/modules"', 'production', false);
