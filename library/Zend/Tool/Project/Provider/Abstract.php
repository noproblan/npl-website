 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
abstract class Zend_Tool_Project_Provider_Abstract extends Zend_Tool_Framework_Provider_Abstract implements 
        Zend_Tool_Framework_Provider_Initializable
{
    ...
    public function initialize ()
    {
        ...
        if (! self::$_isInitialized) {
            ...
            if ($this->_hasProjectProviderDirectory(
                    $profilePath . DIRECTORY_SEPARATOR . '.zfproject.xml')) {
    ...
    /**
     * _getProject is designed to find if there is project file in the context
     * of where
     * the client has been called from..
     * The search order is as follows..
     * - traversing downwards from (PWD) - current working directory
     * - if an enpoint variable has been registered in teh client registry -
    ...
    protected function _loadProfile (
            $loadProfileFlag = self::NO_PROFILE_THROW_EXCEPTION, $projectDirectory = null, 
            $searchParentDirectories = true)
    {
        $foundPath = $this->_findProfileDirectory($projectDirectory, 
            ...
            $searchParentDirectories = true)
    {
        ...
        $parentDirectoriesArray = explode(DIRECTORY_SEPARATOR, 
                ltrim($projectDirectory, DIRECTORY_SEPARATOR));
        while ($parentDirectoriesArray) {
            $projectDirectoryAssembled = implode(DIRECTORY_SEPARATOR, 
            ...
            $profile->setAttribute('projectDirectory', 
                    $projectDirectoryAssembled);
            ...
            $loadProfileFlag = self::NO_PROFILE_THROW_EXCEPTION)
    {
        if (! $this->_loadedProfile) {
            if (($this->_loadProfile($loadProfileFlag) === false) &&
    ...
    {
        $projectProfileFile = $this->_loadedProfile->search(
                'ProjectProfileFile');
    ...
    protected function _getContentForContext (
            Zend_Tool_Project_Context_Interface $context, $methodName, 
            $parameters)
    {
    ...
    {
        // do some static analysis of the file so that we can determin whether
        // or not to incure
        // the cost of loading the profile before the system is fully
        // bootstrapped
