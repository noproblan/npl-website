 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Project_Provider_DbTable extends Zend_Tool_Project_Provider_Abstract implements 
        Zend_Tool_Framework_Provider_Pretendable
{
    ...
    public static function createResource (Zend_Tool_Project_Profile $profile, 
            $dbTableName, $actualTableName, $moduleName = null)
    {
        ...
        if (! ($dbTableDirectory = $modelsDirectory->search('DbTableDirectory'))) {
            $dbTableDirectory = $modelsDirectory->createResource(
                    'DbTableDirectory');
        }
            ...
            $dbTableName, $moduleName = null)
    {
        ...
        if (! ($modelsDirectory instanceof Zend_Tool_Project_Profile_Resource) ||
                 ! ($dbTableDirectory = $modelsDirectory->search(
                        'DbTableDirectory'))) {
            return false;
        }
        
        $dbTableFile = $dbTableDirectory->search(
                array(
                        'DbTableFile' => array(
                                'dbTableName' => $dbTableName
                        )
            ...
            $forceOverwrite = false)
    {
        ...
        if (preg_match('#[_-]#', $name)) {
            throw new Zend_Tool_Project_Provider_Exception(
                    'DbTable names should be camel cased.');
        }
        ...
        if (self::hasResource($this->_loadedProfile, $name, $module)) {
            throw new Zend_Tool_Project_Provider_Exception(
                    'This project already has a DbTable named ' . $name);
        }
        ...
        if ($request->isPretend()) {
            $response->appendContent(
                    'Would create a DbTable at ' .
                             $tableResource->getContext()
                                ->getPath());
        } else {
            $response->appendContent(
                    'Creating a DbTable at ' .
                             $tableResource->getContext()
                                ->getPath());
    ...
    public function createFromDatabase ($module = null, $forceOverwrite = false)
    {
        ...
        foreach ($db->listTables() as $actualTableName) {
            ...
            if (! $forceOverwrite &&
                     self::hasResource($this->_loadedProfile, $dbTableName, 
                            $module)) {
                throw new Zend_Tool_Project_Provider_Exception(
            ...
            $tableResources[] = self::createResource($this->_loadedProfile, 
                    $dbTableName, $actualTableName, $module);
        }
