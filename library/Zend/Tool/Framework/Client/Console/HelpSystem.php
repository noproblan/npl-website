...
class Zend_Tool_Framework_Client_Console_HelpSystem
{
            ...
            Exception $exception = null)
    {
        ...
        $errorMessages = explode(PHP_EOL, 
                wordwrap($errorMessage, 70, PHP_EOL, false));
        
        $text = 'An Error Has Occurred';
        $this->_response->appendContent($text, 
                array(
                        'color' => array(
                                'hiWhite',
                                'bgRed'
        ...
        $this->_response->appendContent($errorMessage, 
                array(
                        'indention' => 1,
                        'blockize' => 72,
                        'color' => array(
    ...
    public function respondWithGeneralHelp ()
    {
        ...
        $this->_response->appendContent('Usage:', array(
            ...
            ->appendContent(' action-name', 
                array_merge(array(
                        'color' => 'cyan'
                ), $noSeparator))
            ->appendContent(' [--action-opts]', $noSeparator)
            ->appendContent(' provider-name', 
                array_merge(array(
                        'color' => 'cyan'
                ), $noSeparator))
            ...
            ->appendContent('Providers and their actions:', 
                array(
                        'color' => 'green'
                ));
    ...
    public function respondWithSpecialtyAndParamHelp ($providerName, $actionName)
    {
        $this->_respondWithHeader();
        $this->_response->appendContent(
                'Details for action "' . $actionName . '" and provider "' .
                         $providerName . '"', array(
                                'color' => 'green'
                        ));
    ...
    protected function _respondWithHeader ()
    {
        ...
        $this->_response->appendContent('Zend Framework', 
                array(
                        'color' => array(
                                'hiWhite'
                        ),
     ...
     * @return Zend_Tool_Framework_Client_Console_HelpSystem
     */
    protected function _respondWithSystemInformation ($providerNameFilter = null, 
            $actionNameFilter = null, $includeAllSpecialties = false)
    {
        ...
        if (is_string($providerNameFilter)) {
            $providerMetadatasSearch = array_merge($providerMetadatasSearch, 
                    array(
        ...
        if (is_string($actionNameFilter)) {
            $actionMetadatasSearch = array_merge($actionMetadatasSearch, 
                    array(
        ...
        $displayProviderMetadatas = $manifest->getMetadatas(
                $providerMetadatasSearch);
        $displayActionMetadatas = $manifest->getMetadatas(
        ...
        foreach ($displayProviderMetadatas as $providerMetadata) {
            ...
            foreach ($providerSignature->getActions() as $actionInfo) {
                ...
                if (($foundActionIndex = array_search($actionName, 
                        $displayActionNames)) === false) {
                    continue;
                } else {
                ...
                if ($actionableGlobalLongParamMetadata) {
                    ...
                    $this->_respondWithCommand($providerMetadata, 
                            $actionMetadata, $specialtyMetadata, 
                ...
                // reset the special flag for single provider action with
                // specialty
                $isSingleSpecialProviderAction = false;
                
                if (! $includeAllSpecialties &&
                         count($actionableGlobalMetadatas) > 1) {
                    $this->_response->appendContent(
                    ...
                    $this->_response->appendContent(
                            'zf ' . $actionMetadata->getValue() . ' ' .
                                     $providerMetadata->getValue() . '.?', 
                                    array(
                                            'color' => 'cyan',
                                            'separator' => false
                                    ));
                    $this->_response->appendContent(
                            ' to get specific help on them.', 
                            array(
                                    'color' => 'yellow'
                            ));
                }
            ...
            if ($providerNameDisplayed) {
                $this->_response->appendContent(null, 
                        array(
                                'separator' => true
                        ));
            }
     ...
     * @return Zend_Tool_Framework_Client_Console_HelpSystem
     */
    protected function _respondWithProviderName (
            Zend_Tool_Framework_Metadata_Tool $providerMetadata)
    {
        $this->_response->appendContent(
                '  ' . $providerMetadata->getProviderName());
        return $this;
    }
                                                                      ...
                                                                      // $parameterShortMetadata)
    {
        ...
        foreach ($parameterLongMetadata->getValue() as $paramName => $consoleParamName) {
            ...
            if (($defaultValue = $methodInfo['parameterInfo'][$paramName]['default']) !=
                     null) {
                $paramString .= '[=' . $defaultValue . ']';
            }
            $this->_response->appendContent($paramString . '', 
                    array(
                            'separator' => false
                    ));
        }
