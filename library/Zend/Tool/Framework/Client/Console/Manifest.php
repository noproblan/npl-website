        ...
        Zend_Tool_Framework_Manifest_MetadataManifestable
{
    ...
    /**
     * setRegistry() - Required for the
     * Zend_Tool_Framework_Registry_EnabledInterface interface
     *
    ...
    public function getMetadata ()
    {
        ...
        // setup the camelCase to dashed filter to use since cli expects dashed
        // named
        $ccToDashedFilter = new Zend_Filter();
        $ccToDashedFilter->addFilter(new Zend_Filter_Word_CamelCaseToDash())->addFilter(
        ...
        foreach ($providerRepository->getProviderSignatures() as $providerSignature) {
            ...
            // create the metadatas for the per provider specialites in
            // providerSpecaltyNames
            ...
            foreach ($providerSignature->getActionableMethods() as $actionableMethodData) {
                ...
                foreach ($actionableMethodData['parameterInfo'] as $parameterInfoData) {
                    ...
                    $methodShortParams[$parameterInfoData['name']] = strtolower(
                            $parameterInfoData['name'][0]);
                }
                
                // create metadata for the long name
                ...
                // create metadata for the short name
