 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Framework_Client_Console_ArgumentParser implements 
        Zend_Tool_Framework_Registry_EnabledInterface
{
    ...
    /**
     * Parse() - This method does the work of parsing the arguments into the
     * enpooint request,
     * this will also (during help operations) fill the response in with
    ...
    public function parse ()
    {
        if ($this->_request == null || $this->_response == null) {
            ...
            throw new Zend_Tool_Framework_Client_Exception(
                    'The client registry must have both a request and response registered.');
        }
        ...
        if (count($this->_argumentsWorking) == 0) {
            $this->_request->setDispatchable(false); // at this point request is
                                                     // not dispatchable
        ...
        if ($this->_helpKnownAction) {
            $helpResponseOptions = array_merge($helpResponseOptions, 
                    array(
        ...
        if (count($this->_argumentsWorking) == 0) {
            ...
            if ($this->_help) {
                ...
                $this->_createHelpResponse(
                        array_merge($helpResponseOptions, 
                                array(
        ...
        // if there are arguments on the command line, lets process them as
        // provider options
        if (count($this->_argumentsWorking) != 0) {
            $this->_parseProviderOptionsPart();
        }
        
        // if there is still arguments lingering around, we can assume something
        // is wrong
        if (count($this->_argumentsWorking) != 0) {
            $this->_request->setDispatchable(false); // at this point request is
                                                     // not dispatchable
            if ($this->_help) {
                ...
                $this->_createHelpResponse(
                        array_merge($helpResponseOptions, 
                                array(
                                        'error' => 'Unknown arguments left on the command line: ' .
                                                 implode(' ', 
    ...
    protected function _parseGlobalPart ()
    {
        ...
        $getoptParser = new Zend_Console_Getopt($getoptOptions, 
    ...
    protected function _parseActionPart ()
    {
        ...
        if (! $actionMetadata) {
            ...
            $actionSearchCriteria['value'] = strtolower(
                    str_replace(array(
                            '-',
                            '_'
                    ), '', $consoleActionName));
    ...
    /**
     * Internal routine for parsing the provider part of the command line
     * arguments
     *
     * @return null
     */
    protected function _parseProviderPart ()
    {
        ...
        if (strstr($consoleProviderFull, '.')) {
            list ($consoleProviderName, $consoleSpecialtyName) = explode('.', 
                    $consoleProviderFull);
        } else {
        ...
        $providerMetadata = $this->_manifestRepository->getMetadata(
                $providerSearchCriteria);
        
        // check for normalized names as well (all lower, no separators)
        if (! $providerMetadata) {
            $providerSearchCriteria['name'] = 'normalizedProviderName';
            $providerSearchCriteria['value'] = strtolower(
                    str_replace(array(
                            '-',
                            '_'
                    ), '', $consoleProviderName));
        ...
        if (! $providerSpecialtyMetadata) {
            require_once 'Zend/Tool/Framework/Client/Exception.php';
            throw new Zend_Tool_Framework_Client_Exception(
                    'Provider \'' . $consoleSpecialtyName .
                             '\' is not a valid specialty.');
        }
        
        $this->_helpKnownSpecialty = true;
        $this->_request->setSpecialtyName(
                $providerSpecialtyMetadata->getSpecialtyName());
        return;
    }
    ...
    // process ParameterInfo into array for command line WORD (argument)
    // matching
...
// if non-option arguments exist, attempt to process them before processing
// options
$wordStack = array();
...
if ($wordStack && $wordArguments) {
    for ($wordIndex = 1; $wordIndex <= count($wordArguments); $wordIndex ++) {
        if (! array_key_exists($wordIndex - 1, $wordStack) ||
                 ! array_key_exists($wordIndex, $wordArguments)) {
            break;
        }
        $this->_request->setProviderParameter(
                $wordArguments[$wordIndex]['parameterName'], 
                $wordStack[$wordIndex - 1]);
        unset($wordStack[$wordIndex - 1]);
    }
}

$getoptParser = new Zend_Console_Getopt($getoptOptions, $this->_argumentsWorking, 
        array(
                'parseAll' => false
        ));
$getoptParser->parse();
...
if (isset($options['actionName']) && isset($options['providerName'])) {
    $helpSystem->respondWithSpecialtyAndParamHelp($options['providerName'], 
            $options['actionName']);
