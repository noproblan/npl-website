...
/**
 * Zend_Tool_Framework_Client_Console - the CLI Client implementation for
 * Zend_Tool_Framework
 *
 * @category Zend
 * @package Zend_Tool
        ...
        Zend_Tool_Framework_Client_Interactive_OutputInterface
{
    ...
    /**
     * _init() - Tasks processed before the constructor, generally setting up
     * objects to use
     */
    protected function _preInit ()
    {
        ...
        if ($this->_storageOptions != null &&
                 isset($this->_storageOptions['directory'])) {
            $storage->setAdapter(
                    new Zend_Tool_Framework_Client_Storage_Directory(
                            $this->_storageOptions['directory']));
        }
        
        // which classes are essential to initializing
        ...
        if ($this->_classesToLoad) {
            if (is_string($this->_classesToLoad)) {
                ...
                $classesToLoad = array_merge($classesToLoad, 
                        $this->_classesToLoad);
            }
        }
        
        // add classes to the basic loader from the config file
        // basicloader.classes.1 ..
        ...
        $this->_registry->setLoader(
                new Zend_Tool_Framework_Loader_BasicLoader(
                        array(
    ...
    protected function _preDispatch ()
    {
        ...
        $response->addContentDecorator(
                new Zend_Tool_Framework_Client_Response_ContentDecorator_Separator())->setDefaultDecoratorOptions(
                array(
                        'separator' => true
                ));
    ...
    protected function _postDispatch ()
    {
        ...
        if ($response->isException()) {
            ...
            $helpSystem->setRegistry($this->_registry)
                ...
                ->respondWithSpecialtyAndParamHelp($request->getProviderName(), 
    ...
    /**
     * handleInteractiveInputRequest() is required by the Interactive
     * InputInterface
     *
     *
     ...
     * @return string
     */
    public function handleInteractiveInputRequest (
            Zend_Tool_Framework_Client_Interactive_InputRequest $inputRequest)
    {
        ...
        return rtrim($inputContent); // remove the return from the end of the
                                     // string
    }
     ...
     *
     * Convert words to client specific naming, in this case is lower, dash
     * separated
     *
