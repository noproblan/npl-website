 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Framework_System_Provider_Manifest implements 
    ...
    public function show ()
    {
        ...
        foreach ($manifestRepository as $metadata) {
            ...
            if (! $metadataAttrs) {
                ...
                $metadataAttrs = urldecode(
                        http_build_query($metadataAttrs, null, ', '));
            }
            ...
            if (! array_key_exists($metadataAttrs, 
            ...
            $longestAttrNameLen = (strlen($metadataAttrs) > $longestAttrNameLen) ? strlen(
                    $metadataAttrs) : $longestAttrNameLen;
            
            $metadataValue = $metadata->getValue();
            if (is_array($metadataValue) && count($metadataValue) > 0) {
                $metadataValue = urldecode(
                        http_build_query($metadataValue, null, ', '));
                ...
                foreach ($metadatasByAttributes as $metadataAttributeName => $metadataValues) {
                    foreach ($metadataValues as $metadataValue) {
                        $string = sprintf(
                                ...
                                $metadataAttributeName) . $metadataValue;
