...
class Zend_Tool_Framework_System_Provider_Config extends Zend_Tool_Framework_Provider_Abstract
{
    ...
    public function show ()
    {
        ...
        $resp->appendContent(
                "User Configuration: " . $userConfig->getConfigFilepath(), 
                array(
                        "color" => "green"
                ));
    ...
    protected function _printTree ($key, $value, $level = 1, $isLast = false)
    {
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $k => $v) {
                $i ++;
                $tree .= $this->_printTree($k, $v, $level + 1, 
                        (count($value) == $i));
            }
        } else {
    ...
    public function disable ()
    {
        $resp = $this->_registry->getResponse();
        $resp->appendContent(
                'Use either "zf disable config.provider" or "zf disable config.manifest".');
    }
    ...
    public function enableProvider ($className)
    {
        ...
        if (! in_array("Zend_Tool_Framework_Provider_Interface", 
                $reflClass->getInterfaceNames())) {
            require_once "Zend/Tool/Framework/Exception.php";
            throw new Zend_Tool_Framework_Exception(
                    "Given class is not a provider");
        }
        ...
        if (! in_array($className, $providerClasses)) {
            ...
            if ($userConfig->save()) {
                $this->_registry->getResponse()->appendContent(
                        "Provider/Manifest '" . $className .
                                 "' was enabled for usage with Zend Tool.", 
                                array(
                                        "color" => "green",
                                        "aligncenter" => true
                                ));
    ...
    public function enableManifest ($className)
    {
        Zend_Loader::loadClass($className);
        $reflClass = new ReflectionClass($className);
        if (! in_array("Zend_Tool_Framework_Manifest_Interface", 
                $reflClass->getInterfaceNames())) {
            require_once "Zend/Tool/Framework/Exception.php";
            throw new Zend_Tool_Framework_Exception(
                    "Given class is not a manifest.");
        }
        $this->_doEnable($className);
    }
    ...
    protected function _loadUserConfigIfExists ()
    {
        ...
        $resp = $this->_registry->getResponse();
        if (! $userConfig->exists()) {
            $resp->appendContent("User has no config file.", 
                    array(
                            "aligncenter" => true,
                            "color" => array(
                                    'hiWhite',
