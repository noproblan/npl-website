 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Framework_Provider_Repository implements 
        ...
        Countable
{
            ...
            $overwriteExistingProvider = false)
    {
        ...
        // if a provider by the given name already exist, and its not set as
        // overwritable, throw exception
        if (! $overwriteExistingProvider && (array_key_exists($providerName, 
                $this->_unprocessedProviders) ||
    ...
    public function process ()
    {
        ...
        while ($this->_unprocessedProviders) {
            ...
            $providerSignature = new Zend_Tool_Framework_Provider_Signature(
                    $provider);
    ...
    /**
     * _parseName - internal method to determine the name of an action when one
     * is not explicity provided.
     *
            ...
            Zend_Tool_Framework_Provider_Interface $provider)
    {
        ...
        if (strpos($providerName, '_') !== false) {
            $providerName = substr($providerName, strrpos($providerName, '_') +
                     1);
        }
