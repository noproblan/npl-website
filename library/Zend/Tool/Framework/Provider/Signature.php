        ...
        Zend_Tool_Framework_Registry_EnabledInterface
{
    ...
    /**
     * getActionableMethod() - Get an actionable method by name, this will
     * return an array of
    ...
    /**
     * getActionableMethodByActionName() - Get an actionable method by its
     * action name, this
     * will return an array of useful information about what can be exectued on
     * this provider
     *
    ...
    /**
     * _process() is called at construction time and is what will build the
        ...
        if ($this->_providerReflection->hasMethod('getSpecialties')) {
            ...
            if (! is_array($specialties)) {
                ...
                throw new Zend_Tool_Framework_Provider_Exception(
                        'Provider ' . get_class($this->_provider) .
    ...
    /**
     * _processActionableMethods() - process all methods that can be called on
     * this provider.
     */
    protected function _processActionableMethods ()
    {
        ...
        foreach ($methods as $method) {
            ...
            if (! $method->getDeclaringClass()->isInstantiable() ||
                     ! $method->isPublic() || $methodName[0] == '_' ||
                     $method->isStatic() ||
                     in_array($methodName, 
                            array(
                                    'getContextClasses',
                                                                                            ...
                                                                                            // here
{
            ...
            /**
             * check to see if the method was a required method by a Zend_Tool_*
             * interface
             */
            foreach ($method->getDeclaringClass()->getInterfaces() as $methodDeclaringClassInterface) {
                if (strpos($methodDeclaringClassInterface->getName(), 
                        'Zend_Tool_') === 0 &&
            ...
            // get the action, and create non-existent actions when they dont
            // exist (the true part below)
            $action = $this->_registry->getActionRepository()->getAction(
                    $actionableMethods[$methodName]['actionName']);
            if ($action == null) {
            ...
            if (! in_array($actionableMethods[$methodName]['action'], 
                    $this->_actions)) {
            ...
            if (($docComment = $method->getDocComment()) != '' &&
                ...
                for ($i = 0; $i <= count($matches[0]) - 1; $i ++) {
                    ...
                    if ($currentParam != '' &&
                             isset($parameterInfo[$currentParam])) {
                        ...
                        if ($descriptionSource != '') {
                            $parameterInfo[$currentParam]['description'] = trim(
                                    $descriptionSource);
                        }
                    }
                }
