...
class Zend_Tool_Framework_Loader_IncludePathLoader extends Zend_Tool_Framework_Loader_Abstract
{
    ...
    protected function _getFiles ()
    {
        ...
        foreach ($paths as $path) {
            ...
            // ensure that we only traverse a single version of Zend Framework
            // on all include paths
            if (file_exists(
                    $realIncludePath .
                ...
                if ($isZendTraversed === false) {
                    ...
                    // use the deny directory pattern that includes the path to
                    // 'Zend', it will not be accepted
                    $filterDenyDirectoryPattern = '.*((/|\\\\).svn|' .
            ...
            foreach ($iterator as $item) {
                ...
                // ensure that the same named file from separate include_paths
                // is not loaded
    ...
    protected function _fileIsBlacklisted ($file)
    {
        $blacklist = array(
                ...
                "Zend" . DIRECTORY_SEPARATOR . "OpenId" . DIRECTORY_SEPARATOR .
                         "Provider"
        );
