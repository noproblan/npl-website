...
class Zend_Tool_Framework_Loader_IncludePathLoader_RecursiveFilterIterator extends RecursiveFilterIterator
{
    ...
    public function __construct (RecursiveIterator $iterator, 
            $denyDirectoryPattern = null, $acceptFilePattern = null)
    {
    ...
    /**
     * accept() - Which iterable items to accept or deny, required by
     * FilterInterface
     *
     * @return unknown
     */
    public function accept ()
    {
        ...
        // if the current node is a directory AND doesn't match the
        // denyDirectory pattern, accept
        if ($currentNode->isDir() &&
                 ! preg_match('#' . $this->_denyDirectoryPattern . '#', 
                        $currentNodeRealPath)) {
            return true;
        }
        
        // if the file matches the accept file pattern, accept
        $acceptable = (preg_match('#' . $this->_acceptFilePattern . '#', 
                $currentNodeRealPath)) ? true : false;
        return $acceptable;
    }

    /**
     * getChildren() - overridden from RecursiveFilterIterator to allow the
     * persistence of
     * the $_denyDirectoryPattern and the $_acceptFilePattern when sub iterators
     * of this filter
    ...
    public function getChildren ()
    {
        ...
        return $this->ref->newInstance($this->getInnerIterator()
            ->getChildren(), $this->_denyDirectoryPattern, 
