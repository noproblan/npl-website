 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Framework_Manifest_Repository implements 
        Zend_Tool_Framework_Registry_EnabledInterface, IteratorAggregate, 
        Countable
{
    ...
    public function addMetadata (
            Zend_Tool_Framework_Metadata_Interface $metadata)
    {
    ...
    /**
     * process() - Process is expected to be called at the end of client
     * construction time.
     * By this time, the loader has run and loaded any found manifests into the
     * repository
     * for loading
     *
     * @return Zend_Tool_Framework_Manifest_Repository
            ...
            if ($manifest instanceof Zend_Tool_Framework_Manifest_MetadataManifestable) {
                ...
                foreach ($metadatas as $metadata) {
                    if (is_array($metadata)) {
                        ...
                        $metadata = new Zend_Tool_Framework_Metadata_Dynamic(
                                $metadata);
                    }
                    ...
                    if (! $metadata instanceof Zend_Tool_Framework_Metadata_Interface) {
                        require_once 'Zend/Tool/Framework/Manifest/Exception.php';
                        throw new Zend_Tool_Framework_Manifest_Exception(
                                'A Zend_Tool_Framework_Metadata_Interface object was not found in manifest ' .
                                         get_class($manifest));
                    }
     ...
     *
     * example: This will retrieve all metadata that matches the following
     * criteria
     ...
     */
    public function getMetadatas (Array $searchProperties = array(), 
            $includeNonExistentProperties = true)
    {
        ...
        foreach ($this->_metadatas as $metadata) {
            ...
            // each value will be retrieved from the metadata, each metadata
            // should
            ...
            foreach ($searchProperties as $searchPropertyName => $searchPropertyValue) {
                if (method_exists($metadata, 'get' . $searchPropertyName)) {
                    if ($metadata->{'get' . $searchPropertyName}() !=
                             $searchPropertyValue) {
                        // if the metadata supports a specific property but the
                        // value does not
                        // match, move on
                        continue 2;
                    }
                } elseif (! $includeNonExistentProperties) {
                    // if the option $includeNonExitentProperties is false, then
                    // move on as
                    // we dont want to include this metadata if non existent
                    // search properties are not inside the target (current)
                    // metadata
                    continue 2;
                }
            }
            
            // all searching has been accounted for, if we reach this point,
            // then the metadata
    ...
    /**
     * getMetadata() - This will proxy to getMetadatas(), but will only return a
     * single metadata.
     * This method
     * should be used in situations where the search criteria is known to only
     * find a single metadata object
     *
     * @param array $searchProperties            
     * @param bool $includeNonExistentProperties            
     * @return Zend_Tool_Framework_Manifest_Metadata
     */
    public function getMetadata (Array $searchProperties = array(), 
            $includeNonExistentProperties = true)
    {
    ...
    public function __toString ()
    {
        ...
        foreach ($metadatasByType as $type => $metadatas) {
            ...
            foreach ($metadatas as $metadata) {
                ...
                // $metadataString = str_replace(PHP_EOL, PHP_EOL . ' ',
                // $metadataString);
