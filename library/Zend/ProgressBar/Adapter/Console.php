...
class Zend_ProgressBar_Adapter_Console extends Zend_ProgressBar_Adapter
{
    ...
    public function setWidth ($width = null)
    {
        if ($width === null || ! is_integer($width)) {
            if (substr(PHP_OS, 0, 3) === 'WIN') {
                ...
                if (preg_match('#\d+ (\d+)#', @shell_exec('stty size'), $match) ===
                         1) {
                    $this->_width = (int) $match[1];
                } else 
                    if (preg_match('#columns = (\d+);#', @shell_exec('stty'), 
                            $match) === 1) {
                        $this->_width = (int) $match[1];
                    }
            }
     ...
     * @throws Zend_ProgressBar_Adapter_Exception When an invalid element is
     *         foudn in the array
     * @return Zend_ProgressBar_Adapter_Console
     */
    public function setElements (array $elements)
    {
        ...
        if (count(array_diff($elements, $allowedElements)) > 0) {
            require_once 'Zend/ProgressBar/Adapter/Exception.php';
            throw new Zend_ProgressBar_Adapter_Exception(
                    'Invalid element found in $elements array');
        }
     ...
     *
     * @param string $action            
     * @throws Zend_ProgressBar_Adapter_Exception When an invalid action is
     *         specified
     * @return Zend_ProgressBar_Adapter_Console
     */
        ...
        foreach ($this->_elements as $element) {
            switch ($element) {
                case self::ELEMENT_BAR:
                    ...
                    $doneWidth = min($visualWidth - $indicatorWidth, 
                            round($visualWidth * $percent));
                    if ($doneWidth > 0) {
                        $bar .= substr(
                                str_repeat($this->_barLeftChar, 
                                        ceil(
                                                $doneWidth /
                                                         strlen(
                                                                $this->_barLeftChar))), 
                ...
                case self::ELEMENT_PERCENT:
                    $renderedElements[] = str_pad(round($percent * 100), 3, ' ', 
                            STR_PAD_LEFT) . '%';
                    break;
                ...
                case self::ELEMENT_ETA:
                    ...
                    if ($timeRemaining === null || $timeRemaining > 86400) {
                        ...
                        $etaFormatted = sprintf('%02d:%02d:%02d', $hours, 
                                $minutes, $seconds);
                    }
                ...
                case self::ELEMENT_TEXT:
                    $renderedElements[] = Zend_Text_MultiByte::strPad(
                            substr($text, 0, $this->_textWidth), 
                            $this->_textWidth, ' ', STR_PAD_RIGHT, 
                            $this->_charset);
                    break;
            }
        }
