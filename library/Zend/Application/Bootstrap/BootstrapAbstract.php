        ...
        Zend_Application_Bootstrap_ResourceBootstrapper
{
    ...
    /**
     *
     * @var array Initializers that have been started but not yet completed
     ...
     * @return void
     * @throws Zend_Application_Bootstrap_Exception When invalid application is
     *         provided
     */
     ...
     * @param mixed $options            
     * @return Zend_Application_Bootstrap_BootstrapAbstract
     * @throws Zend_Application_Bootstrap_Exception When invalid resource is
     *         provided
     */
    public function registerPluginResource ($resource, $options = null)
    {
        ...
        if (! is_string($resource)) {
            throw new Zend_Application_Bootstrap_Exception(
                    'Invalid resource provided to ' . __METHOD__);
        }
     ...
     * @param string|Zend_Application_Resource_Resource $resource            
     * @return Zend_Application_Bootstrap_BootstrapAbstract
     * @throws Zend_Application_Bootstrap_Exception When unknown resource type
     *         is provided
     */
    public function unregisterPluginResource ($resource)
    {
        ...
        if (! is_string($resource)) {
            throw new Zend_Application_Bootstrap_Exception(
                    'Unknown resource type provided to ' . __METHOD__);
        }
    ...
    public function getPluginResource ($resource)
    {
        if (array_key_exists(strtolower($resource), $this->_pluginResources)) {
            ...
            if (! $this->_pluginResources[$resource] instanceof Zend_Application_Resource_Resource) {
                $resourceName = $this->_loadPluginResource($resource, 
                        $this->_pluginResources[$resource]);
                if (! $resourceName) {
                    throw new Zend_Application_Bootstrap_Exception(
                            sprintf(
                                    'Unable to resolve plugin "%s"; no corresponding plugin with that name', 
                                    $resource));
                }
        ...
        foreach ($this->_pluginResources as $plugin => $spec) {
            ...
            if (false !==
                     $pluginName = $this->_loadPluginResource($plugin, $spec)) {
                if (0 === strcasecmp($resource, $pluginName)) {
            ...
            if (class_exists($plugin) &&
                     is_subclass_of($plugin, 
                            'Zend_Application_Resource_Resource')) { // @SEE ZF-7550
                $spec = (array) $spec;
                $spec['bootstrap'] = $this;
     ...
     * @return Zend_Application_Bootstrap_BootstrapAbstract
     * @throws Zend_Application_Bootstrap_Exception When invalid argument was
     *         passed
     */
    ...
    protected function _loadPluginResource ($resource, $options)
    {
        ...
        $className = $this->getPluginLoader()->load(strtolower($resource), 
                false);
