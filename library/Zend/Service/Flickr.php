...
class Zend_Service_Flickr
{
     ...
     * # page: the starting page offset. first result will be (page - 1) *
     * per_page + 1
     * # tag_mode: Either 'any' for an OR combination of tags,
     * or 'all' for an AND combination. Default is 'any'.
     * # min_upload_date: Minimum upload date to search on. Date should be a
     * unix timestamp.
     * # max_upload_date: Maximum upload date to search on. Date should be a
     * unix timestamp.
     * # min_taken_date: Minimum upload date to search on. Date should be a
     * MySQL datetime.
     * # max_taken_date: Maximum upload date to search on. Date should be a
     * MySQL datetime.
     *
    ...
    public function getIdByUsername ($username)
    {
        ...
        return (string) $xpath->query('//user')
            ->item(0)
            ->getAttribute('id');
    }
     ...
     * @param string $id
     *            the NSID
     * @return array of Zend_Service_Flickr_Image, details for the specified
     *         image
     * @throws Zend_Service_Exception
     */
    ...
    protected function _validateUserSearch (array $options)
    {
        ...
        if (! $between->isValid($options['per_page'])) {
            ...
            throw new Zend_Service_Exception(
                    $options['per_page'] .
                             ' is not valid for the "per_page" option');
        }
        ...
        if (! $int->isValid($options['page'])) {
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    $options['page'] . ' is not valid for the "page" option');
        }
        ...
        if ($options['extras']) {
            ...
            /**
             *
             * @todo The following does not do anything [yet], so it is
             *       commented out.
             */
    ...
    /**
     * Throws an exception if and only if the response status indicates a
     * failure
     *
    ...
    protected static function _checkErrors (DOMDocument $dom)
    {
        if ($dom->documentElement->getAttribute('stat') === 'fail') {
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    'Search failed due to error: ' . $err->getAttribute('msg') .
     ...
     */
    protected function _prepareOptions ($method, array $options, 
