...
class Zend_Service_Flickr_ResultSet implements SeekableIterator
{
    ...
    public function __construct (DOMDocument $dom, Zend_Service_Flickr $flickr)
    {
        ...
        $this->totalResultsReturned = ($page == $pages || $pages == 0) ? ($total -
                 ($page - 1) * $perPage) : (int) $perPage;
    ...
    {
        return new Zend_Service_Flickr_Result(
                $this->_results->item($this->_currentIndex), $this->_flickr);
    }
    ...
    public function seek ($index)
    {
        $indexInt = (int) $index;
        if ($indexInt >= 0 &&
                 (null === $this->_results || $indexInt < $this->_results->length)) {
    ...
    {
        return null !== $this->_results &&
                 $this->_currentIndex < $this->_results->length;
    }
}
