...
class Zend_Service_Akismet extends Zend_Service_Abstract
{
    ...
    public function __construct ($apiKey, $blog)
    {
        $this->setBlogUrl($blog)
            ->setApiKey($apiKey)
            ->setUserAgent(
                'Zend Framework/' . Zend_Version::VERSION . ' | Akismet/1.11');
    }
    ...
    protected function _makeApiCall ($path, $params)
    {
        if (empty($params['user_ip']) || empty($params['user_agent'])) {
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    'Missing required Akismet fields (user_ip and user_agent are required)');
        }
        ...
        return $this->_post($this->getApiKey() . '.rest.akismet.com', $path, 
                $params);
    }
     ...
     * optional):
     * - blog: URL of the blog. If not provided, uses value returned by {@link
     * getBlogUrl()}
     ...
     * - comment_type: typically, one of 'blank', 'comment', 'trackback', or
     * 'pingback', but may be any value
