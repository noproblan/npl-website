...
class Zend_Service_Amazon_SimpleDb extends Zend_Service_Amazon_Abstract
{
    ...
    public function setEndpoint ($endpoint)
    {
        ...
        if (! $endpoint->valid()) {
            require_once 'Zend/Service/Amazon/SimpleDb/Exception.php';
            throw new Zend_Service_Amazon_SimpleDb_Exception(
    ...
    public function getAttributes ($domainName, $itemName, $attributeName = null)
    {
        ...
        foreach ($attributeNodes as $attributeNode) {
            ...
            if (isset($attributes[$name])) {
                ...
                $attributes[$name] = new Zend_Service_Amazon_SimpleDb_Attribute(
                        $itemName, $name, $data);
            }
        }
        return $attributes;
            ...
            $replace = array())
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            foreach ($attribute->getValues() as $value) {
                ...
                if (array_key_exists($attributeName, $replace) &&
     ...
     * @return void
     */
    public function batchPutAttributes ($items, $domainName, 
            array $replace = array())
    {
        $params = array();
        ...
        foreach ($items as $name => $attributes) {
            ...
            $attributeIndex = 0;
            foreach ($attributes as $attribute) {
                // attribute value cannot be array, so when several items are
                // passed
                // they are treated as separate values with the same attribute
                // name
                foreach ($attribute->getValues() as $value) {
                    $params['Item.' . $itemIndex . '.Attribute.' .
                             $attributeIndex . '.Name'] = $attribute->getName();
                    $params['Item.' . $itemIndex . '.Attribute.' .
                             $attributeIndex . '.Value'] = $value;
                    if (isset($replace[$name]) &&
                             isset($replace[$name][$attribute->getName()]) &&
                             $replace[$name][$attribute->getName()]) {
                        $params['Item.' . $itemIndex . '.Attribute.' .
                         $attributeIndex . '.Replace'] = 'true';
            }
            $attributeIndex ++;
        }
    }
 ...
 * @return void
 */
public function deleteAttributes ($domainName, $itemName, 
...
foreach ($xml->SelectResult->Item as $item) {
    ...
    foreach ($item->Attribute as $attribute) {
        ...
        $attributes[$itemName][$attributeName] = new Zend_Service_Amazon_SimpleDb_Attribute(
                $itemName, $attributeName, $values);
    }
}
...
try {
    ...
    $request->setRawData(join('&', $params_out), 
...
require_once 'Zend/Crypt/Hmac.php';
$hmac = Zend_Crypt_Hmac::compute($this->_getSecretKey(), 'SHA256', $data, 
 ...
 */
private function _checkForErrors (
    Zend_Service_Amazon_SimpleDb_Response $response)
{
