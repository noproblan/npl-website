...
/**
 * An Amazon EC2 interface to create, describe, attach, detach and delete
 * Elastic Block
...
class Zend_Service_Amazon_Ec2_Ebs extends Zend_Service_Amazon_Ec2_Abstract
{
    ...
    /**
     * Creates a new Amazon EBS volume that you can mount from any Amazon EC2
     * instance.
     *
     * You must specify an availability zone when creating a volume. The volume
     * and
    ...
    public function createNewVolume ($size, $availabilityZone)
    {
        ...
        $return['createTime'] = $xpath->evaluate(
                'string(//ec2:createTime/text())');
        $return['availabilityZone'] = $xpath->evaluate(
                'string(//ec2:availabilityZone/text())');
    ...
    public function createVolumeFromSnapshot ($snapshotId, $availabilityZone)
    {
        ...
        $return['availabilityZone'] = $xpath->evaluate(
                'string(//ec2:availabilityZone/text())');
        $return['snapshotId'] = $xpath->evaluate(
                'string(//ec2:snapshotId/text())');
    ...
    public function describeVolume ($volumeId = null)
    {
        ...
        $xpath = $response->getXPath();
        $nodes = $xpath->query('//ec2:volumeSet/ec2:item', 
        ...
        foreach ($nodes as $node) {
            ...
            $item['volumeId'] = $xpath->evaluate('string(ec2:volumeId/text())', 
                    $node);
            $item['size'] = $xpath->evaluate('string(ec2:size/text())', $node);
            $item['status'] = $xpath->evaluate('string(ec2:status/text())', 
                    $node);
            $item['createTime'] = $xpath->evaluate(
                    'string(ec2:createTime/text())', $node);
            
            $attachmentSet = $xpath->query('ec2:attachmentSet/ec2:item', $node);
            if ($attachmentSet->length == 1) {
                $_as = $attachmentSet->item(0);
                $as = array();
                $as['volumeId'] = $xpath->evaluate(
                        'string(ec2:volumeId/text())', $_as);
                $as['instanceId'] = $xpath->evaluate(
                        'string(ec2:instanceId/text())', $_as);
                $as['device'] = $xpath->evaluate('string(ec2:device/text())', 
                        $_as);
                $as['status'] = $xpath->evaluate('string(ec2:status/text())', 
                        $_as);
                $as['attachTime'] = $xpath->evaluate(
                        'string(ec2:attachTime/text())', $_as);
    ...
    public function describeAttachedVolumes ($instanceId)
    {
        ...
        foreach ($volumes as $vol) {
            if (isset($vol['attachmentSet']) &&
                     $vol['attachmentSet']['instanceId'] == $instanceId) {
                $return[] = $vol;
            }
        }
    ...
    public function attachVolume ($volumeId, $instanceId, $device)
    {
        ...
        $return['status'] = $xpath->evaluate('string(//ec2:status/text())');
        $return['attachTime'] = $xpath->evaluate(
                'string(//ec2:attachTime/text())');
     ...
     *            occur cleanly
     *            (logging into an instance, unmounting the volume, and
     *            detaching normally).
     *            This option can lead to data loss or a corrupted file system.
     *            Use this option
     *            only as a last resort to detach an instance from a failed
     *            instance. The
     ...
     * @return array
     */
    public function detachVolume ($volumeId, $instanceId = null, $device = null, 
            $force = false)
    {
        $params = array();
     ...
     * to launch instances from identical snapshots, and to save data before
    ...
    public function describeSnapshot ($snapshotId = null)
    {
        ...
        $xpath = $response->getXPath();
        $nodes = $xpath->query('//ec2:snapshotSet/ec2:item', 
                $response->getDocument());
        ...
        foreach ($nodes as $node) {
            ...
            $item['snapshotId'] = $xpath->evaluate(
                    'string(ec2:snapshotId/text())', $node);
            $item['volumeId'] = $xpath->evaluate('string(ec2:volumeId/text())', 
                    $node);
            $item['status'] = $xpath->evaluate('string(ec2:status/text())', 
                    $node);
            $item['startTime'] = $xpath->evaluate(
                    'string(ec2:startTime/text())', $node);
            $item['progress'] = $xpath->evaluate('string(ec2:progress/text())', 
                    $node);
