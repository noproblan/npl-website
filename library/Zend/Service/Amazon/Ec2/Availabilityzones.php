...
/**
 * An Amazon EC2 interface to query which Availibity Zones your account has
 * access to.
 *
 * @category Zend
 * @package Zend_Service_Amazon
...
class Zend_Service_Amazon_Ec2_Availabilityzones extends Zend_Service_Amazon_Ec2_Abstract
{
     ...
     * @return array An array that contains all the return items. Keys: zoneName
     *         and zoneState.
     */
    public function describe ($zoneName = null)
    {
        ...
        foreach ($nodes as $k => $node) {
            $item = array();
            $item['zoneName'] = $xpath->evaluate('string(ec2:zoneName/text())', 
                    $node);
            $item['zoneState'] = $xpath->evaluate(
                    'string(ec2:zoneState/text())', $node);
