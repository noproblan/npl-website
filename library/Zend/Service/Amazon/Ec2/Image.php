...
/**
 * An Amazon EC2 interface to register, describe and deregister Amamzon Machine
 * Instances (AMI)
 *
 * @category Zend
 * @package Zend_Service_Amazon
...
class Zend_Service_Amazon_Ec2_Image extends Zend_Service_Amazon_Ec2_Abstract
{
     ...
     * Amazon
     * EC2 retrieves the specified image manifest from Amazon S3 and verifies
     * that
     ...
     * explicit: The owner of the AMI granted launch permissions to a specific
     * user.
     * implicit: A user has implicit launch permissions for all AMIs he or she
     * owns.
     *
     ...
     *
     * If you specify one or more AMI IDs, only AMIs that have the specified IDs
     * are returned.
     * If you specify an invalid AMI ID, a fault is returned. If you specify an
     * AMI ID for which
     ...
     * owners and for
     * which you have access are returned. The results can include the account
     * IDs of the
     * specified owners, amazon for AMIs owned by Amazon or self for AMIs that
     * you own.
     *
     ...
     * AMI(s)), self
     * for AMIs for which you own or have explicit permissions, or all for
     * public AMIs.
     *
    ...
    public function describe ($imageId = null, $owner = null, $executableBy = null)
    {
        ...
        foreach ($nodes as $node) {
            ...
            $item['imageId'] = $xpath->evaluate('string(ec2:imageId/text())', 
                    $node);
            $item['imageLocation'] = $xpath->evaluate(
                    'string(ec2:imageLocation/text())', $node);
            $item['imageState'] = $xpath->evaluate(
                    'string(ec2:imageState/text())', $node);
            $item['imageOwnerId'] = $xpath->evaluate(
                    'string(ec2:imageOwnerId/text())', $node);
            $item['isPublic'] = $xpath->evaluate('string(ec2:isPublic/text())', 
                    $node);
            $item['architecture'] = $xpath->evaluate(
                    'string(ec2:architecture/text())', $node);
            $item['imageType'] = $xpath->evaluate(
                    'string(ec2:imageType/text())', $node);
            $item['kernelId'] = $xpath->evaluate('string(ec2:kernelId/text())', 
                    $node);
            $item['ramdiskId'] = $xpath->evaluate(
                    'string(ec2:ramdiskId/text())', $node);
            $item['platform'] = $xpath->evaluate('string(ec2:platform/text())', 
                    $node);
     ...
     * productCodes: Associates a product code with AMIs. This allows developers
     * to
     ...
     *            See the preceding attributes table for supported operations
     *            for attributes.
     *            Valid Values: add | remove
     *            Required for launchPermssion Attribute
     *            
     * @param string|array $userId
     *            User IDs to add to or remove from the launchPermission
     *            attribute.
     *            Required for launchPermssion Attribute
     * @param string|array $userGroup
     *            User groups to add to or remove from the launchPermission
     *            attribute.
     *            Currently, the all group is available, which will make it a
     *            public AMI.
     *            Required for launchPermssion Attribute
     * @param string $productCode
     *            Attaches a product code to the AMI. Currently only one product
     *            code
     *            can be associated with an AMI. Once set, the product code
     *            cannot be changed or reset.
     *            Required for productCodes Attribute
     * @return boolean
     */
    public function modifyAttribute ($imageId, $attribute, 
            $operationType = 'add', $userId = null, $userGroup = null, $productCode = null)
    {
        $params = array();
        ...
        if ($attribute == 'launchPermission') {
            ...
            if ($lPnodes->length > 0) {
                ...
                foreach ($lPnodes as $node) {
                    $return['launchPermission'][] = $xpath->evaluate(
                            'string(ec2:userId/text())', $node);
                }
            }
        }
        ...
        if ($attribute == 'productCodes') {
            ...
            if ($pCnodes->length > 0) {
                ...
                foreach ($pCnodes as $node) {
                    $return['productCodes'][] = $xpath->evaluate(
                            'string(ec2:productCode/text())', $node);
                }
            }
        }
     ...
     *            launchPermission is supported.
     *            In the case of launchPermission, all public and explicit
