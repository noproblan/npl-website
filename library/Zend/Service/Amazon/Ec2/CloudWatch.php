...
/**
 * An Amazon EC2 interface that allows yout to run, terminate, reboot and
 * describe Amazon
 * Ec2 Instances.
 *
 * @category Zend
...
class Zend_Service_Amazon_Ec2_CloudWatch extends Zend_Service_Amazon_Ec2_Abstract
{
     ...
     *
     * DiskWriteBytes: Bytes written to all disks available to the instance.
     * This
     * metric is used to determine the volume of the data the application writes
     * onto the hard disk of the instance. This can be used to determine the
     * speed
     * of the application for the customer.
     *
     * Latency: Time taken between a request and the corresponding response as
     * seen
     * by the load balancer.
     *
     * RequestCount: The number of requests processed by the LoadBalancer.
     *
     * HealthyHostCount: The number of healthy instances. Both Load Balancing
     * dimensions,
     * LoadBalancerName and AvailabilityZone, should be specified when
     * retreiving
     * HealthyHostCount.
     *
     * UnHealthyHostCount: The number of unhealthy instances. Both Load
     ...
     * instance. You can use the currently available dimensions for EC2
     * instances
     * along with these metrics in order to refine the slice of data you want
     * returned,
     * such as metric CPUUtilization and dimension ImageId to get all
     * CPUUtilization
    ...
    /**
     * Amazon CloudWatch not only aggregates the raw data coming in, it also
     * computes
     * several statistics on the data.
     * The following table lists the statistics that you can request:
     *
     * Minimum: The lowest value observed during the specified period. This can
     * be used to
     * determine low volumes of activity for your application.
     *
     * Maximum: The highest value observed during the specified period. You can
     * use this to
     * determine high volumes of activity for your application.
     *
     * Sum: The sum of all values received (if appropriate, for example a rate
     * would not be
     * summed, but a number of items would be). This statistic is useful for
     * determining
     * the total volume of a metric.
     *
     * Average: The Average of all values received during the specified period.
     * By comparing
     * this statistic with the minimum and maximum statistics, you can determine
     * the full
     * scope of a metric and how close the average use is to the minimum and the
     * maximum.
     * This will allow you to increase or decrease your resources as needed.
     *
     * Samples: The count (number) of measures used. This statistic is always
     * returned to
     * show the user the size of the dataset collected. This will allow the user
     * to properly
     * weight the data.
     *
     * Statistics are computed within a period you specify, such as all
     * CPUUtilization within a
     * five minute period. At a minimum, all data is aggregated into one minute
     * intervals. This
     * is the minimum resolution of the data. It is this data that can be
     * aggregated into larger
     * periods of time that you request.
     *
     * Aggregate data is generally available from the service within one minute
     * from the end of the
     * aggregation period. Delays in data propagation might cause late or
     * partially late data in
     * some cases. If your data is delayed, you should check the service’s
     * Health Dashboard for
     * any current operational issues with either Amazon CloudWatch or the
     * services collecting
     ...
     *
     * AvailabilityZone: This dimension filters the data you request for all
     * instances
     * running in that EC2 Availability Zone.
     *
     * AutoScalingGroupName: This dimension filters the data you request for all
     * instances
     * in a specified capacity group. An AutoScalingGroup is a collection of
     * instances
     * defined by customers of the Auto Scaling service. This dimension is only
     * available
     * for EC2 metrics when the instances are in such an AutoScalingGroup.
     *
     * InstanceId: This dimension filters the data you request for only the
     * identified
     * instance. This allows a user to pinpoint an exact instance from which to
     * monitor data.
     *
     * InstanceType: This dimension filters the data you request for all
     * instances running
     * with this specified instance type. This allows a user to catagorize his
     * data by the
     * type of instance running. For example, a user might compare data from an
     * m1.small instance
     * and an m1.large instance to determine which has the better business value
     * for his application.
     *
     * LoadBalancerName: This dimension filters the data you request for the
     * specified LoadBalancer
     * name. A LoadBalancer is represented by a DNS name and provides the single
     * destination to
     * which all requests intended for your application should be directed. This
     * metric allows
     * you to examine data from all instances connected to a single
     * LoadBalancer.
     *
     * @var array
     */
     ...
     * made that would generate more datapoints than this amount, Amazon
     * CloudWatch
     * will return an error. You can alter your request by narrowing the time
     * range
     * (StartTime, EndTime) or increasing the Period in your single request. You
     * may
     * also get all of the data at the granularity you originally asked for by
     * making
     ...
     *            Load Balancing Metrics are Latency, RequestCount,
     *            HealthyHostCount
     *            UnHealthyHostCount
     *            Statistics: The statistics to be returned for the given
     *            metric. Valid
     *            values are Average, Maximum, Minimum, Samples, Sum. You can
     *            specify
     *            this as a string or as an array of values. If you don't
     *            specify one
     *            it will default to Average instead of failing out. If you
     *            specify an incorrect
     *            option it will just skip it.
     *            ** Optional **
     *            Dimensions: Amazon CloudWatch allows you to specify one
     *            Dimension to further filter
     *            metric data on. If you don't specify a dimension, the service
     *            returns the aggregate
     *            of all the measures with the given measure name and time
     *            range.
     *            Unit: The standard unit of Measurement for a given Measure.
     *            Valid Values: Seconds,
     *            Percent, Bytes, Bits, Count, Bytes/Second, Bits/Second,
     *            Count/Second, and None
     *            Constraints: When using count/second as the unit, you should
     *            use Sum as the statistic
     *            instead of Average. Otherwise, the sample returns as equal to
     *            the number of requests
     *            instead of the number of 60-second intervals. This will cause
     *            the Average to
     *            always equals one when the unit is count/second.
     *            StartTime: The timestamp of the first datapoint to return,
     *            inclusive. For example,
     *            2008-02-26T19:00:00+00:00. We round your value down to the
     *            nearest minute.
     *            You can set your start time for more than two weeks in the
     *            past. However,
     *            you will only get data for the past two weeks. (in ISO 8601
     *            format)
     *            Constraints: Must be before EndTime
     *            EndTime: The timestamp to use for determining the last
     *            datapoint to return. This is
     *            the last datapoint to fetch, exclusive. For example,
    ...
    public function getMetricStatistics (array $options)
    {
        ...
        if (! isset($options['MeasureName']) ||
                 ! in_array($options['MeasureName'], $this->_validMetrics, true)) {
            throw new Zend_Service_Amazon_Ec2_Exception(
                    'Invalid Metric Type: ' . $options['MeasureName']);
        }
        ...
        $nodes = $xpath->query(
                '//ec2:GetMetricStatisticsResult/ec2:Datapoints/ec2:member');
        
        $return = array();
        $return['label'] = $xpath->evaluate(
                'string(//ec2:GetMetricStatisticsResult/ec2:Label/text())');
        foreach ($nodes as $node) {
            $item = array();
            
            $item['Timestamp'] = $xpath->evaluate(
                    'string(ec2:Timestamp/text())', $node);
            $item['Unit'] = $xpath->evaluate('string(ec2:Unit/text())', $node);
            $item['Samples'] = $xpath->evaluate('string(ec2:Samples/text())', 
                    $node);
            foreach ($_usedStatistics as $us) {
                $item[$us] = $xpath->evaluate('string(ec2:' . $us . '/text())', 
                        $node);
            }
    ...
    public function listMetrics ($nextToken = null)
    {
        ...
        foreach ($nodes as $node) {
            ...
            $item['MeasureName'] = $xpath->evaluate(
                    'string(ec2:MeasureName/text())', $node);
            $item['Namespace'] = $xpath->evaluate(
                    'string(ec2:Namespace/text())', $node);
            $item['Deminsions']['name'] = $xpath->evaluate(
                    'string(ec2:Dimensions/ec2:member/ec2:Name/text())', $node);
            $item['Deminsions']['value'] = $xpath->evaluate(
                    'string(ec2:Dimensions/ec2:member/ec2:Value/text())', $node);
