...
/**
 * An Amazon EC2 interface to allocate, associate, describe and release Elastic
 * IP address
 * from your account.
 *
 * @category Zend
...
class Zend_Service_Amazon_Ec2_Elasticip extends Zend_Service_Amazon_Ec2_Abstract
{
    ...
    public function describe ($publicIp = null)
    {
        ...
        foreach ($nodes as $k => $node) {
            $item = array();
            $item['publicIp'] = $xpath->evaluate('string(ec2:publicIp/text())', 
                    $node);
    ...
    /**
     * Disassociates the specified elastic IP address from the instance to which
     * it is assigned.
