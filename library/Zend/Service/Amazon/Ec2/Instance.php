...
class Zend_Service_Amazon_Ec2_Instance extends Zend_Service_Amazon_Ec2_Abstract
{
     ...
     * a security group at launch, the instances start in your default security
     * group.
     ...
     * public
     * key at boot. You can use this key to provide secure access to an instance
     * of an
     * image on a per-instance basis. Amazon EC2 public images use this feature
     * to
     ...
     *            keyName string Name of the key pair with which to launch
     *            instances.
     *            securityGruop string|array Names of the security groups with
     *            which to associate the instances.
     *            userData string The user data available to the launched
     *            instances. This should not be Base64 encoded.
     *            instanceType constant Specifies the instance type.
     *            placement string Specifies the availability zone in which to
     *            launch the instance(s). By default, Amazon EC2 selects an
     *            availability zone for you.
     *            kernelId string The ID of the kernel with which to launch the
     *            instance.
     *            ramdiskId string The ID of the RAM disk with which to launch
     *            the instance.
     *            blockDeviceVirtualName string Specifies the virtual name to
     *            map to the corresponding device name. For example:
     *            instancestore0
     *            blockDeviceName string Specifies the device to which you are
    ...
    public function run (array $options)
    {
        ...
        // set / override the defualt optoins if they are not passed into the
        // array;
        ...
        if (is_array($options['securityGroup']) &&
                 ! empty($options['securityGroup'])) {
        ...
        if (isset($options['blockDeviceVirtualName']) &&
                 isset($options['blockDeviceName'])) {
        ...
        $return['reservationId'] = $xpath->evaluate(
        ...
        foreach ($gs as $gs_node) {
            $return['groupSet'][] = $xpath->evaluate(
                    'string(ec2:groupId/text())', $gs_node);
            unset($gs_node);
        }
        unset($gs);
        ...
        foreach ($is as $is_node) {
            ...
            $item['instanceId'] = $xpath->evaluate(
                    'string(ec2:instanceId/text())', $is_node);
            $item['imageId'] = $xpath->evaluate('string(ec2:imageId/text())', 
                    $is_node);
            $item['instanceState']['code'] = $xpath->evaluate(
                    'string(ec2:instanceState/ec2:code/text())', $is_node);
            $item['instanceState']['name'] = $xpath->evaluate(
                    'string(ec2:instanceState/ec2:name/text())', $is_node);
            $item['privateDnsName'] = $xpath->evaluate(
                    'string(ec2:privateDnsName/text())', $is_node);
            $item['dnsName'] = $xpath->evaluate('string(ec2:dnsName/text())', 
                    $is_node);
            $item['keyName'] = $xpath->evaluate('string(ec2:keyName/text())', 
                    $is_node);
            $item['instanceType'] = $xpath->evaluate(
                    'string(ec2:instanceType/text())', $is_node);
            $item['amiLaunchIndex'] = $xpath->evaluate(
                    'string(ec2:amiLaunchIndex/text())', $is_node);
            $item['launchTime'] = $xpath->evaluate(
                    'string(ec2:launchTime/text())', $is_node);
            $item['availabilityZone'] = $xpath->evaluate(
    ...
    public function describe ($instanceId = null, $ignoreTerminated = false)
    {
        ...
        foreach ($nodes as $node) {
            if ($xpath->evaluate(
                    'string(ec2:instancesSet/ec2:item/ec2:instanceState/ec2:code/text())', 
                    $node) == 48 && $ignoreTerminated)
                continue;
            $item = array();
            
            $item['reservationId'] = $xpath->evaluate(
                    'string(ec2:reservationId/text())', $node);
            $item['ownerId'] = $xpath->evaluate('string(ec2:ownerId/text())', 
                    $node);
            
            $gs = $xpath->query('ec2:groupSet/ec2:item', $node);
            foreach ($gs as $gs_node) {
                $item['groupSet'][] = $xpath->evaluate(
                        'string(ec2:groupId/text())', $gs_node);
                unset($gs_node);
            }
            unset($gs);
            ...
            foreach ($is as $is_node) {
                ...
                $item['instanceId'] = $xpath->evaluate(
                        'string(ec2:instanceId/text())', $is_node);
                $item['imageId'] = $xpath->evaluate(
                        'string(ec2:imageId/text())', $is_node);
                $item['instanceState']['code'] = $xpath->evaluate(
                        'string(ec2:instanceState/ec2:code/text())', $is_node);
                $item['instanceState']['name'] = $xpath->evaluate(
                        'string(ec2:instanceState/ec2:name/text())', $is_node);
                $item['privateDnsName'] = $xpath->evaluate(
                        'string(ec2:privateDnsName/text())', $is_node);
                $item['dnsName'] = $xpath->evaluate(
                        'string(ec2:dnsName/text())', $is_node);
                $item['keyName'] = $xpath->evaluate(
                        'string(ec2:keyName/text())', $is_node);
                $item['productCode'] = $xpath->evaluate(
                        'string(ec2:productCodesSet/ec2:item/ec2:productCode/text())', 
                        $is_node);
                $item['instanceType'] = $xpath->evaluate(
                        'string(ec2:instanceType/text())', $is_node);
                $item['launchTime'] = $xpath->evaluate(
                ...
                $item['availabilityZone'] = $xpath->evaluate(
                        'string(ec2:placement/ec2:availabilityZone/text())', 
                        $is_node);
                $item['kernelId'] = $xpath->evaluate(
                        'string(ec2:kernelId/text())', $is_node);
                $item['ramediskId'] = $xpath->evaluate(
                        'string(ec2:ramediskId/text())', $is_node);
                $item['amiLaunchIndex'] = $xpath->evaluate(
                        'string(ec2:amiLaunchIndex/text())', $is_node);
                $item['monitoringState'] = $xpath->evaluate(
                        'string(ec2:monitoring/ec2:state/text())', $is_node);
    ...
    public function terminate ($instanceId)
    {
        ...
        foreach ($nodes as $node) {
            ...
            $item['instanceId'] = $xpath->evaluate(
                    'string(ec2:instanceId/text())', $node);
            $item['shutdownState']['code'] = $xpath->evaluate(
                    'string(ec2:shutdownState/ec2:code/text())', $node);
            $item['shutdownState']['name'] = $xpath->evaluate(
                    'string(ec2:shutdownState/ec2:name/text())', $node);
            $item['previousState']['code'] = $xpath->evaluate(
                    'string(ec2:previousState/ec2:code/text())', $node);
            $item['previousState']['name'] = $xpath->evaluate(
                    'string(ec2:previousState/ec2:name/text())', $node);
     ...
     * specified instance(s). The operation
     * will succeed if the instances are valid and belong to the user. Requests
    ...
    public function consoleOutput ($instanceId)
    {
        ...
        $return['instanceId'] = $xpath->evaluate(
                'string(//ec2:instanceId/text())');
        $return['timestamp'] = $xpath->evaluate(
                'string(//ec2:timestamp/text())');
        $return['output'] = base64_decode(
                $xpath->evaluate('string(//ec2:output/text())'));
        
        return $return;
    }

    /**
     * Returns true if the specified product code is attached to the specified
     * instance.
     * The operation returns false if the product code is not attached to the
     * instance.
     *
     * The confirmProduct operation can only be executed by the owner of the
     * AMI.
     * This feature is useful when an AMI owner is providing support and wants
     * to
     ...
     * @return array|boolean An array if the product code is attached to the
     *         instance, false if it is not.
     */
    ...
    /**
     * Turn on Amazon CloudWatch Monitoring for an instance or a list of
     * instances
     *
     * @param array|string $instanceId
     *            The instance or list of instances you want to enable
     *            monitoring for
     * @return array
     */
    public function monitor ($instanceId)
    {
        ...
        foreach ($items as $item) {
            $i = array();
            $i['instanceid'] = $xpath->evaluate(
                    'string(//ec2:instanceId/text())', $item);
            $i['monitorstate'] = $xpath->evaluate(
                    'string(//ec2:monitoring/ec2:state/text())');
            $arrReturn[] = $i;
            unset($i);
        }
    ...
    /**
     * Turn off Amazon CloudWatch Monitoring for an instance or a list of
     * instances
     *
     * @param array|string $instanceId
     *            The instance or list of instances you want to disable
     *            monitoring for
     * @return array
     */
