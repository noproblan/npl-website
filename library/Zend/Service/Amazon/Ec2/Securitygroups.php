...
/**
 * An Amazon EC2 interface to create, delete, describe, grand and revoke
 * sercurity permissions.
 *
 * @category Zend
 * @package Zend_Service_Amazon
 * @subpackage Ec2
...
class Zend_Service_Amazon_Ec2_Securitygroups extends Zend_Service_Amazon_Ec2_Abstract
{
     ...
     * Instances
     * within the same security group have unrestricted network access to each
     * other.
     * Instances will reject network access attempts from other instances in a
     * different
     * security group. As the owner of instances you can grant or revoke
     * specific permissions
     * using the {@link authorizeIp}, {@link authorizeGroup}, {@link
     * revokeGroup} and
     ...
     * group is returned.
     * Otherwise, information for all security group is returned. If you specify
     * a group
    ...
    public function describe ($name = null)
    {
        ...
        foreach ($nodes as $node) {
            ...
            $item['ownerId'] = $xpath->evaluate('string(ec2:ownerId/text())', 
                    $node);
            $item['groupName'] = $xpath->evaluate(
                    'string(ec2:groupName/text())', $node);
            $item['groupDescription'] = $xpath->evaluate(
                    'string(ec2:groupDescription/text())', $node);
            ...
            foreach ($ip_nodes as $ip_node) {
                ...
                $sItem['ipProtocol'] = $xpath->evaluate(
                        'string(ec2:ipProtocol/text())', $ip_node);
                $sItem['fromPort'] = $xpath->evaluate(
                        'string(ec2:fromPort/text())', $ip_node);
                $sItem['toPort'] = $xpath->evaluate('string(ec2:toPort/text())', 
                        $ip_node);
                ...
                foreach ($ips as $ip) {
                    $sItem['ipRanges'][] = $xpath->evaluate(
                            'string(ec2:cidrIp/text())', $ip);
                }
     ...
     * security group,
     * a fault is returned. For example, if security group B has a rule that
     * allows access
     * from security group A, security group A cannot be deleted until the allow
     * rule is removed.
     *
     * @param string $name
     ...
     * (by IP range or an Amazon EC2 user-group pair), the source and
     * destination port ranges
     * (for TCP and UDP), and the ICMP codes and types (for ICMP). When
     * authorizing ICMP, -1
     ...
     *            Bottom of port range to authorize access to when operating on
     *            a CIDR IP.
     *            This contains the ICMP type if ICMP is being authorized.
     * @param integer $toPort
     *            Top of port range to authorize access to when operating on a
     *            CIDR IP.
     *            This contains the ICMP code if ICMP is being authorized.
     * @param string $cidrIp
     *            CIDR IP range to authorize access to when operating on a CIDR
     *            IP.
     * @return boolean
     */
     ...
     *            Name of security group to authorize access to when operating
     *            on a user/group pair.
     * @param string $ownerId
     *            Owner of security group to authorize access to when operating
     *            on a user/group pair.
     * @return boolean
     */
     ...
     * @param integer $fromPort
     *            Bottom of port range to revoke access to when operating on a
     *            CIDR IP.
     *            This contains the ICMP type if ICMP is being revoked.
     * @param integer $toPort
     *            Top of port range to revoked access to when operating on a
     *            CIDR IP.
     ...
     *            Name of the group to modify.
     * @param string $groupName
     *            Name of security group to revoke access to when operating on a
     *            user/group pair.
     * @param string $ownerId
     *            Owner of security group to revoke access to when operating on
     *            a user/group pair.
     * @return boolean
     */
