...
class Zend_Service_Amazon_Ec2_Keypair extends Zend_Service_Amazon_Ec2_Abstract
{
    ...
    public function create ($keyName)
    {
        ...
        $return['keyFingerprint'] = $xpath->evaluate(
                'string(//ec2:keyFingerprint/text())');
        $return['keyMaterial'] = $xpath->evaluate(
                'string(//ec2:keyMaterial/text())');
    ...
    public function describe ($keyName = null)
    {
        ...
        foreach ($nodes as $k => $node) {
            ...
            $item['keyName'] = $xpath->evaluate('string(ec2:keyName/text())', 
                    $node);
            $item['keyFingerprint'] = $xpath->evaluate(
                    'string(ec2:keyFingerprint/text())', $node);
