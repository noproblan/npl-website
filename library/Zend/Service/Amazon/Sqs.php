...
class Zend_Service_Amazon_Sqs extends Zend_Service_Amazon_Abstract
{
     ...
     * Constructor
     *
     * The default region is us-east-1. Use the region to set it to one of the
     * regions that is build-in into ZF.
     ...
     * Set SQS endpoint
     *
     * Checks and sets endpoint if region exists in $_sqsEndpoints. If a new SQS
     * region is added by amazon,
     ...
     * Set SQS endpoint
     *
     * You can set SQS to on of the build-in regions. If the region does not
     * exsist it will be added.
     *
     * @param string $region
     *            region
     * @throws Zend_Service_Amazon_Sqs_Exception
     ...
     *
     * Since there is not an SQS webserive to get all possible endpoints, a
     * hardcoded list is available.
...
if (! isset($result->SendMessageResult->MessageId) ||
    ...
    if ((string) $result->SendMessageResult->MD5OfMessageBody != $checksum) {
        require_once 'Zend/Service/Amazon/Sqs/Exception.php';
        throw new Zend_Service_Amazon_Sqs_Exception(
                'MD5 of body does not match message sent');
    }
