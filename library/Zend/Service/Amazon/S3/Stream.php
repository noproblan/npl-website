...
class Zend_Service_Amazon_S3_Stream
{
    ...
    protected function _getS3Client ($path)
    {
        if ($this->_s3 === null) {
            ...
            if (! $this->_s3) {
                ...
                require_once 'Zend/Service/Amazon/S3/Exception.php';
                throw new Zend_Service_Amazon_S3_Exception(
                        "Unknown client for wrapper {$url[0]}");
            }
        }
    ...
    public function stream_read ($count)
    {
        ...
        // Only fetch more data from S3 if we haven't fetched any data yet
        // (postion=0)
        // OR, the range end position plus 1 is greater than the size of the
        // current
        // object buffer
        if ($this->_objectBuffer === null ||
                 $range_end >= strlen($this->_objectBuffer)) {
            $headers = array(
                    'Range' => "bytes=$range_start-$range_end"
            );
            
            $response = $this->_s3->_makeRequest('GET', $this->_objectName, 
                    null, $headers);
    ...
    public function stream_stat ()
    {
        ...
        if (($slash = strchr($this->_objectName, '/')) === false ||
                 $slash == strlen($this->_objectName) - 1) {
            /* bucket */
            $stat['mode'] |= 040000;
        } else {
            $stat['mode'] |= 0100000;
    ...
    {
        return $this->_getS3Client($path)->removeObject(
    ...
    {
        return $this->_getS3Client($path)->createBucket(
    ...
    {
        return $this->_getS3Client($path)->removeBucket(
                parse_url($path, PHP_URL_HOST));
    }
    ...
    public function dir_opendir ($path, $options)
    {
        if (preg_match('@^([a-z0-9+.]|-)+://$@', $path)) {
            ...
            $this->_bucketList = $this->_getS3Client($path)->getObjectsByBucket(
                    $host);
        }
    ...
    public function url_stat ($path, $flags)
    {
        ...
        $name = $this->_getNamePart($path);
        if (($slash = strchr($name, '/')) === false || $slash == strlen($name) -
                 1) {
            /* bucket */
            $stat['mode'] |= 040000;
        } else {
