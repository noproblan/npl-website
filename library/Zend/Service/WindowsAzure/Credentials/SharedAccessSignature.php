...
class Zend_Service_WindowsAzure_Credentials_SharedAccessSignature extends Zend_Service_WindowsAzure_Credentials_CredentialsAbstract
{
    ...
    /**
     * Creates a new Zend_Service_WindowsAzure_Credentials_SharedAccessSignature
     * instance
     *
    ...
    public function __construct (
            ...
            $accountKey = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::DEVSTORE_KEY, $usePathStyleUri = false, $permissionSet = array())
    {
     ...
     * Set permisison set
     *
     * Warning: fine-grained permissions should be added prior to coarse-grained
     * permissions.
     * For example: first add blob permissions, end with container-wide
     * permissions.
     *
     * Warning: the signed access signature URL must match the account name of
     * the
     * Zend_Service_WindowsAzure_Credentials_Zend_Service_WindowsAzure_Credentials_SharedAccessSignature
     * instance
     *
     * @param array $value
     *            Permission set
        ...
        foreach ($value as $url) {
            if (strpos($url, $this->_accountName) === false) {
                throw new Zend_Service_WindowsAzure_Exception(
                        'The permission set can only contain URLs for the account name specified in the Zend_Service_WindowsAzure_Credentials_SharedAccessSignature instance.');
            }
        }
     ...
     * @return string
     */
    public function createSignature ($path = '/', $resource = 'b', $permissions = 'r', 
            $start = '', $expiry = '', $identifier = '')
    {
        ...
        $signature = base64_encode(
                hash_hmac('sha256', $stringToSign, $this->_accountKey, true));
     ...
     * @return string
     */
    public function createSignedQueryString ($path = '/', $queryString = '', 
            ...
            $identifier = '')
    {
        ...
        $parts[] = 'sig=' .
                 urlencode(
                        $this->createSignature($path, $resource, $permissions, 
                                $start, $expiry, $identifier));
     ...
     */
    public function permissionMatchesRequest ($permissionUrl = '', 
            $requestUrl = '', 
            ...
            $requiredPermission = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ)
    {
        ...
        foreach ($permissionParts as $part) {
            ...
            if ($property == 'sr') {
                $matches = $matches &&
                         (strpbrk($value, $requiredResourceType) !== false);
            }
            
            if ($property == 'sp') {
                $matches = $matches &&
                         (strpbrk($value, $requiredPermission) !== false);
            }
        }
        
        // Ok, but... does the resource match?
        $matches = $matches &&
                 (strpos($parsedRequestUrl['path'], 
                        $parsedPermissionUrl['path']) !== false);
            ...
            $requiredPermission = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ)
    {
        ...
        foreach ($this->getPermissionSet() as $permittedUrl) {
            if ($this->permissionMatchesRequest($permittedUrl, $requestUrl, 
     ...
     */
    public function signRequestHeaders ($httpVerb = Zend_Http_Client::GET, $path = '/', 
            $queryString = '', $headers = null, $forTableStorage = false, 
            $resourceType = Zend_Service_WindowsAzure_Storage::RESOURCE_UNKNOWN, 
            $requiredPermission = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ, $rawData = null)
    {
        return $headers;
    }
}
