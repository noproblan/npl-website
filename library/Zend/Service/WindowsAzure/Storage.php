...
class Zend_Service_WindowsAzure_Storage
{
            ...
            Zend_Service_WindowsAzure_RetryPolicy_RetryPolicyAbstract $retryPolicy = null)
    {
        ...
        // Using local storage?
        if (! $this->_usePathStyleUri && ($this->_host == self::URL_DEV_BLOB ||
        ...
        if ($this->_credentials === null) {
            $this->_credentials = new Zend_Service_WindowsAzure_Credentials_SharedKey(
                    $this->_accountName, $this->_accountKey, 
     ...
     */
    public function setHttpClientChannel (
            $adapterInstance = 'Zend_Http_Client_Adapter_Proxy')
    {
     ...
     *            Proxy credentials
     */
    public function setProxy ($useProxy = false, $proxyUrl = '', $proxyPort = 80, 
            $proxyCredentials = '')
    {
     ...
     *            instance to use for request signing.
     */
            ...
            $requiredPermission = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ)
    {
        ...
        $requestUrl = $this->_credentials->signRequestUrl(
                $this->getBaseUrl() . $path . $queryString, $resourceType, 
                $requiredPermission);
        $requestHeaders = $this->_credentials->signRequestHeaders($httpVerb, 
                $path, $queryString, $headers, $forTableStorage, $resourceType, 
                $requiredPermission, $rawData);
    ...
    protected function _parseResponse (Zend_Http_Response $response = null)
    {
        ...
        if ($xml !== false) {
            ...
            $namespaces = array_merge($xml->getNamespaces(true), 
    ...
    protected function _generateMetadataHeaders ($metadata = array())
    {
        ...
        foreach ($metadata as $key => $value) {
            if (strpos($value, "\r") !== false || strpos($value, "\n") !== false) {
                throw new Zend_Service_WindowsAzure_Exception(
                        'Metadata cannot contain newline characters.');
            }
            
            if (! self::isValidMetadataName($key)) {
                throw new Zend_Service_WindowsAzure_Exception(
                        'Metadata name does not adhere to metadata naming conventions. See http://msdn.microsoft.com/en-us/library/aa664670(VS.71).aspx for more information.');
            }
    ...
    {
        // Metadata present?
        if ($element !== null && isset($element->Metadata) &&
    ...
    public function isoDate ($timestamp = null)
    {
        ...
        $returnValue = str_replace('+00:00', '.0000000Z', 
                @date('c', $timestamp));
