...
class Zend_Service_WindowsAzure_Storage_Blob extends Zend_Service_WindowsAzure_Storage
{
    ...
    public function __construct (
            $host = Zend_Service_WindowsAzure_Storage::URL_DEV_BLOB, 
            $accountName = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::DEVSTORE_ACCOUNT, 
            $accountKey = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::DEVSTORE_KEY, $usePathStyleUri = false, 
            Zend_Service_WindowsAzure_RetryPolicy_RetryPolicyAbstract $retryPolicy = null)
    {
        ...
        // SharedAccessSignature credentials
        $this->_sharedAccessSignatureCredentials = new Zend_Service_WindowsAzure_Credentials_SharedAccessSignature(
                $accountName, $accountKey, $usePathStyleUri);
    }
            ...
            $snapshotId = null)
    {
        ...
        if (! self::isValidContainerName($containerName)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name does not adhere to container naming conventions. See http://msdn.microsoft.com/en-us/library/dd135715.aspx for more information.');
        }
        if ($blobName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Blob name is not specified.');
        }
    ...
    public function createContainer ($containerName = '', $metadata = array())
    {
        ...
        if (! is_array($metadata)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Meta data should be an array of key and value pairs.');
        }
        
        // Create metadata headers
        $headers = array();
        $headers = array_merge($headers, 
                $this->_generateMetadataHeaders($metadata));
        
        // Perform request
        $response = $this->_performRequest($containerName, '?restype=container', 
                Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
            return new Zend_Service_WindowsAzure_Storage_BlobContainer(
                    $containerName, $response->getHeader('Etag'), 
                    $response->getHeader('Last-modified'), $metadata);
        } else {
            throw new Zend_Service_WindowsAzure_Exception(
     ...
     *            Display only private/blob/container or display signed
     *            identifiers?
     * @return string Acl, to be compared with
            ...
            $signedIdentifiers = false)
    {
        ...
        // Perform request
        $response = $this->_performRequest($containerName, 
                '?restype=container&comp=acl', Zend_Http_Client::GET, array(), false, 
                null, Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
            ...
            $signedIdentifiers = array())
    {
        ...
        if ($acl != self::ACL_PRIVATE && $acl !== null && $acl != '') {
            $headers[Zend_Service_WindowsAzure_Storage::PREFIX_STORAGE_HEADER .
                     'blob-public-access'] = $acl;
        }
        ...
        if (is_array($signedIdentifiers) && count($signedIdentifiers) > 0) {
            ...
            foreach ($signedIdentifiers as $signedIdentifier) {
                ...
                if ($signedIdentifier->Start != '')
                    $policies .= '      <Start>' . $signedIdentifier->Start .
                             '</Start>' . "\r\n";
                if ($signedIdentifier->Expiry != '')
                    $policies .= '      <Expiry>' . $signedIdentifier->Expiry .
                             '</Expiry>' . "\r\n";
                if ($signedIdentifier->Permissions != '')
                    $policies .= '      <Permission>' .
                             $signedIdentifier->Permissions . '</Permission>' .
                             "\r\n";
                $policies .= '    </AccessPolicy>' . "\r\n";
                $policies .= '  </SignedIdentifier>' . "\r\n";
            }
        ...
        // Perform request
        $response = $this->_performRequest($containerName, 
                '?restype=container&comp=acl', Zend_Http_Client::PUT, $headers, 
                false, $policies, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
    ...
    public function getContainer ($containerName = '')
    {
        ...
        // Perform request
        $response = $this->_performRequest($containerName, '?restype=container', 
                Zend_Http_Client::GET, array(), false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ);
        if ($response->isSuccessful()) {
            // Parse metadata
     ...
     * metadata that is associated with the container. It's not possible to
     * modify an individual name/value pair.
     *
     * @param string $containerName
     *            Container name
     * @param array $metadata
     ...
     *            Additional headers. See
     *            http://msdn.microsoft.com/en-us/library/dd179371.aspx for more
     *            information.
     * @throws Zend_Service_WindowsAzure_Exception
     */
            ...
            $additionalHeaders = array())
    {
        ...
        // Perform request
        $response = $this->_performRequest($containerName, 
                '?restype=container&comp=metadata', Zend_Http_Client::PUT, 
                $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            ...
            $additionalHeaders = array())
    {
        ...
        // Perform request
        $response = $this->_performRequest($containerName, '?restype=container', 
                Zend_Http_Client::DELETE, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
     ...
     *
     * @param string $prefix
     *            Optional. Filters the results to return only containers whose
     *            name begins with the specified prefix.
     * @param int $maxResults
     *            Optional. Specifies the maximum number of containers to return
     *            per call to Azure storage. This does NOT affect list size
     *            returned by this function. (maximum: 5000)
     * @param string $marker
     *            Optional string value that identifies the portion of the list
     *            to be returned with the next list operation.
     * @param string $include
     *            Optional. Include this parameter to specify that the
     *            container's metadata be returned as part of the response body.
    ...
    public function listContainers ($prefix = null, $maxResults = null, $marker = null, 
            $include = null, $currentResultCount = 0)
    {
        // Build query string
        $queryString = array(
                'comp=list'
        ...
        // Perform request
        $response = $this->_performRequest('', $queryString, 
                Zend_Http_Client::GET, array(), false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_CONTAINER, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_LIST);
        if ($response->isSuccessful()) {
            ...
            if ($maxResults !== null && $currentResultCount < $maxResults) {
                if ($xmlMarker !== null && $xmlMarker != '') {
                    $containers = array_merge($containers, 
                            $this->listContainers($prefix, $maxResults, 
                                    $xmlMarker, $include, $currentResultCount));
                }
            }
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function putBlob ($containerName = '', $blobName = '', 
            $localFileName = '', $metadata = array(), $leaseId = null, 
            $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        if ($localFileName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Local file name is not specified.');
        }
        ...
        if (filesize($localFileName) >= self::MAX_BLOB_SIZE) {
            return $this->putLargeBlob($containerName, $blobName, 
                    $localFileName, $metadata, $leaseId);
        }
        
        // Put the data to Windows Azure Storage
        return $this->putBlobData($containerName, $blobName, 
                file_get_contents($localFileName), $metadata, $leaseId, 
                $additionalHeaders);
    }
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function putBlobData ($containerName = '', $blobName = '', $data = '', 
            $metadata = array(), $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        // Specify blob type
        $headers[Zend_Service_WindowsAzure_Storage::PREFIX_STORAGE_HEADER .
                 'blob-type'] = self::BLOBTYPE_BLOCK;
        
        // Resource name
        $resourceName = self::createResourceName($containerName, $blobName);
        
        // Perform request
        $response = $this->_performRequest($resourceName, '', 
                Zend_Http_Client::PUT, $headers, false, $data, 
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function putLargeBlob ($containerName = '', $blobName = '', 
            $localFileName = '', $metadata = array(), $leaseId = null)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        // Check file size
        if (filesize($localFileName) < self::MAX_BLOB_SIZE) {
            return $this->putBlob($containerName, $blobName, $localFileName, 
                    $metadata);
        }
        
        // Determine number of parts
        $numberOfParts = ceil(
                filesize($localFileName) / self::MAX_BLOB_TRANSFER_SIZE);
        ...
        for ($i = 0; $i < $numberOfParts; $i ++) {
            ...
            // Put block
            $this->putBlock($containerName, $blobName, $blockIdentifiers[$i], 
                    $fileContents, $leaseId);
        ...
        // Put block list
        $this->putBlockList($containerName, $blobName, $blockIdentifiers, 
                $metadata, $leaseId);
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function putBlock ($containerName = '', $blobName = '', $identifier = '', 
            $contents = '', $leaseId = null)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        if ($identifier === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Block identifier is not specified.');
        }
        ...
        // Upload
        $response = $this->_performRequest($resourceName, 
                '?comp=block&blockid=' . base64_encode($identifier), 
                Zend_Http_Client::PUT, $headers, false, $contents, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
    ...
    public function putBlockList ($containerName = '', $blobName = '', 
            $blockList = array(), $metadata = array(), $leaseId = null, 
            $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        if (count($blockList) == 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Block list does not contain any elements.');
        }
        if ($containerName === '$root' && strpos($blobName, '/') !== false) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Blobs stored in the root container can not have a name containing a forward slash (/).');
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, '?comp=blocklist', 
                Zend_Http_Client::PUT, $headers, false, $fileContents, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
     ...
     *            Lease identifier
     * @param integer $type
     *            Type of block list to retrieve. 0 = all, 1 = committed, 2 =
     *            uncommitted
     * @return array
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function getBlockList ($containerName = '', $blobName = '', 
            $snapshotId = null, $leaseId = null, $type = 0)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        if ($type < 0 || $type > 2) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Invalid type of block list to retrieve.');
        }
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, $queryString, 
                Zend_Http_Client::GET, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ);
        if ($response->isSuccessful()) {
            // Parse response
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function createPageBlob ($containerName = '', $blobName = '', $size = 0, 
            $metadata = array(), $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        if ($size <= 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Page blob size must be specified.');
        }
        ...
        $headers[Zend_Service_WindowsAzure_Storage::PREFIX_STORAGE_HEADER .
                 'blob-type'] = self::BLOBTYPE_PAGE;
        $headers[Zend_Service_WindowsAzure_Storage::PREFIX_STORAGE_HEADER .
                 'blob-content-length'] = $size;
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, '', 
                Zend_Http_Client::PUT, $headers, false, '', 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
            return new Zend_Service_WindowsAzure_Storage_BlobInstance(
     ...
     *            Write method
     *            (Zend_Service_WindowsAzure_Storage_Blob::PAGE_WRITE_*)
     * @param string $leaseId
     *            Lease identifier
     * @param array $additionalHeaders
     *            Additional headers. See
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function putPage ($containerName = '', $blobName = '', 
            $startByteOffset = 0, $endByteOffset = 0, $contents = '', 
            $writeMethod = self::PAGE_WRITE_UPDATE, $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        if ($startByteOffset % 512 != 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Start byte offset must be a modulus of 512.');
        }
        if (($endByteOffset + 1) % 512 != 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'End byte offset must be a modulus of 512 minus 1.');
        }
        ...
        if ($size >= self::MAX_BLOB_TRANSFER_SIZE) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Page blob size must not be larger than ' +
                             self::MAX_BLOB_TRANSFER_SIZE . ' bytes.');
        }
        ...
        // Write method
        $headers[Zend_Service_WindowsAzure_Storage::PREFIX_STORAGE_HEADER .
                 'page-write'] = $writeMethod;
        
        // Resource name
        $resourceName = self::createResourceName($containerName, $blobName);
        
        // Perform request
        $response = $this->_performRequest($resourceName, '?comp=page', 
                Zend_Http_Client::PUT, $headers, false, $contents, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function getPageRegions ($containerName = '', $blobName = '', 
            $startByteOffset = 0, $endByteOffset = 0, $leaseId = null)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, '?comp=pagelist', 
                Zend_Http_Client::GET, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
            ...
            for ($i = 0; $i < count($xmlRanges); $i ++) {
                $ranges[] = new Zend_Service_WindowsAzure_Storage_PageRegionInstance(
                        (int) $xmlRanges[$i]->Start, (int) $xmlRanges[$i]->End);
            }
     ...
     * @param array $additionalHeaders
     *            Additional headers. See
     *            http://msdn.microsoft.com/en-us/library/dd894037.aspx for more
     *            information.
     * @return object Partial blob properties
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function copyBlob ($sourceContainerName = '', $sourceBlobName = '', 
            $destinationContainerName = '', $destinationBlobName = '', 
            $metadata = array(), $sourceSnapshotId = null, $destinationLeaseId = null, 
            $additionalHeaders = array())
    {
        if ($sourceContainerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Source container name is not specified.');
        }
        if (! self::isValidContainerName($sourceContainerName)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Source container name does not adhere to container naming conventions. See http://msdn.microsoft.com/en-us/library/dd135715.aspx for more information.');
        }
        if ($sourceBlobName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Source blob name is not specified.');
        }
        if ($destinationContainerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Destination container name is not specified.');
        }
        if (! self::isValidContainerName($destinationContainerName)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Destination container name does not adhere to container naming conventions. See http://msdn.microsoft.com/en-us/library/dd135715.aspx for more information.');
        }
        if ($destinationBlobName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Destination blob name is not specified.');
        }
        if ($sourceContainerName === '$root' &&
                 strpos($sourceBlobName, '/') !== false) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Blobs stored in the root container can not have a name containing a forward slash (/).');
        }
        if ($destinationContainerName === '$root' &&
                 strpos($destinationBlobName, '/') !== false) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Blobs stored in the root container can not have a name containing a forward slash (/).');
        }
        ...
        // Resource names
        $sourceResourceName = self::createResourceName($sourceContainerName, 
                $sourceBlobName);
        if ($sourceSnapshotId !== null) {
            $sourceResourceName .= '?snapshot=' . $sourceSnapshotId;
        }
        $destinationResourceName = self::createResourceName(
                $destinationContainerName, $destinationBlobName);
        
        // Set source blob
        $headers["x-ms-copy-source"] = '/' . $this->_accountName . '/' .
                 $sourceResourceName;
        
        // Perform request
        $response = $this->_performRequest($destinationResourceName, '', 
                Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
            return new Zend_Service_WindowsAzure_Storage_BlobInstance(
                    $destinationContainerName, $destinationBlobName, null, 
                    $response->getHeader('Etag'), 
                    $response->getHeader('Last-modified'), 
                    $this->getBaseUrl() . '/' . $destinationContainerName . '/' .
                             $destinationBlobName, 0, '', '', '', false, 
                            $metadata);
        } else {
            throw new Zend_Service_WindowsAzure_Exception(
                    $this->_getErrorMessage($response, 
    ...
    public function getBlob ($containerName = '', $blobName = '', 
            $localFileName = '', $snapshotId = null, $leaseId = null, 
            $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        file_put_contents($localFileName, 
                $this->getBlobData($containerName, $blobName, $snapshotId, 
                        $leaseId, $additionalHeaders));
    }
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function getBlobData ($containerName = '', $blobName = '', 
            $snapshotId = null, $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
            ...
            $snapshotId = null, $leaseId = null, $additionalHeaders = array())
    {
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, $queryString, 
                Zend_Http_Client::HEAD, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ);
        if ($response->isSuccessful()) {
            // Parse metadata
            ...
            // Return blob
            return new Zend_Service_WindowsAzure_Storage_BlobInstance(
                    $containerName, $blobName, $snapshotId, 
                    $response->getHeader('Etag'), 
                    $response->getHeader('Last-modified'), 
                    $this->getBaseUrl() . '/' . $containerName . '/' . $blobName, 
                    ...
                    $response->getHeader('x-ms-lease-status'), false, $metadata);
        } else {
            throw new Zend_Service_WindowsAzure_Exception(
                    $this->_getErrorMessage($response, 
                            'Resource could not be accessed.'));
            ...
            $snapshotId = null, $leaseId = null)
    {
        ...
        return $this->getBlobInstance($containerName, $blobName, $snapshotId, 
                $leaseId)->Metadata;
    }
     ...
     * Set blob metadata
     *
     * Calling the Set Blob Metadata operation overwrites all existing metadata
     * that is associated with the blob. It's not possible to modify an
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function setBlobMetadata ($containerName = '', $blobName = '', 
            $metadata = array(), $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        // Perform request
        $response = $this->_performRequest($containerName . '/' . $blobName, 
                '?comp=metadata', Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
     ...
     * Set blob properties
     *
     * All available properties are listed at
     * http://msdn.microsoft.com/en-us/library/ee691966.aspx and should be
     * provided in the $additionalHeaders parameter.
     *
     * @param string $containerName
     *            Container name
     * @param string $blobName
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function setBlobProperties ($containerName = '', $blobName = '', 
            $leaseId = null, $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        if (count($additionalHeaders) == 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'No additional headers are specified.');
        }
        ...
        // Perform request
        $response = $this->_performRequest($containerName . '/' . $blobName, 
                '?comp=properties', Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function snapshotBlob ($containerName = '', $blobName = '', 
            $metadata = array(), $additionalHeaders = array())
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, '?comp=snapshot', 
                Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
     ...
     * @param string $leaseId
     *            Lease identifier, required to renew the lease or to release
     *            the lease.
     * @return Zend_Service_WindowsAzure_Storage_LeaseInstance Lease instance
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function leaseBlob ($containerName = '', $blobName = '', 
            $leaseAction = self::LEASE_ACQUIRE, $leaseId = null)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        // Perform request
        $response = $this->_performRequest($resourceName, '?comp=lease', 
                Zend_Http_Client::PUT, $headers, false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_WRITE);
        if ($response->isSuccessful()) {
     ...
     *            Container name
     * @param string $prefix
     *            Optional. Filters the results to return only blobs whose name
     *            begins with the specified prefix.
     * @param string $delimiter
     *            Optional. Delimiter, i.e. '/', for specifying folder hierarchy
     * @param int $maxResults
     *            Optional. Specifies the maximum number of blobs to return per
     *            call to Azure storage. This does NOT affect list size returned
     *            by this function. (maximum: 5000)
     * @param string $marker
     *            Optional string value that identifies the portion of the list
     *            to be returned with the next list operation.
     * @param string $include
     *            Optional. Specifies that the response should include one or
     *            more of the following subsets: '', 'metadata', 'snapshots',
     *            'uncommittedblobs'). Multiple values can be added separated
     *            with a comma (,)
     * @param int $currentResultCount
     *            Current result count (internal use)
     * @return array
    ...
    public function listBlobs ($containerName = '', $prefix = '', $delimiter = '', 
            $maxResults = null, $marker = null, $include = null, $currentResultCount = 0)
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Container name is not specified.');
        ...
        // Perform request
        $response = $this->_performRequest($containerName, $queryString, 
                Zend_Http_Client::GET, array(), false, null, 
                Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB, 
                Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_LIST);
        if ($response->isSuccessful()) {
            ...
            if ($xmlBlobs !== null) {
                for ($i = 0; $i < count($xmlBlobs); $i ++) {
                    ...
                    $blobs[] = new Zend_Service_WindowsAzure_Storage_BlobInstance(
                            $containerName, (string) $xmlBlobs[$i]->Name, 
                            ...
                            (string) $properties['LeaseStatus'], false, 
            ...
            if ($xmlBlobs !== null) {
                for ($i = 0; $i < count($xmlBlobs); $i ++) {
                    $blobs[] = new Zend_Service_WindowsAzure_Storage_BlobInstance(
                            $containerName, (string) $xmlBlobs[$i]->Name, null, 
                            '', '', '', 0, '', '', '', '', '', '', true, 
                            $this->_parseMetadataElement($xmlBlobs[$i]));
                }
            }
            ...
            if ($maxResults !== null && $currentResultCount < $maxResults) {
                if ($xmlMarker !== null && $xmlMarker != '') {
                    $blobs = array_merge($blobs, 
                            $this->listBlobs($containerName, $prefix, 
                                    $delimiter, $maxResults, $marker, $include, 
                                    $currentResultCount));
                }
            }
    ...
    public function generateSharedAccessUrl ($containerName = '', $blobName = '', 
            $resource = 'b', $permissions = 'r', $start = '', $expiry = '', 
            $identifier = '')
    {
        if ($containerName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        // Generate URL
        return $this->getBaseUrl() . '/' . $resourceName . '?' . $this->_sharedAccessSignatureCredentials->createSignedQueryString(
                $resourceName, '', $resource, $permissions, $start, $expiry, 
                $identifier);
    }
     ...
     * @return string
     */
    protected function _getErrorMessage (Zend_Http_Response $response, 
