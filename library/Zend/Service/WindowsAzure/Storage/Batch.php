...
class Zend_Service_WindowsAzure_Storage_Batch
{
    ...
    public function __construct (
            Zend_Service_WindowsAzure_Storage_BatchStorageAbstract $storageClient = null, 
            $baseUrl = '')
    {
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function enlistOperation ($path = '/', $queryString = '', 
            $httpVerb = Zend_Http_Client::GET, $headers = array(), $forTableStorage = false, $rawData = null)
    {
        ...
        if ($httpVerb == Zend_Http_Client::GET) {
            if (count($this->_operations) > 0) {
                throw new Zend_Service_WindowsAzure_Exception(
                        "Select operations can only be performed in an empty batch transaction.");
            }
        ...
        $queryString = Zend_Service_WindowsAzure_Storage::urlencode(
                $queryString);
    ...
    {
        // Perform batch
        $response = $this->_storageClient->performBatch($this->_operations, 
                $this->_forTableStorage, $this->_isSingleSelect);
        ...
        // Parse response
        $errors = null;
        preg_match_all('/<message (.*)>(.*)<\/message>/', $response->getBody(), 
                $errors);
        
        // Error?
        if (count($errors[2]) > 0) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'An error has occured while committing a batch: ' .
                             $errors[2][0]);
        }
