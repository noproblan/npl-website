...
class Zend_Service_WindowsAzure_Storage_Queue extends Zend_Service_WindowsAzure_Storage
{
    ...
    public function __construct (
            $host = Zend_Service_WindowsAzure_Storage::URL_DEV_QUEUE, 
    ...
    public function queueExists ($queueName = '')
    {
        ...
        if (! self::isValidQueueName($queueName)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Queue name does not adhere to queue naming conventions. See http://msdn.microsoft.com/en-us/library/dd179349.aspx for more information.');
        }
    ...
    public function createQueue ($queueName = '', $metadata = array())
    {
        ...
        // Perform request
        $response = $this->_performRequest($queueName, '', 
                Zend_Http_Client::PUT, $headers);
    ...
    public function getQueue ($queueName = '')
    {
        ...
        // Perform request
        $response = $this->_performRequest($queueName, '?comp=metadata', 
        ...
        if ($response->isSuccessful()) {
            ...
            $queue = new Zend_Service_WindowsAzure_Storage_QueueInstance(
                    $queueName, $metadata);
            $queue->ApproximateMessageCount = intval(
                    $response->getHeader('x-ms-approximate-message-count'));
            return $queue;
        } else {
            throw new Zend_Service_WindowsAzure_Exception(
                    $this->_getErrorMessage($response, 
     ...
     * Set queue metadata
     *
     * Calling the Set Queue Metadata operation overwrites all existing metadata
     * that is associated with the queue. It's not possible to modify an
     ...
     *
     * @param string $prefix
     *            Optional. Filters the results to return only queues whose name
     *            begins with the specified prefix.
     * @param int $maxResults
     *            Optional. Specifies the maximum number of queues to return per
     ...
     *            Optional. Include this parameter to specify that the queue's
     *            metadata be returned as part of the response body. (allowed
     *            values: '', 'metadata')
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function listQueues ($prefix = null, $maxResults = null, $marker = null, 
            ...
            if ($maxResults !== null && $currentResultCount < $maxResults) {
                if ($xmlMarker !== null && $xmlMarker != '') {
                    $queues = array_merge($queues, 
                            $this->listQueues($prefix, $maxResults, $xmlMarker, 
                                    $include, $currentResultCount));
                }
            }
     ...
     *            Message
     * @param int $ttl
     *            Message Time-To-Live (in seconds). Defaults to 7 days if the
    ...
    public function putMessage ($queueName = '', $message = '', $ttl = null)
    {
        ...
        if (strlen($message) > self::MAX_MESSAGE_SIZE) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Message is too big. Message content should be < 8KB.');
        }
        if ($message == '') {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        if ($ttl !== null && ($ttl <= 0 || $ttl > self::MAX_MESSAGE_SIZE)) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Message TTL is invalid. Maximal TTL is 7 days (' .
        ...
        $rawData .= '    <MessageText>' . base64_encode($message) .
                 '</MessageText>';
        $rawData .= '</QueueMessage>';
        
        // Perform request
        $response = $this->_performRequest($queueName . '/messages', 
                $queryString, Zend_Http_Client::POST, array(), false, $rawData);
        
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Error putting message into queue.');
        }
    }
     ...
     *            messages to retrieve from the queue, up to a maximum of 32. By
     *            default, a single message is retrieved from the queue with
     *            this operation.
     * @param int $visibilityTimeout
     *            Optional. An integer value that specifies the message's
     *            visibility timeout in seconds. The maximum value is 2 hours.
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function getMessages ($queueName = '', $numOfMessages = 1, 
            $visibilityTimeout = null, $peek = false)
    {
        if ($queueName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Queue name is not specified.');
        ...
        if ($numOfMessages < 1 || $numOfMessages > 32 ||
                 intval($numOfMessages) != $numOfMessages) {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Invalid number of messages to retrieve.');
        }
        if ($visibilityTimeout !== null &&
                 ($visibilityTimeout <= 0 || $visibilityTimeout > 7200)) {
            throw new Zend_Service_WindowsAzure_Exception(
        ...
        // Perform request
        $response = $this->_performRequest($queueName . '/messages', 
                $queryString, Zend_Http_Client::GET);
        if ($response->isSuccessful()) {
            // Parse results
     ...
     *            Message to delete from queue. A message retrieved using
     *            "peekMessages" can NOT be deleted!
     * @throws Zend_Service_WindowsAzure_Exception
     */
            ...
            Zend_Service_WindowsAzure_Storage_QueueMessage $message)
    {
        ...
        if ($message->PopReceipt == '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'A message retrieved using "peekMessages" can NOT be deleted! Use "getMessages" instead.');
        }
        
        // Perform request
        $response = $this->_performRequest(
                $queueName . '/messages/' . $message->MessageId, 
                '?popreceipt=' . $message->PopReceipt, Zend_Http_Client::DELETE);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
                    $this->_getErrorMessage($response, 
                            'Resource could not be accessed.'));
