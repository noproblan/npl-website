...
class Zend_Service_WindowsAzure_Storage_TableEntityQuery
{
     ...
     *            Condition, can contain question mark(s) (?) for parameter
     *            insertion.
    ...
    public function assembleQueryString ($urlEncode = false)
    {
        ...
        if (count($this->_where) != 0) {
            ...
            $query[] = '$filter=' .
                     ($urlEncode ? self::encodeQuery($filter) : $filter);
        }
        
        if (count($this->_orderBy) != 0) {
            $orderBy = implode(',', $this->_orderBy);
            $query[] = '$orderby=' .
                     ($urlEncode ? self::encodeQuery($orderBy) : $orderBy);
        }
        ...
        if (! is_array($value)) {
            ...
            while (strpos($text, '?') !== false) {
                if (is_numeric($value[$i])) {
                    $text = substr_replace($text, $value[$i ++], 
                            strpos($text, '?'), 1);
                } else {
