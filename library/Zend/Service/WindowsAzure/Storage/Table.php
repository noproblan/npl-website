...
class Zend_Service_WindowsAzure_Storage_Table extends Zend_Service_WindowsAzure_Storage_BatchStorageAbstract
{
    ...
    public function __construct (
            $host = Zend_Service_WindowsAzure_Storage::URL_DEV_TABLE, 
            ...
            Zend_Service_WindowsAzure_RetryPolicy_RetryPolicyAbstract $retryPolicy = null)
    {
        ...
        $this->_credentials = new Zend_Service_WindowsAzure_Credentials_SharedKeyLite(
                $accountName, $accountKey, $this->_usePathStyleUri);
    ...
    public function tableExists ($tableName = '')
    {
        ...
        // List tables
        $tables = $this->listTables(); // 2009-09-19 does not support
                                       // $this->listTables($tableName); all of
                                       // a sudden...
     ...
     *            Next table name, used for listing tables when total amount of
     *            tables is > 1000.
     * @return array
    ...
    public function listTables ($nextTableName = '')
    {
        ...
        // Perform request
        $response = $this->_performRequest('Tables', $queryString, 
                Zend_Http_Client::GET, null, true);
        if ($response->isSuccessful()) {
            ...
            foreach ($entries as $entry) {
                ...
                $returnValue[] = new Zend_Service_WindowsAzure_Storage_TableInstance(
                        (string) $entry->id, $tableName, 
                        (string) $entry->link['href'], (string) $entry->updated);
            }
    ...
    public function createTable ($tableName = '')
    {
        ...
        // Perform request
        $response = $this->_performRequest('Tables', '', Zend_Http_Client::POST, 
                $headers, true, $requestBody);
        if ($response->isSuccessful()) {
            // Parse response
    ...
    public function deleteTable ($tableName = '')
    {
        ...
        // Perform request
        $response = $this->_performRequest('Tables(\'' . $tableName . '\')', '', 
                Zend_Http_Client::DELETE, $headers, true, null);
        if (! $response->isSuccessful()) {
            throw new Zend_Service_WindowsAzure_Exception(
                    $this->_getErrorMessage($response, 
                            'Resource could not be accessed.'));
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function insertEntity ($tableName = '', 
            Zend_Service_WindowsAzure_Storage_TableEntity $entity = null)
    {
        ...
        $requestBody = $this->_fillTemplate($requestBody, 
                array(
                        'Updated' => $this->isoDate(),
                        'Properties' => $this->_generateAzureRepresentation(
                                $entity)
                ));
        ...
        $response = null;
        if ($this->isInBatch()) {
            $this->getCurrentBatch()->enlistOperation($tableName, '', 
                    Zend_Http_Client::POST, $headers, true, $requestBody);
            return null;
        } else {
            $response = $this->_performRequest($tableName, '', 
                    Zend_Http_Client::POST, $headers, true, $requestBody);
        }
        if ($response->isSuccessful()) {
            // Parse result
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function deleteEntity ($tableName = '', 
            Zend_Service_WindowsAzure_Storage_TableEntity $entity = null, 
            $verifyEtag = false)
    {
        if ($tableName === '') {
        ...
        if ($this->isInBatch()) {
            $this->getCurrentBatch()->enlistOperation(
                    $tableName . '(PartitionKey=\'' . $entity->getPartitionKey() .
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function retrieveEntityById ($tableName = '', $partitionKey = '', 
            $rowKey = '', 
            $entityClass = 'Zend_Service_WindowsAzure_Storage_DynamicTableEntity')
    {
        ...
        if ($rowKey === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Row key is not specified.');
        }
        if ($entityClass === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Entity class is not specified.');
        }
        ...
        if (strlen($partitionKey . $rowKey) >= 256) {
            ...
            if ($this->isInBatch()) {
                throw new Zend_Service_WindowsAzure_Exception(
                        'Entity cannot be retrieved. A transaction is required to retrieve the entity, but another transaction is already active.');
            }
        ...
        $result = $this->retrieveEntities(
                $this->select()
                    ...
                    ->whereRowKey($rowKey), '', $entityClass);
     ...
     *            Table name -or-
     *            Zend_Service_WindowsAzure_Storage_TableEntityQuery instance
     * @param string $filter
     *            Filter condition (not applied when $tableName is a
     *            Zend_Service_WindowsAzure_Storage_TableEntityQuery instance)
     * @param string $entityClass
     *            Entity class name
     * @param string $nextPartitionKey
     *            Next partition key, used for listing entities when total
     *            amount of entities is > 1000.
     * @param string $nextRowKey
     *            Next row key, used for listing entities when total amount of
     *            entities is > 1000.
     * @return array Array of Zend_Service_WindowsAzure_Storage_TableEntity
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function retrieveEntities ($tableName = '', $filter = '', 
            $entityClass = 'Zend_Service_WindowsAzure_Storage_DynamicTableEntity', $nextPartitionKey = null, $nextRowKey = null)
    {
        if ($tableName === '') {
            throw new Zend_Service_WindowsAzure_Exception(
                    'Table name is not specified.');
        ...
        if (is_string($tableName)) {
            ...
            if ($filter !== '') {
                $query[] = '$filter=' .
                         Zend_Service_WindowsAzure_Storage_TableEntityQuery::encodeQuery(
                                $filter);
            }
        ...
        if ($nextPartitionKey !== null && $nextRowKey !== null) {
            ...
            $queryString .= '&NextPartitionKey=' .
                     rawurlencode($nextPartitionKey) . '&NextRowKey=' .
                     rawurlencode($nextRowKey);
        }
        
        // Perform request
        $response = null;
        if ($this->isInBatch() &&
                 $this->getCurrentBatch()->getOperationCount() == 0) {
            $this->getCurrentBatch()->enlistOperation($tableName, $queryString, 
                    Zend_Http_Client::GET, array(), true, null);
            $response = $this->getCurrentBatch()->commit();
            
            // Get inner response (multipart)
            $innerResponse = $response->getBody();
            $innerResponse = substr($innerResponse, 
                    strpos($innerResponse, 'HTTP/1.1 200 OK'));
            $innerResponse = substr($innerResponse, 0, 
                    strpos($innerResponse, '--batchresponse'));
            $response = Zend_Http_Response::fromString($innerResponse);
        } else {
            $response = $this->_performRequest($tableName, $queryString, 
                    Zend_Http_Client::GET, array(), true, null);
        }
        ...
        if ($response->isSuccessful()) {
            ...
            if ($result->entry) {
                ...
                // This one is tricky... If we have properties defined, we have
                // an entity.
            ...
            foreach ($entries as $entry) {
                ...
                // If we have a
                // Zend_Service_WindowsAzure_Storage_DynamicTableEntity, make
                // sure all property types are OK
            ...
            // More entities?
            if ($response->getHeader('x-ms-continuation-NextPartitionKey') !==
                     null &&
                     $response->getHeader('x-ms-continuation-NextRowKey') !==
                     null) {
                if (strpos($queryString, '$top') === false) {
                    $returnValue = array_merge($returnValue, 
                            $this->retrieveEntities($tableName, $filter, 
                                    $entityClass, 
                                    $response->getHeader(
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function updateEntity ($tableName = '', 
            Zend_Service_WindowsAzure_Storage_TableEntity $entity = null, 
            $verifyEtag = false)
    {
        return $this->_changeEntity(Zend_Http_Client::PUT, $tableName, $entity, 
                $verifyEtag);
    }
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    public function mergeEntity ($tableName = '', 
            Zend_Service_WindowsAzure_Storage_TableEntity $entity = null, 
            $verifyEtag = false, $properties = array())
    {
        $mergeEntity = null;
        if (is_array($properties) && count($properties) > 0) {
            // Build a new object
            $mergeEntity = new Zend_Service_WindowsAzure_Storage_DynamicTableEntity(
            ...
            foreach ($azureValues as $key => $value) {
                if (in_array($value->Name, $properties)) {
                    $mergeEntity->setAzureProperty($value->Name, $value->Value, 
                            $value->Type);
                }
            }
        } else {
        ...
        return $this->_changeEntity(Zend_Http_Client::MERGE, $tableName, 
                $mergeEntity, $verifyEtag);
    }
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    protected function _changeEntity ($httpVerb = Zend_Http_Client::PUT, $tableName = '', 
            Zend_Service_WindowsAzure_Storage_TableEntity $entity = null, 
            $verifyEtag = false)
    {
        if ($tableName === '') {
        ...
        if ($timestamp ==
                 Zend_Service_WindowsAzure_Storage_TableEntity::DEFAULT_TIMESTAMP) {
    ...
    protected function _fillTemplate ($templateText, $variables = array())
    {
        foreach ($variables as $key => $value) {
            $templateText = str_replace('{tpl:' . $key . '}', $value, 
                    $templateText);
        }
        return $templateText;
    }
    ...
    /**
     * Generate Azure representation from entity (creates atompub markup from
     * properties)
     *
     * @param Zend_Service_WindowsAzure_Storage_TableEntity $entity            
     * @return string
     ...
     */
    protected function _performRequest ($path = '/', $queryString = '', 
            $httpVerb = Zend_Http_Client::GET, $headers = array(), $forTableStorage = false, $rawData = null, 
            ...
            $requiredPermission = Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PERMISSION_READ)
    {
        ...
        // Perform request
        return parent::_performRequest($path, $queryString, $httpVerb, $headers, 
