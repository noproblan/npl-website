...
class Zend_Service_Twitter extends Zend_Rest_Client
{
    ...
    /**
     * 246 is the current limit for a status message, 140 characters are
     * displayed
     * initially, with the remainder linked from the web UI or client.
     * The limit is
     * applied to a html encoded UTF-8 string (i.e. entities are counted in the
     * limit
            ...
            Zend_Oauth_Consumer $consumer = null)
    {
        ...
        if (isset($options['accessToken']) &&
            ...
            $this->setLocalHttpClient(
                    $options['accessToken']->getHttpClient($options));
        } else {
    ...
    public function setLocalHttpClient (Zend_Http_Client $client)
    {
        ...
        $this->_localHttpClient->setHeaders('Accept-Charset', 
                'ISO-8859-1,utf-8');
        return $this;
    }
    ...
    public function __call ($method, $params)
    {
        if (method_exists($this->_oauthConsumer, $method)) {
            ...
            if ($return instanceof Zend_Oauth_Token_Access) {
                $this->setLocalHttpClient(
                        $return->getHttpClient($this->_options));
            }
            return $return;
        }
     ...
     * - page: return page X of results
     * - count: how many statuses to return
     * - max_id: returns only statuses with an ID less than or equal to the
     * specified ID
     * - user_id: specifies the ID of the user for whom to return the
     * user_timeline
     * - screen_name: specfies the screen name of the user for whom to return
     * the user_timeline
     * - include_rts: whether or not to return retweets
     * - trim_user: whether to return just the user ID or a full user object;
     * omit to return full object
     * - include_entities: whether or not to return entities nodes with tweet
     * metadata
     *
     * @throws Zend_Http_Client_Exception if HTTP request fails or times out
     * @return Zend_Rest_Client_Result
     ...
     * @return Zend_Rest_Client_Result
     * @throws Zend_Http_Client_Exception if HTTP request fails or times out
     * @throws Zend_Service_Twitter_Exception if message is too short or too
     *         long
     */
    public function statusUpdate ($status, $inReplyToStatusId = null)
    {
        ...
        $len = iconv_strlen(htmlspecialchars($status, ENT_QUOTES, 'UTF-8'), 
                'UTF-8');
        if ($len > self::STATUS_MAX_CHARACTERS) {
            include_once 'Zend/Service/Twitter/Exception.php';
            throw new Zend_Service_Twitter_Exception(
                    'Status must be no more than ' . self::STATUS_MAX_CHARACTERS .
     ...
     *
     * @param bool $lite
     *            If true, prevents inline inclusion of current status for
    ...
    {
        $this->_init();
        $path = '/1/direct_messages/destroy/' . $this->_validInteger($id) .
                 '.xml';
        $response = $this->_post($path);
        return new Zend_Rest_Client_Result($response->getBody());
    }
     ...
     *            Instead of returning a boolean return the rest response from
     *            twitter
     ...
     *
     * @param integer $page
     *            Optional. Specifies the page number of the results beginning
     *            at 1. A single page contains 20 ids.
     * @param boolean $returnUserIds
     *            Optional. Returns only the userid's instead of the whole user
     *            object
     * @return Zend_Rest_Client_Result
     */
    ...
    /**
     * Call a remote REST web service URI and return the Zend_Http_Response
     * object
     *
     * @param string $path
