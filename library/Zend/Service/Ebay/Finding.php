...
class Zend_Service_Ebay_Finding extends Zend_Service_Ebay_Abstract
{
     ...
     * @throws Zend_Service_Ebay_Finding_Exception When application id is
     *         missing
     * @return void
     */
     ...
     */
    public function findItemsAdvanced ($keywords, $descriptionSearch = true, 
            $categoryId = null, $options = null)
    {
        // prepare options
        $options = parent::optionsToArray($options);
     ...
     * @return Zend_Service_Ebay_Finding_Response_Items
     */
    public function findItemsByProduct ($productId, $productIdType = null, 
            $options = null)
    {
    ...
    protected function _findItems (array $options, $operation)
    {
        ...
        $response = new Zend_Service_Ebay_Finding_Response_Items(
                $dom->firstChild);
    ...
    public function getHistograms ($categoryId, $options = null)
    {
        ...
        $response = new Zend_Service_Ebay_Finding_Response_Histograms(
                $dom->firstChild);
        return $response->setOperation($operation)->setOption($options);
    }
    ...
    public function getSearchKeywordsRecommendation ($keywords, $options = null)
    {
        ...
        $response = new Zend_Service_Ebay_Finding_Response_Keywords(
                $dom->firstChild);
        return $response->setOperation($operation)->setOption($options);
    }
    ...
    protected function _request ($operation, array $options = null)
    {
        ...
        $response = $client->setUri(self::ENDPOINT_URI)->restGet(
                self::ENDPOINT_PATH, $options);
     ...
     * @throws Zend_Service_Ebay_Finding_Exception When any error occurrs during
     *         request
     * @return DOMDocument
     */
    protected function _parseResponse (Zend_Http_Response $response)
    {
        ...
        if ($response->isError()) {
            $message = $response->getMessage() . ' (HTTP status code #' .
                     $response->getStatus() . ')';
        }
        ...
        if ($severityNode) {
            ...
            // ebay message
            $messageNode = $xpath->query("//$ns:message[1]", $errorNode)->item(
                    0);
            if ($messageNode) {
            ...
            // ebay error id
            $errorIdNode = $xpath->query("//$ns:errorId[1]", $errorNode)->item(
                    0);
            if ($errorIdNode) {
