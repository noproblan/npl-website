...
class Zend_Service_Technorati_CosmosResultSet extends Zend_Service_Technorati_ResultSet
{
    ...
    public function __construct (DomDocument $dom, $options = array())
    {
        ...
        $result = $this->_xpath->query(
                '/tapi/document/result/inboundlinks/text()');
        if ($result->length == 1)
            $this->_inboundLinks = (int) $result->item(0)->data;
        
        $result = $this->_xpath->query(
                '/tapi/document/result/inboundblogs/text()');
        if ($result->length == 1)
        ...
        if ($result->length == 1) {
            try {
                ...
                $this->_url = Zend_Service_Technorati_Utils::normalizeUriHttp(
                        $result->item(0)->data);
    ...
    public function current ()
    {
        ...
        return new Zend_Service_Technorati_CosmosResult(
