...
class Zend_Service_Yahoo
{
     ...
     * 'omit_inlinks' => (none|domain|subdomain) Filter inlinks from these
     * sources
     *
     * @param string $query
     *            the query being run
    ...
    public function inlinkDataSearch ($query, array $options = array())
    {
        ...
        $response = $this->_rest->restGet('/SiteExplorerService/V1/inlinkData', 
                $options);
     ...
     * 'results' => int How many results to return, max is 100
     * 'start' => int The start offset for search results
     * 'domain_only' => bool Data for just the given domain or all sub-domains
     * also
     *
     * @param string $query
     *            the query being run
    ...
    public function pageDataSearch ($query, array $options = array())
    {
        ...
        $this->_rest->getHttpClient()->resetParameters();
        $this->_rest->setUri('http://search.yahooapis.com');
        $response = $this->_rest->restGet('/SiteExplorerService/V1/pageData', 
                $options);
     ...
     * 'results' => int How many results to return, max is 50
     * 'start' => int The start offset for search results
     * 'format' => (any|avi|flash|mpeg|msmedia|quicktime|realmedia) The type of
    ...
    public function videoSearch ($query, array $options = array())
    {
        ...
        $this->_rest->getHttpClient()->resetParameters();
        $this->_rest->setUri('http://search.yahooapis.com');
        $response = $this->_rest->restGet('/VideoSearchService/V1/videoSearch', 
                $options);
     ...
     * 'license' => (any|cc_any|cc_commercial|cc_modifiable) The license of
     * content being searched
     * 'region' => The regional search engine on which the service performs the
     * search. default us.
     *
     * @param string $query
     *            the query being run
    ...
    public function webSearch ($query, array $options = array())
    {
        ...
        $this->_rest->getHttpClient()->resetParameters();
        $this->_rest->setUri('http://search.yahooapis.com');
        $response = $this->_rest->restGet('/WebSearchService/V1/webSearch', 
                $options);
    ...
    protected function _validateInlinkDataSearch (array $options)
    {
        ...
        if (isset($options['results']) &&
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'results': {$options['results']}");
        }
        
        if (isset($options['start']) &&
                 ! $between->setMin(1)
                    ->setMax(1000)
                    ->isValid($options['start'])) {
            /**
             *
             * @see Zend_Service_Exception
             */
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'start': {$options['start']}");
        }
        
        if (isset($options['omit_inlinks'])) {
            $this->_validateInArray('omit_inlinks', $options['omit_inlinks'], 
                    array(
                            'none',
                            'domain',
                            'subdomain'
        ...
        if (isset($options['type'])) {
            switch ($options['type']) {
                ...
                default:
                    ...
                    require_once 'Zend/Service/Exception.php';
                    throw new Zend_Service_Exception(
                            "Invalid value for option 'type': '{$options['type']}'");
            }
        }
        ...
        if (isset($options['format'])) {
            switch ($options['format']) {
                ...
                default:
                    ...
                    require_once 'Zend/Service/Exception.php';
                    throw new Zend_Service_Exception(
                            "Invalid value for option 'format': {$options['format']}");
            }
        }
        ...
        if (isset($options['coloration'])) {
            switch ($options['coloration']) {
                ...
                default:
                    ...
                    require_once 'Zend/Service/Exception.php';
                    throw new Zend_Service_Exception(
                            "Invalid value for option 'coloration': " .
                                     "{$options['coloration']}");
            }
        }
    }
    ...
    protected function _validateLocalSearch (array $options)
    {
        ...
        if (isset($options['longitude']) &&
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'longitude': {$options['longitude']}");
        }
        
        if (isset($options['latitude']) &&
                 ! $between->setMin(- 180)
                    ->setMax(180)
                    ->isValid($options['latitude'])) {
            /**
             *
             * @see Zend_Service_Exception
             */
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'latitude': {$options['latitude']}");
        }
        
        if (isset($options['zip']) &&
                 ! preg_match('/(^\d{5}$)|(^\d{5}-\d{4}$)/', $options['zip'])) {
            /**
             *
             * @see Zend_Service_Exception
             */
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'zip': {$options['zip']}");
        }
        ...
        if (! $hasLocation &&
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    'Location data are required but missing');
        }
        
        if (! in_array($options['sort'], 
                array(
                        'relevance',
                        'title',
                        'distance',
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "Invalid value for option 'sort': {$options['sort']}");
        }
    }
    ...
    protected function _validateNewsSearch (array $options)
    {
        ...
        $this->_validateInArray('type', $options['type'], 
                array(
                        'all',
                        'any',
                        'phrase'
    ...
    protected function _validateVideoSearch (array $options)
    {
        ...
        if (isset($options['format'])) {
            $this->_validateInArray('format', $options['format'], 
                    array(
                            'any',
                            'avi',
                            'flash',
    ...
    protected function _validateWebSearch (array $options)
    {
        ...
        if (isset($options['license'])) {
            $this->_validateInArray('license', $options['license'], 
                    array(
                            'any',
                            'cc_any',
                            'cc_commercial',
        ...
        if (isset($options['region'])) {
            $this->_validateInArray('region', $options['region'], 
                    array(
                            'ar',
                            'au',
                            'at',
     ...
     * @return array
     */
    protected function _prepareOptions ($query, array $options, 
    ...
    protected function _validateLanguage ($lang)
    {
        ...
        if (! in_array($lang, $languages)) {
            ...
            require_once 'Zend/Service/Exception.php';
            throw new Zend_Service_Exception(
                    "The selected language '$lang' is not supported");
        }
    }
     ...
     *            Valid options
     * @return void
     * @throws Zend_Service_Exception if difference is found (e.g., unsupported
     *         query option)
     */
     ...
     * @return void
     * @throws Zend_Service_Exception Thrown when the result from Yahoo! is an
     *         error
     */
    protected static function _checkErrors (DOMDocument $dom)
    {
        ...
        if ($xpath->query('//yapi:Error')->length >= 1) {
            $message = $xpath->query('//yapi:Error/yapi:Message/text()')->item(
                    0)->data;
            /**
             *
             * @see Zend_Service_Exception
