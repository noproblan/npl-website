...
class Zend_Service_Amazon
{
    ...
    public function itemSearch (array $options)
    {
        ...
        $options = $this->_prepareOptions('ItemSearch', $options, 
                $defaultOptions);
    ...
    public function itemLookup ($asin, array $options = array())
    {
        ...
        $options = $this->_prepareOptions('ItemLookup', $options, 
                $defaultOptions);
        $response = $client->restGet('/onca/xml', $options);
            ...
            array $defaultOptions)
    {
        ...
        if ($this->_secretKey !== null) {
            ...
            ;
            ksort($options);
            $options['Signature'] = self::computeSignature($this->_baseUri, 
    ...
    /**
     * Compute Signature for Authentication with Amazon Product Advertising
     * Webservices
     *
     ...
     * @return string
     */
    static public function computeSignature ($baseUri, $secretKey, 
            array $options)
    {
        ...
        return base64_encode(
                Zend_Crypt_Hmac::compute($secretKey, 'sha256', $signature, 
    ...
    static public function buildRawSignature ($baseUri, $options)
    {
        ...
        return sprintf("GET\n%s\n/onca/xml\n%s", 
                str_replace('http://', '', $baseUri), implode("&", $params));
    }
