...
/**
 * Zend_Service_Delicious is a concrete implementation of the del.icio.us web
 * service
 *
 * @category Zend
 * @package Zend_Service
...
class Zend_Service_Delicious
{
    ...
    public function getLastUpdate ()
    {
        ...
        if ($rootNode && $rootNode->nodeName == 'update') {
            ...
            throw new Zend_Service_Delicious_Exception(
                    'del.icio.us web service has returned something odd!');
        }
    }

    /**
     * Get all tags, returning an array with tags as keys and number of
     * corresponding posts as values
     *
    ...
    public function getTags ()
    {
        ...
        return self::_xmlResponseToArray($response, 'tags', 'tag', 'tag', 
                'count');
    }
    ...
    /**
     * Get all bundles, returning an array with bundles as keys and array of
     * tags as values
     *
    ...
    public function getBundles ()
    {
        ...
        $bundles = self::_xmlResponseToArray($response, 'bundles', 'bundle', 
                'name', 'tags');
    ...
    public function addBundle ($bundle, array $tags)
    {
        ...
        $response = $this->makeRequest(self::PATH_BUNDLE_ADD, 
                array(
                        'bundle' => $bundle,
                        'tags' => $tags
                ));
    ...
    {
        $response = $this->makeRequest(self::PATH_BUNDLE_DELETE, 
                array(
                        'bundle' => $bundle
                ));
    ...
    {
        $response = $this->makeRequest(self::PATH_POST_DELETE, 
                array(
                        'url' => $url
                ));
    ...
    public function getDates ($tag = null)
    {
        ...
        return self::_xmlResponseToArray($response, 'dates', 'date', 'date', 
                'count');
    }
    ...
    {
        return new Zend_Service_Delicious_Post($this, 
                array(
                        'title' => $title,
                        'url' => $url
                ));
     ...
     *            Owner of the posts
     * @param int $atleast
     *            Include only tags for which there are at least ### number of
     *            posts
     * @param int $count
    ...
    public function makeRequest ($path, array $parms = array(), $type = 'xml')
    {
        ...
        $this->_rest->getHttpClient()->setAuth($this->_authUname, 
                $this->_authPass);
     ...
     * @throws Zend_Service_Delicious_Exception
     */
    private static function _xmlResponseToArray (DOMDocument $response, $root, 
            $child, $attKey, $attValue)
    {
        $rootNode = $response->documentElement;
        $arrOut = array();
        ...
        if ($rootNode->nodeName == $root) {
            ...
            for ($i = 0; $i < $childNodes->length; $i ++) {
                ...
                if ($currentNode->nodeName == $child) {
                    $arrOut[$currentNode->getAttribute($attKey)] = $currentNode->getAttribute(
                            $attValue);
                }
            }
        } else {
    ...
    private function _parseXmlPostList (DOMDocument $response)
    {
        ...
        if ($rootNode->nodeName == 'posts') {
            return new Zend_Service_Delicious_PostList($this, 
                    $rootNode->childNodes);
        } else {
            /**
             *
    ...
    private static function _evalXmlResult (DOMDocument $response)
    {
        ...
        if ($rootNode && $rootNode->nodeName == 'result') {
            ...
            if ($strResponse != 'done' && $strResponse != 'ok') {
                ...
                require_once 'Zend/Service/Delicious/Exception.php';
                throw new Zend_Service_Delicious_Exception(
                        "del.icio.us web service: '{$strResponse}'");
            }
        } else {
            /**
             *
