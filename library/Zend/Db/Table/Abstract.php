...
abstract class Zend_Db_Table_Abstract
{
     ...
     * @return Zend_Db_Table_Abstract
     */
    public function addReference ($ruleKey, $columns, $refTableClass, 
            $refColumns, $onDelete = null, $onUpdate = null)
    {
        $reference = array(
    ...
    public function getReference ($tableClassname, $ruleKey = null)
    {
        ...
        if ($ruleKey !== null) {
            ...
            if ($refMap[$ruleKey][self::REF_TABLE_CLASS] != $tableClassname) {
                require_once "Zend/Db/Table/Exception.php";
                throw new Zend_Db_Table_Exception(
                        "Reference rule \"$ruleKey\" does not reference table $tableClassname");
            }
        ...
        require_once "Zend/Db/Table/Exception.php";
        throw new Zend_Db_Table_Exception(
                "No reference from table $thisClass to table $tableClassname");
    }
    ...
    /**
     * Gets the Zend_Db_Adapter_Abstract for this particular Zend_Db_Table
     * object.
     *
    ...
    protected function _setupMetadata ()
    {
        ...
        if (null !== $this->_metadataCache) {
            ...
            $port = isset($dbConfig['options']['port']) ? ':' .
                     $dbConfig['options']['port'] : (isset($dbConfig['port']) ? ':' .
                     $dbConfig['port'] : null);
            
            $host = isset($dbConfig['options']['host']) ? ':' .
                     $dbConfig['options']['host'] : (isset($dbConfig['host']) ? ':' .
                     $dbConfig['host'] : null);
            
            // Define the cache identifier where the metadata are saved
            $cacheId = md5( // port:host/dbname:schema.table (based on
                            // availabilty)
        ...
        if (null === $this->_metadataCache ||
                 ! ($metadata = $this->_metadataCache->load($cacheId))) {
            ...
            if (null !== $this->_metadataCache &&
                ...
                trigger_error('Failed saving metadata to metadataCache', 
                        E_USER_NOTICE);
            }
        }
    ...
    protected function _setupPrimaryKey ()
    {
        ...
        $cols = $this->_getCols();
        if (! array_intersect((array) $this->_primary, $cols) ==
                 (array) $this->_primary) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new Zend_Db_Table_Exception(
        ...
        if ($this->_sequence === true &&
            ...
            $this->_sequence = $this->_db->quoteIdentifier(
                    "{$this->_name}_{$pkIdentity}_seq");
            if ($this->_schema) {
                $this->_sequence = $this->_db->quoteIdentifier($this->_schema) .
                         '.' . $this->_sequence;
            }
        }
    }
    ...
    public function select ($withFromPart = self::SELECT_WITHOUT_FROM_PART)
    {
        ...
        if ($withFromPart == self::SELECT_WITH_FROM_PART) {
            $select->from($this->info(self::NAME), 
    ...
    public function insert (array $data)
    {
         ...
         */
        if (! isset($pkSuppliedBySequence) &&
                 array_key_exists($pkIdentity, $data)) {
            if ($data[$pkIdentity] === null || // null
$data[$pkIdentity] === '' || // empty string
is_bool($data[$pkIdentity]) || // boolean
             (is_array($data[$pkIdentity]) && empty($data[$pkIdentity]))) { // empty
                                                                              // array
 ...
 * @return int
 */
public function _cascadeUpdate ($parentTableClassname, array $oldPrimaryKey, 
    array $newPrimaryKey)
{
$this->_setupMetadata();
$rowsAffected = 0;
...
foreach ($args as $keyPosition => $keyValues) {
    ...
    if ($numberTerms == 0) {
        ...
        if ($keyValuesCount != $numberTerms) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new Zend_Db_Table_Exception(
                    "Missing value(s) for the primary key");
        }
...
if (count($whereList)) {
    ...
    foreach ($whereList as $keyValueSets) {
        ...
        foreach ($keyValueSets as $keyPosition => $keyValue) {
            ...
            $columnName = $this->_db->quoteIdentifier($keyNames[$keyPosition], 
                    true);
...
if ($defaultSource == self::DEFAULT_DB) {
    foreach ($this->_metadata as $metadataName => $metadata) {
        if (($metadata['DEFAULT'] != null) &&
                 ...
                 ($metadata['NULLABLE'] === true &&
                 isset($this->_defaultValues[$metadataName]) &&
                 $this->_defaultValues[$metadataName] === true)) &&
                 (! (isset($this->_defaultValues[$metadataName]) &&
                 $this->_defaultValues[$metadataName] === false))) {
