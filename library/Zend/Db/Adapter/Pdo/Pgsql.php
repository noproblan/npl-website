...
/**
 * Class for connecting to PostgreSQL databases and performing common
 * operations.
 *
 * @category Zend
 * @package Zend_Db
...
class Zend_Db_Adapter_Pdo_Pgsql extends Zend_Db_Adapter_Pdo_Abstract
{
    ...
    public function describeTable ($tableName, $schemaName = null)
    {
        ...
        foreach ($result as $key => $row) {
            ...
            if ($row[$type] == 'varchar' || $row[$type] == 'bpchar') {
                if (preg_match('/character(?: varying)?(?:\((\d+)\))?/', 
                ...
                if (preg_match("/^'(.*?)'::(?:character varying|bpchar)$/", 
            ...
            if ($row[$contype] == 'p') {
                $primary = true;
                $primaryPosition = array_search($row[$attnum], 
                        explode(',', $row[$conkey])) + 1;
                $identity = (bool) (preg_match('/^nextval/', 
                        $row[$default_value]));
            }
    ...
    {
        $this->_connect();
        $sequenceName = str_replace($this->getQuoteIdentifierSymbol(), '', 
                (string) $sequenceName);
        $value = $this->fetchOne(
                "SELECT CURRVAL(" .
                         $this->quote(
