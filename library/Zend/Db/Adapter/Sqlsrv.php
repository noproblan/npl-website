...
class Zend_Db_Adapter_Sqlsrv extends Zend_Db_Adapter_Abstract
{
    ...
    public function setTransactionIsolationLevel ($level = null)
    {
        ...
        switch ($level) {
            ...
            default:
                require_once 'Zend/Db/Adapter/Sqlsrv/Exception.php';
                throw new Zend_Db_Adapter_Sqlsrv_Exception(
                        "Invalid transaction isolation level mode '$level' specified");
        }
        
        if (! sqlsrv_query($this->_connection, 
                "SET TRANSACTION ISOLATION LEVEL $sql;")) {
            require_once 'Zend/Db/Adapter/Sqlsrv/Exception.php';
            throw new Zend_Db_Adapter_Sqlsrv_Exception(
                    "Transaction cannot be changed to '$level'");
        }
    ...
    public function isConnected ()
    {
        return (is_resource($this->_connection) &&
                 (get_resource_type($this->_connection) ==
    ...
    public function describeTable ($tableName, $schemaName = null)
    {
        ...
        // 0=TABLE_QUALIFIER 1=TABLE_OWNER 2=TABLE_NAME 3=COLUMN_NAME 4=KEY_SEQ
        // 5=PK_NAME
    ...
    public function limit ($sql, $count, $offset = 0)
    {
        ...
        if ($offset == 0) {
            $sql = preg_replace('/^SELECT\s/i', 'SELECT TOP ' . $count . ' ', 
                    $sql);
        } else {
