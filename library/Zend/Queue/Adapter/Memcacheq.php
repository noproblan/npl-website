...
class Zend_Queue_Adapter_Memcacheq extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function __construct ($options, Zend_Queue $queue = null)
    {
        if (! extension_loaded('memcache')) {
            ...
            throw new Zend_Queue_Exception(
                    'Memcache extension does not appear to be loaded');
        }
    ...
    {
        $response = $this->_sendCommand('delete ' . $name, 
                array(
                        'DELETED',
                        'NOT_FOUND'
                ), true);
    ...
    public function send ($message, Zend_Queue $queue = null)
    {
        ...
        if ($result === false) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'failed to insert message into queue:' . $queue->getName());
        }
    ...
    public function deleteMessage (Zend_Queue_Message $message)
    {
        require_once 'Zend/Queue/Exception.php';
        throw new Zend_Queue_Exception(
                'deleteMessage() is not supported in  ' . get_class($this));
    }
     ...
     */
    protected function _sendCommand ($command, array $terminator, 
            $include_term = false)
    {
        if (! is_resource($this->_socket)) {
            $this->_socket = fsockopen($this->_host, $this->_port, $errno, 
                    $errstr, 10);
        }
        if ($this->_socket === false) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Could not open a connection to $this->_host:$this->_port errno=$errno : $errstr");
        }
