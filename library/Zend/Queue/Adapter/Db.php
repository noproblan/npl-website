...
class Zend_Queue_Adapter_Db extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function __construct ($options, Zend_Queue $queue = null)
    {
        ...
        if (! is_bool($this->_options['options'][Zend_Db_Select::FOR_UPDATE])) {
            ...
            throw new Zend_Queue_Exception(
                    'Options array item: Zend_Db_Select::FOR_UPDATE must be boolean');
        }
    ...
    protected function _initDbAdapter ()
    {
        ...
        if (! array_key_exists('type', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Configuration array must have a key for 'type' for the database type to use");
        }
        
        if (! array_key_exists('host', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Configuration array must have a key for 'host' for the host to use");
        }
        
        if (! array_key_exists('username', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Configuration array must have a key for 'username' for the username to use");
        }
        
        if (! array_key_exists('password', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Configuration array must have a key for 'password' for the password to use");
        }
        
        if (! array_key_exists('dbname', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Configuration array must have a key for 'dbname' for the database to use");
        }
        ...
        try {
            if ($maxMessages > 0) { // ZF-7666 LIMIT 0 clause not included.
                ...
                $query->from($info['name'], array(
                    ...
                    ->where('queue_id=?', $this->getQueueId($queue->getName()))
                    ->where(
                        'handle IS NULL OR timeout+' . (int) $timeout . ' < ' .
                                 (int) $microtime)
                    ->limit($maxMessages);
                ...
                foreach ($db->fetchAll($query) as $data) {
                    ...
                    // update the database
                    $where = array();
                    $where[] = $db->quoteInto('message_id=?', 
                            $data['message_id']);
                    $where[] = 'handle IS NULL OR timeout+' . (int) $timeout .
                             ' < ' . (int) $microtime;
