...
class Zend_Queue_Adapter_PlatformJobQueue extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function __construct ($options, Zend_Queue $queue = null)
    {
        ...
        if (! extension_loaded("jobqueue_client")) {
            ...
            throw new Zend_Queue_Exception(
                    'Platform Job Queue extension does not appear to be loaded');
        }
        
        if (! isset($this->_options['daemonOptions'])) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'Job Queue host and password should be provided');
        }
        ...
        if (! array_key_exists('host', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'Platform Job Queue host should be provided');
        }
        if (! array_key_exists('password', $options)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'Platform Job Queue password should be provided');
        }
        ...
        if (! $this->_zendQueue) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'Platform Job Queue connection failed');
        }
    ...
    public function isExists ($name)
    {
        require_once 'Zend/Queue/Exception.php';
        throw new Zend_Queue_Exception(
                'isExists() is not supported in this adapter');
    }
    ...
    public function create ($name, $timeout = null)
    {
        require_once 'Zend/Queue/Exception.php';
        throw new Zend_Queue_Exception(
                'create() is not supported in ' . get_class($this));
    }
    ...
    public function delete ($name)
    {
        require_once 'Zend/Queue/Exception.php';
        throw new Zend_Queue_Exception(
                'delete() is not supported in ' . get_class($this));
    }
     ...
     *
     * @param
     *            array | ZendAPI_job $message Message to send to the active
     *            queue
