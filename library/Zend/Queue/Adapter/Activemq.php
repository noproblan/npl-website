...
class Zend_Queue_Adapter_Activemq extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function __construct ($options, Zend_Queue $queue = null)
    {
        ...
        if (array_key_exists('stompClient', $options)) {
            ...
            $this->_client = new Zend_Queue_Stomp_Client($options['scheme'], 
        ...
        if ((false !== $response) && ($response->getCommand() != 'CONNECTED')) {
            ...
            throw new Zend_Queue_Exception(
                    "Unable to authenticate to '" . $options['scheme'] . '://' .
                             $options['host'] . ':' . $options['port'] . "'");
        }
    }
            ...
            if ($this->_client->canRead()) {
                for ($i = 0; $i < $maxMessages; $i ++) {
                    ...
                    switch ($response->getCommand()) {
                        case 'MESSAGE':
                            $datum = array(
                                    'message_id' => $response->getHeader(
                                            'message-id'),
                                    'handle' => $response->getHeader(
                                            'message-id'),
                        ...
                        default:
                            ...
                            require_once 'Zend/Queue/Exception.php';
                            throw new Zend_Queue_Exception(
                                    'Invalid response received: ' . $block);
                    }
                }
            }
        }
