 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Queue_Stomp_Client_Connection implements 
        Zend_Queue_Stomp_Client_ConnectionInterface
{
    ...
    public function open ($scheme, $host, $port, array $options = array())
    {
        ...
        if ($this->_socket === false) {
            ...
            throw new Zend_Queue_Exception(
                    "Unable to connect to $str; error = $errstr ( errno = $errno )");
        }
    ...
    public function close ($destructor = false)
    {
        ...
        // When the socket is "closed", it will trigger the below error when php
        // exits
        // Fatal error: Exception thrown without a stack frame in Unknown on
        // line 0
        
        // Danlo: I suspect this is because this has already been claimed by the
        // interpeter
        // thus trying to shutdown this resources, which is already shutdown is
        // a problem.
    ...
    public function canRead ()
    {
        ...
        return stream_select($read, $write, $except, 
                $this->_options['timeout_sec'], $this->_options['timeout_usec']) ==
                 1;
    ...
    public function createFrame ()
    {
        ...
        if (! $frame instanceof Zend_Queue_Stomp_FrameInterface) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    'Invalid Frame class provided; must implement Zend_Queue_Stomp_FrameInterface');
        }
