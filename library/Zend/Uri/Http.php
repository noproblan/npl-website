...
class Zend_Uri_Http extends Zend_Uri
{
    ...
    /**
     * Regular expression grammar rules for validation; values added by
     * constructor
     *
     * @var array
     */
    ...
    /**
     * Constructor accepts a string $scheme (e.g., http, https) and a
    ...
    /**
     * Parse the scheme-specific portion of the URI and place its parts into
     * instance variables.
     *
     * @param string $schemeSpecific
    ...
    protected function _parseUri ($schemeSpecific)
    {
        ...
        $this->_host = isset($matches[5]) === true ? preg_replace(
                '~^\[([^]]+)\]$~', '\1', $matches[5]) : // Strip
                                                                     // wrapper
                                                                     // [] from
                                                                     // IPv6
...
'';
    ...
    public function getUri ()
    {
        ...
        return $this->_scheme . '://' . $auth . $this->_host . $port .
                 $this->_path . $query . $fragment;
    }
    ...
    {
        // Return true if and only if all parts of the URI have passed
        // validation
        return $this->validateUsername() and $this->validatePassword() and
                 $this->validateHost() and $this->validatePort() and
                 $this->validatePath() and $this->validateQuery() and
    ...
    public function validateUsername ($username = null)
    {
        ...
        $status = @preg_match(
                '/^(?:' . $this->_regex['escaped'] . '|[' . self::CHAR_ALNUM .
                         self::CHAR_MARK . ';:&=+$,' . '])+$/', $username);
        
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    'Internal error: username validation failed');
        }
    ...
    public function setUsername ($username)
    {
        if ($this->validateUsername($username) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    "Username \"$username\" is not a valid HTTP username");
        }
    ...
    public function validatePassword ($password = null)
    {
        ...
        // If the password is nonempty, but there is no username, then it is
        // considered invalid
    ...
    public function setPassword ($password)
    {
        if ($this->validatePassword($password) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    "Password \"$password\" is not a valid HTTP password.");
        }
    ...
    public function setHost ($host)
    {
        if ($this->validateHost($host) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    "Host \"$host\" is not a valid HTTP host");
        }
    ...
    public function setPort ($port)
    {
        if ($this->validatePort($port) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    "Port \"$port\" is not a valid HTTP port.");
        }
    ...
    public function validateQuery ($query = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    'Internal error: query validation failed');
        }
    ...
    {
        $queryParams = array_diff_key($this->getQueryAsArray(), 
                array_fill_keys($queryParamKeys, 0));
        return $this->setQuery($queryParams);
    }
    ...
    public function setQuery ($query)
    {
        ...
        if (is_array($query) === true) {
            ...
            // If it is a string, make sure it is valid. If not parse and encode
            // it
            $query = (string) $query;
    ...
    public function setFragment ($fragment)
    {
        if ($this->validateFragment($fragment) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception(
                    "Fragment \"$fragment\" is not a valid HTTP fragment");
        }
