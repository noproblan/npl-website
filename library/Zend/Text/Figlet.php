...
class Zend_Text_Figlet
{
     ...
     * Zend_Text_Figlet::DIRECTION_LEFT_TO_RIGHT. For writing from right to
     * left,
    ...
    public function render ($text, $encoding = 'UTF-8')
    {
        ...
        if ($textLength === false) {
            ...
            throw new Zend_Text_Figlet_Exception(
                    '$text is not encoded with ' . $encoding);
        }
        ...
        for ($charNum = 0; $charNum < $textLength; $charNum ++) {
            ...
            $lastCharWasEol = (ctype_space($char) && $char !== "\t" &&
                     $char !== ' ');
                        ...
                        if ($this->_outlineLength === 0) {
                            for ($i = 0; $i < $this->_charHeight; $i ++) {
                                if ($this->_rightToLeft === 1 &&
                                         $this->_outputWidth > 1) {
                                    $offset = (strlen($this->_currentChar[$i]) -
                                     $this->_outlineLengthLimit);
                            $this->_putString(
                                    substr($this->_currentChar[$i], $offset));
                        } else {
...
if ($this->_outputWidth > 1) {
    ...
    if ($this->_justification > 0) {
        for ($i = 1; ((3 - $this->_justification) * $i + $length +
                 $this->_justification - 2) < $this->_outputWidth; $i ++) {
            $this->_output .= ' ';
        }
    }
}
...
if (($this->_outlineLength + $this->_currentCharWidth - $smushAmount) >
         $this->_outlineLengthLimit ||
...
for ($row = 0; $row < $this->_charHeight; $row ++) {
    if ($this->_rightToLeft === 1) {
        ...
        for ($k = 0; $k < $smushAmount; $k ++) {
            ...
            $tempLine[$position] = $this->_smushem($tempLine[$position], 
                    $this->_outputLine[$row][$k]);
        }
        
        $this->_outputLine[$row] = $tempLine .
                 substr($this->_outputLine[$row], $smushAmount);
    } else {
        for ($k = 0; $k < $smushAmount; $k ++) {
            ...
            $this->_outputLine[$row][$position] = $this->_smushem($leftChar, 
                    $this->_currentChar[$row][$k]);
        }
        
        $this->_outputLine[$row] .= substr($this->_currentChar[$row], 
                $smushAmount);
    }
}
...
if (($this->_smushMode & self::SM_HIERARCHY) > 0) {
    if ($leftChar === '|' && strchr('/\\[]{}()<>', $rightChar) !== false) {
        ...
        if ($rightChar === '|' && strchr('/\\[]{}()<>', $leftChar) !== false) {
            ...
            if (strchr('/\\', $leftChar) &&
                     strchr('[]{}()<>', $rightChar) !== false) {
                return $rightChar;
            } else 
                if (strchr('/\\', $rightChar) &&
                         strchr('[]{}()<>', $leftChar) !== false) {
                    return $leftChar;
                } else 
                    if (strchr('[]', $leftChar) &&
                             strchr('{}()<>', $rightChar) !== false) {
 ...
 * @throws Zend_Text_Figlet_Exception When GZIP library is required but not
 *         found
...
// Get the header
$numsRead = sscanf(fgets($fp, 1000), '%*c%c %d %*d %d %d %d %d %d', 
        $this->_hardBlank, $this->_charHeight, $this->_maxLength, $smush, 
        $cmtLines, $rightToLeft, $this->_fontSmush);

if ($magic !== self::FONTFILE_MAGIC_NUMBER || $numsRead < 5) {
    require_once 'Zend/Text/Figlet/Exception.php';
    throw new Zend_Text_Figlet_Exception(
            $fontFile . ': Not a FIGlet 2 font file');
}
...
while (! feof($fp)) {
    ...
    if (substr($uniCode, 0, 2) === '0x') {
        ...
        if (substr($uniCode, 0, 1) === '0' and $uniCode !== '0' or
