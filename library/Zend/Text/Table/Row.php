...
class Zend_Text_Table_Row
{
    ...
    public function createColumn ($content, array $options = null)
    {
        ...
        $column = new Zend_Text_Table_Column($content, $align, $colSpan, 
                $encoding);
    ...
    public function render (array $columnWidths, 
            Zend_Text_Table_Decorator_Interface $decorator, $padding = 0)
    {
        ...
        if (count($this->_columns) === 0) {
            require_once 'Zend/Text/Table/Column.php';
            $this->appendColumn(
                    new Zend_Text_Table_Column(null, null, count($columnWidths)));
        }
        ...
        foreach ($this->_columns as $column) {
            ...
            $columnWidth = ($colSpan - 1 + array_sum(
                    array_slice($columnWidths, $colNum, $colSpan)));
        ...
        if ($colNum < count($columnWidths)) {
            $remainingWidth = (count($columnWidths) - $colNum - 1) + array_sum(
                    array_slice($columnWidths, $colNum));
            $renderedColumns[] = array(
