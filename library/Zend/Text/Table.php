...
class Zend_Text_Table
{
    ...
    public function __construct ($options = null)
    {
        ...
        if ($this->_columnWidths === null) {
            ...
            throw new Zend_Text_Table_Exception(
                    'You must define the column widths');
        }
    ...
    public function setColumnWidths (array $columnWidths)
    {
        if (count($columnWidths) === 0) {
            require_once 'Zend/Text/Table/Exception.php';
            throw new Zend_Text_Table_Exception(
                    'You must supply at least one column');
        }
        ...
        foreach ($columnWidths as $columnNum => $columnWidth) {
            if (is_int($columnWidth) === false or $columnWidth < 1) {
                require_once 'Zend/Text/Table/Exception.php';
                throw new Zend_Text_Table_Exception(
                        'Column ' . $columnNum . ' has an invalid' .
                                 ' column width');
            }
        }
     ...
     * @throws Zend_Text_Table_Exception When $row is neither an array nor
     *         Zend_Zext_Table_Row
    ...
    public function appendRow ($row)
    {
        if (! is_array($row) && ! ($row instanceof Zend_Text_Table_Row)) {
            require_once 'Zend/Text/Table/Exception.php';
            throw new Zend_Text_Table_Exception(
                    '$row must be an array or instance of Zend_Text_Table_Row');
        }
        ...
        if (is_array($row)) {
            ...
            foreach ($data as $columnData) {
                ...
                $row->appendColumn(
                        new Zend_Text_Table_Column($columnData, $align));
                $colNum ++;
            }
        }
    ...
    public function render ()
    {
        ...
        if (count($this->_rows) === 0) {
            require_once 'Zend/Text/Table/Exception.php';
            throw new Zend_Text_Table_Exception(
                    'No rows were added to the table yet');
        }
        ...
        foreach ($this->_rows as $rowNum => $row) {
            ...
            if ($rowNum === 0) {
                ...
                if ($this->_autoSeparate & self::AUTO_SEPARATE_ALL) {
                    ...
                    if ($rowNum === 1 &&
                             $this->_autoSeparate & self::AUTO_SEPARATE_HEADER) {
                        $drawSeparator = true;
                    } else 
                        if ($rowNum === ($numRows - 1) &&
                                 $this->_autoSeparate &
                                 self::AUTO_SEPARATE_FOOTER) {
                            $drawSeparator = true;
                        } else {
                            $drawSeparator = false;
                        }
                ...
                if ($drawSeparator) {
                    ...
                    foreach ($this->_columnWidths as $columnNum => $columnWidth) {
                        ...
                        $result .= str_repeat(
                                $this->_decorator->getHorizontal(), $columnWidth);
                        ...
                        if ($lastColumnWidths[$currentUpperColumn] ===
                                 $currentUpperWidth) {
                            $connector |= 0x1;
                        ...
                        if ($columnWidths[$currentLowerColumn] ===
                                 $currentLowerWidth) {
                            $connector |= 0x2;
                        ...
                        switch ($connector) {
                            ...
                            default:
                                ...
                                // This can never happen, but the CS tells I
                                // have to have it ...
                                break;
                        }
                    }
