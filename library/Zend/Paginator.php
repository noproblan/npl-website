...
class Zend_Paginator implements Countable, IteratorAggregate
{
    ...
    /**
     * Specifies that the factory should try to detect the proper adapter type
     * first
     *
     * @var string
     */
    ...
    public static function addAdapterPrefixPaths (array $prefixPaths)
    {
        if (isset($prefixPaths['prefix']) && isset($prefixPaths['path'])) {
            self::addAdapterPrefixPath($prefixPaths['prefix'], 
                    $prefixPaths['path']);
        } else {
            foreach ($prefixPaths as $prefix => $path) {
                if (is_array($path) && isset($path['prefix']) &&
                         isset($path['path'])) {
                    $prefix = $path['prefix'];
                    $path = $path['path'];
                }
    ...
    {
        if (isset($prefixPaths['prefix']) && isset($prefixPaths['path'])) {
            self::addScrollingStylePrefixPath($prefixPaths['prefix'], 
                    $prefixPaths['path']);
        } else {
            foreach ($prefixPaths as $prefix => $path) {
                if (is_array($path) && isset($path['prefix']) &&
    ...
    public static function setConfig (Zend_Config $config)
    {
        ...
        if ($prefixPaths != null) {
            self::addScrollingStylePrefixPaths(
                    $prefixPaths->prefixpath->toArray());
        }
    ...
    public function clearPageItemCache ($pageNumber = null)
    {
        ...
        if (null === $pageNumber) {
            foreach (self::$_cache->getIdsMatchingTags(
                    array(
                            $this->_getCacheInternalId()
                    )) as $id) {
                if (preg_match('|' . self::CACHE_TAG_PREFIX . "(\d+)_.*|", $id, 
                        $page)) {
    ...
    public function getCurrentItemCount ()
    {
        if ($this->_currentItemCount === null) {
            $this->_currentItemCount = $this->getItemCount(
    ...
    public function getCurrentItems ()
    {
        if ($this->_currentItems === null) {
            $this->_currentItems = $this->getItemsByPage(
    ...
    public function getItem ($itemNumber, $pageNumber = null)
    {
        ...
        if ($itemNumber > $itemCount) {
            ...
            throw new Zend_Paginator_Exception(
                    'Page ' . $pageNumber . ' does not' . ' contain item number ' .
                             $itemNumber);
        }
    ...
    public function getItemsByPage ($pageNumber)
    {
        ...
        $items = $this->_adapter->getItems($offset, 
    ...
    public function getPageItemCache ()
    {
        ...
        if ($this->_cacheEnabled()) {
            foreach (self::$_cache->getIdsMatchingTags(
                ...
                if (preg_match('|' . self::CACHE_TAG_PREFIX . "(\d+)_.*|", $id, 
                        $page)) {
                    $data[$page[1]] = self::$_cache->load(
                            $this->_getCacheId($page[1]));
                }
            }
        }
        return $data;
    ...
    protected function _getCacheId ($page = null)
    {
        ...
        return self::CACHE_TAG_PREFIX . $page . '_' .
                 $this->_getCacheInternalId();
    }
    ...
    protected function _createPages ($scrollingStyle = null)
    {
        ...
        if ($this->getCurrentItems() !== null) {
            ...
            $pages->firstItemNumber = (($currentPageNumber - 1) *
                     $this->getItemCountPerPage()) + 1;
            $pages->lastItemNumber = $pages->firstItemNumber +
                     $pages->currentItemCount - 1;
        }
    ...
    protected function _loadScrollingStyle ($scrollingStyle = null)
    {
        ...
        switch (strtolower(gettype($scrollingStyle))) {
            ...
            case 'string':
                $className = self::getScrollingStyleLoader()->load(
                        $scrollingStyle);
