...
class Zend_Log_Writer_Mail extends Zend_Log_Writer_Abstract
{
    ...
    public function setLayout ($layout)
    {
        ...
        if (! $layout instanceof Zend_Layout) {
            ...
            throw new Zend_Log_Exception(
                    'Mail must be an instance of Zend_Layout or an array');
        }
        $this->_layout = $layout;
    ...
    protected static function _constructMailFromConfig (array $config)
    {
        ...
        $headerAddresses = array_intersect_key($config, 
                self::$_methodMapHeaders);
    ...
    protected function _write ($event)
    {
        ...
        if ($this->_layout) {
            if ($this->_layoutFormatter) {
                $this->_layoutEventsToMail[] = $this->_layoutFormatter->format(
                        $event);
            } else {
        ...
        try {
            ...
            trigger_error(
                    "unable to send log entries via email; " .
                             "message = {$e->getMessage()}; " .
                             "code = {$e->getCode()}; " . "exception class = " .
                             get_class($e), E_USER_WARNING);
        }
    }
