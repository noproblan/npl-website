...
class Zend_File_Transfer_Adapter_Http extends Zend_File_Transfer_Adapter_Abstract
{
    ...
    public function __construct ($options = array())
    {
        if (ini_get('file_uploads') == false) {
            ...
            throw new Zend_File_Transfer_Exception(
                    'File uploads are not allowed in your php config!');
        }
    ...
    {
        // Workaround for WebServer not conforming HTTP and omitting
        // CONTENT_LENGTH
        $content = 0;
        ...
        // Workaround for a PHP error returning empty $_FILES when form data
        // exceeds php settings
    ...
    public function receive ($files = null)
    {
        ...
        foreach ($check as $file => $content) {
            if (! $content['received']) {
                ...
                // Should never return false when it's tested by the upload
                // validator
    ...
    {
        if (! function_exists('apc_fetch') and
        ...
        if (! empty($id) &&
                 (($id instanceof Zend_ProgressBar_Adapter) ||
                 ($id instanceof Zend_ProgressBar))) {
            $adapter = $id;
            unset($id);
        }
        ...
        if (! empty($id)) {
            if (self::isApcAvailable()) {
                ...
                $call = call_user_func(self::$_callbackApc, 
                        ini_get('apc.rfc1867_prefix') . $id);
                if (is_array($call)) {
                    $status = $call + $status;
                }
            } else 
            ...
            if (! is_array($call)) {
                ...
                if (! empty($status['cancel_upload'])) {
                    ...
                    $status['message'] = self::_toByteString($status['current']) .
                             " - " . self::_toByteString($status['total']);
                }
        ...
        if (isset($adapter) && isset($status['id'])) {
            if ($adapter instanceof Zend_ProgressBar_Adapter) {
                ...
                $adapter = new Zend_ProgressBar($adapter, 0, $status['total'], 
                        $session);
            }
            ...
            if (is_array($content['name'])) {
                foreach ($content as $param => $file) {
                    foreach ($file as $number => $target) {
                        ...
                        $this->_files[$form]['multifiles'][$number] = $form . '_' .
                                 $number . '_';
                    }
                }
