...
abstract class Zend_File_Transfer_Adapter_Abstract
{
    ...
    public static function getProgress ()
    {
        ...
        throw new Zend_File_Transfer_Exception(
                'Method must be implemented within the adapter');
    }
                ...
                if (! isset($this->_loaders[$type])) {
                    $paths = array(
                            'Zend_' . $prefixSegment . '_' => 'Zend/' .
                                     $pathSegment . '/',
                                    'Zend_' . $prefixSegment . '_File' => 'Zend/' .
                                     $pathSegment . '/File'
                    );
                        ...
                        if (isset($validatorInfo['validator'])) {
                            ...
                            if (is_string($name)) {
                                ...
                                switch (true) {
                                    ...
                                    case (2 <= $argc):
                                        $breakChainOnFailure = array_shift(
                                                $validatorInfo);
                                    case (3 <= $argc):
    ...
    public function isValid ($files = null)
    {
        ...
        foreach ($check as $key => $content) {
            if (array_key_exists('validators', $content) &&
                     in_array('Zend_Validate_File_Count', 
                            $content['validators'])) {
        ...
        foreach ($check as $key => $content) {
            ...
            if (array_key_exists('validators', $content)) {
                foreach ($content['validators'] as $class) {
                    ...
                    if (($class === 'Zend_Validate_File_Upload') and
                             (empty($content['tmp_name']))) {
                        $tocheck = $key;
                    } else {
                    ...
                    if (! empty($content['options']['ignoreNoFile']) and
                             (isset($fileerrors['fileUploadErrorNoFile']))) {
     ...
     * @throws Zend_File_Transfer_Exception when the given destination is not a
    ...
    public function getDestination ($files = null)
    {
        ...
        if (empty($files) and is_string($orig)) {
            if (isset($this->_files[$orig]['destination'])) {
                ...
                require_once 'Zend/File/Transfer/Exception.php';
                throw new Zend_File_Transfer_Exception(
                        sprintf('The file transfer adapter can not find "%s"', 
                                $orig));
            }
        }
        ...
        foreach ($files as $key => $value) {
            if (file_exists($value['name'])) {
                ...
                if (file_exists($value['tmp_name'])) {
                    ...
                    if (empty($value['options']['ignoreNoFile'])) {
                        require_once 'Zend/File/Transfer/Exception.php';
                        throw new Zend_File_Transfer_Exception(
                                "The file '{$value['name']}' does not exist");
                    }
        }
     ...
     * Uses fileinfo, when not available mime_magic and as last fallback a
     * manual given mimetype
     *
     * @param string|array $files
        ...
        if (null === $this->_tmpDir) {
            ...
            if (empty($this->_tmpDir)) {
                ...
                if ($tempFile) {
                    ...
                    require_once 'Zend/File/Transfer/Exception.php';
                    throw new Zend_File_Transfer_Exception(
                            'Could not determine a temporary directory');
                }
            }
     ...
     *            (Optional) Allows throwing an exception, otherwise returns an
     *            empty array
        ...
        if (is_array($files)) {
            foreach ($files as $find) {
                ...
                if (empty($found)) {
                    ...
                    throw new Zend_File_Transfer_Exception(
                            sprintf(
                                    'The file transfer adapter can not find "%s"', 
                                    $find));
                }
