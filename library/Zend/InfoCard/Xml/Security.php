...
class Zend_InfoCard_Xml_Security
{
    ...
    static public function validateXMLSignature ($strXMLInput)
    {
        if (! extension_loaded('openssl')) {
            ...
            throw new Zend_InfoCard_Xml_Security_Exception(
        ...
        if ($base64DecodeSupportsStrictParam) {
            $dValue = base64_decode(
                    (string) $sxe->Signature->SignedInfo->Reference->DigestValue, 
                    true);
        } else {
            $dValue = base64_decode(
                    (string) $sxe->Signature->SignedInfo->Reference->DigestValue);
        ...
        if ($base64DecodeSupportsStrictParam) {
            $signatureValue = base64_decode(
                    (string) $sxe->Signature->SignatureValue, true);
        } else {
            $signatureValue = base64_decode(
                    (string) $sxe->Signature->SignatureValue);
        }
        ...
        // The way we are doing our XML processing requires that we specifically
        // add this
        // (even though it's in the <Signature> parent-block).. otherwise, our
        // canonical form
        // fails signature verification
        $sxe->Signature->SignedInfo->addAttribute('xmlns', 
    ...
    {
        // Null pad some data when we get it (integer values > 128 and
        // bitstrings)
        ...
        // I'm fairly confident that this is by no means a complete
        // implementation
