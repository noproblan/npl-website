 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
abstract class Zend_InfoCard_Xml_Element extends SimpleXMLElement implements 
        Zend_InfoCard_Xml_Element_Interface
{
    ...
    static public function convertToDOM (Zend_InfoCard_Xml_Element $e)
    {
        ...
        if (! ($dom instanceof DOMElement)) {
            // Zend_InfoCard_Xml_Element exntes SimpleXMLElement, so this should
            // *never* fail
            // @codeCoverageIgnoreStart
            require_once 'Zend/InfoCard/Xml/Exception.php';
            throw new Zend_InfoCard_Xml_Exception(
                    "Failed to convert between SimpleXML and DOM");
     ...
     *            The name of the class to convert it to (must inhert from
     *            Zend_InfoCard_Xml_Element)
     * @return Zend_InfoCard_Xml_Element a Xml Element object from the DOM
     *         element
     */
    static public function convertToObject (DOMElement $e, $classname)
    {
        ...
        if (! $reflection->isSubclassOf('Zend_InfoCard_Xml_Element')) {
            require_once 'Zend/InfoCard/Xml/Exception.php';
            throw new Zend_InfoCard_Xml_Exception(
                    "DOM element must be converted to an instance of Zend_InfoCard_Xml_Element");
        }
        
        $sxe = simplexml_import_dom($e, $classname);
        
        if (! ($sxe instanceof Zend_InfoCard_Xml_Element)) {
            // Since we just checked to see if this was a subclass of
            // Zend_infoCard_Xml_Element this shoudl never fail
            // @codeCoverageIgnoreStart
            require_once 'Zend/InfoCard/Xml/Exception.php';
            throw new Zend_InfoCard_Xml_Exception(
                    "Failed to convert between DOM and SimpleXML");
            // @codeCoverageIgnoreEnd
        }
