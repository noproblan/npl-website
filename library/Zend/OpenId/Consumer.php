...
class Zend_OpenId_Consumer
{
    ...
    {
        return $this->_checkId(false, $id, $returnTo, $root, $extensions, 
                $response);
    }
    ...
    {
        return $this->_checkId(true, $id, $returnTo, $root, $extensions, 
                $response);
    }
    ...
    public function verify ($params, &$identity = "", $extensions = null)
    {
        ...
        if ($params['openid_mode'] != 'id_res') {
            $this->_setError(
                    "Wrong openid.mode '" . $params['openid_mode'] .
                             "' != 'id_res'");
            return false;
        }
        ...
        if ($params['openid_return_to'] != Zend_OpenId::selfUrl()) {
            ...
            if ($pos === false ||
                     SUBSTR($params['openid_return_to'], 0, $pos) !=
                     Zend_OpenId::selfUrl()) {
                
                $this->_setError(
                        "Wrong openid.return_to '" . $params['openid_return_to'] .
                         "' != '" . Zend_OpenId::selfUrl() . "'");
                return false;
            }
        }
        ...
        if ($version >= 2.0) {
            ...
            if (empty($params['openid_op_endpoint'])) {
                ...
                if (! $this->_storage->isUniqueNonce(
        ...
        if (! empty($params['openid_invalidate_handle'])) {
            if ($this->_storage->getAssociationByHandle(
                    $params['openid_invalidate_handle'], $url, $macFunc, $secret, 
                    $expires)) {
        ...
        if ($this->_storage->getAssociationByHandle(
                $params['openid_assoc_handle'], $url, $macFunc, $secret, 
                $expires)) {
            ...
            if (base64_decode($params['openid_sig']) ==
                ...
                if (! Zend_OpenId_Extension::forAll($extensions, 
                ...
                if (isset($params['openid_claimed_id'])) {
                    ...
                    if (! Zend_OpenId::normalize($id)) {
                        ...
                        if (! $this->_discovery($id, $discovered_server, 
                                $discovered_version)) {
                            $this->_setError(
                                    "Discovery failed: " . $this->getError());
                            return false;
                        } else 
                            if ((! empty($params['openid_identity']) &&
                                ...
                                $this->_setError(
                                        "Discovery information verification failed");
                                return false;
                            }
                }
                return true;
            ...
            if (isset($params['openid_claimed_id'])) {
                ...
                if (isset($params['openid_identity'])) {
                    ...
                    $this->_setError(
                            "Missing openid.claimed_id and openid.identity");
                    return false;
                }
            ...
            /* OpenID 2.0 (11.2) Verifying Discovered Information */
            if ((isset($params['openid_identity']) &&
                     $params["openid_identity"] != $id) || (isset(
                            $params['openid_op_endpoint']) &&
            ...
            foreach ($params as $key => $val) {
                if (strpos($key, 'openid_ns_') === 0) {
                    ...
                    if (strpos($key, 'openid_sreg_') === 0) {
                        $key = 'openid.sreg.' .
                                 substr($key, strlen('openid_sreg_'));
                    } else 
            ...
            if ($status != 200) {
                $this->_setError(
                        "'Dumb' signature verification HTTP request failed");
                return false;
            }
            $r = array();
            if (is_string($ret)) {
            ...
            if (! empty($ret['invalidate_handle'])) {
                if ($this->_storage->getAssociationByHandle(
                        $ret['invalidate_handle'], $url, $macFunc, $secret, 
                        $expires)) {
                    $this->_storage->delAssociation($url);
                }
            }
     ...
     * @return void
     */
    protected function _addAssociation ($url, $handle, $macFunc, $secret, 
            $expires)
    {
        ...
        return $this->_storage->addAssociation($url, $handle, $macFunc, $secret, 
                $expires);
    }
     ...
     * @return void
     */
    protected function _getAssociation ($url, &$handle, &$macFunc, &$secret, 
            &$expires)
    {
        ...
        if ($this->_storage->getAssociation($url, $handle, $macFunc, $secret, 
                $expires)) {
            $this->_cache[$url] = array(
                    $handle,
                    $macFunc,
    ...
    protected function _associate ($url, $version, $priv_key = null)
    {
        ...
        $dh = Zend_OpenId::createDhKey(pack('H*', Zend_OpenId::DH_P), 
                pack('H*', Zend_OpenId::DH_G), $priv_key);
        ...
        if ($version >= 2.0 && isset($ret['ns']) &&
                 $ret['ns'] != Zend_OpenId::NS_2_0) {
            $this->_setError(
                    "Wrong namespace definition in the server response");
            return false;
        }
        
        if (! isset($ret['assoc_handle']) || ! isset($ret['expires_in']) ||
                 ! isset($ret['assoc_type']) ||
                 $params['openid.assoc_type'] != $ret['assoc_type']) {
            if ($params['openid.assoc_type'] != $ret['assoc_type']) {
                $this->_setError(
                        "The returned assoc_type differed from the supplied openid.assoc_type");
            } else {
                $this->_setError(
                        "Missing required data from provider (assoc_handle, expires_in, assoc_type are required)");
            }
            return false;
        }
        ...
        if ((empty($ret['session_type']) ||
            ...
            if (isset($ret['session_type']) && $ret['session_type'] == 'DH-SHA1' &&
                     ! empty($ret['dh_server_public']) &&
                     ! empty($ret['enc_mac_key'])) {
                $dhFunc = 'sha1';
            } else 
                if (isset($ret['session_type']) &&
                         $ret['session_type'] == 'DH-SHA256' && $version >= 2.0 &&
                         ! empty($ret['dh_server_public']) &&
                         ! empty($ret['enc_mac_key'])) {
                    $dhFunc = 'sha256';
                } else {
        ...
        if ($macFunc == 'sha1') {
            ...
            if ($macFunc == 'sha256') {
                if (Zend_OpenId::strlen($secret) != 32) {
                    $this->_setError(
                            "The length of the sha256 secret must be 32");
                    return false;
                }
            }
        $this->_addAssociation($url, $handle, $macFunc, $secret, 
                time() + $expiresIn);
        return true;
    }
    ...
    {
        $realId = $id;
        if ($this->_storage->getDiscoveryInfo($id, $realId, $server, $version, 
                $expire)) {
            $id = $realId;
            return true;
        }
            ...
            $extensions = null, Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if (! $this->_getAssociation($server, $handle, $macFunc, $secret, 
                $expires)) {
            /* Use dumb mode */
            unset($handle);
            unset($macFunc);
        ...
        if (! Zend_OpenId_Extension::forAll($extensions, 'prepareRequest', 
                $params)) {
