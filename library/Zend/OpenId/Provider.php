...
class Zend_OpenId_Provider
{
     ...
     *            or set to null, then $_GET or $_POST superglobal variable is
     *            used
            ...
            Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if (isset($params['openid_mode'])) {
            if ($params['openid_mode'] == 'associate') {
                ...
                if ($params['openid_mode'] == 'checkid_immediate') {
                    ...
                    if ($params['openid_mode'] == 'checkid_setup') {
                        $ret = $this->_checkId($version, $params, 0, 
                                $extensions, $response);
                        if (is_bool($ret))
                            return $ret;
                        if (! empty($params['openid_return_to'])) {
                            Zend_OpenId::redirect($params['openid_return_to'], 
                                    $ret, $response);
                        }
                        return true;
                    } else 
                        if ($params['openid_mode'] == 'check_authentication') {
                            $response = $this->_checkAuthentication($version, 
                                    $params);
                            $ret = '';
                            foreach ($response as $key => $val) {
                                $ret .= $key . ':' . $val . "\n";
    ...
    protected function _associate ($version, $params)
    {
        ...
        $this->_storage->addAssociation($handle, $macFunc, $secret, 
                time() + $expiresIn);
            ...
            Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if (! Zend_OpenId_Extension::forAll($extensions, 'parseRequest', 
                $params)) {
            $ret['openid.mode'] = ($immediate && $version >= 2.0) ? 'setup_needed' : 'cancel';
            return $ret;
        }
        ...
        if (isset($sites[$root])) {
            ...
            foreach ($sites as $site => $t) {
                if (strpos($root, $site) === 0) {
                    ...
                    $n = strpos($site, '://*.');
                    if ($n != false) {
                        $regex = '/^' . preg_quote(substr($site, 0, $n + 3), 
                                '/') . '[A-Za-z1-9_\.]+?' .
        ...
        if (is_array($trusted)) {
            if (! Zend_OpenId_Extension::forAll($extensions, 'checkTrustData', 
                    $trusted)) {
                $trusted = null;
            }
        }
    ...
    {
        if (empty($params['openid_assoc_handle']) || ! $this->_storage->getAssociation(
                $params['openid_assoc_handle'], $macFunc, $secret, $expires)) {
            /* Use dumb mode */
    ...
    protected function _checkAuthentication ($version, $params)
    {
        ...
        if (empty($params['openid_assoc_handle']) ||
                 empty($params['openid_signed']) || empty($params['openid_sig']) || ! $this->_storage->getAssociation(
                        $params['openid_assoc_handle'], $macFunc, $secret, 
                        $expires)) {
            $ret['is_valid'] = 'false';
            return $ret;
        }
