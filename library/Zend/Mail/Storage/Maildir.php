...
class Zend_Mail_Storage_Maildir extends Zend_Mail_Storage_Abstract
{
    ...
    // TODO: getFlags($id) for fast access if headers are not needed (i.e. just
    // setting flags)?
    ...
    public function getSize ($id = null)
    {
        if ($id !== null) {
            ...
            return isset($filedata['size']) ? $filedata['size'] : filesize(
                    $filedata['filename']);
        }
        
        $result = array();
        foreach ($this->_files as $num => $data) {
            $result[$num + 1] = isset($data['size']) ? $data['size'] : filesize(
                    $data['filename']);
        }
    ...
    protected function _openMaildir ($dirname)
    {
        ...
        if ($dh) {
            $this->_getMaildirFiles($dh, $dirname . '/new/', 
                    array(
            ...
            if (file_exists($dirname . '/new/')) {
                ...
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new Zend_Mail_Storage_Exception(
                        'cannot read recent mails in maildir');
            }
    }
