 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Mail_Storage_Writable_Maildir extends Zend_Mail_Storage_Folder_Maildir implements 
        Zend_Mail_Storage_Writable_Interface
{
    ...
    public static function initMaildir ($dir)
    {
        if (file_exists($dir)) {
            if (! is_dir($dir)) {
                ...
                throw new Zend_Mail_Storage_Exception(
                        'maildir must be a directory if already exists');
            }
        } else {
            if (! mkdir($dir)) {
                /**
        ...
        foreach (array(
            ...
            if (! @mkdir($dir . DIRECTORY_SEPARATOR . $subdir)) {
                // ignore if dir exists (i.e. was already valid maildir or two
                // processes try to create one)
                if (! file_exists($dir . DIRECTORY_SEPARATOR . $subdir)) {
                    ...
                    require_once 'Zend/Mail/Storage/Exception.php';
                    throw new Zend_Mail_Storage_Exception(
                            'could not create subdir ' . $subdir);
                }
            }
        }
    }
    ...
    public function __construct ($params)
    {
        ...
        if (! empty($params->create) && isset($params->dirname) &&
    ...
    public function createFolder ($name, $parentFolder = null)
    {
        if ($parentFolder instanceof Zend_Mail_Storage_Folder) {
            ...
            if ($parentFolder != null) {
                $folder = rtrim($parentFolder, $this->_delim) . $this->_delim .
                         $name;
            } else {
                $folder = $name;
            }
        ...
        if (strpos($folder, $this->_delim . $this->_delim) !== false) {
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'invalid name - folder parts may not be empty');
        }
        ...
        // check if we got tricked and would create a dir outside of the rootdir
        // or not as direct child
        if (strpos($folder, DIRECTORY_SEPARATOR) !== false ||
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'invalid name - no directory seprator allowed in folder name');
        }
        ...
        if (! @mkdir($fulldir) || ! @mkdir(
                $fulldir . DIRECTORY_SEPARATOR . 'cur')) {
            /**
             *
             * @see Zend_Mail_Storage_Exception
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'error while creating new folder, may be created incompletly');
        }
        ...
        $this->getFolders($parent)->$localName = new Zend_Mail_Storage_Folder(
                $localName, $folder, true);
    ...
    {
        // TODO: This could fail in the middle of the task, which is not
        // optimal.
        // But there is no defined standard way to mark a folder as removed and
        // there is no atomar fs-op
        // to remove a directory. Also moving the folder to a/the trash folder
        // is not possible, as
        // all parent folders must be created. What we could do is add a dash to
        // the front of the
        // directory name and it should be ignored as long as other processes
        // obey the standard.
        ...
        $parent = strpos($name, $this->_delim) ? substr($name, 0, 
                strrpos($name, $this->_delim)) : null;
    ...
    {
        // TODO: This is also not atomar and has similar problems as
        // removeFolder()
        ...
        if (strpos($newName, $oldName . $this->_delim) === 0) {
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'new folder cannot be a child of old folder');
        }
        
        // check if folder exists and has no children
        $folder = $this->getFolders($oldName);
        
        if ($oldName == 'INBOX' || $oldName == DIRECTORY_SEPARATOR ||
                 $oldName == '/') {
            /**
             *
             * @see Zend_Mail_Storage_Exception
        ...
        if (! $folder->isLeaf()) {
            foreach ($folder as $k => $v) {
                $this->renameFolder($v->getGlobalName(), 
                        $newName . $this->_delim . $k);
            }
        }
        ...
        foreach (array(
            ...
            // using copy or moving files would be even better - but also much
            // slower
            if (! rename($olddir . $subdir, $newdir . $subdir)) {
                ...
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new Zend_Mail_Storage_Exception(
                        'error while moving ' . $subdir);
            }
        }
        // create a dummy if removing fails - otherwise we can't read it next
        // time
     ...
     *
     * This is nearly the format defined in the maildir standard. The
     * microtime() call should already
     * create a uniqueid, the pid is for multicore/-cpu machine that manage to
     * call this function at the
     * exact same time, and uname() gives us the hostname for multiple machines
    ...
    {
        $id = '';
        $id .= function_exists('microtime') ? microtime(true) : (time() . ' ' .
                 rand(0, 100000));
        $id .= '.' .
                 (function_exists('posix_getpid') ? posix_getpid() : rand(50, 
                        65535));
        $id .= '.' . php_uname('n');
    ...
    {
        if ($folder == 'INBOX') {
            $tmpdir = $this->_rootdir . DIRECTORY_SEPARATOR . 'tmp' .
                     DIRECTORY_SEPARATOR;
        } else {
            $tmpdir = $this->_rootdir . '.' . $folder . DIRECTORY_SEPARATOR .
        ...
        if (! file_exists($tmpdir)) {
            if (! mkdir($tmpdir)) {
                ...
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new Zend_Mail_Storage_Exception(
                        'problems creating tmp dir');
            }
        }
        
        // we should retry to create a unique id if a file with the same name
        // exists
        // to avoid a script timeout we only wait 1 second (instead of 2) and
        // stop
        // after a defined retry count
        // if you change this variable take into account that it can take up to
        // $max_tries seconds
        // normally we should have a valid unique name after the first try,
        // we're just following the "standard" here
        $max_tries = 5;
        for ($i = 0; $i < $max_tries; ++ $i) {
            $uniq = $this->_createUniqueId();
            if (! file_exists($tmpdir . $uniq)) {
                // here is the race condition! - as defined in the standard
                // to avoid having a long time between stat()ing the file and
                // creating it we're opening it here
     ...
     *
     * @param array $flags
     *            wanted flags, with the reference you'll get the set flags with
     *            correct key (= char for flag)
     ...
     * @throws Zend_Mail_Storage_Exception
     */
    // not yet * @param string|Zend_Mail_Message|Zend_Mime_Message $message
            ...
            $recent = false)
    {
        ...
        // we're throwing any exception after removing our temp file and saving
        // it to this variable instead
        $exception = null;
        ...
        if (! link($temp_file['filename'], $new_filename)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new Zend_Mail_Storage_Exception(
                    'cannot link message file to final dir');
        }
    ...
    public function copyMessage ($id, $folder)
    {
        ...
        while (($key = array_search(Zend_Mail_Storage::FLAG_RECENT, $flags)) !==
                 false) {
                    unset($flags[$key]);
        }
        $info = $this->_getInfoString($flags);
        ...
        $new_file = $temp_file['dirname'] . DIRECTORY_SEPARATOR . 'cur' .
                 DIRECTORY_SEPARATOR . $temp_file['uniq'] . $info;
    ...
    public function setFlags ($id, $flags)
    {
        ...
        $new_filename = dirname(dirname($filedata['filename'])) .
                 DIRECTORY_SEPARATOR . 'cur' . DIRECTORY_SEPARATOR .
                 "$filedata[uniq]$info";
    ...
    protected function _calculateMaildirsize ()
    {
        ...
        if (is_array($this->_quota)) {
            ...
            try {
                ...
                throw new Zend_Mail_Storage_Exception(
                        'no quota definition found', 0, $e);
            }
        }
        
        $folders = new RecursiveIteratorIterator($this->getFolders(), 
        ...
        foreach ($folders as $folder) {
            ...
            foreach (array(
                ...
                $dirname = $this->_rootdir . $subdir . DIRECTORY_SEPARATOR .
                         $subsubdir . DIRECTORY_SEPARATOR;
                if (! file_exists($dirname)) {
                    continue;
                }
                // NOTE: we are using mtime instead of "the latest timestamp".
                // The latest would be atime
                // and as we are accessing the directory it would make the whole
                // calculation useless.
                $timestamps[$dirname] = filemtime($dirname);
                
                $dh = opendir($dirname);
                // NOTE: Should have been checked in constructor. Not throwing
                // an exception here, quotas will
                // therefore not be fully enforeced, but next request will fail
                // anyway, if problem persists.
                if (! $dh) {
                    continue;
                }
    ...
    protected function _calculateQuota ($forceRecalc = false)
    {
        ...
        $messages = 0;
        $maildirsize = '';
        if (! $forceRecalc && file_exists($this->_rootdir . 'maildirsize') &&
                 filesize($this->_rootdir . 'maildirsize') < 5120) {
            $fh = fopen($this->_rootdir . 'maildirsize', 'r');
        }
        if ($fh) {
        ...
        $over_quota = $over_quota ||
                 (isset($quota['size']) && $total_size > $quota['size']);
        $over_quota = $over_quota ||
                 (isset($quota['count']) && $messages > $quota['count']);
        // NOTE: $maildirsize equals false if it wasn't set (AKA we
        // recalculated) or it's only
        // one line, because $maildirsize[0] gets unsetted.
        // Also we're using local time to calculate the 15 minute offset.
        // Touching a file just for known the
        // local time of the file storage isn't worth the hassle.
        if ($over_quota &&
                 ($maildirsize ||
                 filemtime($this->_rootdir . 'maildirsize') > time() - 900)) {
            $result = $this->_calculateMaildirsize();
            $total_size = $result['size'];
            $messages = $result['count'];
            $quota = $result['quota'];
