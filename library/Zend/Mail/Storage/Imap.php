 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Mail_Storage_Imap extends Zend_Mail_Storage_Abstract implements 
        Zend_Mail_Storage_Folder_Interface, Zend_Mail_Storage_Writable_Interface
{
    // TODO: with an internal cache we could optimize this class, or create an
    // extra class with
    // such optimizations. Especially the various fetch calls could be combined
    // to one cache call
        ...
        if ($params instanceof Zend_Mail_Protocol_Imap) {
            ...
            try {
                ...
                throw new Zend_Mail_Storage_Exception(
                        'cannot select INBOX, is this a valid transport?', 0, $e);
            }
            return;
        }
    ...
    public function removeMessage ($id)
    {
        ...
        // TODO: expunge here or at close? we can handle an error here better
        // and are more fail safe
        if (! $this->_protocol->expunge()) {
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'message marked as deleted, but could not expunge');
        }
    }
        ...
        foreach ($folders as $globalName => $data) {
            do {
                if (! $parent || strpos($globalName, $parent) === 0) {
                    ...
                    $selectable = ! $data['flags'] ||
                             ! in_array('\\Noselect', $data['flags']);
                    
                    array_push($stack, $parent);
                    $parent = $globalName . $data['delim'];
                    $folder = new Zend_Mail_Storage_Folder($localName, 
                            $globalName, $selectable);
            ...
            if (! $stack) {
                ...
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new Zend_Mail_Storage_Exception(
                        'error while constructing folder tree');
            }
        }
    ...
    {
        // TODO: we assume / as the hierarchy delim - need to get that from the
        // folder class!
    ...
    public function copyMessage ($id, $folder)
    {
        if (! $this->_protocol->copy($folder, $id)) {
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'cannot copy message, does the folder exist?');
        }
    }
