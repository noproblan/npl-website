...
class Zend_Mail_Storage_Mbox extends Zend_Mail_Storage_Abstract
{
    ...
    /**
     * start and end position of messages as array('start' => start, 'seperator'
     * => headersep, 'end' => end)
     * 
     * @var array
     */
    ...
    /**
     * used message class, change it in an extened class to extend the returned
     * message class
     * 
     * @var string
     */
    ...
    public function getMessage ($id)
    {
        ...
        if (strtolower($this->_messageClass) == 'zend_mail_message_file' ||
        ...
        if ($bodyLines) {
            $message .= "\n";
            while ($bodyLines -- &&
                     ftell($this->_fh) < $this->_positions[$id - 1]['end']) {
    ...
    public function getRawHeader ($id, $part = null, $topLines = 0)
    {
        ...
        $messagePos = $this->_getPos($id);
        // TODO: toplines
        return stream_get_contents($this->_fh, 
                $messagePos['separator'] - $messagePos['start'], 
                $messagePos['start']);
    }
    ...
    public function getRawContent ($id, $part = null)
    {
        ...
        $messagePos = $this->_getPos($id);
        return stream_get_contents($this->_fh, 
                $messagePos['end'] - $messagePos['separator'], 
                $messagePos['separator']);
    }
    ...
    public function __construct ($params)
    {
        ...
        if (! isset($params->filename) /* || Zend_Loader::isReadable($params['filename']) */) {
            ...
            throw new Zend_Mail_Storage_Exception(
                    'no valid filename given in params');
        }
    ...
    protected function _openMboxFile ($filename)
    {
        ...
        if (! $this->_isMboxFile($this->_fh, false)) {
            ...
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception(
                    'file is not a valid mbox format');
        }
        ...
        while (($line = fgets($this->_fh)) !== false) {
            if (strpos($line, 'From ') === 0) {
                $messagePos['end'] = ftell($this->_fh) - strlen($line) - 2; // +
                                                                            // newline
     ...
     * message number.
     * That shouldn't be a problem, because we can't change mbox files. Therefor
     * the message
