 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Mail_Storage_Folder_Maildir extends Zend_Mail_Storage_Maildir implements 
        Zend_Mail_Storage_Folder_Interface
{
    ...
    protected function _buildFolderTree ()
    {
        ...
        $this->_rootFolder->INBOX = new Zend_Mail_Storage_Folder('INBOX', 
                'INBOX', true);
            ...
            do {
                if (strpos($dir, $parent) === 0) {
                    ...
                    if (strpos($local, $this->_delim) !== false) {
                        ...
                        require_once 'Zend/Mail/Storage/Exception.php';
                        throw new Zend_Mail_Storage_Exception(
                                'error while reading maildir');
                    }
                    array_push($stack, $parent);
                    $parent = $dir . $this->_delim;
                    $folder = new Zend_Mail_Storage_Folder($local, 
                            substr($dir, 1), true);
    ...
    public function selectFolder ($globalName)
    {
        ...
        try {
            $this->_openMaildir(
                    $this->_rootdir . '.' . $folder->getGlobalName());
