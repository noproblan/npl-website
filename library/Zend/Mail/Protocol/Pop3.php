...
class Zend_Mail_Protocol_Pop3
{
     ...
     *
     * @param string $host
     *            hostname or IP address of POP3 server, if given connect() is
     *            called
     * @param int|null $port
    ...
    public function connect ($host, $port = null, $ssl = false)
    {
        ...
        if ($ssl === 'TLS') {
            $this->request('STLS');
            $result = stream_socket_enable_crypto($this->_socket, true, 
     ...
     * supported
     *
     * The fallback makes normale RETR call, which retrieves the whole message.
     * Additional
     ...
     *            number of message
     * @param int $lines
     *            number of wanted body lines (empty line is inserted after
     *            header lines)
     * @param bool $fallback
    ...
    public function top ($msgno, $lines = 0, $fallback = false)
    {
        if ($this->hasTop === false) {
            if ($fallback) {
                ...
                require_once 'Zend/Mail/Protocol/Exception.php';
                throw new Zend_Mail_Protocol_Exception(
                        'top not supported and no fallback wanted');
            }
        }
        $this->hasTop = true;
