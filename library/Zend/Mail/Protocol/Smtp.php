...
class Zend_Mail_Protocol_Smtp extends Zend_Mail_Protocol_Abstract
{
            ...
            switch (strtolower($config['ssl'])) {
                ...
                default:
                    ...
                    throw new Zend_Mail_Protocol_Exception(
                            $config['ssl'] . ' is unsupported SSL type');
                    break;
            }
        }
        
        // If no port has been specified then check the master PHP ini file.
        // Defaults to 25 if the ini setting is null.
        if ($port == null) {
    ...
    public function connect ()
    {
        return $this->_connect(
                $this->_transport . '://' . $this->_host . ':' . $this->_port);
    }
    ...
    {
        // Respect RFC 2821 and disallow HELO attempts if session is already
        // initiated.
        if ($this->_sess === true) {
            /**
             *
        ...
        $this->_expect(220, 300); // Timeout set for 5 minutes as per RFC 2821
                                  // 4.5.3.2
        $this->_ehlo($host);
    ...
    {
        // Support for older, less-compliant remote servers. Tries multiple
        // attempts of EHLO or HELO.
        try {
            $this->_send('EHLO ' . $host);
            $this->_expect(250, 300); // Timeout set for 5 minutes as per RFC
                                      // 2821 4.5.3.2
    ...
    public function mail ($from)
    {
        ...
        $this->_expect(250, 300); // Timeout set for 5 minutes as per RFC 2821
                                  // 4.5.3.2
                                  
        // Set mail to true, clear recipients and any existing data flags as per
                                  // 4.1.1.2 of RFC 2821
        $this->_mail = true;
        $this->_rcpt = false;
        $this->_data = false;
    ...
    public function rcpt ($to)
    {
        ...
        $this->_expect(array(
                250,
                251
        ), 300); // Timeout set for 5 minutes as
                                              // per RFC 2821 4.5.3.2
        $this->_rcpt = true;
    }
    ...
    public function data ($data)
    {
        ...
        $this->_send('DATA');
        $this->_expect(354, 120); // Timeout set for 2 minutes as per RFC 2821
                                  // 4.5.3.2
        ...
        $this->_send('.');
        $this->_expect(250, 600); // Timeout set for 10 minutes as per RFC 2821
                                  // 4.5.3.2
        $this->_data = true;
    }
     ...
     *
     * Not used by Zend_Mail, could be used to keep a connection alive or check
     * if it is still open.
     *
     * @return void
     */
    ...
    public function vrfy ($user)
    {
        ...
        $this->_expect(array(
                ...
                251,
                252
        ), 300); // Timeout set for 5 minutes
                                                   // as per RFC 2821 4.5.3.2
    }
    ...
    public function quit ()
    {
        if ($this->_sess) {
            $this->_send('QUIT');
            $this->_expect(221, 300); // Timeout set for 5 minutes as per RFC
                                      // 2821 4.5.3.2
            $this->_stopSession();
        }
    }
