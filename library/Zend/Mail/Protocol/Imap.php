...
class Zend_Mail_Protocol_Imap
{
     ...
     *
     * @param string $host
     *            hostname or IP address of IMAP server, if given connect() is
     *            called
     * @param int|null $port
    ...
    public function connect ($host, $port = null, $ssl = false)
    {
        ...
        if (! $this->_socket) {
            ...
            throw new Zend_Mail_Protocol_Exception(
                    'cannot connect to host; error = ' . $errstr . ' (errno = ' .
                             $errno . ' )');
        }
    ...
    protected function _decodeLine ($line)
    {
         ...
         * {bytes}<NL>literal
         * (literals*)
         * All tokens are returned in an array. Literals in braces (the last
         * unterstood
         * token in the list) are returned as an array of tokens. I.e. the
         * following response:
         * "foo" baz {3}<NL>bar ("f\\\"oo" bar)
         * would be returned as:
         * array('foo', 'baz', 'bar', array('f\\\"oo', 'bar'));
         *
         * // TODO: add handling of '[' and ']' to parser for easier handling of
         * response text
         */
        ...
        while (($pos = strpos($line, ' ')) !== false) {
            ...
            if ($stack && $token[strlen($token) - 1] == ')') {
                // closing braces are not seperated by spaces, so we need to
                // count them
                $braces = strlen($token);
                $token = rtrim($token, ')');
     ...
     * @param array $tokens
     *            additional parameters to command, use escapeString() to
     *            prepare
     * @param string $tag
    ...
    {
        return $this->requestAndResponse('LOGIN', 
                $this->escapeString($user, $password), true);
    }
     ...
     *         returned as string
     *         if items of one message are fetched it's returned as (name =>
     *         value)
     *         if one items of messages are fetched it's returned as (msgno =>
     *         value)
    ...
    public function fetch ($items, $from, $to = null)
    {
        ...
        while (! $this->readLine($tokens, $tag)) {
            ...
            // if we want only one message we can ignore everything else and
            // just return
            if ($to === null && ! is_array($from) && $tokens[0] == $from) {
        ...
        if ($to === null && ! is_array($from)) {
            ...
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Zend_Mail_Protocol_Exception(
                    'the single id was not found in response');
        }
    ...
    {
        $result = array();
        $list = $this->requestAndResponse('LIST', 
                $this->escapeString($reference, $mailbox));
        if (! $list || $list === true) {
            return $result;
        }
     ...
     *            '+' to add flags, '-' to remove flags, everything else sets
     *            the flags as given
     * @param bool $silent
     *            if false the return values are the new flags for the wanted
     *            messages
    ...
    public function copy ($folder, $from, $to = null)
    {
        ...
        return $this->requestAndResponse('COPY', 
                array(
                        $set,
    ...
    {
        return $this->requestAndResponse('CREATE', 
                array(
                        $this->escapeString($folder)
                ), true);
    }
    ...
    {
        return $this->requestAndResponse('RENAME', 
                $this->escapeString($old, $new), true);
    }
    ...
    {
        return $this->requestAndResponse('DELETE', 
                array(
                        $this->escapeString($folder)
                ), true);
    }
     ...
     * do a search request
     *
     * This method is currently marked as internal as the API might change and
     * is not
