 ...
 *
 * Provides low-level methods for concrete adapters to communicate with a remote
 * mail server and track requests and responses.
 *
 * @category Zend
 * @package Zend_Mail
 * @subpackage Protocol
...
abstract class Zend_Mail_Protocol_Abstract
{
    ...
    /**
     * String template for parsing server responses using sscanf (default: 3
     * digit code and response string)
     * 
     * @var resource
    ...
    protected function _connect ($remote)
    {
        ...
        if (($result = $this->_setStreamTimeout(self::TIMEOUT_CONNECTION)) ===
                 false) {
            /**
             *
             * @see Zend_Mail_Protocol_Exception
    ...
    protected function _send ($request)
    {
        if (! is_resource($this->_socket)) {
            ...
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Zend_Mail_Protocol_Exception(
                    'No connection has been established to ' . $this->_host);
        }
        ...
        if ($result === false) {
            ...
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Zend_Mail_Protocol_Exception(
                    'Could not send request to ' . $this->_host);
        }
    ...
    protected function _receive ($timeout = null)
    {
        ...
        // Adapters may wish to supply per-commend timeouts according to
        // appropriate RFC
        if ($timeout !== null) {
        ...
        if ($reponse === false) {
            ...
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Zend_Mail_Protocol_Exception(
                    'Could not read from ' . $this->_host);
        }
    ...
    protected function _expect ($code, $timeout = null)
    {
        ...
        do {
            ...
            list ($cmd, $more, $msg) = preg_split('/([\s-]+)/', $result, 2, 
        ...
        } while (strpos($more, '-') === 0); // The '-' message prefix indicates
                                            // an information string instead of
                                            // a response string.
