...
class Zend_Mail_Part implements RecursiveIterator, Zend_Mail_Part_Interface
{
    ...
    public function __construct (array $params)
    {
        if (isset($params['handler'])) {
            if (! $params['handler'] instanceof Zend_Mail_Storage_Abstract) {
                ...
                throw new Zend_Mail_Exception(
                        'handler is not a valid mail handler');
            }
            if (! isset($params['id'])) {
                /**
                 *
        ...
        if (isset($params['raw'])) {
            Zend_Mime_Decode::splitMessage($params['raw'], $this->_headers, 
                    $this->_content);
        } else 
            if (isset($params['headers'])) {
                if (is_array($params['headers'])) {
                    ...
                    if (! empty($params['noToplines'])) {
                        Zend_Mime_Decode::splitMessage($params['headers'], 
                                $this->_headers, $null);
                    } else {
                        Zend_Mime_Decode::splitMessage($params['headers'], 
                                $this->_headers, $this->_topLines);
                    }
                }
    ...
    protected function _cacheContent ()
    {
        ...
        if (! $boundary) {
            ...
            require_once 'Zend/Mail/Exception.php';
            throw new Zend_Mail_Exception(
                    'no boundary found in content type to split message');
        }
     ...
     * other as string. If $format
     * is set to string implode is used to concat the values (with
     * Zend_Mime::LINEEND as delim).
     *
     * @param string $name
     *            name of header, matches case-insensitive, but camel-case is
    ...
    public function getHeader ($name, $format = null)
    {
        ...
        if ($this->headerExists($name) == false) {
            ...
            if ($this->headerExists($lowerName) == false) {
                ...
                require_once 'Zend/Mail/Exception.php';
                throw new Zend_Mail_Exception(
                        "no Header with Name $name or $lowerName found");
            }
        }
        $name = $lowerName;
     ...
     *            the wanted part, default is first, if null an array with all
     *            parts is returned
    ...
    {
        return Zend_Mime_Decode::splitHeaderField(
                current($this->getHeader($name, 'array')), $wantedPart, 
                $firstName);
    }
    ...
    public function hasChildren ()
    {
        ...
        return $current && $current instanceof Zend_Mail_Part &&
                 $current->isMultipart();
    }
