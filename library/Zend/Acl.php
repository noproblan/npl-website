...
class Zend_Acl
{
     ...
     * over
     * parents that were previously added. In other words, the first parent
     * added
    ...
    public function deny ($roles = null, $resources = null, $privileges = null, 
            Zend_Acl_Assert_Interface $assert = null)
    {
        return $this->setRule(self::OP_ADD, self::TYPE_DENY, $roles, $resources, 
                $privileges, $assert);
    }
     ...
     * OP_REMOVE specifics:
     *
     * The rule is removed only in the context of the given Roles, Resources,
     * and privileges.
     * Existing rules to which the remove operation does not apply would remain
     * in the
     * ACL.
     *
     ...
     * and objects is ok - to indicate the Resources and Roles to which the rule
     * applies. If either
     ...
     * privilege with a string, and multiple privileges may be specified as an
     * array of strings.
     *
     * If $assert is provided, then its assert() method must return true in
     * order for
     * the rule to apply. If $assert is provided with $roles, $resources, and
     * $privileges all
    ...
    public function setRule ($operation, $type, $roles = null, $resources = null, 
            $privileges = null, Zend_Acl_Assert_Interface $assert = null)
    {
        ...
        if ($resources !== null) {
            ...
            $allResources = array(); // this might be used later if resource
        ...
        switch ($operation) {
            ...
            case self::OP_REMOVE:
                if ($resources !== null) {
                    ...
                    foreach ($roles as $role) {
                        /**
                         * since null (all resources) was passed to this
                         * setRule() call, we need
                         * clean up all the rules for the global allResources,
                         * as well as the indivually
     ...
     * or all Resources,
     * respectively. Both may be null to query whether the ACL has a "blacklist"
     * rule
     * (allow everything to all). By default, Zend_Acl creates a "whitelist"
     * rule (deny
     ...
     * only if the
     * Role is denied access to at least one privilege upon the Resource. In
     * other words, this
     * method returns true if and only if the Role is allowed all privileges on
     * the Resource.
     *
     * This method checks Role inheritance using a depth-first traversal of the
     * Role registry.
     * The highest priority parent (i.e., the parent most recently added) is
     * checked first,
    ...
    public function isAllowed ($role = null, $resource = null, $privilege = null)
    {
        ...
        if (null === $privilege) {
            ...
            do {
                ...
                if (null !== $role &&
                         null !==
                         ($result = $this->_roleDFSAllPrivileges($role, 
                                $resource, $privilege))) {
                    return $result;
                }
                ...
                if (null !== ($rules = $this->_getRules($resource, null))) {
                    foreach ($rules['byPrivilegeId'] as $privilege => $rule) {
                        if (self::TYPE_DENY ===
                                 ($ruleTypeOnePrivilege = $this->_getRuleType(
                                        $resource, null, $privilege))) {
                            return false;
                        }
                    }
                    if (null !==
                             ($ruleTypeAllPrivileges = $this->_getRuleType(
                                    $resource, null, null))) {
            ...
            do {
                ...
                if (null !== $role &&
                         null !==
                         ($result = $this->_roleDFSOnePrivilege($role, 
                                $resource, $privilege))) {
                    return $result;
                }
                ...
                if (null !==
                         ($ruleType = $this->_getRuleType($resource, null, 
                                $privilege))) {
     ...
     * order to find a rule
     * allowing/denying $role access to all privileges upon $resource
     *
     * This method returns true if a rule is found and allows access. If a rule
     * exists and denies access,
     * then this method returns false. If no applicable rule is found, then this
     * method returns null.
     *
     * @param Zend_Acl_Role_Interface $role            
     * @param Zend_Acl_Resource_Interface $resource            
     * @return boolean|null
     */
    protected function _roleDFSAllPrivileges (Zend_Acl_Role_Interface $role, 
            Zend_Acl_Resource_Interface $resource = null)
    {
        ...
        if (null !==
                 ($result = $this->_roleDFSVisitAllPrivileges($role, $resource, 
                        $dfs))) {
            return $result;
        }
        ...
        while (null !== ($role = array_pop($dfs['stack']))) {
            if (! isset($dfs['visited'][$role->getRoleId()])) {
                if (null !==
                         ($result = $this->_roleDFSVisitAllPrivileges($role, 
                                $resource, $dfs))) {
                    return $result;
                }
            }
            ...
            Zend_Acl_Resource_Interface $resource = null, &$dfs = null)
    {
        ...
        if (null !== ($rules = $this->_getRules($resource, $role))) {
            foreach ($rules['byPrivilegeId'] as $privilege => $rule) {
                if (self::TYPE_DENY ===
                         ($ruleTypeOnePrivilege = $this->_getRuleType($resource, 
                                $role, $privilege))) {
                    return false;
                }
            }
            if (null !==
                     ($ruleTypeAllPrivileges = $this->_getRuleType($resource, 
                            $role, null))) {
                return self::TYPE_ALLOW === $ruleTypeAllPrivileges;
            }
        }
            ...
            Zend_Acl_Resource_Interface $resource = null, $privilege = null)
    {
        ...
        if (null !==
                 ($result = $this->_roleDFSVisitOnePrivilege($role, $resource, 
                        $privilege, $dfs))) {
            return $result;
        }
        ...
        while (null !== ($role = array_pop($dfs['stack']))) {
            if (! isset($dfs['visited'][$role->getRoleId()])) {
                if (null !==
                         ($result = $this->_roleDFSVisitOnePrivilege($role, 
     ...
     * @throws Zend_Acl_Exception
     */
    protected function _roleDFSVisitOnePrivilege (Zend_Acl_Role_Interface $role, 
            Zend_Acl_Resource_Interface $resource = null, $privilege = null, 
            &$dfs = null)
    {
        if (null === $privilege) {
        ...
        if (null !==
                 ($ruleTypeOnePrivilege = $this->_getRuleType($resource, $role, 
                        $privilege))) {
    ...
    protected function _getRuleType (
            Zend_Acl_Resource_Interface $resource = null, 
            Zend_Acl_Role_Interface $role = null, $privilege = null)
    {
        ...
        if ($rule['assert']) {
            ...
            $assertionValue = $assertion->assert($this, 
     ...
     * are for all Resources or all Roles,
     * respectively. Both can be null to return the default rule set for all
     * Resources and all Roles.
     *
     ...
     * @return array|null
     */
    protected function &_getRules (Zend_Acl_Resource_Interface $resource = null, 
            Zend_Acl_Role_Interface $role = null, $create = false)
    {
