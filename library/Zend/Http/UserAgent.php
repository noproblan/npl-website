...
class Zend_Http_UserAgent implements Serializable
{
    ...
    /**
     * 'desktop' by default if the sequence return false for each item or is
     * empty
     */
     ...
     *
     * The comparison is case-insensitive : the browser signatures must be in
     * lower
     * case
     *
     ...
     * - the identification sequence (can be empty) => desktop browser type is
     * the
        ...
        if (! is_array($config) && ! $config instanceof Traversable) {
            ...
            throw new Zend_Http_UserAgent_Exception(
                    sprintf(
                            ...
                            (is_object($config) ? get_class($config) : gettype(
                                    $config))));
        }
        ...
        if (! is_array($server) && ! $server instanceof Traversable) {
            ...
            throw new Zend_Http_UserAgent_Exception(
                    sprintf(
                            ...
                            (is_object($server) ? get_class($server) : gettype(
                                    $server))));
        }
    ...
    /**
     * Run the identification sequence to match the right browser type according
     * to the
     * user agent
     *
    ...
    protected function _matchUserAgent ()
    {
        ...
        // If a browser type is already configured, push that to the front of
        // the list
    ...
    protected function _createDevice ()
    {
        ...
        $classname = $this->_getUserAgentDevice($browserType);
        $this->_device = new $classname($this->getUserAgent(), 
                $this->getServer(), $this->getConfig());
    }
}
