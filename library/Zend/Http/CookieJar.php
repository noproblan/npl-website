...
/**
 * A Zend_Http_CookieJar object is designed to contain and maintain HTTP
 * cookies, and should
 * be used along with Zend_Http_Client in order to manage cookies across HTTP
 * requests and
 * responses.
 *
 * The class contains an array of Zend_Http_Cookie objects. Cookies can be added
 * to the jar
 * automatically from a request or manually. Then, the jar can find and return
 * the cookies
 * needed for a specific HTTP request.
 *
 * A special parameter can be passed to all methods of this class that return
 * cookies: Cookies
 * can be returned either in their native form (as Zend_Http_Cookie objects) or
 * as strings -
 * the later is suitable for sending as the value of the "Cookie" header in an
 * HTTP request.
 * You can also choose, when returning more than one cookie, whether to get an
 * array of strings
 * (by passing Zend_Http_CookieJar::COOKIE_STRING_ARRAY) or one unified string
 * for all cookies
...
class Zend_Http_CookieJar implements Countable, IteratorAggregate
{
     ...
     *
     * @param int $ret_as
     *            Whether to return cookies as objects of Zend_Http_Cookie or as
     *            strings
     * @return array|string
     */
     ...
     * the request URI,
     * whether session cookies should be sent or not, and the time to consider
     * as "now" when
            ...
            $ret_as = self::COOKIE_OBJECT, $now = null)
    {
        ...
        // First, reduce the array of cookies to only those matching domain and
        // path
        $cookies = $this->_matchDomain($uri->getHost());
        $cookies = $this->_matchPath($cookies, $uri->getPath());
        $cookies = $this->_flattenCookiesArray($cookies, self::COOKIE_OBJECT);
        
        // Next, run Cookie->match on all cookies to check secure, time and
        // session mathcing
        $ret = array();
        foreach ($cookies as $cookie)
            ...
            // Now, use self::_flattenCookiesArray again - only to convert to
        // the return format ;)
