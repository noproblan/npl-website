...
/**
 * Zend_Http_Cookie is a class describing an HTTP cookie and all it's
 * parameters.
 *
 * Zend_Http_Cookie is a class describing an HTTP cookie and all it's
 * parameters. The
 * class also enables validating whether the cookie should be sent to the server
 * in
 * a specified scenario according to the request URI, the expiry time and
 * whether
 * session cookies should be used or not. Generally speaking cookies should be
 * contained in a Cookiejar object, or instantiated manually and added to an
 * HTTP
 * request.
 *
...
class Zend_Http_Cookie
{
     ...
     * @param bool $secure            
     */
    public function __construct ($name, $value, $domain, $expires = null, 
            $path = null, $secure = false)
    {
        if (preg_match("/[=,; \t\r\n\013\014]/", $name)) {
     ...
     *
     * Always returns false if the cookie is a session cookie (has no expiry
     * time)
     *
     * @param int $now
    ...
    public function match ($uri, $matchSessionCookies = true, $now = null)
    {
        ...
        if (! ($uri->valid() &&
            ...
            require_once 'Zend/Http/Exception.php';
            throw new Zend_Http_Exception(
                    'Passed URI is not a valid HTTP or HTTPS URI');
        }
    ...
    /**
     * Get the cookie as a string, suitable for sending as a "Cookie" header in
     * an
     * HTTP request
     *
     * @return string
     ...
     * @return Zend_Http_Cookie A new Zend_Http_Cookie object or false on
     *         failure.
     */
    public static function fromString ($cookieStr, $refUri = null, 
            $encodeValue = true)
    {
        // Set default values
        if (is_string($refUri)) {
                ...
                switch (strtolower($k)) {
                    case 'expires':
                        if (($expires = strtotime($v)) === false) {
                            /**
                             * The expiration is past Tue, 19 Jan 2038 03:14:07
                             * UTC
    ...
    public static function matchCookieDomain ($cookieDomain, $host)
    {
        if (! $cookieDomain) {
            require_once 'Zend/Http/Exception.php';
            throw new Zend_Http_Exception(
                    "\$cookieDomain is expected to be a cookie domain");
        }
    ...
    public static function matchCookiePath ($cookiePath, $path)
    {
        if (! $cookiePath) {
            require_once 'Zend/Http/Exception.php';
            throw new Zend_Http_Exception(
                    "\$cookiePath is expected to be a cookie path");
        }
