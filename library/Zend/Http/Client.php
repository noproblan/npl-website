...
class Zend_Http_Client
{
     ...
     * $this->auth = array('user' => 'username', 'password' => 'password',
     * 'type' => 'basic')
     * Where 'type' should be one of the supported authentication types (see the
     * AUTH_*
     ...
     * 'name' => array('filename.txt', 'text/plain', 'This is the actual file
     * contents')
     *
     * @var array
     */
    ...
    public function setMethod ($method = self::GET)
    {
        ...
        if (($method == self::POST || $method == self::PUT ||
                 $method == self::DELETE) && $this->enctype === null) {
    ...
    /**
     * Set a GET or POST parameter - used by SetParameterGet and
     * SetParameterPost
     *
     * @param string $type
     *            GET or POST
     ...
     *
     * A cookie jar is an object that holds and maintains cookies across HTTP
     * requests
     * and responses.
     *
     * @param Zend_Http_CookieJar|boolean $cookiejar
     *            Existing cookiejar object, true to create a new one, false to
     *            disable
     * @return Zend_Http_Client
     * @throws Zend_Http_Client_Exception
     */
        ...
        if (isset($this->cookiejar)) {
            if ($cookie instanceof Zend_Http_Cookie) {
                ...
                $cookie = Zend_Http_Cookie::fromString("{$cookie}={$value}", 
                        $this->uri, $this->config['encodecookies']);
                $this->cookiejar->addCookie($cookie);
            }
        } else {
            if ($cookie instanceof Zend_Http_Cookie) {
     ...
     * file which
     * will be read and sent. Will try to guess the content type using
     * mime_content_type().
     * 2. $data is set - $filename is sent as the file name, but $data is sent
     * as the file
     * contents and no file is read from the file system. In this case, you need
     * to
    ...
    public function setFileUpload ($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            if (($data = @file_get_contents($filename)) === false) {
                ...
                require_once 'Zend/Http/Client/Exception.php';
                throw new Zend_Http_Client_Exception(
                        "Unable to read file '{$filename}' for upload");
            }
     ...
     * 1. For advanced user who would like to set their own data, already
     * encoded
     * 2. For backwards compatibilty: If someone uses the old post($data)
     * method.
    ...
    public function setAdapter ($adapter)
    {
        if (is_string($adapter)) {
            try {
                ...
                require_once 'Zend/Http/Client/Exception.php';
                throw new Zend_Http_Client_Exception(
                        "Unable to load adapter '$adapter': {$e->getMessage()}", 
                        0, $e);
            }
    ...
    protected function _openTempStream ()
    {
        ...
        if (false === ($fp = @fopen($this->_stream_name, "w+b"))) {
            ...
            require_once 'Zend/Http/Client/Exception.php';
            throw new Zend_Http_Client_Exception(
                    "Could not open temp file {$this->_stream_name}");
        }
    ...
    public function request ($method = null)
    {
        ...
        do {
            ...
            if (! empty($this->paramsGet)) {
                ...
                // @see ZF-11671 to unmask for some services to
                // foo=val1&foo=val2
                if ($this->getUnmaskStatus()) {
                    if ($this->_queryBracketsEscaped) {
                        $query = preg_replace('/%5B(?:[0-9]|[1-9][0-9]+)%5D=/', 
                                '=', $query);
                    } else {
                        $query = preg_replace('/\\[(?:[0-9]|[1-9][0-9]+)\\]=/', 
                                '=', $query);
                    }
                }
            ...
            $this->last_request = $this->adapter->write($this->method, $uri, 
                    $this->config['httpversion'], $headers, $body);
            ...
            if ($this->config['output_stream']) {
                ...
                $response = Zend_Http_Response_Stream::fromStream($response, 
                        $stream);
            ...
            if ($response->isRedirect() &&
                ...
                // Check whether we send the exact same request again, or drop
                // the parameters
                // and send a GET request
                if ($response->getStatus() == 303 || ((! $this->config['strictredirects']) && ($response->getStatus() ==
                         302 || $response->getStatus() == 301))) {
                ...
                if (($scheme = substr($location, 0, 6)) &&
                    ...
                    if (strpos($location, '/') === 0) {
                        ...
                        // Get the current path directory, removing any trailing
                        // slashes
    ...
    protected function _prepareHeaders ()
    {
        ...
        if (! isset($this->headers['host'])) {
            ...
            if (! (($this->uri->getScheme() == 'http' &&
                     $this->uri->getPort() == 80) ||
                     ($this->uri->getScheme() == 'https' &&
                     $this->uri->getPort() == 443))) {
        ...
        if (($this->method == self::POST || $this->method == self::PUT) &&
                 (! isset($this->headers[strtolower(self::CONTENT_TYPE)]) &&
                 isset($this->enctype))) {
        ...
        if (! isset($this->headers['user-agent']) &&
                 isset($this->config['useragent'])) {
            $headers[] = "User-Agent: {$this->config['useragent']}";
        }
        
        // Set HTTP authentication if needed
        if (is_array($this->auth)) {
            $auth = self::encodeAuthHeader($this->auth['user'], 
                    $this->auth['password'], $this->auth['type']);
            $headers[] = "Authorization: {$auth}";
        }
        
        // Load cookies from cookie jar
        if (isset($this->cookiejar)) {
            $cookstr = $this->cookiejar->getMatchingCookies($this->uri, true, 
        ...
        if (count($this->paramsPost) > 0 || count($this->files) > 0) {
            switch ($this->enctype) {
                case self::ENC_FORMDATA:
                    ...
                    $this->setHeaders(self::CONTENT_TYPE, 
                            self::ENC_FORMDATA . "; boundary={$boundary}");
                    
                    // Encode all files and POST vars in the order they were
                    // given
                            ...
                            case self::VTYPE_SCALAR:
                                if (isset($this->paramsPost[$fieldName])) {
                                    if (is_array($this->paramsPost[$fieldName])) {
                                        $flattened = self::_flattenParametersArray(
                                                $this->paramsPost[$fieldName], 
                                                $fieldName);
                                        foreach ($flattened as $pp) {
                                            $body .= self::encodeFormData(
                                                    $boundary, $pp[0], $pp[1]);
                                        }
                                    } else {
                                        $body .= self::encodeFormData($boundary, 
     ...
     * @param string $filename            
     * @param array $headers
     *            Associative array of optional headers @example
     *            ("Content-Transfer-Encoding" => "binary")
     * @return string
     */
    public static function encodeFormData ($boundary, $name, $value, 
            $filename = null, $headers = array())
    {
     ...
     * @throws Zend_Http_Client_Exception
     */
    public static function encodeAuthHeader ($user, $password, 
        ...
        foreach ($parray as $name => $value) {
            ...
            if (is_array($value)) {
                $parameters = array_merge($parameters, 
                        self::_flattenParametersArray($value, $key));
            } else {
                $parameters[] = array(
                        $key,
                        $value
