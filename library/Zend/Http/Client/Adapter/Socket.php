        ...
        Zend_Http_Client_Adapter_Interface, Zend_Http_Client_Adapter_Stream
{
    ...
    {
        // If the URI should be accessed via SSL, prepend the Hostname with
        // ssl://
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            ...
            if ($secure || $this->config['sslusecontext']) {
                if ($this->config['sslcert'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 
                            'local_cert', $this->config['sslcert'])) {
                        require_once 'Zend/Http/Client/Adapter/Exception.php';
                        throw new Zend_Http_Client_Adapter_Exception(
            ...
            $this->socket = @stream_socket_client($host . ':' . $port, $errno, 
                    $errstr, (int) $this->config['timeout'], $flags, $context);
    ...
    public function read ()
    {
        ...
        if ($statusCode == 100 || $statusCode == 101)
            ...
            // Check headers to see what kind of connection / transfer encoding
        // we have
...
if (isset($headers['transfer-encoding'])) {
    ...
    if (strtolower($headers['transfer-encoding']) == 'chunked') {
        ...
        do {
            ...
            do {
                ...
                if ($this->out_stream) {
                    if (stream_copy_to_stream($this->socket, $this->out_stream, 
                            $read_to - $current_pos) == 0) {
                        $this->_checkSocketReadTimeout();
                        break;
                    }
                } else {
        ...
        require_once 'Zend/Http/Client/Adapter/Exception.php';
        throw new Zend_Http_Client_Adapter_Exception(
                'Cannot handle "' . $headers['transfer-encoding'] .
                         '" transfer encoding');
    }
    ...
    if (is_array($headers['content-length'])) {
        $contentLength = $headers['content-length'][count(
    ...
    for ($read_to = $current_pos + $contentLength; $read_to > $current_pos; $current_pos = ftell(
            $this->socket)) {
