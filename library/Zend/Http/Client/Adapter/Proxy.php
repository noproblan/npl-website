...
class Zend_Http_Client_Adapter_Proxy extends Zend_Http_Client_Adapter_Socket
{
    ...
    public function connect ($host, $port = 80, $secure = false)
    {
        ...
        return parent::connect($this->config['proxy_host'], 
                $this->config['proxy_port'], false);
    }
    ...
    public function write ($method, $uri, $http_ver = '1.1', $headers = array(), $body = '')
    {
        ...
        if ($this->config['proxy_user'] &&
            ...
            $headers['proxy-authorization'] = Zend_Http_Client::encodeAuthHeader(
                    $this->config['proxy_user'], $this->config['proxy_pass'], 
                    $this->config['proxy_auth']);
        }
        
        // if we are proxying HTTPS, preform CONNECT handshake with the proxy
        if ($uri->getScheme() == 'https' && (! $this->negotiated)) {
            $this->connectHandshake($uri->getHost(), $uri->getPort(), $http_ver, 
                    $headers);
     ...
     * @param array $headers            
     */
    protected function connectHandshake ($host, $port = 443, $http_ver = '1.1', 
            array &$headers = array())
    {
