 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
abstract class Zend_Http_UserAgent_AbstractDevice implements 
        Zend_Http_UserAgent_Device
{
     ...
     * @return void
     */
    public function __construct ($userAgent = null, array $server = array(), 
            array $config = array())
    {
    ...
    /**
     * Sets all the standard features extracted from the User Agent chain and
     * $this->_server
     * vars
     *
     * @return void
    ...
    protected function _getDefaultFeatures ()
    {
        ...
        $this->setFeature('is_' . strtolower($this->getType()), true, 
                'product_info');
        ...
        if (isset($this->list) && empty($this->_browser)) {
            ...
            foreach ($this->list as $browser_signature) {
                if (strpos($lowerUserAgent, $browser_signature) !== false) {
                    ...
                    $this->setFeature('browser_name', $this->_browser, 
                            'product_info');
                }
            }
        }
        ...
        if (isset($this->_server['remote_addr'])) {
            $this->setFeature('client_ip', $this->_server['remote_addr'], 
                    'product_info');
        } elseif (isset($this->_server['http_x_forwarded_for'])) {
            $this->setFeature('client_ip', 
                    $this->_server['http_x_forwarded_for'], 'product_info');
        } elseif (isset($this->_server['http_client_ip'])) {
            $this->setFeature('client_ip', $this->_server['http_client_ip'], 
                    'product_info');
        }
        
        /* sets the server infos */
        if (isset($this->_server['server_software'])) {
            if (strpos($this->_server['server_software'], 'Apache') !== false ||
                     strpos($this->_server['server_software'], 'LiteSpeed') !==
                     false) {
                $server['version'] = 1;
                if (strpos($this->_server['server_software'], 'Apache/2') !==
                         false) {
                    $server['version'] = 2;
                }
            ...
            if (strpos($this->_server['server_software'], 'Microsoft-IIS') !==
                     false) {
                $server['server'] = 'iis';
            }
            ...
            if (preg_match('/Apache\/([0-9\.]*)/', 
                    $this->_server['server_software'], $arr)) {
                if ($arr[1]) {
        ...
        if (isset($server['version'])) {
            $this->setFeature('server_os_version', $server['version'], 
                    'server_info');
        }
        if (isset($this->_server['http_accept'])) {
            $this->setFeature('server_http_accept', 
                    $this->_server['http_accept'], 'server_info');
        }
        if (isset($this->_server['http_accept_language'])) {
            $this->setFeature('server_http_accept_language', 
                    $this->_server['http_accept_language'], 'server_info');
        }
        if (isset($this->_server['server_addr'])) {
            $this->setFeature('server_ip', $this->_server['server_addr'], 
                    'server_info');
        }
        if (isset($this->_server['server_name'])) {
            $this->setFeature('server_name', $this->_server['server_name'], 
                    'server_info');
        }
    }
    ...
    public static function extractFromUserAgent ($userAgent)
    {
        ...
        if (empty($result['device_os_token']) &&
                 ! empty($result['compatibility_flag'])) {
        ...
        if (isset($result['compatibility_flag']) && isset($result['comment'])) {
            $compatibleOrIe = ($result['compatibility_flag'] == 'compatible' ||
                     strpos($result['comment']['full'], "MSIE") !== false);
        }
        if ($product == 'mozilla' && $compatibleOrIe) {
            if (! empty($result['browser_token'])) {
                // Classic Mozilla chain
                preg_match_all('/([^\/\s].*)(\/|\s)(.*)/i', 
                        $result['browser_token'], $real);
            } else {
        ...
        if (isset($result['others'])) {
            if ($result['others']['detail'][0][1] == 'AppleWebKit') {
                ...
                if (isset($result['others']['detail'][1]) &&
                         $result['others']['detail'][1][1] == 'Version') {
                    $result['browser_version'] = $result['others']['detail'][1][2];
                } else {
                    $result['browser_version'] = $result['others']['detail'][count(
                            $result['others']['detail']) - 1][2];
                }
                if (isset($comment[3])) {
                    $result['browser_language'] = trim($comment[3]);
                }
                
                $last = $result['others']['detail'][count(
                        $result['others']['detail']) - 1][1];
                
                if (empty($result['others']['detail'][2][1]) ||
                         $result['others']['detail'][2][1] == 'Safari') {
                    if (isset($result['others']['detail'][1])) {
                        $result['browser_name'] = ($result['others']['detail'][1][1] &&
                         $result['others']['detail'][1][1] != 'Version' ? $result['others']['detail'][1][1] : 'Safari');
                ...
                $result['browser_name'] = ($result['others']['detail'][0][1] &&
                         $result['others']['detail'][0][1] != 'Version' ? $result['others']['detail'][0][1] : 'Safari');
                $result['browser_version'] = $result['others']['detail'][0][2];
            }
        } else {
        ...
        if (strpos($result['browser_version'], '.') > 2 ||
                 (int) $result['browser_version'] > 20) {
    ...
    if ($result['others']['detail'][0][1] == 'Gecko') {
        $searchRV = true;
        if (! empty($result['others']['detail'][1][1]) &&
                 ! empty(
                        $result['others']['detail'][count(
                                $result['others']['detail']) - 1][2]) ||
                 strpos(strtolower($result['others']['full']), 'opera') !== false) {
            $searchRV = false;
            ...
            // Netscape
            if ($result['browser_name'] == 'Navigator' ||
                     $result['browser_name'] == 'Netscape6') {
                $result['browser_name'] = 'Netscape';
            }
        }
        if ($searchRV) {
...
if (isset($result['browser_name']) && isset($result['browser_engine'])) {
    if ($result['browser_name'] == 'Opera' &&
             $result['browser_engine'] == 'Gecko' &&
             empty($result['browser_version'])) {
        $result['browser_version'] = $result['others']['detail'][count(
                $result['others']['detail']) - 1][1];
    }
}
...
if (empty($config['classname'])) {
    ...
    throw new Zend_Http_UserAgent_Exception(
            'The ' . $this->getType() .
                     ' features adapter must have a "classname" config parameter defined');
}
...
if (! class_exists($className)) {
    if (isset($config['path'])) {
        ...
        throw new Zend_Http_UserAgent_Exception(
                'The ' . $this->getType() .
                         ' features adapter must have a "path" config parameter defined');
    }
    ...
    if (false === include_once ($path)) {
        ...
        throw new Zend_Http_UserAgent_Exception(
                'The ' . $this->getType() .
                         ' features adapter path that does not exist');
    }
}
