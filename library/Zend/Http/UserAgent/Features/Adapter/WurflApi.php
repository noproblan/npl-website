 ...
 * Download : http://sourceforge.net/projects/wurfl/files/WURFL
 * PHP/1.1/wurfl-php-1.1.tar.gz/download
 *
 * @package Zend_Http
 * @subpackage UserAgent
 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Http_UserAgent_Features_Adapter_WurflApi implements 
        Zend_Http_UserAgent_Features_Adapter
{
    ...
    public static function getFromRequest ($request, array $config)
    {
        ...
        if (empty($config['wurfl_lib_dir'])) {
            require_once 'Zend/Http/UserAgent/Features/Exception.php';
            throw new Zend_Http_UserAgent_Features_Exception(
                    'The "wurfl_lib_dir" parameter is not defined');
        }
        if (empty($config['wurfl_config_file']) &&
                 empty($config['wurfl_config_array'])) {
            require_once 'Zend/Http/UserAgent/Features/Exception.php';
            throw new Zend_Http_UserAgent_Features_Exception(
                    'The "wurfl_config_file" parameter is not defined');
        }
        ...
        switch ($config['wurfl_api_version']) {
            case '1.0':
                ...
                // Zend_Http_UserAgent::$config['wurfl_config_file'] must be an
                // XML file
                require_once ($config['wurfl_lib_dir'] .
                         'WURFLManagerProvider.php');
                $wurflManager = WURFL_WURFLManagerProvider::getWURFLManager(
                        Zend_Http_UserAgent::$config['wurfl_config_file']);
                break;
            case '1.1':
                ...
                if (! empty($config['wurfl_config_file'])) {
                    ...
                    $wurflConfig->wurflFile($c['wurfl']['main-file'])
                        ...
                        ->persistence($c['persistence']['provider'], 
                            $c['persistence']['dir']);
                }
            ...
            default:
                require_once 'Zend/Http/UserAgent/Features/Exception.php';
                throw new Zend_Http_UserAgent_Features_Exception(
                        sprintf('Unknown API version "%s"', 
                                $config['wurfl_api_version']));
        }
        
        $device = $wurflManager->getDeviceForHttpRequest(
                array_change_key_case($request, CASE_UPPER));
