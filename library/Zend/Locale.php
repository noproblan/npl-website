...
class Zend_Locale
{
    ...
    /**
     * Sets a new default locale which will be used when no locale can be
     * detected
    ...
    public static function setDefault ($locale, $quality = 1)
    {
        if (($locale === 'auto') or ($locale === 'root') or
                 ($locale === 'default') or ($locale === 'environment') or
                 ($locale === 'browser')) {
            require_once 'Zend/Locale/Exception.php';
            throw new Zend_Locale_Exception(
                    'Only full qualified locales can be used as default!');
        }
        
        if (($quality < 0.1) or ($quality > 100)) {
            require_once 'Zend/Locale/Exception.php';
            throw new Zend_Locale_Exception(
                    "Quality must be between 0.1 and 100");
        }
        ...
        if (isset(self::$_localeData[(string) $locale]) === true) {
            ...
            if (isset(self::$_localeData[$elocale[0]]) === true) {
                ...
                require_once 'Zend/Locale/Exception.php';
                throw new Zend_Locale_Exception(
                        "Unknown locale '" . (string) $locale .
                                 "' can not be set as default!");
            }
        }
        
        self::$_auto = self::getBrowser() + self::getEnvironment() +
                 self::getDefault();
    }
        ...
        foreach ($languages as $locale) {
            ...
            if ($language !== 'C') {
                ...
                $language = str_ireplace(
                        array_keys(
                                Zend_Locale_Data_Translation::$languageTranslation), 
                        array_values(
                                Zend_Locale_Data_Translation::$languageTranslation), 
                        (string) $language);
                ...
                $language = str_ireplace(
                        array_keys(
                                Zend_Locale_Data_Translation::$regionTranslation), 
                        array_values(
                                Zend_Locale_Data_Translation::$regionTranslation), 
                        $language);
                ...
                if (isset(self::$_localeData[$language]) === true) {
                    ...
                    if (strpos($language, '_') !== false) {
                        $languagearray[substr($language, 0, 
                                strpos($language, '_'))] = 1;
                    }
                }
            }
    ...
    public static function getBrowser ()
    {
        ...
        foreach ($accepted as $accept) {
            ...
            if ((isset($languages[$region]) === false) ||
     ...
     * @param string|Zend_Locale $locale
     *            (Optional) Locale|Language for which this informations should
     *            be returned
     * @param string $value
     *            (Optional) Value for detail list
     * @return array Array with the wished information in the given language
     */
    public static function getTranslationList ($path = null, $locale = null, 
            $value = null)
    {
    ...
    /**
     * Returns an array with the name of all languages translated to the given
     * language
     *
     * @param string|Zend_Locale $locale
    ...
    {
        trigger_error(
                "The method getLanguageTranslationList is deprecated. Use getTranslationList('language', $locale) instead", 
                E_USER_NOTICE);
        return self::getTranslationList('language', $locale);
    }

    /**
     * Returns an array with the name of all scripts translated to the given
     * language
     *
     * @param string|Zend_Locale $locale
    ...
    {
        trigger_error(
                "The method getScriptTranslationList is deprecated. Use getTranslationList('script', $locale) instead", 
                E_USER_NOTICE);
        return self::getTranslationList('script', $locale);
    }

    /**
     * Returns an array with the name of all countries translated to the given
     * language
     *
     * @param string|Zend_Locale $locale
    ...
    {
        trigger_error(
                "The method getCountryTranslationList is deprecated. Use getTranslationList('territory', $locale, 2) instead", 
                E_USER_NOTICE);
        return self::getTranslationList('territory', $locale, 2);
    }

    /**
     * Returns an array with the name of all territories translated to the given
     * language
    ...
    {
        trigger_error(
                "The method getTerritoryTranslationList is deprecated. Use getTranslationList('territory', $locale, 1) instead", 
                E_USER_NOTICE);
        return self::getTranslationList('territory', $locale, 1);
    }

    /**
     * Returns a localized information string, supported are several types of
     * informations.
     * For detailed information about the types look into the documentation
     *
     * @param string $value
    ...
    {
        trigger_error(
                "The method getLanguageTranslation is deprecated. Use getTranslation($value, 'language', $locale) instead", 
                E_USER_NOTICE);
    ...
    {
        trigger_error(
                "The method getScriptTranslation is deprecated. Use getTranslation($value, 'script', $locale) instead", 
                E_USER_NOTICE);
    ...
    {
        trigger_error(
                "The method getCountryTranslation is deprecated. Use getTranslation($value, 'country', $locale) instead", 
                E_USER_NOTICE);
    ...
    {
        trigger_error(
                "The method getTerritoryTranslation is deprecated. Use getTranslation($value, 'territory', $locale) instead", 
                E_USER_NOTICE);
     ...
     *
     * @param string|Zend_Locale $locale
     *            (Optional) Locale for language translation (defaults to $this
     *            locale)
     * @return array
     */
     ...
     *            (DEPRECATED) Only for internal usage, brakes compatibility
     *            mode
     ...
     * Detects registry locale and when all fails tries to detect a automatic
     * locale
     * Returns the found locale as string
     *
     * @param string $locale            
     * @throws Zend_Locale_Exception When the given locale is no locale or the
     *         autodetection fails
     * @return string
     */
     ...
     * Only real locales are returned, the internal locales 'root', 'auto',
     * 'browser'
     ...
     * @throws Zend_Locale_Exception When no locale is set which is only
     *         possible when the class was wrong extended
     * @return string
     */
    private static function _prepareLocale ($locale, $strict = false)
    {
        ...
        // This can only happen when someone extends Zend_Locale and erases the
        // default
        if ($locale === null) {
            require_once 'Zend/Locale/Exception.php';
            throw new Zend_Locale_Exception(
        ...
        if (! isset(self::$_localeData[$parts[0]])) {
            if ((count($parts) == 1) &&
                     array_key_exists($parts[0], self::$_territoryData)) {
    ...
    public static function getOrder ($order = null)
    {
        switch ($order) {
            case self::ENVIRONMENT:
                self::$_breakChain = true;
                $languages = self::getEnvironment() + self::getBrowser() +
                         self::getDefault();
                break;
            
            case self::ZFDEFAULT:
                self::$_breakChain = true;
                $languages = self::getDefault() + self::getEnvironment() +
                         self::getBrowser();
                break;
            
            default:
                self::$_breakChain = true;
                $languages = self::getBrowser() + self::getEnvironment() +
                         self::getDefault();
                break;
        }
