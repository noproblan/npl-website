...
class Zend_Markup_Parser_Bbcode implements Zend_Markup_Parser_ParserInterface
{
    ...
    /**
     * Prepare the parsing of a bbcode string, the real parsing is done in
     * {@link _parse()}
     *
     * @param string $value            
        ...
        while ($this->_pointer < $this->_valueLen) {
            switch ($this->_state) {
                ...
                case self::STATE_SCANATTRS:
                    $matches = array();
                    $regex = '#\G((?<end>\s*\])|\s+(?<attribute>[' .
                             self::NAME_CHARSET . ']+)(?<eq>=?))#';
    ...
    public function _createTree ()
    {
        foreach ($this->_tokens as $token) {
            // first we want to know if this tag is a stopper, or at least a
            // searched one
            if ($this->_isStopper($token['tag'])) {
                // find the stopper
                $oldItems = array();
                
                while (! in_array($token['tag'], 
                        $this->_tags[$this->_current->getName()]['stoppers'])) {
