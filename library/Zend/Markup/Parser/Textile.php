...
class Zend_Markup_Parser_Textile implements Zend_Markup_Parser_ParserInterface
{
    ...
    /**
     * Prepare the parsing of a Textile string, the real parsing is done in
     * {@link _parse()}
     *
     * @param string $value            
     *
    ...
    public function parse ($value)
    {
        if (! is_string($value)) {
            ...
            throw new Zend_Markup_Parser_Exception(
                    'Value to parse should be a string.');
        }
        if (empty($value)) {
            /**
             *
            ...
            require_once 'Zend/Markup/Parser/Exception.php';
            throw new Zend_Markup_Parser_Exception(
                    'Value to parse cannot be left empty.');
        }
            ...
            switch ($state) {
                case self::STATE_SCAN:
                    ...
                    $regex = '#\G(?<text>.*?)(?:' .
                             ...
                             '|":(?<url>[^\s]+)|")' . "(?:{$attrsMatch})*)" .
                             ')#si';
                    preg_match($regex, $this->_value, $matches, null, 
                            $this->_pointer);
                    ...
                    if (! empty($matches['nl_paragraph'])) {
                        ...
                        if (isset($this->_simpleTags[$matches['name']])) {
                            ...
                            $this->_tokens[] = array(
                                    ...
                                    'attributes' => $this->_extractAttributes(
                                            $matches)
                            );
                        } else {
                ...
                case self::STATE_NEW_PARAGRAPH:
                    ...
                    if (! preg_match($regex, $this->_value, $matches, null, 
                            $this->_pointer)) {
                    ...
                    if ($matches['name'] == 'p') {
                        ...
                        $this->_temp['attributes'] = $this->_extractAttributes(
                                $matches);
            ...
            if ($this->_isStopper($token, $this->_current)) {
                ...
                if ($token['name'] == 'break') {
                    ...
                    $this->_current->addChild(
                            new Zend_Markup_Token($token['tag'], 
                                    Zend_Markup_Token::TYPE_NONE, '', array(), 
                                    $this->_current));
                } else {
                    // handle a list item
                    if ($token['name'] == 'list') {
                        ...
                        if ($this->_current->getName() != 'list') {
                            ...
                            $child = new Zend_Markup_Token('', 
                                    Zend_Markup_Token::TYPE_TAG, 'list', 
                                    $attributes, $this->_current);
                        ...
                        $token['name'] = 'li';
                    } elseif (($token['name'] == 'img') ||
                             ($token['name'] == 'url')) {
                        $inside = false;
                    }
                    
                    // add the token
                    $child = new Zend_Markup_Token($token['tag'], 
                            Zend_Markup_Token::TYPE_TAG, $token['name'], 
                            $token['attributes'], $this->_current);
