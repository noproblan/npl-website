...
abstract class Zend_Markup_Renderer_RendererAbstract
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (! isset($options['useDefaultFilters']) ||
                 ($options['useDefaultFilters'] === true)) {
    ...
    public function addMarkup ($name, $type, array $options)
    {
        if (! isset($options['group']) && ($type ^ self::TYPE_ALIAS)) {
            ...
            throw new Zend_Markup_Renderer_Exception(
                    "There is no render group defined.");
        }
    ...
    protected function _execute (Zend_Markup_Token $token)
    {
        ...
        if (! isset($this->_markups[$token->getName()])) {
            ...
            $return = $this->_filter($token->getTag()) . $this->_render($token) .
                     $token->getStopper();
        ...
        $empty = (is_array($markup) && array_key_exists('empty', $markup) &&
                 $markup['empty']);
        ...
        if (is_array($markup) &&
                 ! in_array($markup['group'], $this->_groups[$this->_group])) {
            $oldToken = $this->_token;
            $return = $this->_filter($token->getTag()) . $this->_render($token) .
                     $token->getStopper();
            $this->_token = $oldToken;
        ...
        if (! isset($markup['filter']) ||
                 (! ($markup['filter'] instanceof Zend_Filter_Interface) &&
                 ($markup['filter'] !== false))) {
        ...
        if (is_array($markup) && ($markup['type'] & self::TYPE_CALLBACK)) {
            ...
            if (! ($markup['callback'] instanceof Zend_Markup_Renderer_TokenConverterInterface)) {
                ...
                if (! ($markup['callback'] instanceof Zend_Markup_Renderer_TokenConverterInterface)) {
                    require_once 'Zend/Markup/Renderer/Exception.php';
                    throw new Zend_Markup_Renderer_Exception(
                            "Callback for tag '$name' found, but it isn't valid.");
                }
            ...
            if ($markup['type'] && ! $empty) {
                $return = $markup['callback']->convert($token, 
                        $this->_render($token));
            } else {
     ...
     * @return void
     */
    public function addDefaultFilter (Zend_Filter_Interface $filter, 
 ...
 * @return Zend_Markup_Renderer_RendererAbstract
 */
public function addFilter (Zend_Filter_Interface $filter, $markup, 
    $placement = Zend_Filter::CHAIN_APPEND)
{
$markup = $this->_resolveMarkupName($markup);
 ...
 * @return void
 */
public function addGroup ($name, array $allowedInside = array(), 
