...
class Zend_Markup_Renderer_Html extends Zend_Markup_Renderer_RendererAbstract
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (! isset($options['useDefaultMarkups']) &&
                 isset($options['useDefaultTags'])) {
            $options['useDefaultMarkups'] = $options['useDefaultTags'];
        }
        if (isset($options['useDefaultMarkups']) &&
                 ($options['useDefaultMarkups'] !== false)) {
     ...
     * @return string
     */
    public static function renderAttributes (Zend_Markup_Token $token, 
            array $attributes = array())
    {
        ...
        if (isset($tokenAttributes['align'])) {
            $tokenAttributes['style'] .= 'text-align: ' .
                     $tokenAttributes['align'] . ';';
            unset($tokenAttributes['align']);
        }
        if (isset($tokenAttributes['color']) &&
                 self::checkColor($tokenAttributes['color'])) {
...
foreach ($attributes as $attribute => $value) {
    if (isset($tokenAttributes[$attribute]) &&
             ! empty($tokenAttributes[$attribute])) {
        $return .= ' ' . $attribute . '="' .
         htmlentities($tokenAttributes[$attribute], ENT_QUOTES, 
                self::getEncoding()) . '"';
} elseif (! empty($value)) {
$return .= ' ' . $attribute . '="' .
         htmlentities($value, ENT_QUOTES, self::getEncoding()) . '"';
}
}
