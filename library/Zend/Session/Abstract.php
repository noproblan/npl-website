...
abstract class Zend_Session_Abstract
{
    ...
    /**
     * Since expiring data is handled at startup to avoid __destruct
     * difficulties,
    ...
    /**
     * namespaceIsset() - check to see if a namespace or a variable within a
     * namespace is set
     *
    ...
    protected static function _namespaceIsset ($namespace, $name = null)
    {
        ...
        if ($name === null) {
            return (isset($_SESSION[$namespace]) ||
                     isset(self::$_expiringData[$namespace]));
        } else {
            return (isset($_SESSION[$namespace][$name]) ||
                     isset(self::$_expiringData[$namespace][$name]));
        }
    }
    ...
    protected static function _namespaceUnset ($namespace, $name = null)
    {
        ...
        // check to see if the api wanted to remove a var from a namespace or a
        // namespace
        if ($name === '') {
    ...
    /**
     * namespaceGet() - Get $name variable from $namespace, returning by
     * reference.
     *
     * @param string $namespace            
     * @param string $name            
    ...
    protected static function &_namespaceGet ($namespace, $name = null)
    {
        ...
        if ($name === null) {
            if (isset($_SESSION[$namespace])) { // check session first for data
                                                // requested
                return $_SESSION[$namespace];
            } elseif (isset(self::$_expiringData[$namespace])) { // check
                                                                 // expiring
                                                                 // data for
                                                                 // data
                                                                 // reqeusted
            ...
            } elseif (isset(self::$_expiringData[$namespace][$name])) { // check
                                                                        // expiring
                                                                        // data
                return self::$_expiringData[$namespace][$name];
            } else {
                return $_SESSION[$namespace][$name]; // satisfy return by
                                                     // reference
            }
        }
    }
    ...
    /**
     * namespaceGetAll() - Get an array containing $namespace, including
     * expiring data.
     *
     * @param string $namespace            
     * @param string $name            
    ...
    {
        $currentData = (isset($_SESSION[$namespace]) &&
                 is_array($_SESSION[$namespace])) ? $_SESSION[$namespace] : array();
        $expiringData = (isset(self::$_expiringData[$namespace]) &&
                 is_array(self::$_expiringData[$namespace])) ? self::$_expiringData[$namespace] : array();
