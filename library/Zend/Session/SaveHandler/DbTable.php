 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Session_SaveHandler_DbTable extends Zend_Db_Table_Abstract implements 
        Zend_Session_SaveHandler_Interface
{
     ...
     * containing configuration options for
     * Zend_Session_SaveHandler_DbTable and Zend_Db_Table_Abstract. These are
     * the configuration options for
     * Zend_Session_SaveHandler_DbTable:
     *
     * primaryAssignment => (string|array) Session table primary key value
     * assignment
     * (optional; default: 1 => sessionId) You have to assign a value to each
     * primary key of your session table.
     * The value of this configuration option is either a string if you have
     * only one primary key or an array if
     * you have multiple primary keys. The array consists of numeric keys
     * starting at 1 and string values. There
     ...
     *
     * lifetime => (integer) Session lifetime (optional; default:
     * ini_get('session.gc_maxlifetime'))
     *
     * overrideLifetime => (boolean) Whether or not the lifetime of an existing
     * session should be overridden
    ...
    public function write ($id, $data)
    {
        ...
        if (count($rows)) {
            $data[$this->_lifetimeColumn] = $this->_getLifetime(
                    $rows->current());
            
            if ($this->update($data, 
                    $this->_getPrimary($id, self::PRIMARY_TYPE_WHERECLAUSE))) {
                $return = true;
            }
        } else {
            ...
            if ($this->insert(
                    array_merge(
                            $this->_getPrimary($id, self::PRIMARY_TYPE_ASSOC), 
                            $data))) {
                $return = true;
            }
        }
    ...
    public function gc ($maxlifetime)
    {
        $this->delete(
                $this->getAdapter()
                    ->quoteIdentifier($this->_modifiedColumn, true) . ' + ' .
                         $this->getAdapter()
                            ->quoteIdentifier($this->_lifetimeColumn, true) .
                         ' < ' . $this->getAdapter()
                            ->quote(time()));
    ...
    /**
     * Calls other protected methods for individual setup tasks and requirement
     * checks
     *
     * @return void
     */
    ...
    {
        if (empty($this->_name) &&
                 basename(($this->_name = session_save_path())) != $this->_name) {
            /**
             *
             * @see Zend_Session_SaveHandler_Exception
             */
    ...
    protected function _setupPrimaryAssignment ()
    {
        ...
        if (count($this->_primaryAssignment) !== count($this->_primary)) {
            ...
            throw new Zend_Session_SaveHandler_Exception(
                    "Value for configuration option '" . self::PRIMARY_ASSIGNMENT .
                             "' must have an assignment " .
                             "for each session table primary key.");
        } else 
            if (! in_array(self::PRIMARY_ASSIGNMENT_SESSION_ID, 
                ...
                throw new Zend_Session_SaveHandler_Exception(
                        "Value for configuration option '" .
                                 ...
                                 "' must have an assignment " .
                                 "for the session id ('" .
                                 self::PRIMARY_ASSIGNMENT_SESSION_ID . "').");
            }
    }
    ...
    protected function _checkRequiredColumns ()
    {
        if ($this->_modifiedColumn === null) {
            ...
            throw new Zend_Session_SaveHandler_Exception(
                    "Configuration must define '" . self::MODIFIED_COLUMN .
                             "' which names the " .
                             "session table last modification time column.");
        } else 
            if ($this->_lifetimeColumn === null) {
                ...
                throw new Zend_Session_SaveHandler_Exception(
                        "Configuration must define '" . self::LIFETIME_COLUMN .
                                 "' which names the " .
                                 "session table lifetime column.");
            } else 
                if ($this->_dataColumn === null) {
                    ...
                    throw new Zend_Session_SaveHandler_Exception(
                            "Configuration must define '" . self::DATA_COLUMN .
                                     "' which names the " .
                                     "session table data column.");
                }
    }
        ...
        foreach ($this->_primary as $index => $primary) {
            ...
            switch ((string) $type) {
                ...
                case self::PRIMARY_TYPE_WHERECLAUSE:
                    $primaryArray[] = $this->getAdapter()->quoteIdentifier(
                            $primary, true) . ' = ' .
     ...
     * Zend_Session_SaveHandler_DbTable::OVERRIDE_LIFETIME
     *
