 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Session_Namespace extends Zend_Session_Abstract implements 
        IteratorAggregate
{

    /**
     * used as option to constructor to prevent additional instances to the same
     * namespace
     */
    const SINGLE_INSTANCE = true;

    /**
     * Namespace - which namespace this instance of zend-session is
     * saving-to/getting-from
     *
     * @var string
     */
    ...
    /**
     * __construct() - Returns an instance object bound to a particular,
     * isolated section
     * of the session, identified by $namespace name (defaulting to 'Default').
     * The optional argument $singleInstance will prevent construction of
     * additional
     ...
     *            - prevent creation of additional accessor instance objects for
     *            this namespace
     * @return void
     */
    public function __construct ($namespace = 'Default', $singleInstance = false)
    {
        if ($namespace === '') {
            ...
            throw new Zend_Session_Exception(
                    'Session namespace must be a non-empty string.');
        }
        ...
        if ($namespace[0] == "_") {
            ...
            require_once 'Zend/Session/Exception.php';
            throw new Zend_Session_Exception(
                    'Session namespace must not start with an underscore.');
        }
        ...
        if (preg_match('#(^[0-9])#i', $namespace[0])) {
            ...
            require_once 'Zend/Session/Exception.php';
            throw new Zend_Session_Exception(
                    'Session namespace must not start with a number.');
        }
        ...
        Zend_Session::start(true); // attempt auto-start (throws exception if
                                   // strict option set)
            ...
            if (isset($_SESSION['__ZF'][$namespace])) {
                ...
                if (isset($_SESSION['__ZF'][$namespace]['ENVNH'])) {
                    foreach ($_SESSION['__ZF'][$namespace]['ENVNH'] as $variable => $hops) {
                        ...
                        if ($_SESSION['__ZF'][$namespace]['ENVNH'][$variable] ===
                                 0) {
     ...
     *
     * @param string $name
     *            - programmatic name of a key, in a <key,value> pair in the
     *            current namespace
     * @return mixed
     */
    ...
    /**
     * apply() - enables applying user-selected function, such as array_merge()
     * to the namespace
     * Parameters following the $callback argument are passed to the callback
     * function.
     * Caveat: ignores members expiring now.
     *
     * Example:
     * $namespace->apply('array_merge', array('tree' => 'apple', 'fruit' =>
     * 'peach'), array('flower' => 'rose'));
    ...
    /**
     * applySet() - enables applying user-selected function, and sets entire
     * namespace to the result
     ...
     *
     * Example:
     * $namespace->applySet('array_merge', array('tree' => 'apple', 'fruit' =>
     * 'peach'), array('flower' => 'rose'));
     *
     * @param string|array $callback
     *            - callback function
    ...
    public function applySet ($callback)
    {
        ...
        if (! is_array($result)) {
            ...
            require_once 'Zend/Session/Exception.php';
            throw new Zend_Session_Exception(
                    'Result must be an array. Got: ' . gettype($result));
        }
    ...
    /**
     * setExpirationSeconds() - expire the namespace, or specific variables
     * after a specified
     * number of seconds
     *
     * @param int $seconds
