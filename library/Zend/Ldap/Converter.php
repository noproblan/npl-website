...
/**
 * Zend_Ldap_Converter is a collection of useful LDAP related conversion
 * functions.
 *
 * @category Zend
 * @package Zend_Ldap
...
class Zend_Ldap_Converter
{
    ...
    public static function toLdap ($value, $type = self::STANDARD)
    {
        try {
            ...
            throw new Zend_Ldap_Converter_Exception($e->getMessage(), 
                    $e->getCode(), $e);
        }
    }
                ...
                if (is_string($date)) {
                    ...
                    if ($date instanceof Zend_Date) {
                        ...
                        throw new InvalidArgumentException(
                                'Parameter $date is not of the expected type');
                    }
        }
    ...
    public static function fromLdapDateTime ($date, $asUtc = true)
    {
        ...
        if ($datepart[1] < 4) {
            throw new InvalidArgumentException(
                    'Invalid date format found (too short)');
        }
        ...
        if ($length >= 6) {
            ...
            if ($month < 1 || $month > 12) {
                throw new InvalidArgumentException(
                        'Invalid date format found (invalid month)');
            }
            $time['month'] = $month;
        }
        ...
        if ($length >= 8) {
            ...
            if ($day < 1 || $day > 31) {
                throw new InvalidArgumentException(
                        'Invalid date format found (invalid day)');
            }
            $time['day'] = $day;
        }
        ...
        if ($length >= 10) {
            ...
            if ($hour < 0 || $hour > 23) {
                throw new InvalidArgumentException(
                        'Invalid date format found (invalid hour)');
            }
            $time['hour'] = $hour;
        }
        ...
        if ($length >= 12) {
            ...
            if ($minute < 0 || $minute > 59) {
                throw new InvalidArgumentException(
                        'Invalid date format found (invalid minute)');
            }
            $time['minute'] = $minute;
        }
        ...
        if ($length >= 14) {
            ...
            if ($second < 0 || $second > 59) {
                throw new InvalidArgumentException(
                        'Invalid date format found (invalid second)');
            }
            $time['second'] = $second;
        }
            ...
            if ($offset == '+' || $offset == '-') {
                ...
                if (isset($off[2])) {
                    ...
                    if ($offsetHours < 0 || $offsetHours > 12) {
                        throw new InvalidArgumentException(
                                'Invalid date format found (invalid offset hour)');
                    }
        ...
        $timestring = $time['year'] . '-' .
                 ...
                 str_pad($time['second'], 2, '0', STR_PAD_LEFT) . $time['offdir'] .
                 str_pad($time['offsethours'], 2, '0', STR_PAD_LEFT) .
                 str_pad($time['offsetminutes'], 2, '0', STR_PAD_LEFT);
