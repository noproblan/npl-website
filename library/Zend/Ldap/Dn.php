...
class Zend_Ldap_Dn implements ArrayAccess
{
    ...
    public function getParentDn ($levelUp = 1)
    {
        ...
        if ($levelUp < 1 || $levelUp >= count($this->_dn)) {
            ...
            require_once 'Zend/Ldap/Exception.php';
            throw new Zend_Ldap_Exception(null, 
                    'Cannot retrieve parent DN with given $levelUp');
        }
    ...
    public function get ($index, $length = 1, $caseFold = null)
    {
        ...
        if ($length === 1) {
            ...
            return self::_caseFoldDn(
                    array_slice($this->_dn, $index, $length, false), $caseFold);
        }
    }
    ...
    protected static function _assertRdn (array $value)
    {
        if (count($value) < 1) {
            ...
            require_once 'Zend/Ldap/Exception.php';
            throw new Zend_Ldap_Exception(null, 
                    'RDN Array is malformed: it must have at least one item');
        }
        ...
        foreach (array_keys($value) as $key) {
            if (! is_string($key)) {
                ...
                require_once 'Zend/Ldap/Exception.php';
                throw new Zend_Ldap_Exception(null, 
                        'RDN Array is malformed: it must use string keys');
            }
        }
    }
    ...
    {
        $this->_caseFold = self::_sanitizeCaseFold($caseFold, 
                self::$_defaultCaseFold);
    }
    ...
    {
        self::$_defaultCaseFold = self::_sanitizeCaseFold($caseFold, 
     ...
     * special meaning in RFC 2252
     * are preceeded by ba backslash. Control characters with an ASCII code < 32
     * are represented as \hexpair.
     * Finally all leading and trailing spaces are converted to sequences of
     * \20.
     * 
            ...
            $caseFold = self::ATTR_CASEFOLD_NONE)
    {
        ...
        for ($i = 0; $i < count($k); $i ++) {
            if (is_array($k[$i]) && is_array($v[$i]) &&
                ...
                if (is_string($k[$i]) && is_string($v[$i])) {
                    $ret[] = array(
                            $k[$i] => $v[$i]
                    );
                }
            ...
            $caseFold = self::ATTR_CASEFOLD_NONE)
    {
         ...
         * until a comma (,) or semicolon (;) is encountered after which we
         * switch back
         * to state 1. If a backslash (\) is encountered, state 3 is used to
         * collect the
            ...
            switch ($state) {
                case 1: // collect key
                    if ($ch === '=') {
                        ...
                        if ($caseFold == self::ATTR_CASEFOLD_LOWER)
                            ...
                            if ($caseFold == self::ATTR_CASEFOLD_UPPER)
                                $key = strtoupper($key);
                        if (is_array($multi)) {
                ...
                case 2: // collect value
                    if ($ch === '\\') {
                        ...
                        if ($ch === ',' || $ch === ';' || $ch === 0 ||
                                 $ch === '+') {
                            $value = self::unescapeValue(
                                    trim(substr($dn, $vo, $di - $vo)));
                            if (is_array($multi)) {
                                $va[count($va) - 1][] = $value;
                            } else {
                                $va[] = $value;
     ...
     * @throws Zend_Ldap_Exception
     */
    public static function implodeDn (array $dnArray, $caseFold = null, 
            $separator = ',')
    {
        $parts = array();
