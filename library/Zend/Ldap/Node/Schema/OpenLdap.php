...
/**
 * Zend_Ldap_Node_Schema_OpenLdap provides a simple data-container for the
 * Schema node of
 * an OpenLDAP server.
 *
 * @category Zend
...
class Zend_Ldap_Node_Schema_OpenLdap extends Zend_Ldap_Node_Schema
{
    ...
    protected function _parseAttributeType ($value)
    {
        ...
        if (array_key_exists('syntax', $attributeType)) {
            ...
            if (preg_match('/^(.+){(\d+)}$/', $attributeType['syntax'], 
                    $matches)) {
     ...
     */
    protected function _resolveInheritance (Zend_Ldap_Node_Schema_Item $node, 
            array $repository)
    {
        $data = $node->getData();
        $parents = $data['sup'];
        ...
        foreach ($parents as $parent) {
            if (! array_key_exists($parent, $repository))
                continue;
            if (! array_key_exists('_parents', $data) ||
    ...
    protected function _parseMatchingRuleUse ($value)
    {
        ...
        $tokens = $this->_tokenizeString($value);
        $matchingRuleUse['oid'] = array_shift($tokens); // first token is the
                                                        // oid
    ...
    protected function _parseLdapSchemaSyntax (array &$data, array $tokens)
    {
        ...
        while (count($tokens) > 0) {
            ...
            if (in_array($token, $noValue)) {
                ...
                if ($data[$token] == '(') {
                    // this creates the list of values and cycles through the
                    // tokens
                    ...
                    while ($tmp = array_shift($tokens)) {
                        if ($tmp == ')')
                            break;
                        if ($tmp != '$') {
                            $data[$token][] = Zend_Ldap_Attribute::convertFromLdapValue(
                                    $tmp);
                        }
                    }
                } else {
    ...
    protected function _tokenizeString ($value)
    {
        ...
        for ($i = 0; $i < $cMatches; $i ++) { // number of tokens (full pattern
                                             // match)
