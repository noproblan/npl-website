...
/**
 * Zend_Ldap_Ldif_Encoder provides methods to encode and decode LDAP data
 * into/from LDIF.
 *
 * @category Zend
 * @package Zend_Ldap
...
class Zend_Ldap_Ldif_Encoder
{
    ...
    protected function _encodeAttribute ($name, $value)
    {
        ...
        foreach ($value as $v) {
            ...
            if (isset($this->_options['wrap']) &&
                     strlen($attribute) > $this->_options['wrap']) {
                $attribute = trim(
                        chunk_split($attribute, $this->_options['wrap'], 
                                PHP_EOL . ' '));
            }
    ...
    protected function _encodeAttributes (array $attributes)
    {
        ...
        if (! $this->_versionWritten && array_key_exists('dn', $attributes) &&
                 isset($this->_options['version']) &&
                 array_key_exists('objectclass', $attributes)) {
            $string .= sprintf('version: %d', $this->_options['version']) .
             PHP_EOL;
