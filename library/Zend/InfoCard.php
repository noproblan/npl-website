...
class Zend_InfoCard
{
    ...
    /**
     * An array of certificate pair files and optional passwords for them to
     * search
     * when trying to determine which certificate was used to encrypt the
     * transient key
     *
     * @var Array
     */
    ...
    /**
     * The InfoCard Adapter to use for callbacks into the application using the
     * component
    ...
    /**
     * Sets the adapter uesd for callbacks into the application using the
     * component, used
    ...
    /**
     * Retrieves the adapter used for callbacks into the application using the
     * component.
     * If no adapter was set then an instance of Zend_InfoCard_Adapter_Default
     * is used
     *
     ...
     * @throws Zend_InfoCard_Exception
     * @param string $key_id
     *            The Certificate Key ID returned from adding the certificate
     *            pair
     * @return Zend_InfoCard
     */
     ...
     * @param string $type
     *            (optional) The URI for the type of key pair this is (default
     ...
     * @return array An array containing the path to the private/public key
     *         files,
    ...
    /**
     * Retrieve the digest of a given public key / certificate using the
     * provided digest
     * method
     *
     * @throws Zend_InfoCard_Exception
     ...
     * @return mixed The Key ID of the matching certificate pair or false if not
     *         found
     */
     ...
     * @return string The XML of the Signed Token inside of the EncryptedData
     *         block
     */
    protected function _extractSignedToken ($strXmlToken)
    {
        $encryptedData = Zend_InfoCard_Xml_EncryptedData::getInstance(
                $strXmlToken);
        ...
        $PKcipher = Zend_InfoCard_Cipher::getInstanceByURI(
        ...
        $symCipher = Zend_InfoCard_Cipher::getInstanceByURI(
    ...
    /**
     * Process an input Infomation Card EncryptedData block sent from the
     * client,
     * validate it, and return the claims contained within it on success or an
     * error message on error
     *
     * @param string $strXmlToken
     *            The XML token sent to the server from the client
     * @return Zend_Infocard_Claims The Claims object containing the claims, or
    ...
    public function process ($strXmlToken)
    {
        ...
        try {
            $assertions = Zend_InfoCard_Xml_Assertion::getInstance(
                    $signedAssertionsXml);
        } catch (Zend_InfoCard_Exception $e) {
        ...
        if (! ($reference_id = Zend_InfoCard_Xml_Security::validateXMLSignature(
                $assertions->asXML()))) {
            $retval->setError(
        ...
        // Make sure the signature is in reference to the same document as the
        // assertions
        ...
        // Validate we haven't seen this before and the conditions are
        // acceptable
