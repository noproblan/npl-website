...
class Zend_Auth_Adapter_Ldap implements Zend_Auth_Adapter_Interface
{
     ...
     * @return void
     */
    public function __construct (array $options = array(), $username = null, 
            $password = null)
    {
        ...
        foreach ($this->_options as $name => $options) {
            ...
            if (! is_array($options)) {
                ...
                throw new Zend_Auth_Adapter_Exception(
                        'Adapter options array not an array');
            }
            ...
            try {
                ...
                /*
                 * Fixes problem when authenticated user is not allowed to
                 * retrieve
                 * group-membership information or own account.
                 * This requires that the user specified with "username" and
                 * optionally
                 * "password" in the Zend_Ldap options is able to retrieve the
                 * required
                 * information.
                 */
                $requireRebind = false;
                ...
                $dn = $ldap->getCanonicalAccountName($canonicalName, 
                        Zend_Ldap::ACCTNAME_FORM_DN);
                
                $groupResult = $this->_checkGroupMembership($ldap, 
                        $canonicalName, $dn, $adapterOptions);
                if ($groupResult === true) {
                 ...
                 * diagnose. Therefore
                 * we bend over backwards to capture and record every possible
                 * bit of
     ...
     * @return string|true
     */
    protected function _checkGroupMembership (Zend_Ldap $ldap, $canonicalName, 
            $dn, array $adapterOptions)
    {
        ...
        $groupName = Zend_Ldap_Filter::equals($adapterOptions['groupAttr'], 
                $adapterOptions['group']);
        $membership = Zend_Ldap_Filter::equals($adapterOptions['memberAttr'], 
                $user);
        ...
        $result = $ldap->count($group, $adapterOptions['groupDn'], 
                $adapterOptions['groupScope']);
     ...
     * Zend_Auth_Adapter_DbTable::getResultRowObject()}.
     * Closes ZF-6813
     *
            ...
            array $omitAttribs = array())
    {
        ...
        $entry = $this->getLdap()->getEntry($this->_authenticatedDn, 
                $returnAttribs, true);
