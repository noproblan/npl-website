...
class Zend_Validate_EmailAddress extends Zend_Validate_Abstract
{
     ...
     *
     * @param boolean $mx
     *            Set allowed to true to validate for MX records, and false to
     *            not validate them
     * @return Zend_Validate_EmailAddress Fluid Interface
     */
     ...
     *
     * @param boolean $deep
     *            Set deep to true to perform a deep validation process for MX
     *            records
     * @return Zend_Validate_EmailAddress Fluid Interface
     */
    ...
    private function _isReserved ($host)
    {
        ...
        if ((int) $octet[0] >= 224) {
            ...
            if (array_key_exists($octet[0], $this->_invalidIp)) {
                foreach ((array) $this->_invalidIp[$octet[0]] as $subnetData) {
                    // we skip the first loop as we already know that octet
                    // matches
                    for ($i = 1; $i < 4; $i ++) {
                    ...
                    $tmp = explode(".", $host[0]);
                    for ($i = 0; $i < 4; $i ++) {
                        $binaryHost .= str_pad(decbin($tmp[$i]), 8, "0", 
                                STR_PAD_LEFT);
                    }
                    
                    $segmentData = array(
                            'network' => (int) $this->_toIp(
                                    str_pad(substr($binaryHost, 0, $host[1]), 
                                            32, 0)),
                            'broadcast' => (int) $this->_toIp(
                                    str_pad(substr($binaryHost, 0, $host[1]), 
                                            32, 1))
                    );
    ...
    private function _validateLocalPart ()
    {
        ...
        if (preg_match('/^[' . $atext . ']+(\x2e+[' . $atext . ']+)*$/', 
            ...
            // Quoted-string characters are: DQUOTE *([FWS] qtext/quoted-pair)
            // [FWS] DQUOTE
            // qtext: Non white space controls, and the rest of the US-ASCII
            // characters not
            // including "\" or the quote character
            $noWsCtl = '\x01-\x08\x0b\x0c\x0e-\x1f\x7f';
            $qtext = $noWsCtl . '\x21\x23-\x5b\x5d-\x7e';
            $ws = '\x20\x09';
            if (preg_match('/^\x22([' . $ws . $qtext . '])*[$ws]?\x22$/', 
                    $this->_localPart)) {
                $result = true;
            } else {
    ...
    {
        $hostname = $this->_options['hostname']->setTranslator(
                $this->getTranslator())
