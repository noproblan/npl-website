...
class Zend_Validate_Ip extends Zend_Validate_Abstract
{
     ...
     *
     * @param array $options
     *            OPTIONAL Options to set, see the manual for all available
     *            options
     * @return void
     */
    ...
    public function isValid ($value)
    {
        ...
        if (($this->_options['allowipv4'] && ! $this->_options['allowipv6'] &&
                 ! $this->_validateIPv4($value)) ||
                 (! $this->_options['allowipv4'] && $this->_options['allowipv6'] &&
                 ! $this->_validateIPv6($value)) ||
                 ($this->_options['allowipv4'] && $this->_options['allowipv6'] &&
                 ! $this->_validateIPv4($value) && ! $this->_validateIPv6(
                        $value))) {
    ...
    protected function _validateIPv6 ($value)
    {
        ...
        if (strpos($value, '.')) {
            ...
            if (! ($lastcolon &&
                     $this->_validateIPv4(substr($value, $lastcolon + 1)))) {
                return false;
            }
        ...
        if (strpos($value, '::') === false) {
            return preg_match('/\A(?:[a-f0-9]{1,4}:){7}[a-f0-9]{1,4}\z/i', 
                    $value);
        }
