...
abstract class Zend_Validate_Db_Abstract extends Zend_Validate_Abstract
{
     ...
     * with `field` and `value` keys
     * to define the where clause added to the sql.
     * A database adapter may optionally be supplied to avoid using the
     * registered default adapter.
     *
    ...
    public function setAdapter ($adapter)
    {
        if (! ($adapter instanceof Zend_Db_Adapter_Abstract)) {
            ...
            throw new Zend_Validate_Exception(
                    'Adapter option must be a database adapter!');
        }
    ...
    public function setSelect ($select)
    {
        if (! $select instanceof Zend_Db_Select) {
            throw new Zend_Validate_Exception(
                    'Select option must be a valid ' . 'Zend_Db_Select object');
        }
        $this->_select = $select;
        return $this;
    }
    ...
    public function getSelect ()
    {
        if (null === $this->_select) {
            ...
            if ($db->supportsParameters('named')) {
                $select->where(
                        $db->quoteIdentifier($this->_field, true) . ' = :value'); // named
            } else {
    ...
    protected function _query ($value)
    {
        ...
        $result = $select->getAdapter()->fetchRow($select, 
                array(
                        'value' => $value
                ), // this should work whether db
                                          // supports positional or named params
Zend_Db::FETCH_ASSOC);
