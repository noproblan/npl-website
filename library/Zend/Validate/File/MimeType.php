...
class Zend_Validate_File_MimeType extends Zend_Validate_Abstract
{
    ...
    /**
     * If no $_ENV['MAGIC'] is set, try and autodiscover it based on common
     * locations
     * 
     * @var array
     */
    ...
    public function getMagicFile ()
    {
        if (null === $this->_magicfile) {
            ...
            } elseif (! (@ini_get("safe_mode") == 'On' ||
                     @ini_get("safe_mode") === 1)) {
                require_once 'Zend/Validate/Exception.php';
                foreach ($this->_magicFiles as $file) {
                    // supressing errors which are thrown due to openbase_dir
                    // restrictions
                    try {
     ...
     *            Returns the values as array, when false an concated string is
     *            returned
     * @return string|array
     */
     ...
     *
     * Returns true if the mimetype of the file matches the given ones. Also
     * parts
