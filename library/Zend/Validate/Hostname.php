 ...
 * with file encoding.
 *
 * The first is tests/Zend/Validate/HostnameTestStandalone.php which is designed
 * to be run on
 * the command line.
 *
 * The second is tests/Zend/Validate/HostnameTestForm.php which is designed to
 * be run via HTML
...
class Zend_Validate_Hostname extends Zend_Validate_Abstract
{
     ...
     *      https://nic.switch.ch/reg/ocView.action?res=EF6GW2JBPVTG67DLNIQXU234MN6SC33JNQQGI7L6#anhang1
     *      (.LT) Lithuania
     *      http://www.domreg.lt/static/doc/public/idn_symbols-en.pdf
     ...
     *      (.SE) Sweden
     *      http://www.iis.se/english/IDN_campaignsite.shtml?lang=en
     ...
     *      (.VE) Venice
     *      http://www.iana.org/domains/idn-tables/tables/ve_es_1.0.html
     *      (.VN) Vietnam
     ...
     *            Set allowed to true to validate IDNs, and false to not
     *            validate them
     */
     ...
     *
     * @param boolean $allowed
     *            Set allowed to true to validate TLDs, and false to not
     *            validate them
     */
     ...
     *
     * Returns true if and only if the $value is a valid hostname with respect
     * to the current allow option
     *
     * @param string $value            
     * @throws Zend_Validate_Exception if a fatal error occurs for validation
     *         process
     * @return boolean
     */
    public function isValid ($value)
    {
        ...
        if (preg_match('/^[0-9a-f:.]*$/i', $value) &&
                 $this->_options['ip']->setTranslator($this->getTranslator())
                    ->isValid($value)) {
        ...
        if ((count($domainParts) == 4) && preg_match('/^[0-9.a-e:.]*$/i', 
                $value) &&
                 $this->_options['ip']->setTranslator($this->getTranslator())
                    ->isValid($value)) {
            $this->_error(self::INVALID_LOCAL_NAME);
        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) &&
                 (strlen($value) <= 254)) {
            $status = false;
            ...
            do {
                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    // Hostname characters are: *(label dot)(label dot label);
                    // max 254 chars
                     ...
                     * Note: Keep label regex short to avoid issues with long
                     * patterns when matching IDN hostnames
                     * 
                    ...
                    if ($this->_options['idn'] &&
                             isset($this->_validIdns[strtoupper($this->_tld)])) {
                        if (is_string(
                                $this->_validIdns[strtoupper($this->_tld)])) {
                            $regexChars += include ($this->_validIdns[strtoupper(
                                    $this->_tld)]);
                        } else {
                            $regexChars += $this->_validIdns[strtoupper(
                                    $this->_tld)];
                        }
                    }
                    ...
                    foreach ($domainParts as $domainPart) {
                        ...
                        if (strpos($domainPart, 'xn--') === 0) {
                            $domainPart = $this->decodePunycode(
                                    substr($domainPart, 4));
                            if ($domainPart === false) {
                                return false;
                            }
                        ...
                        // Check dash (-) does not start, end or appear in 3rd
                        // and 4th positions
                        if ((strpos($domainPart, '-') === 0) ||
                                 ((strlen($domainPart) > 2) &&
                                 (strpos($domainPart, '-', 2) == 2) &&
                                 (strpos($domainPart, '-', 3) == 3)) ||
                                 (strpos($domainPart, '-') ===
                                 (strlen($domainPart) - 1))) {
                        ...
                        foreach ($regexChars as $regexKey => $regexChar) {
                            ...
                            if ($status > 0) {
                                $length = 63;
                                if (array_key_exists(strtoupper($this->_tld), 
                                        $this->_idnLength) &&
                                         (array_key_exists($regexKey, 
                                                $this->_idnLength[strtoupper(
                                                        $this->_tld)]))) {
                                    $length = $this->_idnLength[strtoupper(
                                            $this->_tld)];
                                }
                    ...
                    // If one of the labels doesn't match, the hostname is
                    // invalid
            ...
            // If the input passes as an Internet domain name, and domain names
            // are allowed, then the hostname
            // passes validation
        ...
        // Check input against local network name schema; last chance to pass
        // validation
        $regexLocal = '/^(([a-zA-Z0-9\x2d]{1,63}\x2e)*[a-zA-Z0-9\x2d]{1,63}[\x2e]{0,1}){1,254}$/';
        $status = @preg_match($regexLocal, $value);
        
        // If the input passes as a local network name, and local network names
    ...
    protected function decodePunycode ($encoded)
    {
        ...
        for ($indexe = ($separator) ? ($separator + 1) : 0; $indexe < $lengthe; ++ $lengthd) {
            for ($old_index = $index, $pos = 1, $key = 36; 1; $key += 36) {
                ...
                $digit = ($hex - 48 < 10) ? $hex - 22 : (($hex - 65 < 26) ? $hex -
                         65 : (($hex - 97 < 26) ? $hex - 97 : 36));
                
                $index += $digit * $pos;
                $tag = ($key <= $base) ? 1 : (($key >= $base + 26) ? 26 : ($key -
                         $base));
                if ($digit < $tag) {
                    break;
                }
            ...
            $delta = intval(
                    $init ? (($index - $old_index) / 700) : (($index - $old_index) /
                             2));
