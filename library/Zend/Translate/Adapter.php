...
abstract class Zend_Translate_Adapter
{
     ...
     * 'clear' => when true, clears already loaded translations when adding new
     * files
    ...
    public function addTranslation ($options = array())
    {
        ...
        if (! isset($options['content']) || empty($options['content'])) {
            ...
            throw new Zend_Translate_Exception(
                    "Required option 'content' is missing");
        }
        ...
        if ((array_key_exists('log', $options)) &&
                 ! ($options['log'] instanceof Zend_Log)) {
            require_once 'Zend/Translate/Exception.php';
            throw new Zend_Translate_Exception(
                    'Instance of Zend_Log expected for option log');
        }
        
        try {
            if (! ($options['content'] instanceof Zend_Translate) &&
            ...
            require_once 'Zend/Translate/Exception.php';
            throw new Zend_Translate_Exception(
                    "The given Language '{$options['locale']}' does not exist", 
                    0, $e);
        }
        ...
        if (is_string($options['content']) and is_dir($options['content'])) {
            ...
            $iterator = new RecursiveIteratorIterator(
                    new RecursiveRegexIterator(
                            new RecursiveDirectoryIterator($options['content'], 
            ...
            foreach ($iterator as $directory => $info) {
                ...
                if (is_array($options['ignore'])) {
                    foreach ($options['ignore'] as $key => $ignore) {
                        if (strpos($key, 'regex') !== false) {
                            if (preg_match($ignore, $directory)) {
                                // ignore files matching the given regex from
                                // option 'ignore' and all files below
                                continue 2;
                            }
                        } else 
                            if (strpos($directory, 
                                    DIRECTORY_SEPARATOR . $ignore) !== false) {
                                // ignore files matching first characters from
                                // option 'ignore' and all files below
                                continue 2;
                            }
                    }
                    ...
                    if (strpos($directory, 
                            DIRECTORY_SEPARATOR . $options['ignore']) !== false) {
                        // ignore files matching first characters from option
                        // 'ignore' and all files below
                        continue;
                    }
                }
                ...
                if ($info->isDir()) {
                    // pathname as locale
                    if (($options['scan'] === self::LOCALE_DIRECTORY) and
                             (Zend_Locale::isLocale($file, true, false))) {
                        $options['locale'] = $file;
        ...
        if ((isset($this->_translate[$originate]) === true) and
                 (count($this->_translate[$originate]) > 0)) {
    ...
    public function setOptions (array $options = array())
    {
        ...
        foreach ($options as $key => $option) {
            if ($key == 'locale') {
                ...
                if ((isset($this->_options[$key]) and
                         ($this->_options[$key] != $option)) or
     ...
     * @return array
     */
    abstract protected function _loadTranslationData ($data, $locale, 
            array $options = array());
     ...
     * If the options 'clear' is true, then the translation data for the
     * specified
    ...
    private function _addTranslationData ($options = array())
    {
        ...
        if (($options['content'] instanceof Zend_Translate) ||
            ...
            if (! empty($options['locale']) && ($options['locale'] !== 'auto')) {
                $options['content'] = $options['content']->getMessages(
                        $options['locale']);
            } else {
        ...
        $read = true;
        if (isset(self::$_cache)) {
            $id = 'Zend_Translate_' . md5(serialize($options['content'])) . '_' .
                     $this->toString();
        ...
        if ($read) {
            if (! empty($options['usetranslateadapter'])) {
                ...
                $temp = $this->_loadTranslationData($options['content'], 
     ...
     *            when true, a translation for 'en_US' would give false when it
     *            can
