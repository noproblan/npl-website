...
class Zend_Translate_Adapter_Tmx extends Zend_Translate_Adapter
{
     ...
     * @param string $locale
     *            Locale has no effect for TMX because TMX defines all languages
     *            within
     *            the source file
     * @param array $option
                ...
                case 'header':
                    if (empty($this->_useId) && isset($attrib['srclang'])) {
                        if (Zend_Locale::isLocale($attrib['srclang'])) {
                            $this->_srclang = Zend_Locale::findLocale(
                                    $attrib['srclang']);
                        } else {
                            if (! $this->_options['disableNotices']) {
                                if ($this->_options['log']) {
                                    $this->_options['log']->notice(
                ...
                case 'tuv':
                    if (isset($attrib['xml:lang'])) {
                        if (Zend_Locale::isLocale($attrib['xml:lang'])) {
                            $this->_tuv = Zend_Locale::findLocale(
                                    $attrib['xml:lang']);
                        } else {
                            if (! $this->_options['disableNotices']) {
                                if ($this->_options['log']) {
    ...
    protected function _endElement ($file, $name)
    {
        if (($this->_seg !== null) and ($name !== 'seg')) {
            ...
            switch (strtolower($name)) {
                ...
                case 'seg':
                    $this->_seg = null;
                    if (! empty($this->_srclang) &&
                             ($this->_srclang == $this->_tuv)) {
                        $this->_tu = $this->_content;
                    }
                    
                    if (! empty($this->_content) or
                             (! isset($this->_data[$this->_tuv][$this->_tu]))) {
    ...
    {
        if (($this->_seg !== null) and ($this->_tu !== null) and
                 ($this->_tuv !== null)) {
            $this->_content .= $data;
        }
    }
