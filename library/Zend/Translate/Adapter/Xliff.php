...
class Zend_Translate_Adapter_Xliff extends Zend_Translate_Adapter
{
     ...
     *
     * @param string $locale
     *            Locale/Language to add data for, identical with locale
     *            identifier,
            ...
            array $options = array())
    {
        ...
        if (! is_readable($filename)) {
            ...
            throw new Zend_Translate_Exception(
                    'Translation file \'' . $filename . '\' is not readable.');
        }
    ...
    private function _endElement ($file, $name)
    {
        if (($this->_stag === true) and ($name !== 'source')) {
            ...
            if (($this->_ttag === true) and ($name !== 'target')) {
                $this->_tcontent .= "</" . $name . ">";
            } else {
                switch (strtolower($name)) {
                    case 'trans-unit':
                    ...
                    case 'source':
                        if ($this->_useId) {
                            if (! empty($this->_scontent) &&
                                     ! empty($this->_langId) &&
                                     ! isset(
                                            $this->_data[$this->_source][$this->_langId])) {
                                $this->_data[$this->_source][$this->_langId] = $this->_scontent;
                            }
                        } else {
                            if (! empty($this->_scontent) &&
                                     ! isset(
                                            $this->_data[$this->_source][$this->_scontent])) {
                    ...
                    case 'target':
                        if ($this->_useId) {
                            if (! empty($this->_tcontent) &&
                                     ! empty($this->_langId) &&
                                     ! isset(
                                            $this->_data[$this->_target][$this->_langId])) {
                                $this->_data[$this->_target][$this->_langId] = $this->_tcontent;
                            }
                        } else {
                            if (! empty($this->_tcontent) &&
                                     ! empty($this->_scontent) &&
                                     ! isset(
                                            $this->_data[$this->_target][$this->_scontent])) {
    ...
    {
        if (($this->_transunit !== null) and ($this->_source !== null) and
                 ($this->_stag === true)) {
            $this->_scontent .= $data;
        }
        
        if (($this->_transunit !== null) and ($this->_target !== null) and
                 ($this->_ttag === true)) {
            $this->_tcontent .= $data;
        }
    }
    ...
    private function _findEncoding ($filename)
    {
        ...
        if (strpos($file, "encoding") !== false) {
            ...
            $encoding = substr($encoding, 1, 
                    strpos($encoding, $encoding[0], 1) - 1);
            return $encoding;
        }
        return 'UTF-8';
    }
