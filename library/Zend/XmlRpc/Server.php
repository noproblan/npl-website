...
class Zend_XmlRpc_Server extends Zend_Server_Abstract
{
    ...
    public function __call ($method, $params)
    {
        ...
        if (! method_exists($system, $method)) {
            ...
            throw new Zend_XmlRpc_Server_Exception(
                    'Unknown instance method called on server: ' . $method);
        }
    ...
    public function addFunction ($function, $namespace = '')
    {
        ...
        foreach ($function as $func) {
            ...
            $reflection = Zend_Server_Reflection::reflectFunction($func, $argv, 
                    $namespace);
    ...
    public function setClass ($class, $namespace = '', $argv = null)
    {
        ...
        $dispatchable = Zend_Server_Reflection::reflectClass($class, $argv, 
                $namespace);
    ...
    public function loadFunctions ($definition)
    {
        if (! is_array($definition) &&
            ...
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Zend_XmlRpc_Server_Exception(
                    'Unable to load server definition; must be an array or Zend_Server_Definition, received ' .
                             $type, 612);
        }
    ...
    public function setResponseClass ($class)
    {
        if (! class_exists($class) or
                 ($c = new ReflectionClass($class) and
                 ! $c->isSubclassOf('Zend_XmlRpc_Response'))) {
     ...
     * how to handle arguments. If set to true, all methods including
     * constructor
     * will receive the arguments. If set to false, only constructor will
     * receive the
     * arguments
     */
     ...
     *         Zend_XmlRpcServer_Exceptions are thrown for internal errors;
     *         otherwise,
    ...
    protected function _handle (Zend_XmlRpc_Request $request)
    {
        ...
        if (! $this->_table->hasMethod($method)) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Zend_XmlRpc_Server_Exception(
                    'Method "' . $method . '" does not exist', 620);
        }
        ...
        if (! $matched) {
            require_once 'Zend/XmlRpc/Server/Exception.php';
            throw new Zend_XmlRpc_Server_Exception(
                    'Calling parameters do not match signature', 623);
        }
