 ...
 * called by the Zend_XmlRpc_Client object and as the return value for those
 * calls.
 *
 * This object as a very important static function
 * Zend_XmlRpc_Value::getXmlRpcValue, this
 * function acts likes a factory for the Zend_XmlRpc_Value objects
 *
 * Using this function, users/Zend_XmlRpc_Client object can create the
 * Zend_XmlRpc_Value objects
...
abstract class Zend_XmlRpc_Value
{
     ...
     *
     * If the native type of this object is array or struct, this will be an
     * array
    ...
    /**
     * Get the native XML-RPC type (the type is one of the
     * Zend_XmlRpc_Value::XMLRPC_TYPE_* constants)
     *
     * @return string
     */
     ...
     * 2. By specifing the native type ($type is one of the
     * Zend_XmlRpc_Value::XMLRPC_TYPE_* constants)
     ...
     *
     */
    public static function getXmlRpcValue ($value, 
            $type = self::AUTO_DETECT_TYPE)
    {
        switch ($type) {
            case self::AUTO_DETECT_TYPE:
                
                // Auto detect the XML-RPC native type from the PHP type of
                // $value
                return self::_phpVarToNativeXmlRpc($value);
            
            case self::XML_STRING:
                
                // Parse the XML string given in $value and get the XML-RPC
                // value in it
            ...
            default:
                ...
                throw new Zend_XmlRpc_Value_Exception(
                        'Given type is not a ' . __CLASS__ . ' constant');
        }
    }
    ...
    public static function getXmlRpcTypeByValue ($value)
    {
        ...
        throw new Zend_XmlRpc_Value_Exception(
                sprintf('No matching XMLRPC type found for php type %s.', 
                        gettype($value)));
    }
    ...
    protected static function _phpVarToNativeXmlRpc ($value)
    {
        ...
        switch (self::getXmlRpcTypeByValue($value)) {
            ...
            default:
                ...
                // If type isn't identified (or identified as string), it
                // treated as string
                require_once 'Zend/XmlRpc/Value/String.php';
                return new Zend_XmlRpc_Value_String($value);
        }
    ...
    protected static function _xmlStringToNativeXmlRpc ($xml)
    {
        ...
        switch ($type) {
            ...
            case self::XMLRPC_TYPE_STRING:
                ...
                break;
            case self::XMLRPC_TYPE_DATETIME: // The value should already be in a
                                             // iso8601 format
                require_once 'Zend/XmlRpc/Value/DateTime.php';
                $xmlrpcValue = new Zend_XmlRpc_Value_DateTime($value);
                break;
            case self::XMLRPC_TYPE_BASE64: // The value should already be base64
                                           // encoded
                require_once 'Zend/XmlRpc/Value/Base64.php';
            ...
            case self::XMLRPC_TYPE_STRUCT:
                ...
                foreach ($value->member as $member) {
                    // @todo? If a member doesn't have a <value> tag, we don't
                    // add it to the struct
                    // Maybe we want to throw an exception here ?
                    if (! isset($member->value) or ! isset($member->name)) {
                        continue;
                        // throw new Zend_XmlRpc_Value_Exception('Member of the
                    // '. self::XMLRPC_TYPE_STRUCT .' XML-RPC native type must
                    // contain a VALUE tag');
                    }
                    $values[(string) $member->name] = self::_xmlStringToNativeXmlRpc(
                            $member->value);
                }
                require_once 'Zend/XmlRpc/Value/Struct.php';
            ...
            default:
                ...
                throw new Zend_XmlRpc_Value_Exception(
                        'Value type \'' . $type .
                                 '\' parsed from the XML string is not a known XML-RPC native type');
                break;
        }
    ...
    protected static function _createSimpleXMLElement (&$xml)
    {
        ...
        try {
            ...
            require_once 'Zend/XmlRpc/Value/Exception.php';
            throw new Zend_XmlRpc_Value_Exception(
                    'Failed to create XML-RPC value from XML string: ' .
     ...
     * @return void
     */
    protected static function _extractTypeAndValue (SimpleXMLElement $xml, 
            &$type, &$value)
    {
