...
abstract class Zend_Test_PHPUnit_ControllerTestCase extends PHPUnit_Framework_TestCase
{
     ...
     *
     * If {@link $bootstrap} is a callback, executes it; if it is a file, it
     * include's
    ...
    final public function bootstrap ()
    {
        ...
        if (null !== $this->bootstrap) {
            if ($this->bootstrap instanceof Zend_Application) {
                ...
                $this->_frontController = $this->bootstrap->getBootstrap()->getResource(
                        'frontcontroller');
    ...
    /**
     * Assert against DOM selection; should contain at least this number of
     * nodes
     *
     * @param string $path
     *            CSS selector path
    ...
    /**
     * Assert against XPath selection; should contain no more than this number
     * of nodes
     *
     * @param string $path
     *            XPath path
    ...
    /**
     * Assert response header does not exist and/or does not contain the given
     * string
     *
     * @param string $header            
     * @param string $match            
    ...
    /**
     * Assert response header does not exist and/or does not match the given
     * regex
     *
     * @param string $header            
     * @param string $pattern            
    ...
    public function assertNotModule ($module, $message = '')
    {
        $this->_incrementAssertionCount();
        if ($module == $this->request->getModuleName()) {
            $msg = sprintf('Failed asserting last module used was NOT "%s"', 
                    $module);
            if (! empty($message)) {
                $msg = $message . "\n" . $msg;
            }
    ...
    public function assertAction ($action, $message = '')
    {
        ...
        if ($action != $this->request->getActionName()) {
            $msg = sprintf('Failed asserting last action used <"%s"> was "%s"', 
    ...
    public function assertNotRoute ($route, $message = '')
    {
        ...
        $router = $this->frontController->getRouter();
        if ($route == $router->getCurrentRouteName()) {
            $msg = sprintf('Failed asserting route matched was NOT "%s"', 
                    $route);
            if (! empty($message)) {
                $msg = $message . "\n" . $msg;
            }
    ...
    public function url ($urlOptions = array(), $name = null, $reset = false, $encode = true)
    {
        ...
        if (! $router instanceof Zend_Controller_Router_Rewrite) {
            throw new Exception(
                    'This url helper utility function only works when the router is of type Zend_Controller_Router_Rewrite');
        }
    ...
    public function urlizeOptions ($urlOptions, 
        ...
        foreach (debug_backtrace() as $step) {
            if (isset($step['object']) &&
                ...
                if (version_compare(PHPUnit_Runner_Version::id(), '3.3.0', 'lt')) {
                    break;
                } elseif (version_compare(PHPUnit_Runner_Version::id(), '3.3.3', 
                        'lt')) {
