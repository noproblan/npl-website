...
class Zend_Ldap
{
    ...
    /**
     *
     * @deprecated will be removed, use {@see
     ...
     * @return boolean True if the DN was successfully parsed or false if the
     *         string is
     *         not a valid DN.
     */
            ...
            if (array_key_exists($key, $options)) {
                ...
                switch ($key) {
                    ...
                    case 'tryUsernameSplit':
                        $permittedOptions[$key] = ($val === true || $val === '1' ||
    ...
    /**
     *
     * @return string The hostname of the LDAP server being used to authenticate
     *         accounts
     */
    ...
    /**
     *
     * @return integer Either ACCTNAME_FORM_BACKSLASH, ACCTNAME_FORM_PRINCIPAL
     *         or
    ...
    {
        /*
         * Account names should always be qualified with a domain. In some
         * scenarios
         * using non-qualified account names can lead to security
         * vulnerabilities. If
    ...
    /**
     *
     * @return string A format string for building an LDAP search filter to
     *         match
     *         an account
     */
    ...
    protected function _getAccountDn ($acctname)
    {
        ...
        if (Zend_Ldap_Dn::checkDn($acctname))
            return $acctname;
        $acctname = $this->getCanonicalAccountName($acctname, 
    ...
    public function getCanonicalAccountName ($acctname, $form = 0)
    {
        ...
        if (! $uname) {
            ...
            require_once 'Zend/Ldap/Exception.php';
            throw new Zend_Ldap_Exception(null, 
                    "Invalid account name syntax: $acctname");
        }
        ...
        switch ($form) {
            ...
            default:
                ...
                require_once 'Zend/Ldap/Exception.php';
                throw new Zend_Ldap_Exception(null, 
                        "Unknown canonical name form: $form");
        }
    }
    ...
    public function connect ($host = null, $port = null, $useSsl = null, $useStartTls = null)
    {
         ...
         */
        $hosts = array();
        if (preg_match_all('~ldap(?:i|s)?://~', $host, $hosts, PREG_SET_ORDER) >
                 0) {
         ...
         * use the old form.
         */
        $resource = ($useUri) ? @ldap_connect($this->_connectString) : @ldap_connect(
                $host, $port);
        ...
        if (is_resource($resource) === true) {
            ...
            if (@ldap_set_option($resource, LDAP_OPT_PROTOCOL_VERSION, 3) &&
                     @ldap_set_option($resource, LDAP_OPT_REFERRALS, 
                            $optReferrals)) {
        ...
        require_once 'Zend/Ldap/Exception.php';
        throw new Zend_Ldap_Exception(null, 
                "Failed to connect to LDAP server: $host:$port");
    }
    ...
    public function bind ($username = null, $password = null)
    {
                ...
                if ($this->_getBindRequiresDn()) {
                    ...
                    if ($moreCreds) {
                        try {
                            ...
                            throw new Zend_Ldap_Exception(null, 
                                    'Failed to retrieve DN for account: ' .
                                             $username . ' [' .
                                             $zle->getMessage() . ']', 
        ...
        if ($username !== null && $password === '' &&
                 $this->_getAllowEmptyPassword() !== true) {
            /**
             *
             * @see Zend_Ldap_Exception
             */
    ...
    public function search ($filter, $basedn = null, $scope = self::SEARCH_SCOPE_SUB, 
            array $attributes = array(), $sort = null, $collectionClass = null, $sizelimit = 0, 
            $timelimit = 0)
    {
        if (is_array($filter)) {
        ...
        switch ($scope) {
            case self::SEARCH_SCOPE_ONE:
                $search = @ldap_list($this->getResource(), $basedn, $filter, 
                        $attributes, 0, $sizelimit, $timelimit);
                break;
            case self::SEARCH_SCOPE_BASE:
                $search = @ldap_read($this->getResource(), $basedn, $filter, 
                        $attributes, 0, $sizelimit, $timelimit);
                break;
            case self::SEARCH_SCOPE_SUB:
            default:
                $search = @ldap_search($this->getResource(), $basedn, $filter, 
                        $attributes, 0, $sizelimit, $timelimit);
                break;
        }
    ...
    protected function _createCollection (
            Zend_Ldap_Collection_Iterator_Default $iterator, $collectionClass)
    {
    ...
    {
        return ($this->count('(objectClass=*)', $dn, self::SEARCH_SCOPE_BASE) ==
                 1);
    }
     ...
     * @throws Zend_Ldap_Exception
     */
    public function searchEntries ($filter, $basedn = null, 
            $scope = self::SEARCH_SCOPE_SUB, array $attributes = array(), $sort = null, $reverseSort = false, 
     ...
     * @throws Zend_Ldap_Exception
     */
    public function getEntry ($dn, array $attributes = array(), 
            $throwOnNotFound = false)
    {
        try {
            $result = $this->search("(objectClass=*)", $dn, 
                    self::SEARCH_SCOPE_BASE, $attributes, null);
                    ...
                    if ($v === null)
                        ...
                        if (! is_scalar($v)) {
                            throw new InvalidArgumentException(
                                    'Only scalar values allowed in LDAP data');
                        } else {
                            $v = (string) $v;
                            if (strlen($v) == 0) {
                                unset($value[$i]);
    ...
    public function update ($dn, array $entry)
    {
        ...
        foreach ($rdnParts as $key => $value) {
            $value = Zend_Ldap_Dn::unescapeValue($value);
            if (array_key_exists($key, $entry) && ! in_array($value, 
                    $entry[$key])) {
                $entry[$key] = array_merge(array(
                        $value
                ), $entry[$key]);
        ...
        if (count($entry) > 0) {
            $isModified = @ldap_modify($this->getResource(), $dn->toString(), 
                    $entry);
            if ($isModified === false) {
                /**
                 *
                 ...
                 */
                require_once 'Zend/Ldap/Exception.php';
                throw new Zend_Ldap_Exception($this, 
                        'updating: ' . $dn->toString());
            }
        }
        return $this;
    }
    ...
    protected function _getChildrenDns ($parentDn)
    {
        ...
        $children = array();
        $search = @ldap_list($this->getResource(), $parentDn, '(objectClass=*)', 
                array(
                        'dn'
                ));
        for ($entry = @ldap_first_entry($this->getResource(), $search); $entry !==
                 false; $entry = @ldap_next_entry($this->getResource(), $entry)) {
     ...
     * @throws Zend_Ldap_Exception
     */
    public function moveToSubtree ($from, $to, $recursively = false, 
            $alwaysEmulate = false)
    {
     ...
     *
     * This method implicitely moves the entry to another location within the
     * tree.
     *
     * @param string|Zend_Ldap_Dn $from            
     * @param string|Zend_Ldap_Dn $to            
    ...
    public function rename ($from, $to, $recursively = false, $alwaysEmulate = false)
    {
        ...
        if ($emulate === false) {
            ...
            $isOK = @ldap_rename($this->getResource(), $from, $newRdn, 
                    $newParent, true);
            if ($isOK === false) {
                /**
                 *
