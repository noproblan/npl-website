 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cloud_StorageService_Adapter_S3 implements 
        Zend_Cloud_StorageService_Adapter
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (isset($options[self::HTTP_ADAPTER])) {
            $this->_s3->getHttpClient()->setAdapter(
    ...
    public function fetchItem ($path, $options = array())
    {
        ...
        try {
            if (! empty($options[self::FETCH_STREAM])) {
                return $this->_s3->getObjectStream($fullPath, 
                        $options[self::FETCH_STREAM]);
            } else {
    ...
    {
        try {
            // TODO We *really* need to add support for object copying in the S3
            // adapter
    ...
    public function moveItem ($sourcePath, $destinationPath, $options = array())
    {
        try {
            ...
            return $this->_s3->moveObject($fullSourcePath, $fullDestPath, 
                    empty($options[self::METADATA]) ? null : $options[self::METADATA]);
        } catch (Zend_Service_Amazon_S3_Exception $e) {
            throw new Zend_Cloud_StorageService_Exception(
    ...
    {
        try {
            // TODO Support 'prefix' parameter for
            // Zend_Service_Amazon_S3::getObjectsByBucket()
            ...
            $options = array())
    {
        require_once 'Zend/Cloud/OperationNotAvailableException.php';
        throw new Zend_Cloud_OperationNotAvailableException(
                'Storing separate metadata is not supported, use storeItem() with \'metadata\' option key');
    }
    ...
    protected function _getFullPath ($path, $options)
    {
        if (isset($options[self::BUCKET_NAME])) {
            ...
            if (isset($this->_defaultBucketName)) {
                $bucket = $this->_defaultBucketName;
            } else {
                require_once 'Zend/Cloud/StorageService/Exception.php';
                throw new Zend_Cloud_StorageService_Exception(
                        'Bucket name must be specified for S3 adapter.');
            }
        ...
        if (isset($options[self::BUCKET_AS_DOMAIN])) {
            ...
            require_once 'Zend/Cloud/StorageService/Exception.php';
            throw new Zend_Cloud_StorageService_Exception(
                    'The S3 adapter does not currently support buckets in domain names.');
        }
