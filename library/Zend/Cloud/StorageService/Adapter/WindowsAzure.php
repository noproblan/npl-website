 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cloud_StorageService_Adapter_WindowsAzure implements 
        Zend_Cloud_StorageService_Adapter
{
    ...
    public function fetchItem ($path, $options = null)
    {
        ...
        // Fetch the blob
        try {
            $this->_storageClient->getBlob($this->_container, $path, 
                    $returnPath);
    ...
    public function storeItem ($destinationPath, $data, $options = null)
    {
        ...
        try {
            // Upload data
            $this->_storageClient->putBlob($this->_container, $destinationPath, 
                    $temporaryFilePath);
        } catch (Zend_Service_WindowsAzure_Exception $e) {
    ...
    {
        try {
            $this->_storageClient->copyBlob($this->_container, $sourcePath, 
    ...
    {
        // Options
        $returnType = self::RETURN_NAMES; // 1: return list of paths, 2: return
                                          // raw output from underlying provider
        ...
        try {
            // Fetch list
            $blobList = $this->_storageClient->listBlobs($this->_container, 
                    $path);
        } catch (Zend_Service_WindowsAzure_Exception $e) {
            throw new Zend_Cloud_StorageService_Exception(
    ...
    {
        try {
            return $this->_storageClient->getBlobMetaData($this->_container, 
                    $path);
        } catch (Zend_Service_WindowsAzure_Exception $e) {
    ...
    {
        try {
            $this->_storageClient->setBlobMetadata($this->_container, 
                    $destinationPath, $metadata);
        } catch (Zend_Service_WindowsAzure_Exception $e) {
            if (strpos($e->getMessage(), "could not be accessed") === false) {
                throw new Zend_Cloud_StorageService_Exception(
                        'Error on store metadata: ' . $e->getMessage(), 
                        $e->getCode(), $e);
            }
        }
    }
    ...
    public function deleteMetadata ($path, $options = null)
    {
        try {
            ...
            if (strpos($e->getMessage(), "could not be accessed") === false) {
                throw new Zend_Cloud_StorageService_Exception(
                        'Error on delete metadata: ' . $e->getMessage(), 
                        $e->getCode(), $e);
            }
        }
    }
