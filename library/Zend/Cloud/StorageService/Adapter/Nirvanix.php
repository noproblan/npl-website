 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Cloud_StorageService_Adapter_Nirvanix implements 
        Zend_Cloud_StorageService_Adapter
{
    ...
    function __construct ($options = array())
    {
        ...
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on create: ' . $e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function fetchItem ($path, $options = null)
    {
        ...
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on fetch: ' . $e->getMessage(), $e->getCode(), $e);
        }
        return $item;
    }
    ...
    public function storeItem ($destinationPath, $data, $options = null)
    {
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on store: ' . $e->getMessage(), $e->getCode(), $e);
        }
        return true;
    }
    ...
    public function deleteItem ($path, $options = null)
    {
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on delete: ' . $e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function copyItem ($sourcePath, $destinationPath, $options = null)
    {
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on copy: ' . $e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function moveItem ($sourcePath, $destinationPath, $options = null)
    {
        try {
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on move: ' . $e->getMessage(), $e->getCode(), $e);
        }
    }
     ...
     * @param string $path            
     * @param array $options            
     * @return array An associative array of key/value pairs specifying the
    ...
    public function fetchMetadata ($path, $options = null)
    {
        $path = $this->_getFullPath($path);
        try {
            $metadataNode = $this->_metadataNs->getMetadata(
                    array(
                            'path' => $path
                    ));
        } catch (Zend_Service_Nirvanix_Exception $e) {
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on fetching metadata: ' . $e->getMessage(), 
                    $e->getCode(), $e);
        }
     ...
     *            associative array specifying the key/value pairs for the
     *            metadata.
     * @param array $options            
     * @return void
     */
    public function storeMetadata ($destinationPath, $metadata, $options = null)
    {
        ...
        if ($metadata != null) {
            try {
                ...
                throw new Zend_Cloud_StorageService_Exception(
                        'Error on storing metadata: ' . $e->getMessage(), 
                        $e->getCode(), $e);
            }
        }
    }
     ...
     * @param string $path            
     * @param array $metadata
     *            - An associative array specifying the key/value pairs for the
     *            metadata
     *            to be deleted. If null, all metadata associated with the
     *            object will
     *            be deleted.
     * @param array $options            
     * @return void
    ...
    public function deleteMetadata ($path, $metadata = null, $options = null)
    {
        ...
        try {
            if ($metadata == null) {
                ...
                foreach ($metadata as $key => $value) {
                    $this->_metadataNs->DeleteMetadata(
                            array(
                                    'path' => $path,
                                    'metadata' => $key
                            ));
            ...
            throw new Zend_Cloud_StorageService_Exception(
                    'Error on deleting metadata: ' . $e->getMessage(), 
                    $e->getCode(), $e);
        }
    }
     ...
     * @param array& $resultArray reference to the array that contains the path
     * names
     * for each folder.
     */
    private function getAllFolders ($path, &$resultArray)
    {
        $response = $this->_imfNs->ListFolder(
                array(
                        'folderPath' => $path,
                        'pageNumber' => 1,
        ...
        if ($numFolders == 0) {
            ...
            // Need to special case this as Nirvanix returns an array if there
            // is
            // more than one, but doesn't return an array if there is only one.
            if ($numFolders == 1) {
    ...
    public function listItems ($path, $options = null)
    {
        ...
        if (! isset($path)) {
            ...
            try {
                ...
                throw new Zend_Cloud_StorageService_Exception(
                        'Error on list: ' . $e->getMessage(), $e->getCode(), $e);
            }
            ...
            if ($numFiles != 0) {
                // Need to special case this as Nirvanix returns an array if
                // there is
                // more than one, but doesn't return an array if there is only
                // one.
                if ($numFiles == 1) {
