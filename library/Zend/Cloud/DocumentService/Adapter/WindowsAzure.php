...
class Zend_Cloud_DocumentService_Adapter_WindowsAzure extends Zend_Cloud_DocumentService_Adapter_AbstractAdapter
{
    ...
    public function __construct ($options = array())
    {
        ...
        try {
            ...
            if (! empty($options[self::PROXY_HOST])) {
                ...
                $this->_storageClient->setProxy(true, $proxyHost, $proxyPort, 
                        $proxyCredentials);
            }
            if (isset($options[self::HTTP_ADAPTER])) {
                $this->_storageClient->setHttpClientChannel(
                        $options[self::HTTP_ADAPTER]);
            }
    ...
    public function createCollection ($name, $options = null)
    {
        ...
        try {
            ...
            if (strpos($e->getMessage(), "table specified already exists") ===
                     false) {
                throw new Zend_Cloud_DocumentService_Exception(
            ...
            if (isset($document[self::ROW_KEY])) {
                ...
                if (isset($document[self::PARTITION_KEY])) {
                    ...
                    unset($document[self::PARTITION_KEY]);
                         ...
                         ($partitionKey = $this->getDefaultPartitionKey())) {
                    $key = array(
                            $partitionKey,
                            $rowKey
                    );
    ...
    public function replaceDocument ($collectionName, $document, $options = null)
    {
        ...
        try {
            ...
            $this->_storageClient->updateEntity($collectionName, $entity, 
                    isset($options[self::VERIFY_ETAG]));
        } catch (Zend_Service_WindowsAzure_Exception $e) {
            throw new Zend_Cloud_DocumentService_Exception(
     ...
     * @param
     *            null|array|Zend_Cloud_DocumentService_Document Fields to
     *            update (or new fields))
     * @param array $options            
     * @return boolean
     */
    ...
    {
        if (null === $fieldset &&
                 $documentId instanceof Zend_Cloud_DocumentService_Document) {
        ...
        try {
            ...
            $this->_storageClient->mergeEntity($collectionName, $entity, 
                    isset($options[self::VERIFY_ETAG]));
        } catch (Zend_Service_WindowsAzure_Exception $e) {
            throw new Zend_Cloud_DocumentService_Exception(
            ...
            $options = null)
    {
        ...
        try {
            ...
            $this->_storageClient->deleteEntity($collectionName, $entity, 
                    isset($options[self::VERIFY_ETAG]));
        } catch (Zend_Service_WindowsAzure_Exception $e) {
            if (strpos($e->getMessage(), "does not exist") === false) {
                throw new Zend_Cloud_DocumentService_Exception(
    ...
    public function fetchDocument ($collectionName, $documentId, $options = null)
    {
        $documentId = $this->_validateDocumentId($documentId, $collectionName);
        try {
            $entity = $this->_storageClient->retrieveEntityById($collectionName, 
                    $documentId[0], $documentId[1]);
            $documentClass = $this->getDocumentClass();
            return new $documentClass($this->_resolveAttributes($entity), 
                    array(
    ...
    public function query ($collectionName, $query, $options = null)
    {
        try {
            if ($query instanceof Zend_Cloud_DocumentService_Adapter_WindowsAzure_Query) {
                $entities = $this->_storageClient->retrieveEntities(
                        $query->assemble());
            } else {
                $entities = $this->_storageClient->retrieveEntities(
    ...
    public function select ($fields = null)
    {
        ...
        if (! $query instanceof $defaultClass) {
            throw new Zend_Cloud_DocumentService_Exception(
                    'Query class must extend ' . self::DEFAULT_QUERY_CLASS);
        }
     ...
     * @return array
     */
    protected function _resolveAttributes (
     ...
     * - if a collection name is provided, it will use that for the partition
     * key
    ...
    protected function _validateDocumentId ($documentId, $collectionName = false)
    {
        ...
        if (! is_string($documentId)) {
            throw new Zend_Cloud_DocumentService_Exception(
                    'Invalid document identifier; must be a string or an array');
        }
    ...
    public function _validateFields ($document)
    {
        if ($document instanceof Zend_Cloud_DocumentService_Document) {
            ...
            throw new Zend_Cloud_DocumentService_Exception(
                    'Cannot inspect fields; invalid type provided');
        }
    ...
    public function _validateFieldKey ($key)
    {
        if (! preg_match('/^[_A-Za-z][-._A-Za-z0-9]*$/', $key)) {
            throw new Zend_Cloud_DocumentService_Exception(
                    'Field keys must conform to XML names (^[_A-Za-z][-._A-Za-z0-9]*$); key "' .
                             $key . '" does not match');
        }
    }
}
