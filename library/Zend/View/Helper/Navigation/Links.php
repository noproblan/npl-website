...
class Zend_View_Helper_Navigation_Links extends Zend_View_Helper_Navigation_HelperAbstract
{
    ...
    public function __call ($method, array $arguments = array())
    {
        if (@preg_match('/find(Rel|Rev)(.+)/', $method, $match)) {
            return $this->findRelation($arguments[0], strtolower($match[1]), 
                    strtolower($match[2]));
        }
     ...
     */
    protected function _findFromProperty (Zend_Navigation_Page $page, $rel, 
            $type)
    {
    ...
    public function searchRelChapter (Zend_Navigation_Page $page)
    {
        ...
        foreach ($root as $chapter) {
            ...
            if ($chapter !== $page && ! in_array($chapter, $start) &&
                     $this->accept($chapter)) {
                $found[] = $chapter;
            }
        }
    ...
    public function renderLink (Zend_Navigation_Page $page, $attrib, $relation)
    {
        ...
        return '<link' . $this->_htmlAttribs($attribs) .
