...
class Zend_View_Helper_Navigation_Menu extends Zend_View_Helper_Navigation_HelperAbstract
{
    ...
    public function htmlify (Zend_Navigation_Page $page)
    {
        ...
        return '<' . $element . $this->_htmlAttribs($attribs) . '>' .
                 $this->view->escape($label) . '</' . $element . '>';
    }
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth < $minDepth || ! $this->accept($page)) {
                ...
                if ($onlyActive && ! $isActive) {
                    // page is not active itself, but might be in the active
                    // branch
                    $accept = false;
                    if ($foundPage) {
                        if ($foundPage->hasPage($page)) {
                            // accept if page is a direct child of the active
                            // page
                            $accept = true;
                        } else 
                            if ($foundPage->getParent()->hasPage($page)) {
                                ...
                                if (! $foundPage->hasPages() ||
                                    ...
                                    // accept if active page has no children, or
                                    // the
            ...
            array $options = array())
    {
        ...
        if ($options['onlyActiveBranch'] && ! $options['renderParents']) {
            $html = $this->_renderDeepestMenu($container, $options['ulClass'], 
                    $options['indent'], $options['minDepth'], 
                    $options['maxDepth']);
        } else {
            $html = $this->_renderMenu($container, $options['ulClass'], 
                    $options['indent'], $options['minDepth'], 
                    $options['maxDepth'], $options['onlyActiveBranch']);
        }
