...
class Zend_View_Helper_HeadMeta extends Zend_View_Helper_Placeholder_Container_Standalone
{
     ...
     */
    public function headMeta ($content = null, $keyValue = null, $keyType = 'name', 
            $modifiers = array(), 
        ...
        switch ($type) {
            ...
            default:
                ...
                $e = new Zend_View_Exception(
                        sprintf('Invalid type "%s" passed to _normalizeType', 
                                $type));
                $e->setView($this->view);
                throw $e;
        }
    ...
    {
        if ((! $item instanceof stdClass) || ! isset($item->type) ||
    ...
    public function itemToString (stdClass $item)
    {
        if (! in_array($item->type, $this->_typeKeys)) {
            require_once 'Zend/View/Exception.php';
            $e = new Zend_View_Exception(
                    sprintf('Invalid type "%s" provided for meta', $item->type));
            $e->setView($this->view);
            throw $e;
        }
        $type = $item->type;
        ...
        $meta = sprintf($tpl, $type, $this->_escape($item->$type), 
                $this->_escape($item->content), $modifiersString);
        return $meta;
    }
    ...
    public function toString ($indent = null)
    {
        ...
        return $indent .
                 implode($this->_escape($this->getSeparator()) . $indent, 
                        $items);
    }
