...
class Zend_Feed
{
    ...
    public static function import ($uri)
    {
        ...
        if ($response->getStatus() !== 200) {
            ...
            throw new Zend_Feed_Exception(
                    'Feed failed to load, got response code ' .
                             $response->getStatus());
        }
    ...
    public static function findFeeds ($uri)
    {
        ...
        if ($response->getStatus() !== 200) {
            ...
            require_once 'Zend/Feed/Exception.php';
            throw new Zend_Feed_Exception(
                    "Failed to access $uri, got response code " .
                             $response->getStatus());
        }
        ...
        if (isset($matches[1]) && count($matches[1]) > 0) {
            foreach ($matches[1] as $link) {
                ...
                if (! isset($attributes['rel']) ||
                         ! @preg_match('~^(?:alternate|service\.feed)~i', 
                                $attributes['rel'])) {
                    continue;
                }
                if (! isset($attributes['type']) ||
                         ! @preg_match('~^application/(?:atom|rss|rdf)\+xml~', 
                                $attributes['type'])) {
                    continue;
                }
                ...
                try {
                    ...
                    try {
                        ...
                        if (substr($path, 0, 1) != '/') {
                            ...
                            $path = rtrim($client->getUri()->getPath(), '/') .
                                     '/' . $path;
                        }
    ...
    /**
     * Construct a new Zend_Feed_Abstract object from a
     * Zend_Feed_Builder_Interface data source
     *
     ...
     * @return Zend_Feed_Abstract
     */
    public static function importBuilder (Zend_Feed_Builder_Interface $builder, 
            $format = 'atom')
    {
        $obj = 'Zend_Feed_' . ucfirst(strtolower($format));
        if (! class_exists($obj)) {
