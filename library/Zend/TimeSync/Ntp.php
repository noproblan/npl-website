...
class Zend_TimeSync_Ntp extends Zend_TimeSync_Protocol
{
    ...
    protected function _prepare ()
    {
         ...
         * Clock Dispersion
         *
         * Indicates the maximum error relative to the primary reference source
         * at the
        ...
        /*
         * The local time, in timestamp format, at the peer when its latest NTP
         * message
    ...
    protected function _read ()
    {
        ...
        $result = array(
                ...
                'referencestamp' => $this->_getTimestamp(
                        fread($this->_socket, 8)),
                'originatestamp' => $this->_getTimestamp(
                        fread($this->_socket, 8)),
                'receivestamp' => $this->_getTimestamp(fread($this->_socket, 8)),
                'transmitstamp' => $this->_getTimestamp(
                        fread($this->_socket, 8)),
    ...
    protected function _extract ($binary)
    {
        ...
        switch ($binary['stratum']) {
            case 0:
                if (substr($refid, 0, 3) === 'DCN') {
                    ...
                    if (substr($refid, 0, 4) === 'NIST') {
                        $ntpserviceid = 'NIST public modem';
                    } else 
                        if (substr($refid, 0, 3) === 'TSP') {
                            $ntpserviceid = 'TSP time protocol';
                        } else 
                            if (substr($refid, 0, 3) === 'DTS') {
        ...
        /*
         * Indicates the total roundtrip delay to the primary reference source
         * at the
         * root of the synchronization subnet, in seconds.
         *
         * Both positive and negative values, depending on clock precision and
         * skew, are
         * possible.
         */
