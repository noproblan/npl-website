...
class Zend_Session extends Zend_Session_Abstract
{
     ...
     *
     * @var bool|bitset This could also be a combiniation of error codes to
     *      catch
     */
    ...
    /**
     * Default number of seconds the session will be remembered for when asked
     * to be remembered
     *
     * @var int
     */
    ...
    /**
     * Whether the default options listed in Zend_Session::$_localOptions have
     * been set
     *
     * @var bool
     */
     ...
     * session is started. If called prior to session starting, session id will
     * be regenerated
     * at start time.
     *
     * @throws Zend_Session_Exception
    ...
    /**
     * rememberMe() - Write a persistent cookie that expires after a number of
     * seconds in the future.
     * If no number of
     * seconds is specified, then this defaults to self::$_rememberMeSeconds.
     * Due to clock errors on end users' systems,
     * large values are recommended to avoid undesirable expiration of session
     * cookies.
     *
     * @param int $seconds
    ...
    /**
     * forgetMe() - Write a volatile session cookie, removing any persistent
     * cookie that may have existed.
     * The session
    ...
    /**
     * rememberUntil() - This method does the work of changing the state of the
     * session cookie and making
    ...
    public static function rememberUntil ($seconds = 0)
    {
        ...
        session_set_cookie_params($seconds, $cookieParams['path'], 
     ...
     *            OPTIONAL Either user supplied options, or flag indicating if
     *            start initiated automatically
     * @throws Zend_Session_Exception
     * @return void
     */
    public static function start ($options = false)
    {
        ...
        // In strict mode, do not allow auto-starting Zend_Session, such as via
        ...
        if (! self::$_unitTestEnabled) {
            ...
            if (! $startedCleanly ||
                     Zend_Session_Exception::$sessionStartError != null) {
                if (self::$_throwStartupExceptions) {
                    set_error_handler(
                            array(
                                    'Zend_Session_Exception',
                ...
                if (self::$_throwStartupExceptions) {
                    ...
                    throw new Zend_Session_Exception(
                            __CLASS__ . '::' . __FUNCTION__ . '() - ' .
                                     Zend_Session_Exception::$sessionStartError);
                }
            }
        }
    ...
    private static function _processStartupMetadataGlobal ()
    {
        ...
        if (isset($_SESSION['__ZF'])) {
            ...
            foreach ($_SESSION['__ZF'] as $namespace => $namespace_metadata) {
                ...
                if (isset($namespace_metadata['ENT']) &&
                         ($namespace_metadata['ENT'] > 0) &&
                         (time() > $namespace_metadata['ENT'])) {
                    unset($_SESSION[$namespace]);
                    unset($_SESSION['__ZF'][$namespace]);
                }
                
                // Expire Namespace by Global Hop (ENGH) if it wasnt expired
                // above
                if (isset($_SESSION['__ZF'][$namespace]) &&
                         isset($namespace_metadata['ENGH']) &&
                         $namespace_metadata['ENGH'] >= 1) {
                ...
                if (isset($namespace_metadata['ENVGH'])) {
                    foreach ($namespace_metadata['ENVGH'] as $variable => $hops) {
                        ...
                        if ($_SESSION['__ZF'][$namespace]['ENVGH'][$variable] ===
                                 0) {
    ...
    /**
     * isStarted() - convenience method to determine if the session is already
     * started.
     *
     * @return bool
     */
    ...
    /**
     * isRegenerated() - convenience method to determine if
    ...
    /**
     * registerValidator() - register a validator that will attempt to validate
     * this session for
    ...
    /**
     * writeClose() - Shutdown the sesssion, close writing and detach $_SESSION
     * from the back-end storage mechanism.
     * This will complete the internal data transformation on this request.
     *
     * @param bool $readonly
     *            - OPTIONAL remove write access (i.e. throw error if
     *            Zend_Session's attempt writes)
     * @return void
     */
    ...
    /**
     * destroy() - This is used to destroy session data, and optionally, the
     * session cookie itself
     *
     * @param bool $remove_cookie
     *            - OPTIONAL remove session id cookie, defaults to true (remove
     *            cookie)
     * @param bool $readonly
     *            - OPTIONAL remove write access (i.e. throw error if
     *            Zend_Session's attempt writes)
    ...
    /**
     * expireSessionCookie() - Sends an expired session id cookie, causing the
     * client to delete the session cookie
     *
     * @return void
     */
...
$cookie_params['path'], $cookie_params['domain'], $cookie_params['secure']);
    ...
    /**
     * _processValidator() - internal function that is called in the existence
     * of VALID metadata
     *
     * @throws Zend_Session_Exception
     * @return void
    ...
    /**
     * getIterator() - return an iteratable object for use in foreach and the
     * like,
    ...
    public static function getIterator ()
    {
        ...
        return new ArrayObject(
                array_merge($spaces, array_keys(parent::$_expiringData)));
    }

    /**
     * isWritable() - returns a boolean indicating if namespaces can write (use
     * setters)
     *
     * @return bool
     */
    ...
    /**
     * isReadable() - returns a boolean indicating if namespaces can write (use
     * setters)
     *
     * @return bool
     */
