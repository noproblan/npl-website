...
class Zend_Mime
{
    ...
    /**
     * Encode a given string with the QUOTED_PRINTABLE mechanism for Mail
     * Headers.
     *
    ...
    public static function encodeQuotedPrintableHeader ($str, $charset, 
            $lineLength = self::LINELENGTH, $lineEnd = self::LINEEND)
    {
        // Reduce line-length by the length of the required delimiter, charsets
        // and encoding
        ...
        while (strlen($str) > 0) {
            ...
            if ($token == '=20') {
                ...
                // the
                // tempstring it to the current line or start a new line if
                // necessary.
     ...
     * @return string
     */
    public static function encodeBase64Header ($str, $charset, 
            $lineLength = self::LINELENGTH, $lineEnd = self::LINEEND)
    {
        $prefix = '=?' . $charset . '?B?';
        $suffix = '?=';
        ...
        $encodedValue = str_replace($lineEnd, 
                $suffix . $lineEnd . ' ' . $prefix, $encodedValue);
     ...
     * @return string
     */
    public static function encodeBase64 ($str, $lineLength = self::LINELENGTH, 
