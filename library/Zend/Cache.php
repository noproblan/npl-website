...
abstract class Zend_Cache
{
            ...
            $customFrontendNaming = false, $customBackendNaming = false, $autoload = false)
    {
        if (is_string($backend)) {
            $backendObject = self::_makeBackend($backend, $backendOptions, 
                    $customBackendNaming, $autoload);
        } else {
            if ((is_object($backend)) &&
                     (in_array('Zend_Cache_Backend_Interface', 
                            class_implements($backend)))) {
            ...
            $customBackendNaming = false, $autoload = false)
    {
        ...
        if (in_array($backend, Zend_Cache::$standardBackends)) {
            ...
            if (! $autoload) {
                $file = str_replace('_', DIRECTORY_SEPARATOR, $backendClass) .
                         '.php';
    ...
    public static function _makeFrontend ($frontend, $frontendOptions = array(), 
            $customFrontendNaming = false, $autoload = false)
    {
        ...
        if (in_array($frontend, self::$standardFrontends)) {
            ...
            // the Core itself
            $frontendClass = 'Zend_Cache_' .
                     ($frontend != 'Core' ? 'Frontend_' : '') . $frontend;
            // security controls are explicit
            ...
            if (! $autoload) {
                $file = str_replace('_', DIRECTORY_SEPARATOR, $frontendClass) .
                         '.php';
                if (! (self::_isReadable($file))) {
                    self::throwException("file $file not found in include_path");
                }
