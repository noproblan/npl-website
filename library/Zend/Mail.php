...
class Zend_Mail extends Zend_Mime_Message
{
     ...
     */
    public static function setDefaultTransport (
     ...
     *
     * @param string $encoding
     *            Zend_Mime::ENCODING_QUOTEDPRINTABLE or
     ...
     *
     * @param
     *            bool textOnly Whether to return just the body text content or
     ...
     *
     * @param bool $htmlOnly
     *            Whether to return the body HTML only, or the MIME part;
     *            defaults to false, the MIME part
     * @return false|Zend_Mime_Part|string
     */
    ...
    public function createAttachment ($body, 
            ...
            $encoding = Zend_Mime::ENCODING_BASE64, $filename = null)
    {
    ...
    protected function _encodeHeader ($value)
    {
        if (Zend_Mime::isPrintable($value) === false) {
            if ($this->getHeaderEncoding() ===
                     Zend_Mime::ENCODING_QUOTEDPRINTABLE) {
                $value = Zend_Mime::encodeQuotedPrintableHeader($value, 
                        $this->getCharset(), Zend_Mime::LINELENGTH, 
                        Zend_Mime::LINEEND);
            } else {
                $value = Zend_Mime::encodeBase64Header($value, 
                        $this->getCharset(), Zend_Mime::LINELENGTH, 
                        Zend_Mime::LINEEND);
            }
        }
    ...
    protected function _addRecipientAndHeader ($headerName, $email, $name)
    {
        ...
        $this->_storeHeader($headerName, $this->_formatAddress($email, $name), 
                true);
    }
    ...
    public function addTo ($email, $name = '')
    {
        ...
        foreach ($email as $n => $recipient) {
            $this->_addRecipientAndHeader('To', $recipient, 
                    is_int($n) ? '' : $n);
            $this->_to[] = $recipient;
        }
    ...
    public function addCc ($email, $name = '')
    {
        ...
        foreach ($email as $n => $recipient) {
            $this->_addRecipientAndHeader('Cc', $recipient, 
                    is_int($n) ? '' : $n);
        }
    ...
    public function setReplyTo ($email, $name = null)
    {
        ...
        $name = $this->_filterName($name);
        $this->_replyTo = $email;
        $this->_storeHeader('Reply-To', $this->_formatAddress($email, $name), 
                true);
