...
/**
 * Wrapper around Zend_Http_Client to facilitate Google's "Account
 * Authentication
...
class Zend_Gdata_AuthSub
{
     ...
     *            accessed.
     *            The resulting token will enable access to the specified
     *            service only.
     *            Some services may limit scope further, such as read-only
     *            access.
     * @param string $scope
     *            (required) URL identifying the service to be
     ...
     */
    public static function getAuthSubTokenUri ($next, $scope, $secure = 0, 
            $session = 0, $request_uri = self::AUTHSUB_REQUEST_URI)
    {
        $querystring = '?next=' . urlencode($next) . '&scope=' .
                 urldecode($scope) . '&secure=' . urlencode($secure) .
    ...
    public static function getHttpClient ($token, $client = null)
    {
        ...
        if (! $client instanceof Zend_Gdata_HttpClient) {
            ...
            throw new Zend_Gdata_App_HttpException(
                    'Client is not an instance of Zend_Gdata_HttpClient.');
        }
