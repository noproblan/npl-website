...
class Zend_Gdata_YouTube extends Zend_Gdata_Media
{
    ...
    public function __construct ($client = null, 
            $applicationId = 'MyCompany-MyApp-1.0', $clientId = null, $developerKey = null)
    {
     ...
     *            entry. Only possible if entry belongs to currently
     *            authenticated
    ...
    {
        $uri = self::USER_URI . "/default/" . self::UPLOADS_URI_SUFFIX .
                 "/$videoId";
        return parent::getEntry($uri, 'Zend_Gdata_YouTube_VideoEntry');
    }
    ...
    public static function parseFormUploadTokenResponse ($response)
    {
        ...
        if ($responseElement != null) {
            ...
            $tokenElement = $responseElement->getElementsByTagName('token')->item(
                    0);
            
            if ($urlElement && $urlElement->hasChildNodes() && $tokenElement &&
                     $tokenElement->hasChildNodes()) {
     ...
     * @param
     *            Zend_Gdata_YouTube_VideoEntry (optional) The video entry to
     *            send
     * @param string $videoId
     ...
     *        
     */
    public function sendVideoMessage ($body, $videoEntry = null, $videoId = null, 
            $recipientUserName)
    {
    ...
    public function replyToCommentEntry ($commentEntry, $commentText)
    {
        ...
        $commentFeedPostUrl = self::VIDEO_URI . '/' . $commentIdArray[3] .
                 '/comments';
        return $this->insertEntry($newComment, $commentFeedPostUrl, 
