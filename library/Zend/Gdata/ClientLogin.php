...
class Zend_Gdata_ClientLogin
{
     ...
     *            account to be authenticated is a google or a hosted account.
     *            Defaults to
     *            'HOSTED_OR_GOOGLE'. See:
    ...
    public static function getHttpClient ($email, $password, $service = 'xapi', 
            $client = null, $source = self::DEFAULT_SOURCE, $loginToken = null, $loginCaptcha = null, 
            $loginUri = self::CLIENTLOGIN_URI, $accountType = 'HOSTED_OR_GOOGLE')
    {
        if (! ($email && $password)) {
        ...
        if ($response->getStatus() == 200) {
            ...
            if (array_key_exists('Error', $goog_resp) &&
                ...
                require_once 'Zend/Gdata/App/AuthException.php';
                throw new Zend_Gdata_App_AuthException(
                        'Authentication with Google failed. Reason: ' .
