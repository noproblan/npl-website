 ...
 * - separator: string with which to separate passed in content and generated
 * content
 * - placement: whether to append or prepend the generated content to the passed
 * in content
...
class Zend_Dojo_Form_Decorator_DijitElement extends Zend_Form_Decorator_ViewHelper
{
     ...
     * @return string
     * @throws Zend_Form_Decorator_Exception if element or view are not
     *         registered
     */
    public function render ($content)
    {
        ...
        if ($view->dojo()->hasDijit($id)) {
            trigger_error(
                    sprintf(
                            'Duplicate dijit ID detected for id "%s; temporarily generating uniqid"', 
                            $id), E_USER_NOTICE);
            $base = $id;
            do {
                $id = $base . '-' . uniqid();
        ...
        $elementContent = $view->$helper($name, $value, $dijitParams, $attribs, 
                $options);
