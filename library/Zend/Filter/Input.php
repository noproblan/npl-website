...
class Zend_Filter_Input
{
     ...
     *            OPTIONAL
     */
    public function __construct ($filterRules, $validatorRules, 
            array $data = null, array $options = null)
    {
        if ($options) {
     ...
     * @deprecated since 1.5.0RC1 - use addFilterPrefixPath() or
     *             addValidatorPrefixPath instead.
     */
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new Zend_Filter_Exception(
                        sprintf(
                                'Invalid type "%s" provided to setPluginLoader()', 
                                $type));
        }
        ...
        if (! isset($this->_loaders[$type])) {
            switch ($type) {
                ...
                default:
                    ...
                    throw new Zend_Filter_Exception(
                            sprintf(
                                    'Invalid type "%s" provided to getPluginLoader()', 
                                    $type));
            }
    ...
    public function setDefaultEscapeFilter ($escapeFilter)
    {
        ...
        if (! $escapeFilter instanceof Zend_Filter_Interface) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    'Escape filter specified does not implement Zend_Filter_Interface');
        }
            ...
            switch ($option) {
                ...
                case self::VALIDATOR_NAMESPACE:
                    ...
                    foreach ($value as $prefix) {
                        $this->addValidatorPrefixPath($prefix, 
                ...
                case self::FILTER_NAMESPACE:
                    ...
                    foreach ($value as $prefix) {
                        $this->addFilterPrefixPath($prefix, 
                                str_replace('_', DIRECTORY_SEPARATOR, $prefix));
                    }
                    break;
    ...
    protected function _filter ()
    {
        foreach ($this->_filterRules as $ruleName => &$filterRule) {
             ...
             * Don't typecast to (array) because it might be a Zend_Filter
             * object
             */
            ...
            /**
             * Filters are indexed by integer, metacommands are indexed by
             * string.
    ...
    protected function _filterRule (array $filterRule)
    {
        ...
        if (is_array($this->_data[$field])) {
            foreach ($this->_data[$field] as $key => $value) {
                $this->_data[$field][$key] = $filterRule[self::FILTER_CHAIN]->filter(
                        $value);
            }
        } else {
            $this->_data[$field] = $filterRule[self::FILTER_CHAIN]->filter(
                    $this->_data[$field]);
        }
    }
    ...
    protected function _getDefaultEscapeFilter ()
    {
        ...
        return $this->setDefaultEscapeFilter(
                $this->_defaults[self::ESCAPE_FILTER]);
    }
    ...
    protected function _validate ()
    {
        ...
        // remember the default not empty message in case we want to temporarily
        // change it
        ...
        foreach ($this->_validatorRules as $ruleName => &$validatorRule) {
             ...
             * Don't typecast to (array) because it might be a Zend_Validate
             * object
             */
            ...
            /**
             * Validators are indexed by integer, metacommands are indexed by
             * string.
            ...
            if (! isset($validatorRule[self::MESSAGES])) {
                ...
                if (! is_array($validatorRule[self::MESSAGES])) {
                    ...
                    if (array_intersect_key($validatorList, 
                            $validatorRule[self::MESSAGES])) {
                        // this seems pointless... it just re-adds what it
                        // already has...
                        // I can disable all this and not a single unit test
                        // fails...
                        // There are now corresponding numeric keys in the
                        // validation rule messages array
                        // Treat it as a named messages list for all rule
                        // validators
                ...
                foreach ($validatorList as $key => $validator) {
                    ...
                    if (isset($validatorRule[self::MESSAGES][$key])) {
                        ...
                        if ($validator instanceof Zend_Validate_NotEmpty) {
                            /**
                             * we are changing the defaults here, this is
                             * alright if all subsequent validators are also a
                             * not empty
                             * validator, but it goes wrong if one of them is
                             * not AND is required!!!
                             * that is why we restore the default value at the
                             * end of this loop
                             */
                            if (is_array($value)) {
                                $temp = $value; // keep the original value
                                $this->_defaults[self::NOT_EMPTY_MESSAGE] = array_pop(
                                        $temp);
                                unset($temp);
                            } else {
                    ...
                    $validatorRule[self::VALIDATOR_CHAIN]->addValidator(
                            $validator, $validatorRule[self::BREAK_CHAIN]);
                }
                $validatorRule[self::VALIDATOR_CHAIN_COUNT] = count(
                        $validatorList);
            }
         ...
         */
        foreach (array_merge(array_keys($this->_missingFields), 
                array_keys($this->_invalidMessages)) as $rule) {
        ...
        foreach ((array) $validatorRule[self::FIELDS] as $key => $field) {
            if (array_key_exists($field, $this->_data)) {
                ...
                if (isset($validatorRule[self::DEFAULT_VALUE])) {
                    /**
                     * @todo according to this code default value can't be an
                     * array.
                    ...
                    if (! is_array($validatorRule[self::DEFAULT_VALUE])) {
                        ...
                        // Default value is an array. Search for corresponding
                        // key
                        if (isset($validatorRule[self::DEFAULT_VALUE][$key])) {
                            $data[$field] = $validatorRule[self::DEFAULT_VALUE][$key];
                        } else 
                            if ($validatorRule[self::PRESENCE] ==
                                     self::PRESENCE_REQUIRED) {
                                // Default value array is provided, but it
                                // doesn't have an entry for current field
                                // and presence is required
                                $this->_missingFields[$validatorRule[self::RULE]][] = $this->_getMissingMessage(
                                        $validatorRule[self::RULE], $field);
                            }
                    }
                } else 
        ...
        if (count((array) $validatorRule[self::FIELDS]) > 1) {
            if (! $validatorRule[self::ALLOW_EMPTY]) {
                ...
                foreach ($data as $fieldKey => $field) {
                    // if there is no Zend_Validate_NotEmpty instance in the
                    // rules, we will use the default
                    if (! ($notEmptyValidator = $this->_getNotEmptyValidatorInstance(
                            $validatorRule))) {
                        $notEmptyValidator = $this->_getValidator('NotEmpty');
                        $notEmptyValidator->setMessage(
                                $this->_getNotEmptyMessage(
                                        $validatorRule[self::RULE], $fieldKey));
                    }
            ...
            if (count($data) > 0) {
                ...
                // if there is no Zend_Validate_NotEmpty instance in the rules,
                ...
                if (! ($notEmptyValidator = $this->_getNotEmptyValidatorInstance(
                    ...
                    $notEmptyValidator->setMessage(
                            $this->_getNotEmptyMessage(
                                    $validatorRule[self::RULE], $fieldName));
                }
     ...
     * The purpose is to preserve things like a custom message, that may have
     * been
     * set on the validator outside Zend_Filter_Input.
     * 
     * @param array $validatorRule            
     * @return mixed false if none is found, Zend_Validate_NotEmpty instance if
     *         found
     */
    ...
    protected function _getFilterOrValidator ($type, $classBaseName)
    {
        ...
        $className = $this->getPluginLoader($type)->load(
                ucfirst($classBaseName));
        ...
        if (! $class->implementsInterface($interfaceName)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    "Class '$className' based on basename '$classBaseName' must implement the '$interfaceName' interface");
        }
