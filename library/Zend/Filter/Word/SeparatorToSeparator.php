...
class Zend_Filter_Word_SeparatorToSeparator extends Zend_Filter_PregReplace
{
     ...
     *
     * Returns the string $value, replacing the searched seperators with the
     * defined ones
     *
     * @param string $value            
     * @return string
    ...
    /**
     * Do the real work, replaces the seperator to search for with the
    ...
    protected function _separatorToSeparatorFilter ($value)
    {
        if ($this->_searchSeparator == null) {
            ...
            throw new Zend_Filter_Exception(
                    'You must provide a search separator for this filter to work.');
        }
        
        $this->setMatchPattern(
                '#' . preg_quote($this->_searchSeparator, '#') . '#');
