...
class Zend_Filter_Inflector implements Zend_Filter_Interface
{
    ...
    public function setOptions ($options)
    {
        ...
        if (array_key_exists('throwTargetExceptionsOn', $options)) {
            $this->setThrowTargetExceptionsOn(
    ...
    /**
     * getRule() returns a rule set by setFilterRule(), a numeric index must be
     * provided
     *
     * @param string $spec            
     * @param int $index            
    ...
    public function filter ($source)
    {
        ...
        $pregQuotedTargetReplacementIdentifier = preg_quote(
                $this->_targetReplacementIdentifier, '#');
        ...
        foreach ($this->_rules as $ruleName => $ruleValue) {
            if (isset($source[$ruleName])) {
                if (is_string($ruleValue)) {
                    ...
                    $processedParts['#' . $pregQuotedTargetReplacementIdentifier .
        ...
        // all of the values of processedParts would have been str_replace('\\',
        // '\\\\', ..)'d to disable preg_replace backreferences
        $inflectedTarget = preg_replace(array_keys($processedParts), 
                array_values($processedParts), $this->_target);
        
        if ($this->_throwTargetExceptionsOn &&
                 (preg_match(
                        '#(?=' . $pregQuotedTargetReplacementIdentifier .
                         '[A-Za-z]{1})#', $inflectedTarget) == true)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    'A replacement identifier ' .
                             $this->_targetReplacementIdentifier .
                             ' was found inside the inflected target, perhaps a rule was not satisfied with a target source?  Unsatisfied inflected target: ' .
                             $inflectedTarget);
        }
    ...
    protected function _getRule ($rule)
    {
        ...
        if (! $ruleObject instanceof Zend_Filter_Interface) {
            ...
            throw new Zend_Filter_Exception(
                    'No class named ' . $rule .
                             ' implementing Zend_Filter_Interface could be found');
        }
