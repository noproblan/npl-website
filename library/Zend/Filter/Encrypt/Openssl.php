...
class Zend_Filter_Encrypt_Openssl implements Zend_Filter_Encrypt_Interface
{
    ...
    public function __construct ($options = array())
    {
        if (! extension_loaded('openssl')) {
            ...
            throw new Zend_Filter_Exception(
                    'This filter needs the openssl extension');
        }
    ...
    public function encrypt ($value)
    {
        ...
        if (count($this->_keys['public']) == 0) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    'Openssl can not encrypt without public keys');
        }
        ...
        if ($this->_package) {
            ...
            foreach ($this->_keys['envelope'] as $key => $envKey) {
                $header .= pack('H32n', $fingerprints[$key], strlen($envKey)) .
                         $envKey;
            }
    ...
    public function decrypt ($value)
    {
        ...
        if (count($this->_keys['private']) !== 1) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    'Please give a private key for decryption with Openssl');
        }
        
        if (! $this->_package && empty($envelope)) {
            require_once 'Zend/Filter/Exception.php';
            throw new Zend_Filter_Exception(
                    'Please give a envelope key for decryption with Openssl');
        }
