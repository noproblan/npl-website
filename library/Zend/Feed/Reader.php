...
class Zend_Feed_Reader
{
        ...
        if (self::$_httpConditionalGet && $cache) {
            ...
            if ($response->getStatus() == 304) {
                ...
                if ($response->getHeader('ETag')) {
                    $cache->save($response->getHeader('ETag'), $cacheId .
                             '_etag');
                }
                if ($response->getHeader('Last-Modified')) {
                    $cache->save($response->getHeader('Last-Modified'), 
                            $cacheId . '_lastmodified');
                }
            }
            if (empty($responseXml)) {
                require_once 'Zend/Feed/Exception.php';
                throw new Zend_Feed_Exception(
                        'Feed failed to load, got empty response body');
            }
    ...
    public static function importString ($string)
    {
        ...
        if (substr($type, 0, 3) == 'rss') {
            ...
            $reader = new Zend_Feed_Reader_Entry_Atom($dom->documentElement, 0, 
                    Zend_Feed_Reader::TYPE_ATOM_10);
