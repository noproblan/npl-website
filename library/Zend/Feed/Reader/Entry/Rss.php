 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Feed_Reader_Entry_Rss extends Zend_Feed_Reader_EntryAbstract implements 
        Zend_Feed_Reader_EntryInterface
{
    ...
    public function __construct (DOMElement $entry, $entryKey, $type = null)
    {
        ...
        $this->_extensions['DublinCore_Entry'] = new $dublinCoreClass($entry, 
                $entryKey, $type);
        
        $contentClass = $pluginLoader->getClassName('Content_Entry');
        $this->_extensions['Content_Entry'] = new $contentClass($entry, 
                $entryKey, $type);
        
        $atomClass = $pluginLoader->getClassName('Atom_Entry');
        $this->_extensions['Atom_Entry'] = new $atomClass($entry, $entryKey, 
                $type);
        
        $wfwClass = $pluginLoader->getClassName('WellFormedWeb_Entry');
        $this->_extensions['WellFormedWeb_Entry'] = new $wfwClass($entry, 
                $entryKey, $type);
        
        $slashClass = $pluginLoader->getClassName('Slash_Entry');
        $this->_extensions['Slash_Entry'] = new $slashClass($entry, $entryKey, 
                $type);
        
        $threadClass = $pluginLoader->getClassName('Thread_Entry');
        $this->_extensions['Thread_Entry'] = new $threadClass($entry, $entryKey, 
                $type);
    }
    ...
    public function getDateModified ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $dateModified = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRss . '/pubDate)');
            if ($dateModified) {
    ...
    public function getDescription ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $description = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRss . '/description)');
        } else {
            $description = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRdf . '/rss:description)');
        }
    ...
    public function getId ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $id = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRss . '/guid)');
        }
    ...
    public function getCategories ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $list = $this->_xpath->query($this->_xpathQueryRdf .
                     '//rss:category');
        }
    ...
    public function getTitle ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $title = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRss . '/title)');
        } else {
            $title = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRdf . '/rss:title)');
        }
    ...
    /**
     * Returns a URI pointing to the HTML page where comments can be made on
     * this entry
     *
     * @return string
     */
    public function getCommentLink ()
    {
        ...
        if ($this->getType() !== Zend_Feed_Reader::TYPE_RSS_10 &&
            ...
            $commentlink = $this->_xpath->evaluate(
                    'string(' . $this->_xpathQueryRss . '/comments)');
        }
