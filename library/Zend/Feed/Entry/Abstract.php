...
abstract class Zend_Feed_Entry_Abstract extends Zend_Feed_Element
{
     ...
     *
     * The Zend_Feed_Entry_Abstract constructor takes the URI of the feed the
     * entry
    ...
    public function __construct ($uri = null, $element = null)
    {
        if (! ($element instanceof DOMElement)) {
            if ($element) {
                ...
                if (! $status) {
                    // prevent the class to generate an undefined variable
                    // notice (ZF-2590)
                ...
                $element = $doc->getElementsByTagName($this->_rootElement)->item(
                        0);
                if (! $element) {
                    /**
                     *
                ...
                if ($this->_rootNamespace !== null) {
                    $element = $doc->createElementNS(
                            Zend_Feed::lookupNamespace($this->_rootNamespace), 
                            $this->_rootElement);
                } else {
