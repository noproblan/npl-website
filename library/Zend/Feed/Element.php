...
class Zend_Feed_Element implements ArrayAccess
{
    ...
    {
        $this->_element = $this->_element->ownerDocument->importNode($element, 
                true);
    }
    ...
    public function __get ($var)
    {
        ...
        if ($length == 1) {
            ...
            return array_map(
                    create_function('$e', 'return new Zend_Feed_Element($e);'), 
                    $nodes);
        } else {
            ...
            if (strpos($var, ':') !== false) {
                ...
                $node = $this->_element->ownerDocument->createElementNS(
                        Zend_Feed::lookupNamespace($ns), $elt);
            } else {
    ...
    public function __set ($var, $val)
    {
        ...
        if (! $nodes) {
            if (strpos($var, ':') !== false) {
                list ($ns, $elt) = explode(':', $var, 2);
                $node = $this->_element->ownerDocument->createElementNS(
                        Zend_Feed::lookupNamespace($ns), $var, 
            ...
            throw new Zend_Feed_Exception(
                    'Cannot set the value of multiple tags simultaneously.');
        } else {
     ...
     *
     * @param string $var
     *            Tagname to match, can be either namespace:tagName or just
     *            tagName.
     * @return array
     */
    ...
    public function offsetExists ($offset)
    {
        if (strpos($offset, ':') !== false) {
            ...
            return $this->_element->hasAttributeNS(
    ...
    public function offsetSet ($offset, $value)
    {
        ...
        if (strpos($offset, ':') !== false) {
            list ($ns, $attr) = explode(':', $offset, 2);
            // DOMElement::setAttributeNS() requires $qualifiedName to have a
            // prefix
            return $this->_element->setAttributeNS(
                    Zend_Feed::lookupNamespace($ns), $offset, $value);
        } else {
