 ...
 *
 * Users are encouraged to make their own classes to implement
...
class Zend_Feed_Builder implements Zend_Feed_Builder_Interface
{
     ...
     * 'webmaster' => 'email address for person responsible for technical
     * issues' // optional, ignored if atom is used
     ...
     * 'rating' => 'The PICS rating for the channel.', // optional, ignored if
     * atom is used
     * 'cloud' => array(
     ...
     * 'registerProcedure' => 'procedure to call, e.g. myCloud.rssPleaseNotify'
     * // required
     ...
     * // required
     * ) // a text input box that can be displayed with the feed // optional,
     * ignored if atom is used
     * 'skipHours' => array(
     * 'hour in 24 format', // e.g 13 (1pm)
     * // up to 24 rows whose value is a number between 0 and 23
     * ) // Hint telling aggregators which hours they can skip // optional,
     * ignored if atom is used
     * 'skipDays ' => array(
     * 'a day to skip', // e.g Monday
     * // up to 7 rows whose value is a Monday, Tuesday, Wednesday, Thursday,
     * Friday, Saturday or Sunday
     * ) // Hint telling aggregators which days they can skip // optional,
     * ignored if atom is used
     * 'itunes' => array(
     ...
     * 'image' => 'album/podcast art' // optional, default to the main image
     * value
     * 'subtitle' => 'short description' // optional, default to the main
     * description value
     * 'summary' => 'longer description' // optional, default to the main
     * description value
     ...
     * 'description' => 'short version of a feed entry', // only text, no html,
     * required
    ...
    protected function _createHeader (array $data)
    {
        ...
        $this->_header = new Zend_Feed_Builder_Header($data['title'], 
                $data['link'], $data['charset']);
        ...
        if (isset($data['cloud'])) {
            ...
            $uri_str = 'http://' . $data['cloud']['domain'] .
                     $data['cloud']['path'];
            $this->_header->setCloud($uri_str, 
                    $data['cloud']['registerProcedure'], 
                    $data['cloud']['protocol']);
        }
        if (isset($data['textInput'])) {
            ...
            $this->_header->setTextInput($data['textInput']['title'], 
                    ...
                    $data['textInput']['name'], $data['textInput']['link']);
        }
        ...
        if (isset($data['itunes'])) {
            $itunes = new Zend_Feed_Builder_Header_Itunes(
    ...
    protected function _createEntries (array $data)
    {
        foreach ($data as $row) {
            ...
            $entry = new Zend_Feed_Builder_Entry($row['title'], $row['link'], 
                    $row['description']);
            if (isset($row['author'])) {
