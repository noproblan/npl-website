...
class Zend_Feed_Pubsubhubbub_Subscriber
{
    ...
    /**
     * An instance of Zend_Feed_Pubsubhubbub_Model_SubscriptionInterface used to
     * background
    ...
    /**
     * Sets an instance of Zend_Feed_Pubsubhubbub_Model_SubscriptionInterface
     * used to background
     * save any verification tokens associated with a subscription or other.
     *
    ...
    protected function _getRequestParameters ($hubUrl, $mode)
    {
        ...
        if (isset($params['hub.lease_seconds'])) {
            $expires = $now->add($params['hub.lease_seconds'], 
                    Zend_Date::SECOND)->get('yyyy-MM-dd HH:mm:ss');
        }
        $data = array(
                'id' => $key,
        ...
        foreach ($params as $key => $value) {
            if (is_array($value)) {
                ...
                foreach ($value as $duplicateKey) {
                    $encoded[$ekey][] = Zend_Feed_Pubsubhubbub::urlencode(
                            $duplicateKey);
                }
            } else {
                $encoded[Zend_Feed_Pubsubhubbub::urlencode($key)] = Zend_Feed_Pubsubhubbub::urlencode(
                        $value);
            }
        }
        return $encoded;
