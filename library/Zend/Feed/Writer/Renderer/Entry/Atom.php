 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Feed_Writer_Renderer_Entry_Atom extends Zend_Feed_Writer_Renderer_RendererAbstract implements 
        Zend_Feed_Writer_Renderer_RendererInterface
{
    ...
    public function render ()
    {
        ...
        $entry = $this->_dom->createElementNS(
                Zend_Feed_Writer::NAMESPACE_ATOM_10, 'entry');
    ...
    protected function _setTitle (DOMDocument $dom, DOMElement $root)
    {
        ...
        $cdata = $dom->createCDATASection($this->getDataContainer()
            ->getTitle());
    ...
    protected function _setLink (DOMDocument $dom, DOMElement $root)
    {
        ...
        $link->setAttribute('href', $this->getDataContainer()
            ->getLink());
    }
    ...
    protected function _setId (DOMDocument $dom, DOMElement $root)
    {
        ...
        if (! Zend_Uri::check($this->getDataContainer()->getId()) && ! preg_match(
                "#^urn:[a-zA-Z0-9][a-zA-Z0-9\-]{1,31}:([a-zA-Z0-9\(\)\+\,\.\:\=\@\;\$\_\!\*\-]|%[0-9a-fA-F]{2})*#", 
                $this->getDataContainer()->getId()) &&
                 ! $this->_validateTagUri($this->getDataContainer()
                    ->getId())) {
            require_once 'Zend/Feed/Exception.php';
    ...
    protected function _loadXhtml ($content)
    {
        ...
        $dom->loadXML(
                '<xhtml:div xmlns:xhtml="http://www.w3.org/1999/xhtml">' . $xhtml .
                         '</xhtml:div>');
