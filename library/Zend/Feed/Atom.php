...
class Zend_Feed_Atom extends Zend_Feed_Abstract
{
    ...
    public function __wakeup ()
    {
        ...
        if (! $element) {
            ...
            $element = $this->_element->getElementsByTagName(
                    $this->_entryElementName)->item(0);
            if (! $element) {
                /**
                 *
     ...
     * $elt->link('self'): returns the href from the first <link rel="self"> in
     * the entry.
     *
     * @param string $rel
    ...
    protected function _mapFeedHeaders ($array)
    {
        ...
        $updated = $this->_element->createElement('updated', 
                date(DATE_ATOM, $updated));
        $feed->appendChild($updated);
        
        if (isset($array->published)) {
            $published = $this->_element->createElement('published', 
                    date(DATE_ATOM, $array->published));
        ...
        if (isset($array->description)) {
            ...
            $subtitle->appendChild(
                    $this->_element->createCDATASection($array->description));
            $feed->appendChild($subtitle);
        }
        
        if (isset($array->copyright)) {
            $copyright = $this->_element->createElement('rights', 
                    $array->copyright);
    ...
    protected function _mapFeedEntries (DOMElement $root, $array)
    {
        foreach ($array as $dataentry) {
            ...
            $id = $this->_element->createElement('id', 
                    isset($dataentry->guid) ? $dataentry->guid : $dataentry->link);
            ...
            if (isset($dataentry->comments)) {
                $comments = $this->_element->createElementNS(
                        'http://wellformedweb.org/CommentAPI/', 'wfw:comment', 
                        $dataentry->comments);
