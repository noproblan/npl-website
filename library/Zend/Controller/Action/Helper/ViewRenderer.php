...
class Zend_Controller_Action_Helper_ViewRenderer extends Zend_Controller_Action_Helper_Abstract
{
    ...
    /**
     * Whether or not to autorender postDispatch; per controller/action setting
     * (reset
     * at next invocation)
     * 
     * @var boolean
     ...
     *
     * @param Zend_View_Interface $view            
     * @return Zend_Controller_Action_Helper_ViewRenderer Provides a fluent
     *         interface
     */
    ...
    public function getModuleDirectory ()
    {
        ...
        $moduleDir = $this->getFrontController()->getControllerDirectory(
                $module);
        if ((null === $moduleDir) || is_array($moduleDir)) {
            ...
            throw new Zend_Controller_Action_Exception(
                    'ViewRenderer cannot locate module directory for module "' .
                             $module . '"');
        }
...
addRules(
     ...
     *            Whether the moduleDir, target, and suffix should be set as
     *            references to ViewRenderer properties
     * @return Zend_Controller_Action_Helper_ViewRenderer Provides a fluent
     *         interface
     */
            ...
            $reference = false)
    {
        ...
        if ($reference) {
            $this->_inflector->setStaticRuleReference('suffix', 
                    $this->_viewSuffix)
    ...
    protected function _getBasePath ()
    {
        ...
        $parts = array(
                'module' => (($moduleName = $request->getModuleName()) != '') ? $dispatcher->formatModuleName(
                        $moduleName) : $moduleName,
                'controller' => $request->getControllerName(),
                'action' => $dispatcher->formatActionName(
     ...
     * - neverRender - flag dis/enabling postDispatch() autorender (affects all
     * subsequent calls)
     * - noController - flag indicating whether or not to look for view scripts
     * in subdirectories named after the controller
     * - noRender - flag indicating whether or not to autorender postDispatch()
     * - responseSegment - which named response segment to render a view script
     * to
     * - scriptAction - what action script to render
     * - viewBasePathSpec - specification to use for determining view base path
     * - viewScriptPathSpec - specification to use for determining view script
     * paths
     * - viewScriptPathNoControllerSpec - specification to use for determining
     * view script paths when noController flag is set
     * - viewSuffix - what view script filename suffix to use
     *
     * @param string $path            
     * @param string $prefix            
    ...
    public function initView ($path = null, $prefix = null, array $options = array())
    {
        ...
        if ((null !== $this->_actionController) &&
                 (null === $this->_actionController->view)) {
    ...
    /**
     * Retrieve the current view script path specification string (no controller
     * variant)
     *
     * @return string
     */
     ...
     *
     * If {@link $_noController} is set, uses {@link
    ...
    public function getViewScript ($action = null, array $vars = array())
    {
        ...
        if ($this->getNoController() || $this->getNeverController()) {
            $this->_setInflectorTarget(
                    $this->getViewScriptPathNoControllerSpec());
        } else {
    ...
    /**
     * Set the noController flag (i.e., whether or not to render into controller
     * subdirectories)
     *
     * @param boolean $flag            
     * @return Zend_Controller_Action_Helper_ViewRenderer Provides a fluent
    ...
    /**
     * Set the neverController flag (i.e., whether or not to render into
     ...
     *            Whether or not to render within a subdirectory named after the
     *            controller
     * @return Zend_Controller_Action_Helper_ViewRenderer Provides a fluent
     *         interface
     */
     ...
     *
     * Pass an action, and one or more specification variables (view script
     * suffix)
     ...
     * - request has not been re-dispatched (i.e., _forward() has not been
     * called)
    ...
    protected function _shouldRender ()
    {
        return (! $this->getFrontController()->getParam('noViewRenderer') &&
                 ! $this->_neverRender && ! $this->_noRender &&
