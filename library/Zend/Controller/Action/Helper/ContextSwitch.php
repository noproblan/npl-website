...
class Zend_Controller_Action_Helper_ContextSwitch extends Zend_Controller_Action_Helper_Abstract
{
     ...
     *
     * @param array $options            
     * @return Zend_Controller_Action_Helper_ContextSwitch Provides a fluent
     *         interface
     */
        ...
        if (null !==
            ...
            if (is_string($callback) && method_exists($this, $callback)) {
                ...
                throw new Zend_Controller_Action_Exception(
                        sprintf(
                                'Invalid context callback registered for context "%s"', 
                                $context));
            }
        }
     ...
     *            Whether or not to prepend the new suffix to the viewrenderer
     *            suffix
     * @throws Zend_Controller_Action_Exception
     * @return Zend_Controller_Action_Helper_ContextSwitch Provides a fluent
     *         interface
            ...
            $prependViewRendererSuffix = true)
    {
        if (! isset($this->_contexts[$context])) {
            ...
            require_once 'Zend/Controller/Action/Exception.php';
            throw new Zend_Controller_Action_Exception(
                    sprintf('Cannot set suffix; invalid context type "%s"', 
                            $context));
        }
    ...
    public function getSuffix ($type)
    {
        if (! isset($this->_contexts[$type])) {
            ...
            require_once 'Zend/Controller/Action/Exception.php';
            throw new Zend_Controller_Action_Exception(
                    sprintf('Cannot retrieve suffix; invalid context type "%s"', 
                            $type));
        }
     ...
     * @throws Zend_Controller_Action_Exception if context does not exist and
    ...
    public function addHeader ($context, $header, $content)
    {
        ...
        if (isset($this->_contexts[$context]['headers'][$header])) {
            ...
            throw new Zend_Controller_Action_Exception(
                    sprintf(
                            'Cannot add "%s" header to context "%s": already exists', 
                            $header, $context));
        }
    ...
    protected function _validateTrigger ($trigger)
    {
        ...
        if (! in_array($trigger, array(
            ...
            require_once 'Zend/Controller/Action/Exception.php';
            throw new Zend_Controller_Action_Exception(
                    sprintf('Invalid trigger "%s"', $trigger));
        }
    ...
    public function setDefaultContext ($type)
    {
        if (! isset($this->_contexts[$type])) {
            ...
            throw new Zend_Controller_Action_Exception(
                    sprintf(
                            'Cannot set default context; invalid context type "%s"', 
                            $type));
        }
    ...
    public function postDispatch ()
    {
        ...
        if (null !== $context) {
            if (null !==
                     ($callback = $this->getCallback($context, 
                            self::TRIGGER_POST))) {
                if (is_string($callback) && method_exists($this, $callback)) {
                    $this->$callback();
                } elseif (is_string($callback) && function_exists($callback)) {
     ...
     * @return Zend_Controller_Action_Helper_ContextSwitch|void Provides a
     *         fluent interface
     */
    ...
    public function hasActionContext ($action, $context)
    {
        ...
        if (! is_array($allContexts)) {
            ...
            require_once 'Zend/Controller/Action/Exception.php';
            throw new Zend_Controller_Action_Exception(
                    "Invalid contexts found for controller");
        }
    ...
    protected function _getViewRenderer ()
    {
        if (null === $this->_viewRenderer) {
            $this->_viewRenderer = Zend_Controller_Action_HelperBroker::getStaticHelper(
                    'viewRenderer');
        }
