...
class Zend_Controller_Router_Route extends Zend_Controller_Router_Route_Abstract
{
     ...
     * requirement or null. In case of a static part, it holds only it's direct
     * value.
    ...
    /**
     * Holds user submitted regular expression patterns for route's variables'
     * values.
     * Name and value pairs.
     * 
     * @var array
     ...
     * @param array $reqs
     *            Regular expression requirements for variables (keys as
     *            variable names)
        ...
        if ($route !== '') {
            foreach (explode($this->_urlDelimiter, $route) as $pos => $part) {
                if (substr($part, 0, 1) == $this->_urlVariable &&
                         substr($part, 1, 1) != $this->_urlVariable) {
                    $name = substr($part, 1);
    ...
    public function match ($path, $partial = false)
    {
        ...
        if ($path !== '') {
            ...
            foreach ($path as $pos => $pathPart) {
                ...
                // If it's a wildcard, get the rest of URL as wildcard data and
                // stop matching
                if ($this->_parts[$pos] == '*') {
                    $count = count($path);
                    for ($i = $pos; $i < $count; $i += 2) {
                        ...
                        if (! isset($this->_wildcardData[$var]) &&
                                 ...
                                 ! isset($values[$var])) {
                            $this->_wildcardData[$var] = (isset($path[$i + 1])) ? urldecode(
                                    $path[$i + 1]) : null;
                        }
                    }
                ...
                if ($this->_isTranslated &&
                         (substr($part, 0, 1) === '@' &&
                         substr($part, 1, 1) !== '@' && $name === null) ||
                         $name !== null && in_array($name, $this->_translatable)) {
                    if (substr($part, 0, 1) === '@') {
                        $part = substr($part, 1);
                    }
                    
                    if (($originalPathPart = array_search($pathPart, 
                ...
                // If it's a variable with requirement, match a regex. If not -
                // everything matches
                if ($part !== null &&
                         ! preg_match(
                                $this->_regexDelimiter . '^' . $part . '$' .
                                 $this->_regexDelimiter . 'iu', $pathPart)) {
                    return false;
                }
     ...
     * @param boolean $reset
     *            Whether or not to set route defaults with those provided in
     *            $data
    ...
    public function assemble ($data = array(), $reset = false, $encode = false, $partial = false)
    {
        ...
        foreach ($this->_parts as $key => $part) {
            ...
            if (isset($name)) {
                ...
                if ($this->_isTranslated && in_array($name, 
                        $this->_translatable)) {
                ...
                if ($this->_isTranslated && substr($part, 0, 1) === '@') {
                    if (substr($part, 1, 1) !== '@') {
                        $url[$key] = $translator->translate(substr($part, 1), 
                                $locale);
                    } else {
        ...
        foreach (array_reverse($url, true) as $key => $value) {
            ...
            if (isset($this->_variables[$key])) {
                ...
                if ($this->_isTranslated && $defaultValue !== null &&
                    ...
                    $defaultValue = $translator->translate($defaultValue, 
                            $locale);
                }
            }
