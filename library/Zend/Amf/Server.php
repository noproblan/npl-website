...
/**
 * An AMF gateway server implementation to allow the connection of the Adobe
 * Flash Player to
 * Zend Framework
 *
...
class Zend_Amf_Server implements Zend_Server_Interface
{
    ...
    {
        Zend_Amf_Parse_TypeLoader::setResourceLoader(
    ...
    {
        if ($source) {
            if (($mapped = Zend_Amf_Parse_TypeLoader::getMappedClassName(
                    $source)) !== false) {
                $source = $mapped;
            }
        }
        ...
        if (! isset($this->_table[$qualifiedName])) {
            ...
            if ($source) {
                ...
                if (class_exists($className, false) &&
                         ! isset($this->_classAllowed[$className])) {
                    require_once 'Zend/Amf/Server/Exception.php';
                    throw new Zend_Amf_Server_Exception(
                            'Can not call "' . $className . '" - use setClass()');
                }
                try {
        ...
        if ($info instanceof Zend_Server_Reflection_Function) {
            ...
            require_once 'Zend/Amf/Server/Exception.php';
            throw new Zend_Amf_Server_Exception(
                    'Method missing implementation ' . get_class($info));
        }
     ...
     */
    protected function _loadCommandMessage (
        ...
        switch ($message->operation) {
            ...
            case Zend_Amf_Value_Messaging_CommandMessage::LOGIN_OPERATION:
                ...
                if (empty($userid)) {
                    require_once 'Zend/Amf/Server/Exception.php';
                    throw new Zend_Amf_Server_Exception(
                            'Login failed: username not supplied');
                }
            ...
            default:
                require_once 'Zend/Amf/Server/Exception.php';
                throw new Zend_Amf_Server_Exception(
                        'CommandMessage::' . $message->operation .
                                 ' not implemented');
                break;
        }
        return $return;
     ...
     */
    protected function _errorMessage ($objectEncoding, $message, $description, 
            $detail, $code, $line)
    {
        $return = null;
    ...
    protected function _handle (Zend_Amf_Request $request)
    {
        ...
        if (isset($headers[Zend_Amf_Constants::CREDENTIALS_HEADER]) &&
            ...
            try {
                if ($this->_handleAuth(
                    ...
                    $response->addAmfHeader(
                            new Zend_Amf_Value_MessageHeader(
                                    Zend_Amf_Constants::PERSISTENT_HEADER, false, 
                                    new Zend_Amf_Value_MessageHeader(
                ...
                $error = $this->_errorMessage($objectEncoding, '', 
                        $e->getMessage(), $e->getTraceAsString(), $e->getCode(), 
                        $e->getLine());
        ...
        foreach ($request->getAmfBodies() as $body) {
            if ($error) {
                ...
                $newBody = new Zend_Amf_Value_MessageBody($responseURI, null, 
                        $error);
                ...
                switch ($objectEncoding) {
                    case Zend_Amf_Constants::AMF0_OBJECT_ENCODING:
                        ...
                        if ($source) {
                            ...
                            $return = $this->_dispatch($targetURI, 
                                    $body->getData());
                        }
                        break;
                    case Zend_Amf_Constants::AMF3_OBJECT_ENCODING:
                    default:
                        ...
                        if ($message instanceof Zend_Amf_Value_Messaging_CommandMessage) {
                            ...
                            $return = new Zend_Amf_Value_Messaging_AcknowledgeMessage(
                                    $message);
                            $return->body = $this->_dispatch(
                                    $message->operation, $message->body, 
                                    $message->source);
                        } else {
                            ...
                            if ($source) {
                                // Break off method name from namespace into
                                // source
                                $method = substr(strrchr($targetURI, '.'), 1);
                                $return = $this->_dispatch($method, 
                                        $body->getData(), $source);
        ...
        if ($this->isSession()) {
            ...
            // create a new AMF message header with the session id as a
            // variable.
            $sessionValue = $joint . $this->_sessionName . "=" . $currentID;
            $sessionHeader = new Zend_Amf_Value_MessageHeader(
                    Zend_Amf_Constants::URL_APPEND_HEADER, false, $sessionValue);
    ...
    public function handle ($request = null)
    {
        ...
        try {
            ...
            require_once 'Zend/Amf/Server/Exception.php';
            throw new Zend_Amf_Server_Exception(
                    'Handle error: ' . $e->getMessage() . ' ' . $e->getLine(), 0, 
                    $e);
        }
    ...
    public function setClass ($class, $namespace = '', $argv = null)
    {
        ...
        $this->_methods[] = Zend_Server_Reflection::reflectClass($class, $argv, 
                $namespace);
    ...
    public function addFunction ($function, $namespace = '')
    {
        ...
        foreach ($function as $func) {
            ...
            $this->_methods[] = Zend_Server_Reflection::reflectFunction($func, 
                    $argv, $namespace);
        }
        ...
        foreach ($this->_methods as $key => $dispatchable) {
            if ($dispatchable instanceof Zend_Server_Reflection_Function_Abstract) {
                ...
                if (isset($table[$name])) {
                    require_once 'Zend/Amf/Server/Exception.php';
                    throw new Zend_Amf_Server_Exception(
                            'Duplicate method registered: ' . $name);
                }
