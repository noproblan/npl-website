 ...
 * @todo Class could be implemented as Factory Class with each data type it's
 *       own class.
 * @package Zend_Amf
 * @subpackage Parse_Amf3
...
class Zend_Amf_Parse_Amf3_Deserializer extends Zend_Amf_Parse_Deserializer
{
     ...
     *
     * - array = 0x09 integer-data ( [ 1OCTET *amf3-data ] | [OCTET *amf3-data
     * 1] | [ OCTET *amf-data ] )
     *
     * @return array
     */
     ...
     *
     * @todo Rather than using an array of traitsInfo create
    ...
    public function readObject ()
    {
        ...
        if ($storedObject) {
            ...
            if ($storedClass) {
                ...
                // The class was not in the reference tables. Start reading
                // rawdata to build traits.
                // Create a traits table. Zend_Amf_Value_TraitsInfo would be
                // ideal
            ...
            // We now have the object traits defined in variables. Time to go to
            // work:
            if (! $className) {
            ...
            switch ($encoding) {
                case (Zend_Amf_Constants::ET_EXTERNAL):
                    ...
                    // Externalizable object such as {ArrayCollection} and
                    // {ObjectProxy}
                    if (! $storedClass) {
                ...
                case (Zend_Amf_Constants::ET_DYNAMIC):
                    ...
                    // not a reference object read name value properties from
                    // byte stream
                    do {
                ...
                default:
                    ...
                    if (! $storedClass) {
                        $count = $traitsInfo; // Number of properties in the
                                              // list
                        for ($i = 0; $i < $count; $i ++) {
