...
class Zend_Amf_Parse_Amf0_Serializer extends Zend_Amf_Parse_Serializer
{
    ...
    public function writeTypeMarker (&$data, $markerType = null, $dataByVal = false)
    {
        ...
        if (null !== $markerType) {
            ...
            if (! $this->writeObjectReference($data, $markerType)) {
                // Write the Type Marker to denote the following action script
                // data type
    ...
    public function writeDate ($data)
    {
        if ($data instanceof DateTime) {
            ...
            require_once 'Zend/Amf/Exception.php';
            throw new Zend_Amf_Exception(
                    'Invalid date specified; must be a DateTime or Zend_Date object');
        }
        $dateString *= 1000;
    ...
    protected function getClassName ($object)
    {
        ...
        switch (true) {
            ...
            case Zend_Amf_Parse_TypeLoader::getMappedClassName(
                    get_class($object)):
                $className = Zend_Amf_Parse_TypeLoader::getMappedClassName(
                        get_class($object));
                break;
