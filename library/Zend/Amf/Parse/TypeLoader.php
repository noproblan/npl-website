 ...
 *
 * @todo PHP 5.3 can drastically change this class w/ namespace and the new
 *       call_user_func w/ namespace
 * @package Zend_Amf
 * @subpackage Parse
...
final class Zend_Amf_Parse_TypeLoader
{
     ...
     */
    public static function setResourceLoader (
    ...
    public static function handleResource ($resource)
    {
        if (! self::$_resourceLoader) {
            ...
            throw new Zend_Amf_Exception(
                    'Unable to handle resources - resource plugin loader not set');
        }
        try {
            while (is_resource($resource)) {
                ...
                if (! $resclass) {
                    require_once 'Zend/Amf/Exception.php';
                    throw new Zend_Amf_Exception(
                            'Can not serialize resource type: ' .
                                     get_resource_type($resource));
                }
                $parser = new $resclass();
                if (is_callable(array(
                    ...
                    require_once 'Zend/Amf/Exception.php';
                    throw new Zend_Amf_Exception(
                            "Could not call parse() method on class $resclass");
                }
            }
            return $resource;
            ...
            throw new Zend_Amf_Exception(
                    'Can not serialize resource type: ' .
                             get_resource_type($resource), 0, $e);
        }
    }
}
