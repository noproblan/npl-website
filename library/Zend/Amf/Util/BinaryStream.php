...
/**
 * Utility class to walk through a data stream byte by byte with conventional
 * names
 *
 * @package Zend_Amf
 * @subpackage Util
...
class Zend_Amf_Util_BinaryStream
{
     ...
     *
     * @todo Should check that there are enough bytes left in the stream we are
     *       about to read.
     * @param int $length            
     * @return string
    ...
    public function readBytes ($length)
    {
        if (($length + $this->_needle) > $this->_streamLength) {
            ...
            throw new Zend_Amf_Exception(
                    'Buffer underrun at needle position: ' . $this->_needle .
                             ' while requesting length: ' . $length);
        }
     ...
     *
     * @return string A UTF-8 string produced by the byte representation of
     *         characters
     */
    ...
    /**
     * Reads an IEEE 754 double-precision floating point number from the data
     * stream.
     *
    ...
    /**
     * Writes an IEEE 754 double-precision floating point number from the data
     * stream.
     *
