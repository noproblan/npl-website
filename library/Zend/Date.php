...
class Zend_Date extends Zend_Date_DateObject
{
    ...
    private static $_options = array(
            ...
            'extend_month' => false, // false - addMonth like SQL, true like
                                     // excel
...
;
    ...
    public function __construct ($date = null, $part = null, $locale = null)
    {
        ...
        if (($date !== null) and ! is_array($date) and
                 ! ($date instanceof Zend_TimeSync_Protocol) and
                 ! ($date instanceof Zend_Date) and ! defined($date) and
                 Zend_Locale::isLocale($date, true, false)) {
            $locale = $date;
            $date = null;
            $part = null;
        } else 
            if (($part !== null) and ! defined($part) and
                     Zend_Locale::isLocale($part, true, false)) {
                $locale = $part;
                $part = null;
            }
        ...
        // set datepart
        if (($part !== null && $part !== self::TIMESTAMP) or (! is_numeric(
                $date))) {
    ...
    /**
     * Sets class wide options, if no option was given, the actual set options
     * will be returned
     *
     * @param array $options
     *            Options to set
                    ...
                    case 'format_type':
                        if ((strtolower($value) != 'php') &&
                            ...
                            throw new Zend_Date_Exception(
                                    "Unknown format type ($value) for dates, only 'iso' and 'php' supported", 
                                    0, null, $value);
                        }
                        break;
                    case 'fix_dst':
                        if (! is_bool($value)) {
                            require_once 'Zend/Date/Exception.php';
                            throw new Zend_Date_Exception(
                                    "'fix_dst' has to be boolean", 0, null, 
                                    $value);
                        }
                        break;
                    case 'extend_month':
                        if (! is_bool($value)) {
                            require_once 'Zend/Date/Exception.php';
                            throw new Zend_Date_Exception(
                                    "'extend_month' has to be boolean", 0, null, 
                                    $value);
                        }
                        break;
                    case 'cache':
                        if ($value === null) {
                            ...
                            if (! $value instanceof Zend_Cache_Core) {
                                require_once 'Zend/Date/Exception.php';
                                throw new Zend_Date_Exception(
                                        "Instance of Zend_Cache expected");
                            }
                    ...
                    case 'timesync':
                        if ($value === null) {
                            ...
                            if (! $value instanceof Zend_TimeSync_Protocol) {
                                require_once 'Zend/Date/Exception.php';
                                throw new Zend_Date_Exception(
                                        "Instance of Zend_TimeSync expected");
                            }
     ...
     * Zend_Date::TIMESTAMP).
     * If the timestamp is too large for integers, then the return value will be
     * a string.
     ...
     *            Timestamp to calculate, when null the actual timestamp is
     *            calculated
     ...
     * G - era, y - year, Y - ISO year, M - month, w - week of year, D - day of
     * year, d - day of month
     * E - day of week, e - number of weekday (1-7), h - hour 1-12, H - hour
     * 0-23, m - minute, s - second
     * A - milliseconds of day, z - timezone, Z - timezone offset, S -
     * fractional second, a - period of day
     *
     * Additionally format tokens but non ISO conform are:
     * SS - day suffix, eee - php number of weekday(0-6), ddd - number of days
     * per month
     * l - Leap year, B - swatch internet time, I - daylight saving time, X -
     * timezone offset in seconds
     * r - RFC2822 format, U - unix timestamp
     *
     * Not supported ISO tokens are
     * u - extended year, Q - quarter, q - quarter, L - stand alone month, W -
     * week of month
     * F - day of week of month, g - modified julian, c - stand alone weekday, k
     * - hour 0-11, K - hour 1-24
     * v - wall zone
     *
     * @param string $format
     *            OPTIONAL Rule for formatting output. If null the default date
     *            format is used
     * @param string $type
     *            OPTIONAL Type for the format string which overrides the
     *            standard setting
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
     * @return string
    ...
    public function toString ($format = null, $type = null, $locale = null)
    {
        ...
        if (($format !== null) && ! defined($format) && ($format != 'ee') &&
                 ($format != 'ss') && ($format != 'GG') && ($format != 'MM') &&
                 ($format != 'EE') && ($format != 'TT') &&
        ...
        if ($format === null) {
            $format = Zend_Locale_Format::getDateFormat($locale) . ' ' .
                     Zend_Locale_Format::getTimeFormat($locale);
        } else 
            if (((self::$_options['format_type'] == 'php') && ($type === null)) or
                     ($type == 'php')) {
                $format = Zend_Locale_Format::convertPhpToIsoFormat($format);
            }
        
        return $this->date($this->_toToken($format, $locale), 
     ...
     *
     * @param string $part
     *            OPTIONAL Part of the date to return, if null the timestamp is
     *            returned
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
    ...
    public function get ($part = null, $locale = null)
    {
        ...
        if (($part !== null) && ! defined($part) && ($part != 'ee') &&
                 ($part != 'ss') && ($part != 'GG') && ($part != 'MM') &&
                 ($part != 'EE') && ($part != 'TT') &&
        ...
        return $this->date($this->_toToken($part, $locale), 
                $this->getUnixTimestamp(), false);
    }
    ...
    private function _parseIsoToDate ($token, $locale)
    {
        switch ($token) {
            ...
            case self::WEEKDAY_SHORT:
                $weekday = strtolower(
                        $this->date('D', $this->getUnixTimestamp(), false));
                $day = Zend_Locale_Data::getContent($locale, 'day', 
                        array(
                                'gregorian',
                                'format',
                                'wide',
            ...
            case self::WEEKDAY:
                $weekday = strtolower(
                        $this->date('D', $this->getUnixTimestamp(), false));
                return $this->_toComment(
                        Zend_Locale_Data::getContent($locale, 'day', 
                                array(
                                        'gregorian',
                                        'format',
                                        'wide',
            ...
            case 'ee':
                return $this->_toComment(
                        str_pad(
                                $this->date('N', $this->getUnixTimestamp(), 
                                        false), 2, '0', STR_PAD_LEFT));
                break;
            ...
            case 'DDD':
                return $this->_toComment(
                        str_pad(
                                $this->date('z', $this->getUnixTimestamp(), 
                                        false), 3, '0', STR_PAD_LEFT));
                break;
            ...
            case self::MONTH_NAME:
                ...
                return $this->_toComment(
                        Zend_Locale_Data::getContent($locale, 'month', 
                                array(
                                        'gregorian',
                                        'format',
                                        'wide',
            ...
            case self::MONTH_NAME_NARROW:
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                $mon = Zend_Locale_Data::getContent($locale, 'month', 
                        array(
                                'gregorian',
                                'format',
                                'abbreviated',
            ...
            case self::YEAR_SHORT_8601:
                return $this->_toComment(
                        substr(
                                $this->date('o', $this->getUnixTimestamp(), 
                                        false), - 2, 2));
                break;
            ...
            case self::MERIDIEM:
                ...
                if ($am == 'am') {
                    return $this->_toComment(
                            Zend_Locale_Data::getContent($locale, 'am'));
                }
                
                return $this->_toComment(
                        Zend_Locale_Data::getContent($locale, 'pm'));
                break;
            ...
            case self::MINUTE:
                return $this->_toComment(
                        str_pad(
                                $this->date('i', $this->getUnixTimestamp(), 
                                        false), 2, '0', STR_PAD_LEFT));
                break;
            ...
            case self::SECOND:
                return $this->_toComment(
                        str_pad(
                                $this->date('s', $this->getUnixTimestamp(), 
                                        false), 2, '0', STR_PAD_LEFT));
                break;
            ...
            case 'GGG':
                ...
                if ($year < 0) {
                    return $this->_toComment(
                            Zend_Locale_Data::getContent($locale, 'era', 
                                    array(
                                            'gregorian',
                                            'Abbr',
                                            '0'
            ...
            case self::DATES:
                return $this->_toToken(
                        Zend_Locale_Format::getDateFormat($locale), $locale);
                break;
            
            case self::DATE_FULL:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'date', 
                                array(
                                        'gregorian',
                                        'full'
                                )), $locale);
            ...
            case self::TIMES:
                return $this->_toToken(
                        Zend_Locale_Format::getTimeFormat($locale), $locale);
                break;
            
            case self::TIME_FULL:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'time', 'full'), 
                        $locale);
                break;
            
            case self::TIME_LONG:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'time', 'long'), 
                        $locale);
                break;
            
            case self::TIME_MEDIUM:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'time', 'medium'), 
                        $locale);
                break;
            
            case self::TIME_SHORT:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'time', 'short'), 
                        $locale);
                break;
            
            case self::DATETIME:
                return $this->_toToken(
                        Zend_Locale_Format::getDateTimeFormat($locale), $locale);
                break;
            
            case self::DATETIME_FULL:
                return $this->_toToken(
                        Zend_Locale_Data::getContent($locale, 'datetime', 
                                array(
                                        'gregorian',
                                        'full'
                                )), $locale);
        ...
        switch ($token[0]) {
            case 'y':
                if ((strlen($token) == 4) &&
                         (abs($this->getUnixTimestamp()) <= 0x7FFFFFFF)) {
                    return 'Y';
                }
                ...
                return $this->_toComment(
                        str_pad(
                                $this->date('Y', $this->getUnixTimestamp(), 
                                        false), $length, '0', STR_PAD_LEFT));
                break;
            ...
            case 'A':
                ...
                $result += $this->date('s', $this->getUnixTimestamp(), false) *
                         1000;
                $result += $this->date('i', $this->getUnixTimestamp(), false) *
                         60000;
                $result += $this->date('H', $this->getUnixTimestamp(), false) *
                         3600000;
                
                return $this->_toComment(
                        str_pad($result, $length, '0', STR_PAD_LEFT));
                break;
        }
    ...
    private function _getDigitFromName ($name)
    {
        switch ($name) {
            ...
            default:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        'Month ($name) is not a known month');
        }
    }
    ...
    /**
     * Sets the given date as new date or a given datepart as new datepart
     * returning the new datepart
    ...
    /**
     * Adds a date or datepart to the existing date, by extracting $part from
     * $date,
     * and modifying this object by adding that part.
     * The $part is then extracted from
     * this object and returned as an integer or numeric string (for large
     * values, or $part's
     * corresponding to pre-defined formatted date strings).
     * This could be for example a ISO 8601 date, the hour the monthname or only
     * the minute.
     * There are about 50 different supported date parts.
     * For a complete list of supported datepart values look into the docu.
     *
     ...
     * @param string $part
     *            OPTIONAL Part of the date to add, if null the timestamp is
     *            added
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
     * @return Zend_Date Provides fluid interface
     ...
     * the year or only the timestamp. There are about 50 different supported
     * date parts.
     * For a complete list of supported datepart values look into the docu
     ...
     * @param string $part
     *            OPTIONAL Part of the date to sub, if null the timestamp is
     *            subtracted
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
     * @return Zend_Date Provides fluid interface
     ...
     * @param string $part
     *            OPTIONAL Part of the date to compare, if null the timestamp is
     *            subtracted
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
     * @return integer 0 = equal, 1 = later, -1 = earlier
     ...
     * If a date part is copied, all other date parts are set to standard
     * values.
     ...
     * 01-01-1970 HOUR:00:00 (so $this contains a timestamp equal to a timestamp
     * of 0 plus HOUR).
     *
     * @param string $part
     *            Part of the date to compare, if null the timestamp is
     *            subtracted
     * @param string|Zend_Locale $locale
     *            OPTIONAL New object's locale. No adjustments to timezone are
     *            made.
    ...
    public function getTimezoneFromString ($zone)
    {
        ...
        if (! empty($match) and ($match[count($match) - 1] <= 12) and
                 ($match[count($match) - 1] >= - 12)) {
            $zone = "Etc/GMT";
        ...
        preg_match('/([[:alpha:]\/]{3,30})(?!.*([[:alpha:]\/]{3,30}))/', $zone, 
                $match);
        try {
     ...
     * @return integer|string|Zend_Date new timestamp or Zend_Date depending on
     *         calculation
     */
    private function _assign ($calc, $date, $comp = 0, $dst = false)
    {
            ...
            case 'set':
                if (! empty($comp)) {
                    $this->setUnixTimestamp(
                            call_user_func(Zend_Locale_Math::$sub, 
                                    $this->getUnixTimestamp(), $comp));
                }
                $this->setUnixTimestamp(
                        call_user_func(Zend_Locale_Math::$add, 
                                $this->getUnixTimestamp(), $date));
        ...
        // dst-correction if 'fix_dst' = true and dst !== false but only for non
        // UTC and non GMT
        if ((self::$_options['fix_dst'] === true) and ($dst !== false) and
                 ($this->_dst === true)) {
    ...
    private function _calculate ($calc, $date, $part, $locale)
    {
            ...
            if (empty($part) === false) {
                switch ($part) {
                    ...
                    default:
                        require_once 'Zend/Date/Exception.php';
                        throw new Zend_Date_Exception(
                                "datepart for part ($part) not found in array");
                        break;
                }
            } else {
                $hours = 0;
                ...
                return $this->_assign($calc, 
                        $this->mktime($hours, $minutes, $seconds, $months, 
                                $days, $years, true), 
                        $this->mktime($hour, $minute, $second, $month, $day, 
                                $year, true), $hour);
            }
        }
        ...
        switch ($part) {
            ...
            case self::DAY:
                if (is_numeric($date)) {
                    return $this->_assign($calc, 
                            $this->mktime(0, 0, 0, 1, 1 + intval($date), 1970, 
                                    true), 
                            $this->mktime(0, 0, 0, 1, 1 + intval($day), 1970, 
                                    true), $hour);
                }
                
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, day expected", 0, null, 
                        $date);
                break;
            
            case self::WEEKDAY_SHORT:
                $daylist = Zend_Locale_Data::getList($locale, 'day');
                $weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', 
                        $locale);
                $cnt = 0;
                
                foreach ($daylist as $key => $value) {
                    if (strtoupper(iconv_substr($value, 0, 3, 'UTF-8')) ==
                             strtoupper($date)) {
                        $found = $cnt;
                        break;
                    }
            ...
            case self::WEEKDAY_8601:
                $weekday = (int) $this->toString(self::WEEKDAY_8601, 'iso', 
                        $locale);
            ...
            case self::DAY_SUFFIX:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('day suffix not supported', 0, 
                        null, $date);
                break;
            ...
            case self::WEEKDAY_DIGIT:
                $weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', 
                        $locale);
                if (is_numeric($date) and (intval($date) >= 0) and
                         (intval($date) < 7)) {
                    return $this->_assign($calc, 
            ...
            case self::WEEKDAY_NARROW:
                ...
                $cnt = 0;
                foreach ($daylist as $key => $value) {
                    if (strtoupper(iconv_substr($value, 0, 1, 'UTF-8')) ==
                             strtoupper($date)) {
                        $found = $cnt;
                        break;
                    }
            ...
            case self::WEEK:
                if (is_numeric($date)) {
                    ...
                    return $this->_assign($calc, 
                            parent::mktime(0, 0, 0, 1, 1 + ($date * 7), 1970, 
                                    true), 
                            parent::mktime(0, 0, 0, 1, 1 + ($week * 7), 1970, 
                                    true), $hour);
                }
                
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, week expected", 0, null, 
                        $date);
                break;
            ...
            case self::MONTH_NAME:
                ...
                if ($cnt < 12) {
                    ...
                    if ($calc == 'add') {
                        ...
                        if (self::$_options['extend_month'] == false) {
                            $parts = $this->getDateParts(
                                    $this->mktime($hour, $minute, $second, 
                                            $date, $day, $year, false));
                            if ($parts['mday'] != $day) {
                                $fixday = ($parts['mday'] < $day) ? - $parts['mday'] : ($parts['mday'] -
                                         $day);
                            }
                        }
                    } else 
                    ...
                    return $this->_assign($calc, 
                            $this->mktime(0, 0, 0, $date, $day + $fixday, $year, 
                                    true), 
                            $this->mktime(0, 0, 0, $month, $day, $year, true), 
                            $hour);
                }
                ...
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, month expected", 0, null, 
                        $date);
                break;
            ...
            case self::MONTH_DAYS:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('month days not supported', 0, 
                        null, $date);
                break;
            ...
            case self::LEAPYEAR:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('leap year not supported', 0, null, 
                        $date);
                break;
            ...
            case self::YEAR_8601:
                if (is_numeric($date)) {
                    ...
                    return $this->_assign($calc, 
                            $this->mktime(0, 0, 0, $month, $day, intval($date), 
                                    true), 
                            $this->mktime(0, 0, 0, $month, $day, $year, true), 
                            false);
                }
                ...
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, year expected", 0, null, 
                        $date);
                break;
            ...
            case self::MERIDIEM:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('meridiem not supported', 0, null, 
                        $date);
                break;
            ...
            case self::SWATCH:
                if (is_numeric($date)) {
                    ...
                    return $this->_assign($calc, 
                            $this->mktime($hours, $minutes, $seconds, 1, 1, 
                                    1970, true), 
                            $this->mktime($hour, $minute, $second, 1, 1, 1970, 
                                    true), false);
                }
            ...
            case self::HOUR_SHORT_AM:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, hour expected", 0, null, 
                        $date);
                break;
            ...
            case self::MINUTE:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, minute expected", 0, null, 
                        $date);
                break;
            ...
            case self::SECOND:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid date ($date) operand, second expected", 0, null, 
                        $date);
                break;
            ...
            case self::TIMEZONE_SECS:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('timezone not supported', 0, null, 
                        $date);
                break;
            
            case self::DAYLIGHT:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('daylight not supported', 0, null, 
                        $date);
                break;
            ...
            case self::GMT_DIFF_SEP:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('gmtdiff not supported', 0, null, 
                        $date);
                break;
            ...
            case self::ISO_8601:
                ...
                // (-)YYYY-MM-dd
                preg_match('/^(-{0,1}\d{4})-(\d{2})-(\d{2})/', $date, 
                        $datematch);
                // (-)YY-MM-dd
                if (empty($datematch)) {
                    preg_match('/^(-{0,1}\d{2})-(\d{2})-(\d{2})/', $date, 
                            $datematch);
                }
                // (-)YYYYMMdd
                if (empty($datematch)) {
                ...
                if (! empty($datematch)) {
                    ...
                    $tmpdate = iconv_substr($date, $dateMatchCharCount, 
                            iconv_strlen($date, 'UTF-8') - $dateMatchCharCount, 
                            'UTF-8');
                }
                // (T)hh:mm:ss
                preg_match('/[T,\s]{0,1}(\d{2}):(\d{2}):(\d{2})/', $tmpdate, 
                        $timematch);
                if (empty($timematch)) {
                    preg_match('/[T,\s]{0,1}(\d{2})(\d{2})(\d{2})/', $tmpdate, 
                            $timematch);
                ...
                if (empty($datematch) and empty($timematch)) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "unsupported ISO8601 format ($date)", 0, null, $date);
                }
                if (! empty($timematch)) {
                    $timeMatchCharCount = iconv_strlen($timematch[0], 'UTF-8');
                    $tmpdate = iconv_substr($tmpdate, $timeMatchCharCount, 
                ...
                return $this->_assign($calc, 
                        $this->mktime($timematch[1], $timematch[2], 
                                $timematch[3], 1 + $datematch[2], 
                                1 + $datematch[3], 1970 + $datematch[1], false), 
                        $this->mktime($hour, $minute, $second, 1 + $month, 
                                1 + $day, 1970 + $year, false), false);
                break;
            ...
            case self::RFC_2822:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception("no RFC 2822 format ($date)", 
                            0, null, $date);
                }
                ...
                return $this->_assign($calc, 
                        $this->mktime($match[4], $match[5], $match[6], 
                                1 + $months, 1 + $match[1], 1970 + $match[3], 
                                false), 
                        $this->mktime($hour, $minute, $second, 1 + $month, 
                                1 + $day, 1970 + $year, false), false);
                break;
            ...
            case self::ERA_NAME:
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception('era not supported', 0, null, 
                        $date);
                break;
            
            case self::DATES:
                try {
                    $parsed = Zend_Locale_Format::getDate($date, 
                            array(
                                    'locale' => $locale,
                                    'format_type' => 'iso',
                                    'fix_date' => true
                    ...
                    return $this->_assign($calc, 
                            $this->mktime(0, 0, 0, 1 + $parsed['month'], 
                                    1 + $parsed['day'], 1970 + $parsed['year'], 
                                    true), 
                            $this->mktime(0, 0, 0, 1 + $month, 1 + $day, 
                                    1970 + $year, true), $hour);
            ...
            case self::TIMES:
                try {
                    ...
                    $parsed = Zend_Locale_Format::getTime($date, 
                            array(
                                    'locale' => $locale,
                                    'format_type' => 'iso',
                                    'fix_date' => true
                    ...
                    return $this->_assign($calc, 
                            $this->mktime($parsed['hour'], $parsed['minute'], 
                                    $parsed['second'], $month, $day, $year, true), 
                            $this->mktime($hour, $minute, $second, $month, $day, 
                                    $year, true), false);
                } catch (Zend_Locale_Exception $e) {
                    require_once 'Zend/Date/Exception.php';
            ...
            case self::DATETIME:
                try {
                    $parsed = Zend_Locale_Format::getDateTime($date, 
                            array(
                                    'locale' => $locale,
                                    'format_type' => 'iso',
                                    'fix_date' => true
                    ...
                    return $this->_assign($calc, 
                            $this->mktime($parsed['hour'], $parsed['minute'], 
                                    $parsed['second'], 1 + $parsed['month'], 
                                    1 + $parsed['day'], 1970 + $parsed['year'], 
                                    true), 
                            $this->mktime($hour, $minute, $second, 1 + $month, 
                                    1 + $day, 1970 + $year, true), $hour);
                } catch (Zend_Locale_Exception $e) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception($e->getMessage(), 0, $e, $date);
                }
            ...
            case self::RFC_3339:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, ATOM format expected", 
                            0, null, $date);
                }
                ...
                return $this->_assign($calc, 
                        $this->mktime($match[4], $match[5], $match[6], 
                                1 + $match[2], 1 + $match[3], 1970 + $match[1], 
                                true), 
                        $this->mktime($hour, $minute, $second, 1 + $month, 
                                1 + $day, 1970 + $year, true), false);
                break;
            ...
            case self::COOKIE:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, COOKIE format expected", 
                            0, null, $date);
                }
                $matchStartPos = iconv_strpos($match[0], ' ', 0, 'UTF-8') + 1;
                $match[0] = iconv_substr($match[0], $matchStartPos, 
            ...
            case self::RFC_1036:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, RFC 822 date format expected", 
                            0, null, $date);
                }
            ...
            case self::RFC_850:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, RFC 850 date format expected", 
                            0, null, $date);
                }
            ...
            case self::RFC_1123:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, RFC 1123 date format expected", 
                            0, null, $date);
                }
            ...
            case self::RSS:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, RSS date format expected", 
                            0, null, $date);
                }
            ...
            case self::W3C:
                ...
                if (! $result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "invalid date ($date) operand, W3C date format expected", 
                            0, null, $date);
                }
            ...
            default:
                if (! is_numeric($date) || ! empty($part)) {
                    try {
                        ...
                        if ((strpos(strtoupper($part), 'YY') !== false) and
                                 (strpos(strtoupper($part), 'YYYY') === false)) {
                            $parsed['year'] = self::getFullYear($parsed['year']);
                        }
                        ...
                        return $this->_assign($calc, 
                                $this->mktime(
                                        ...
                                        isset($parsed['month']) ? (1 +
                                                 $parsed['month']) : 1, 
                                                isset($parsed['day']) ? (1 +
                                                 $parsed['day']) : 1, 
                                                isset($parsed['year']) ? (1970 +
                                                 $parsed['year']) : 1970, false), 
                                $this->getUnixTimestamp(), false);
                    } catch (Zend_Locale_Exception $e) {
                        if (! is_numeric($date)) {
                            require_once 'Zend/Date/Exception.php';
                            throw new Zend_Date_Exception($e->getMessage(), 0, 
                                    $e, $date);
                        }
                    }
                }
     ...
     * For example:
     * 15.May.2000 <-> 15.June.2000 Equals only for Day or Year... all other
     * will return false
     *
     * @param string|integer|array|Zend_Date $date
     ...
     * For example:
     * 15.May.2000 <-> 13.June.1999 will return true for day, year and date, but
     * not for month
     *
     * @param string|integer|array|Zend_Date $date
     ...
     * For example:
     * 15.May.2000 <-> 13.June.1999 will return true for month but false for
     * day, year and date
     ...
     * For example:
     * 15.May.2000 10:11:23 will return a dateobject equal to 01.Jan.1970
     * 10:11:23
     *
     * @param string|Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
    ...
    private function _time ($calc, $time, $format, $locale)
    {
        if ($time === null) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    'parameter $time must be set, null is not allowed');
        }
        ...
        if ($time instanceof Zend_Date) {
            ...
            if (is_array($time)) {
                if ((isset($time['hour']) === true) or
                         (isset($time['minute']) === true) or
                         (isset($time['second']) === true)) {
                    $parsed = $time;
                } else {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "no hour, minute or second given in array");
                }
            } else {
                if (self::$_options['format_type'] == 'php') {
                    $format = Zend_Locale_Format::convertPhpToIsoFormat($format);
                ...
                try {
                    ...
                    $parsed = Zend_Locale_Format::getTime($time, 
                            array(
                                    'date_format' => $format,
                                    'locale' => $locale,
                                    'format_type' => 'iso'
     ...
     * For example: dd.MMMM.yyTHH:mm' and 'ss sec'-> 10.May.07T25:11 and 44 sec
     * => 1h11min44sec + 1 day
     * Returned is the new date object and the existing date is left as it was
     * before
     *
     * @param string|integer|array|Zend_Date $time
     *            Time to set
    ...
    private function _date ($calc, $date, $format, $locale)
    {
        ...
        if ($date instanceof Zend_Date) {
            ...
            if (is_array($date)) {
                if ((isset($date['year']) === true) or
                         (isset($date['month']) === true) or
                         (isset($date['day']) === true)) {
                    $parsed = $date;
                } else {
                    require_once 'Zend/Date/Exception.php';
                    throw new Zend_Date_Exception(
                            "no day,month or year given in array");
                }
            } else {
                if ((self::$_options['format_type'] == 'php') &&
                         ! defined($format)) {
     ...
     * Also a complete date with time can be given, but only the date is used
     * for setting.
     ...
     * Always the complete ISO 8601 specifiction is used. If an other ISO date
     * is needed
     ...
     * For example: 050901 -> 01.Sept.2005 00:00:00, 20050201T10:00:30 ->
     * 01.Feb.2005 10h00m30s
    ...
    private function _checkLocation ($location)
    {
        ...
        if (($location['longitude'] > 180) or ($location['longitude'] < - 180)) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    'Longitude must be between -180 and 180', 0, null, $location);
        }
    ...
    /**
     * Returns the time of sunrise for this date and a given location as new
     * date object
     * For a list of cities and correct locations use the class Zend_Date_Cities
     *
     * @param array $location
     *            location of sunrise
     *            ['horizon'] -> civil, nautic, astronomical, effective
     *            (default)
    ...
    public function getSunset ($location)
    {
        $horizon = $this->_checkLocation($location);
        $result = clone $this;
        $result->set($this->calcSun($location, $horizon, false), 
                self::TIMESTAMP);
        return $result;
    }
    ...
    public function getSunInfo ($location)
    {
        ...
        for ($i = 0; $i < 4; ++ $i) {
            ...
            $horizon = $this->_checkLocation($location);
            $result = clone $this;
            $result->set($this->calcSun($location, $horizon, true), 
                    self::TIMESTAMP);
    ...
    public function isYesterday ()
    {
        list ($year, $month, $day) = explode('-', 
                $this->date('Y-m-d', $this->_getTime()));
        // adjusts for leap days and DST changes that are timezone specific
        $yesterday = $this->date('Ymd', 
                $this->mktime(0, 0, 0, $month, $day - 1, $year));
        $day = $this->date('Ymd', $this->getUnixTimestamp());
    ...
    private function _calcvalue ($calc, $value, $type, $parameter, $locale)
    {
        if ($value === null) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    "parameter $type must be set, null is not allowed");
        }
        ...
        if ($value instanceof Zend_Date) {
            ...
            if (! is_array($value) && ! is_numeric($value) && ($type != 'iso') &&
                     ($type != 'arpa')) {
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception("invalid $type ($value) operand", 
                        0, null, $value);
            }
    ...
    /**
     * Compares the year with the existing date object, ignoring other date
     * parts.
    ...
    private function _month ($calc, $month, $locale)
    {
        if ($month === null) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    'parameter $month must be set, null is not allowed');
        }
        ...
        if ($month instanceof Zend_Date) {
            ...
            if (is_numeric($month)) {
                ...
                if (is_array($month)) {
                    ...
                    $monthlist = Zend_Locale_Data::getList($locale, 'month');
                    $monthlist2 = Zend_Locale_Data::getList($locale, 'month', 
                            array(
                                    'gregorian',
                                    'format',
                                    'abbreviated'
                    ...
                    if ($found == 0) {
                        require_once 'Zend/Date/Exception.php';
                        throw new Zend_Date_Exception(
                                "unknown month name ($month)", 0, null, $month);
                    }
                }
        }
     ...
     * will result in adding or subtracting the relevant year. (12 months equal
     * one year)
     * If a localized monthname is given it will be parsed with the default
     * locale or the optional
     * set locale.
     * Returned is the new date object
     *
     * @param string|integer|array|Zend_Date $month
     ...
     * The month can be a number or a string. Adding months lower then 0 and
     * greater then 12
     * will result in adding or subtracting the relevant year. (12 months equal
     * one year)
     * If a localized monthname is given it will be parsed with the default
     ...
     * The month can be a number or a string. Subtracting months lower then 0
     * and greater then 12
     * will result in adding or subtracting the relevant year. (12 months equal
     * one year)
     * If a localized monthname is given it will be parsed with the default
    ...
    /**
     * Compares the month with the existing date object, ignoring other date
     * parts.
    ...
    private function _day ($calc, $day, $locale)
    {
        if ($day === null) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    'parameter $day must be set, null is not allowed');
        }
     ...
     * If a localized dayname is given it will be parsed with the default locale
     * or the optional
     * set locale.
     * Returned is the new date object
     * Example: setDay('Montag', 'de_AT'); will set the monday of this week as
     * day.
     *
     * @param string|integer|array|Zend_Date $month
     *            Day to set
     ...
     * The day can be a number or a string. Adding days lower then 0 or greater
     * than the number of this months days
     * will result in adding or subtracting the relevant month.
     * If a localized dayname is given it will be parsed with the default locale
     * or the optional
     * set locale.
     ...
     * The day can be a number or a string. Subtracting days lower then 0 or
     * greater than the number of this months days
     * will result in adding or subtracting the relevant month.
     * If a localized dayname is given it will be parsed with the default locale
     * or the optional
     * set locale.
    ...
    /**
     * Compares the day with the existing date object, ignoring other date
     * parts.
     ...
     * Example: 09-Jan-2007 -> 2 = Tuesday -> 02-Jan-1970 (when 02.01.1970 is
     * also Tuesday)
     *
     * @param Zend_Locale $locale
     *            OPTIONAL Locale for parsing input
     ...
     *            Weekday to calculate, when null the actual weekday is
     *            calculated
     * @param Zend_Locale $locale
     *            Locale for parsing input
     * @return Zend_Date|integer
    ...
    private function _weekday ($calc, $weekday, $locale)
    {
        if ($weekday === null) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception(
                    'parameter $weekday must be set, null is not allowed');
        }
     ...
     * then it will be parsed as a date in $locale (defaults to the same locale
     * as $this).
     * Returned is the new date object.
     ...
     * set locale.
     * Returned is the new date object
     * Example: addWeekday(3); will add the difference of days from the begining
     * of the month until
     * wednesday.
     *
     * @param string|integer|array|Zend_Date $month
     ...
     * set locale.
     * Returned is the new date object
     * Example: subWeekday(3); will subtract the difference of days from the
     * begining of the month until
     * wednesday.
     *
     * @param string|integer|array|Zend_Date $month
     *            Weekday to sub
    ...
    {
        return $this->_calcvalue('set', $day, 'day of year', self::DAY_OF_YEAR, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('add', $day, 'day of year', self::DAY_OF_YEAR, 
                $locale);
    }
     ...
     * The day of year is always a number.
     * Returned is the new date object
     * Example: subDayOfYear(10); will subtract 10 days from the existing date
     * object.
     *
     * @param string|integer|array|Zend_Date $day
     *            Day of Year to sub
    ...
    {
        return $this->_calcvalue('sub', $day, 'day of year', self::DAY_OF_YEAR, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('cmp', $day, 'day of year', self::DAY_OF_YEAR, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('set', $hour, 'hour', self::HOUR_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('add', $hour, 'hour', self::HOUR_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('sub', $hour, 'hour', self::HOUR_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('cmp', $hour, 'hour', self::HOUR_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('set', $minute, 'minute', self::MINUTE_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('add', $minute, 'minute', self::MINUTE_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('sub', $minute, 'minute', self::MINUTE_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('cmp', $minute, 'minute', self::MINUTE_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('set', $second, 'second', self::SECOND_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('add', $second, 'second', self::SECOND_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('sub', $second, 'second', self::SECOND_SHORT, 
                $locale);
    }
    ...
    {
        return $this->_calcvalue('cmp', $second, 'second', self::SECOND_SHORT, 
                $locale);
    }
    ...
    public function setFractionalPrecision ($precision)
    {
        ...
        if ($this->_precision < strlen($this->_fractional)) {
            $this->_fractional = substr($this->_fractional, 0, 
                    $this->_precision);
        } else {
            $this->_fractional = str_pad($this->_fractional, $this->_precision, 
                    '0', STR_PAD_RIGHT);
        }
     ...
     *            (Optional) Millisecond to set, when null the actual
     *            millisecond is set
     * @param integer $precision
    ...
    public function setMilliSecond ($milli = null, $precision = null)
    {
        if ($milli === null) {
            ...
            if (! is_numeric($milli)) {
                require_once 'Zend/Date/Exception.php';
                throw new Zend_Date_Exception(
                        "invalid milli second ($milli) operand", 0, null, $milli);
            }
     ...
     *
     * @param integer|Zend_Date $milli
     *            (Optional) Millisecond to add, when null the actual
     ...
     *
     * @param integer|Zend_Date $milli
     *            (Optional) Millisecond to sub, when null the actual
     *            millisecond is subtracted
     * @param integer $precision
     *            (Optional) Fractional precision for the given milliseconds
     * @return Zend_Date Provides fluid interface
     */
     ...
     *
     * @param integer|Zend_Date $milli
     *            OPTIONAL Millisecond to compare, when null the actual
     *            millisecond is compared
     * @param integer $precision
     *            OPTIONAL Fractional precision for the given milliseconds
     ...
     * Returns false if a expected datepart is missing or a datepart exceeds its
     * possible border.
     * But the check will only be done for the expected dateparts which are
     * given by format.
     * If no format is given the standard dateformat for the actual locale is
     * used.
    ...
    public static function isDate ($date, $format = null, $locale = null)
    {
        if (! is_string($date) && ! is_numeric($date) &&
                 ! ($date instanceof Zend_Date) && ! is_array($date)) {
            return false;
        }
        
        if (($format !== null) && ($format != 'ee') && ($format != 'ss') &&
                 ($format != 'GG') && ($format != 'MM') && ($format != 'EE') &&
                 ($format != 'TT') &&
                 (Zend_Locale::isLocale($format, null, false))) {
            $locale = $format;
            $format = null;
        ...
        $timestamp = $date->mktime($parsed['hour'], $parsed['minute'], 
                $parsed['second'], $parsed['month'], $parsed['day'], 
                $parsed['year']);
