...
class Zend_Queue implements Countable
{
     ...
     *
     * @param
     *            string|Zend_Queue_Adapter|array|Zend_Config|null String or
     *            adapter instance, or options array or Zend_Config instance
    ...
    public function __construct ($spec, $options = array())
    {
        ...
        if ((null === $adapter) &&
            ...
            throw new Zend_Queue_Exception(
                    'No valid params passed to constructor');
        }
     ...
     *
     * @param string $name            
     * @return null|mixed Returns null if option does not exist; option value
     *         otherwise
     */
    ...
    public function setAdapter ($adapter)
    {
        if (is_string($adapter)) {
            if (null ===
                     ($adapterNamespace = $this->getOption('adapterNamespace'))) {
        ...
        if (! $adapter instanceof Zend_Queue_Adapter_AdapterInterface) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    "Adapter class '" . get_class($adapterName) .
                             "' does not implement Zend_Queue_Adapter_AdapterInterface");
        }
    ...
    public function receive ($maxMessages = null, $timeout = null)
    {
        if (($maxMessages !== null) && ! is_integer($maxMessages)) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
                    '$maxMessages must be an integer or null');
        }
    ...
    public function getQueues ()
    {
        if (! $this->isSupported('getQueues')) {
            throw new Zend_Queue_Exception(
                    __FUNCTION__ . '() is not supported by ' .
