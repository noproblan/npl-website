...
class Zend_Oauth_Client extends Zend_Http_Client
{
     ...
     *
     * @return void
     * @throws Zend_Oauth_Exception If POSTBODY scheme requested, but GET
     *         request method used; or if invalid request scheme provided
     */
    public function prepareOauth ()
    {
        ...
        if ($requestScheme == Zend_Oauth::REQUEST_SCHEME_HEADER) {
            $oauthHeaderValue = $this->getToken()->toHeader($this->getUri(true), 
                    $this->_config, $this->_getSignableParametersAsQueryString(), 
                    $this->getRealm());
            ...
            $raw = $this->getToken()->toQueryString($this->getUri(true), 
                    $this->_config, $this->_getSignableParametersAsQueryString());
            ...
            if ($query) {
                ...
                foreach ($queryParts as $queryPart) {
                    ...
                    $params[urldecode($kvTuple[0])] = (array_key_exists(1, 
                            $kvTuple) ? urldecode($kvTuple[1]) : null);
                }
            }
            if (! empty($this->paramsPost)) {
                $params = array_merge($params, $this->paramsPost);
                $query = $this->getToken()->toQueryString($this->getUri(true), 
                        $this->_config, $params);
            }
            $query = $this->getToken()->toQueryString($this->getUri(true), 
                    $this->_config, $params);
            ...
            require_once 'Zend/Oauth/Exception.php';
            throw new Zend_Oauth_Exception(
                    'Invalid request scheme: ' . $requestScheme);
        }
    }
     ...
     * It's that instance
     * which holds all configuration methods and values this object also
     * presents
     * as it's API.
     *
     * @param string $method            
