...
class LanController extends Zend_Controller_Action
{
        ...
        foreach($this->_currentLans as $lan) {
            ...
            foreach ($tickets as $ticket) {
                ...
                $ticketsByUser[] = array(
                    ...
                    'bill' => $this->_getCalculatedExtras($ticket->getExtrasSplitted()),
	...
	private function _getCalculatedExtras($extrasArray) {
		$result = 40;
		if (isset($extrasArray['breakfast'])) $result += 5;
		if (isset($extrasArray['dinner']))    $result += 15;
		if (isset($extrasArray['cable']))     $result += 10;
		return $result;
	}
	...
	private function _sendPrepaidMail(Application_Model_Ticket $ticket) {
		...
		try {
			$mail->send();
			$this->_flashMessenger->setNamespace('success')->addMessage('Eine E-Mail zur Bestätigung der Vorauszahlung wurde verschickt.');
		} catch (Zend_Mail_Transport_Exception $e) {
			$this->_flashMessenger->setNamespace('error')->addMessage('Die Bestätigung der Vorauszahlung konnte nicht an ' . $user->getUsername() . ' (' . $user->getMail() . ') versendet werden.');
		}
	}
	...
	private function _createNewTicket($data) {
        ...
        switch ($data['extras']) {
            case 'none': $ticket->setExtras(''); break;
            case 'breakfast': $ticket->setExtras('breakfast'); break;
            case 'dinner': $ticket->setExtras('dinner'); break;
            case 'both':
            default: $ticket->setExtras('breakfast,dinner');
        }

        switch ($data['status']) {
            case 'prepaid': $ticket->setStatus(Application_Model_Ticket::STATUS_PREPAID); break;
            case 'paid': $ticket->setStatus(Application_Model_Ticket::STATUS_PAID); break;
            case 'notpaid':
            default: $ticket->setStatus(Application_Model_Ticket::STATUS_NOTPAID);
        }
